...
```commandline
http-server-endpoint = 127.0.0.1:8889
```
...
```commandline
./eosc --wallet-port 8889 create account inita currency PUBLIC_KEY_1 PUBLIC_KEY_2 
```
...
```commandline
./eosc --wallet-port 8889 set contract currency ../../../contracts/currency/currency.wast ../../../contracts/currency/currency.abi
```
...
```commandline
./eosc  --wallet-port 8889 create account inita tester PUBLIC_USER_KEY_1 PUBLIC_USER_KEY_2 
```

After this we can send a message to the contract:

```commandline
./eosc  --wallet-port 8889 push message currency transfer '{"from":"currency","to":"inita","amount":50}' --scope currency,inita --permission currency@active
```
