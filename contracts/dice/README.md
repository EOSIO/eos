...
````bash
cleos create account eosio enumivo.token EOS7ijWCBmoXBi3CgtK7DJxentZZeTkeUnaSDvyro9dq7Sd1C3dC4 EOS7ijWCBmoXBi3CgtK7DJxentZZeTkeUnaSDvyro9dq7Sd1C3dC4
````

##### Set enumivo.token contract to enumivo.token account
````bash
cleos set contract enumivo.token build/contracts/enumivo.token -p enumivo.token
````
...
````bash
cleos push action enumivo.token create '[ "eosio", "1000000000.0000 EOS", 0, 0, 0]' -p enumivo.token
````
...
````bash
cleos push action enumivo.token issue '[ "alice", "1000.0000 EOS", "" ]' -p eosio
````

##### Issue 1000 EOS to bob
````bash
cleos push action enumivo.token issue '[ "bob", "1000.0000 EOS", "" ]' -p eosio
````
...
````bash
cleos get currency balance enumivo.token alice eos
1003.0000 EOS
````
