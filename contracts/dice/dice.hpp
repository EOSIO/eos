 ...
 *  iterate over offers by either {bet,owner,commitment} or
 * {commitment,bet,owner}
 *
 *  Only the first 128 bits of commitment ID are used in the index, but there
 * will
 *  be a requirement that these first 128 bits be unique and any collisions will
 *  be rejected.
 *
 *  If there is no existing offer of equal bet size then gameid is set as 0, if
 * it is
 *  matched then a new Game record is created and the Offer is modified so that
 * bet
 *  goes to 0 (and therefore moves to end of queue and not matched with other
 * bets) and
...
using Offers =
    Table<N(dice), N(dice), N(global), GlobalDice, OfferPrimaryKey, uint128_t>;
