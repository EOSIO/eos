...
void apply_offer(const OfferBet &offer) {
  ...
  assert(!hasOffer(offer.commitment),
   ...
   *          update lowerbound.primary.bet = 0 and lwoerbound.gameid =
   * GlobalDice.nextgameid
   *          Create Offer entry in Offers table with bet = 0 and gameid == --^
   *     else
   *          Create Offer entry in Offers table with bet = offer.bet and gameid
   * = 0
   */
}
