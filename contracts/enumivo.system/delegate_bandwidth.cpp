...
namespace enumivosystem {
   ...
   void system_contract::buyram( account_name payer, account_name receiver, asset quant )
   {
      ...
      INLINE_ACTION_SENDER(enumivo::token, transfer)( N(enumivo.token), {payer,N(active)},
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      INLINE_ACTION_SENDER(enumivo::token, transfer)( N(enumivo.token), {N(enumivo.ram),N(active)},
                                                       { N(enumivo.ram), account, asset(tokens_out), std::string("sell ram") } );
      auto fee = tokens_out.amount / 200;
      if( fee > 0 ) {
         INLINE_ACTION_SENDER(enumivo::token, transfer)( N(enumivo.token), {account,N(active)},
                                   ...
                                   const asset stake_net_delta, const asset stake_cpu_delta, bool transfer )
   {
      ...
      if ( N(enumivo.stk) != source_stake_from ) { //for enumivo both transfer and refund make no sense
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(enumivo::token, transfer)( N(enumivo.token), {source_stake_from, N(active)},
                                       ...
                                       asset unstake_net_quantity, asset unstake_cpu_quantity )
   {
      ...
      enumivo_assert( _gstate.total_activated_stake >= min_activated_stake, "not enough has been staked for users to unstake" );
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      INLINE_ACTION_SENDER(enumivo::token, transfer)( N(enumivo.token), {N(enumivo.stk),N(active)},
