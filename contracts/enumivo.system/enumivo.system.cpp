...
namespace enumivosystem {
    ...
    _rammarket(_self,_self)
   {
      ...
      if( itr == _rammarket.end() ) {
         auto system_token_supply   = enumivo::token(N(enumivo.token)).get_supply(enumivo::symbol_type(system_token_symbol).name()).amount;
   ...
   void system_contract::bidname( account_name bidder, account_name newname, asset bid ) {
      ...
      INLINE_ACTION_SENDER(enumivo::token, transfer)( N(enumivo.token), {bidder,N(active)},
      ...
      if( current == bids.end() ) {
         ...
         INLINE_ACTION_SENDER(enumivo::token, transfer)( N(enumivo.token), {N(enumivo.name),N(active)},
