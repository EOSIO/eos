...
asset connector::convert_from_exchange(exchange_state &ex, const asset &input) {
    ...
    real_type CONNECTOR_PAID_OUT =
            CONNECTOR_AMT * (std::pow(ONE + EXC_TO_CONVERT / DEDUCTED_EXC_AMT, ONE / WEIGHT) - ONE);
...
void connector_weight_test(exchange_state &origin_state) {
    ...
    state = origin_state;
    state.base.weight = 0.1;
    state.quote.weight = 0.9;
    sensitivity_analysis(state);

    state = origin_state;
    state.base.weight = 0.5;
    state.quote.weight = 0.5;
    sensitivity_analysis(state);

    state = origin_state;
    state.base.weight = 0.9;
    state.quote.weight = 0.1;
    sensitivity_analysis(state);
