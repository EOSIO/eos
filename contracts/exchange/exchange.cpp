 ...
 *
 *  The exchange and one of the currency contracts are forced to execute in the
 * same
 ...
 *
 *  creating, canceling, and filling orders do not require blocking either
 * currency
...
void apply_currency_transfer(const currency::Transfer &transfer) {
  if (transfer.to == N(exchange)) {
    ...
    requireAuth(transfer.to); /// require the receiver of funds (account owner)
...
void apply_exchange_buy(BuyOrder order) {
  ...
  print(Name(bid.buyer.name), " created bid for ", order.quantity,
        " currency at price: ", order.price, "\n");

  Bid existing_bid;
  assert(!BidsById::get(bid.buyer, existing_bid),
  ...
  if (bid.quantity && !order.fill_or_kill)
    buyer_account.open_orders++;
  save(buyer_account);
...
void apply_exchange_sell(SellOrder order) {
  ...
  Ask existing_ask;
  assert(!AsksById::get(ask.seller, existing_ask),
         "order with this id already exists");
  ...
  if (ask.quantity && !order.fill_or_kill)
    seller_account.open_orders++;
  save(seller_account);
  if (ask.quantity) {
    assert(!order.fill_or_kill, "order not completely filled");
...
void apply_exchange_cancel_buy(OrderID order) {
  ...
  Bid bid_to_cancel;
  assert(BidsById::get(order, bid_to_cancel),
...
void apply_exchange_cancel_sell(OrderID order) {
  ...
  Ask ask_to_cancel;
  assert(AsksById::get(order, ask_to_cancel),
