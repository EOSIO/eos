...
struct contract {
   ...
   *
   *  @note the change in code does not take effect until the start of the next
   * block
   */
  void setcode(Bytes code, Abi abi, uint8_t vm = 0,
  ...
  /**
   *  @brief set the local named permission required for `this` account/contract
   * to
   *     call `con::act(...)`
   *
   *  <h3> Required Authority </h3>
   ...
   *  @param myperm - the name of a permission set by @ref setauth on `this`
   * contract
   */
 ...
 *   @ingroup contracts
 *
 *   @details The EOS contract is a *native* currency contract implemented with
 * account named `eos`. This contract enables
 *   users to transfer EOS tokens to each other. This contract is designed to
 * work the @ref stakedcontract and
...
struct eos : public contract {
  ...
  This action requires access to `from` and `to` account scopes. It does not
  require
  access to the `eos` scope which means that multiple transfers can execute in
  parallel
  as long as they don't have any overlapping scopes.

  <h3> Required Recipients </h3>

  This message requires that the accounts `from` and `to` are listed in the
  required recipients. This ensures
  ...
  */
  void transfer(
      ...
      uint64_t amount ///< must be greater than 0 and less or equal to `from`'s
                      ///balance
      );
