...
namespace arisensystem {
   ...
   struct user_resources {
      ...
      RSNLIB_SERIALIZE( user_resources, (owner)(net_weight)(cpu_weight)(ram_bytes) )
   };
   ...
   struct delegated_bandwidth {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      RSNLIB_SERIALIZE( delegated_bandwidth, (from)(to)(net_weight)(cpu_weight) )
   ...
   struct refund_request {
      account_name  owner;
      time          request_time;
      arisen::asset  net_amount;
      arisen::asset  cpu_amount;

      uint64_t  primary_key()const { return owner; }

      // explicit serialization macro is not necessary, used here only to improve compilation time
      RSNLIB_SERIALIZE( refund_request, (owner)(request_time)(net_amount)(cpu_amount) )
   };
    ...
    */
   typedef arisen::multi_index< N(userres), user_resources>      user_resources_table;
   typedef arisen::multi_index< N(delband), delegated_bandwidth> del_bandwidth_table;
   typedef arisen::multi_index< N(refunds), refund_request>      refunds_table;
