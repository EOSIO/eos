...
void test_transaction::send_action_large() {
   ...
   act.send();
   arisen_assert(false, "send_message_large() should've thrown an error");
}
...
void test_transaction::test_tapos_block_prefix() {
   ...
   arisen_assert( tbp == tapos_block_prefix(), "tapos_block_prefix does not match" );
}
...
void test_transaction::test_tapos_block_num() {
   ...
   arisen_assert( tbn == tapos_block_num(), "tapos_block_num does not match" );
}
...
void test_transaction::test_read_transaction() {
   ...
   arisen_assert( size == read, "read_transaction failed");
   sha256(buf, read, &h);
   printhex( &h, sizeof(h) );
}
...
void test_transaction::test_transaction_size() {
   ...
   arisen_assert( trans_size == transaction_size(), "transaction size does not match" );
}
...
void test_transaction::send_transaction_empty(uint64_t receiver, uint64_t, uint64_t) {
   ...
   arisen_assert(false, "send_transaction_empty() should've thrown an error");
}
...
void test_transaction::assert_false_error_handler(const arisen::transaction& dtrx) {
   arisen_assert(dtrx.actions.size() == 1, "transaction should only have one action");
   arisen_assert(dtrx.actions[0].account == N(testapi), "transaction has wrong code");
   arisen_assert(dtrx.actions[0].name == WASM_TEST_ACTION("test_action", "assert_false"), "transaction has wrong name");
   arisen_assert(dtrx.actions[0].authorization.size() == 1, "action should only have one authorization");
   arisen_assert(dtrx.actions[0].authorization[0].actor == N(testapi), "action's authorization has wrong actor");
   arisen_assert(dtrx.actions[0].authorization[0].permission == N(active), "action's authorization has wrong permission");
}
...
void test_transaction::send_transaction_large(uint64_t receiver, uint64_t, uint64_t) {
   ...
   arisen_assert(false, "send_transaction_large() should've thrown an error");
}
...
void test_transaction::cancel_deferred_transaction_success() {
   ...
   arisen_assert( (bool)r, "transaction was not found" );
}

void test_transaction::cancel_deferred_transaction_not_found() {
   using namespace arisen;
   auto r = cancel_deferred( 0xffffffffffffffff ); //use the same id (0) as in send_deferred_transaction
   arisen_assert( !r, "transaction was canceled, whild should not be found" );
}
...
void test_transaction::send_cf_action_fail() {
   ...
   act.send_context_free();
   arisen_assert(false, "send_cfa_action_fail() should've thrown an error");
}
...
void test_transaction::new_feature() {
   arisen_assert(false == is_feature_active((int64_t)N(newfeature)), "we should not have new features unless hardfork");
}
