...
void test_action::test_dummy_action() {
   ...
   arisen_assert( act.authorization.back().actor == N(testapi), "incorrect permission actor" );
   arisen_assert( act.authorization.back().permission == N(active), "incorrect permission name" );
   arisen_assert( arisen::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
   arisen_assert( act.account == N(testapi), "expected testapi account" );
...
void test_action::test_cf_action() {
   ...
   if ( cfa.payload == 100 ) {
      ...
      arisen::bytes cfd( static_cast<size_t>(size) );
...
void test_action::require_notice(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if( receiver == N(testapi) ) {
      arisen::require_recipient( N(acc1) );
      arisen::require_recipient( N(acc2) );
      arisen::require_recipient( N(acc1), N(acc2) );
...
void test_action::require_notice_tests(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if( receiver == N( testapi ) ) {
      arisen::print( "require_recipient( N(acc5) )" );
      arisen::require_recipient( N( acc5 ) );
   } else if( receiver == N( acc5 ) ) {
      arisen::print( "require_recipient( N(testapi) )" );
      arisen::require_recipient( N( testapi ) );
   }
}

void test_action::require_auth() {
   prints("require_auth");
   arisen::require_auth( N(acc3) );
   arisen::require_auth( N(acc4) );
}
...
void test_action::test_current_receiver(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   arisen_assert( receiver == cur_rec, "the current receiver does not match" );
}
...
void test_action::test_ram_billing_in_notify(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if( code == receiver ) {
      arisen::require_recipient( to_notify );
   } else {
