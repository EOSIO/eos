...
unsigned int test_string::construct_with_data_partially() {
  ...
  WASM_ASSERT(str.get_data() == data + offset,
...
unsigned int test_string::copy_constructor() {
  ...
  WASM_ASSERT(str1.get_size() == str2.get_size(),
              "str1.get_size() == str2.get_size()");
  WASM_ASSERT(str1.get_data() == str2.get_data(),
              "str1.get_data() == str2.getget_data_size()");
  WASM_ASSERT(str1.is_own_memory() == str2.is_own_memory(),
              "str1.is_own_memory() == str2.is_own_memory()");
  WASM_ASSERT(str1.get_refcount() == str2.get_refcount(),
...
unsigned int test_string::substring() {
  ...
  WASM_ASSERT(substr.get_size() == substr_size,
              "str.get_size() == substr_size");
  WASM_ASSERT(substr.get_data() == str.get_data() + offset,
              "substr.get_data() == str.get_data() + offset");
  for (uint8_t i = offset; i < substr_size; i++) {
    WASM_ASSERT(substr[i] == str[offset + i], "substr[i] == str[offset + i]");
  }
  WASM_ASSERT(substr.is_own_memory() == false,
...
unsigned int test_string::concatenation_null_terminated() {
  ...
  WASM_ASSERT(str1.get_size() == size1 + size2 - 1,
...
unsigned int test_string::concatenation_non_null_terminated() {
  ...
  WASM_ASSERT(str1.get_data() != data1, "str1.get_data() != data1");
  WASM_ASSERT(str1.get_size() == size1 + size2,
