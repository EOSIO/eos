...
void multisig::propose() {
   ...
   eosio_assert( trx_header.expiration >= arisen::time_point_sec(now()), "transaction expired" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      eosio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < arisen::time_point_sec(now()), "cannot cancel until expiration" );
   }
