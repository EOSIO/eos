...
void apply_storage_transfer(const TOKEN_NAME::Transfer &transfer) {
  ...
  from.balance -=
      transfer.quantity;           /// token subtraction has underflow assertion
...
uint32_t readLinkFromBuffer(const char *buffer, uint32_t bufferlen,
                            TOKEN_NAME::Link &link, uint32_t &eospathlen,
...
void apply_storage_setlink() {
  ...
  auto linklen =
      readLinkFromBuffer(tmp, bufferlen, link, eospathlen, ipfspathlen);
  ...
  ::store_str(currentCode(), N(storage), link.eospath, eospathlen,
              (char *)&link, linklen);
}

void apply_storage_removelink(char *eospath, uint32_t eospathlen) {
  char tmp[4098];
  auto len = ::load_str(currentCode(), currentCode(), N(storage), eospath,
                        eospathlen, tmp, 4098);
  TOKEN_NAME::Link link;
  uint32_t ipfspathlen;
...
void apply_storage_createstore(char *eospath, uint32_t eospathlen) {
  ...
  // logic goes here to reduce number of tokens and increase quote used using
  // bancor algorithm
  link.accept = 1;
  ::store_str(currentCode(), N(storage), link.eospath, eospathlen,
              (char *)&link, len);
...
void apply(uint64_t code, uint64_t action) {
  if (code == N(storage)) {
    if (action == N(transfer)) {
      TOKEN_NAME::apply_storage_transfer(
          eos::currentMessage<TOKEN_NAME::Transfer>());
