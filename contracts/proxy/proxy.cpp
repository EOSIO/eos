...
namespace proxy {
   ...
   void apply_transfer(uint64_t receiver, account_name /* code */, const T& transfer) {
      ...
      if (transfer.from == self) {
         ...
         transaction out;
         out.actions.emplace_back(permission_level{self, N(active)}, N(enumivo.tkn), N(transfer), new_transfer);
   ...
   void apply_onerror(uint64_t receiver, const onerror& error ) {
      ...
      enumivo::print("Resending Transaction: ", error.sender_id, " as ", id, "\n");
