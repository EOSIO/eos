...
*  @brief Defines the ABI for interfacing with standard-compatible token
* messages and database tables.
*  @ingroup contractdev
*
* @{
*/
...
*  @tparam CurrencyType - type of the currency (e.g. eos) represented as an
* unsigned 64 bit integer
*  @ingroup tokens
*
*  @details Base token structure with checks for proper types and
* over/underflows.
*  It supports the following operator: +=, -=, +, -, <=, <, ==, !=, >=, >, bool
* and also print functionality
*
*  Example:
*  @code
...
*  @{
*/
template <typename NumberType, uint64_t CurrencyType = N(eos)> struct token {
  /**
  ...
  */
  inline void print() { eos::print(quantity, " ", Name(CurrencyType)); }
};
/// @}
...
*  A price is written as  X  Base/Quote. Where X is a power of 10 which makes it
* simpler to just shift the decimal.
*  It supports the following operator: /, \, <=, <, ==, !=, >=, > and also print
* functionality
*
*  Example:
*  @code
...
*  MyBaseToQuotePrice zeroBaseToQuote(zeroBaseToken, zeroQuoteToken); // throws
...
*  @{
*/
template <typename BaseToken, typename QuoteToken> struct price {
  ...
  * @brief Operator returns a quote token given a base token and the conversion
  * price.
  * @param b - base token
  * @param q - price
  * @return quote token
  */
  friend QuoteToken operator/(BaseToken b, const price &q) {
    eos::print("operator/ ", uint128(b.quantity), " * ", uint128(precision),
               " / ", q.base_per_quote, "\n");
    return QuoteToken(uint64_t(
        (uint128(b.quantity) * uint128(precision) / q.base_per_quote)));
  }

  /**
  * Operator returns a base token given a quote token and the conversion price.
  * @brief Operator returns a base token given a quote token and the conversion
  * price.
  * @param b - quote token
  * @param q - price
  * @return base token
  */
  friend BaseToken operator*(const QuoteToken &b, const price &q) {
    eos::print("b: ", b, " \n");
    eos::print("operator* ", uint128(b.quantity), " * ",
               uint128(q.base_per_quote), " / ", precision, "\n");
    // return QuoteToken( uint64_t( mult_div_i128( b.quantity, q.base_per_quote,
    // precision ) ) );
  ...
  * @return true if base per quote of a is less than  or equal to base per quote
  * of b
  */
  friend bool operator<=(const price &a, const price &b) {
    return a.base_per_quote <= b.base_per_quote;
  }
  ...
  * @param a price to be compared
  * @param b price to be compared
  * @return true if base per quote of a is greater than or equal to base per
  * quote of b
  */
  friend bool operator>=(const price &a, const price &b) {
    return a.base_per_quote >= b.base_per_quote;
  ...
  */
  inline void print() {
    eos::print(base_per_quote, ".", " ", Name(base_token_type::currency_type),
               "/", Name(quote_token_type::currency_type));
  }
...
/**
*  @struct eos::Transfer
*  @brief The binary structure of the `transfer` message type for the `eos`
* contract.
*  @ingroup tokens
*
*  @details
