 ...
 *
 * @note There are some methods from the @ref transactioncapi that can be used
 * directly from C++
 *
 * @{
 */
...
private:
  void assertValidHandle() {
    assert(handle != InvalidMessageHandle,
...
class Transaction {
public:
  Transaction() : handle(transactionCreate()) {}
  ...
  TransactionHandle get() { return handle; }

private:
  void assertValidHandle() {
    assert(handle != InvalidTransactionHandle,
