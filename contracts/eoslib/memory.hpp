...
public:
  memory_manager()
      // NOTE: it appears that WASM has an issue with initialization lists if
      // the object is globally allocated,
      //       and seems to just initialize members to 0
      : _heaps_actual_size(0), _active_heap(0), _active_free_heap(0) {}
...
private:
  ...
  memory *next_active_heap() {
    ...
    // make sure we will not exceed the 1M limit (needs to match
    // wasm_interface.cpp _max_memory)
    ...
    const uint32_t new_heap_size =
        remaining > NEW_HEAP_SIZE ? NEW_HEAP_SIZE : remaining;
  ...
  void *realloc(void *ptr, uint32_t size) {
    ...
    if (ptr != nullptr) {
      ...
      for (memory *realloc_heap = _available_heaps;
           realloc_heap < _available_heaps + _heaps_actual_size &&
           realloc_heap->is_init();
           ++realloc_heap) {
        if (realloc_heap->is_in_heap(char_ptr)) {
          realloc_ptr =
              realloc_heap->realloc_in_place(char_ptr, size, &orig_ptr_size);
  ...
  void free(void *ptr) {
    ...
    for (memory *free_heap = _available_heaps;
         free_heap < _available_heaps + _heaps_actual_size &&
         free_heap->is_init();
         ++free_heap) {
  ...
  public:
    ...
    uint32_t is_init() const { return _heap != nullptr; }
    ...
    char *malloc_from_freed(uint32_t size) {
      assert(_offset == _heap_size, "malloc_from_freed was designed to only be "
                                    "called after _heap was completely "
                                    "allocated");
    ...
    char *realloc_in_place(char *const ptr, uint32_t size,
  ...
  private:
    ...
    public:
      buffer_ptr(void *ptr, const char *const heap_end)
          : _ptr(static_cast<char *>(ptr)),
            _size(*reinterpret_cast<uint32_t *>(static_cast<char *>(ptr) -
                                                SIZE_MARKER) &
                  ~ALLOC_MEMORY_MASK),
            _heap_end(heap_end) {}
      ...
      void size(uint32_t val) {
        ...
        const uint32_t memory_state =
            *reinterpret_cast<uint32_t *>(_ptr - SIZE_MARKER) &
            ALLOC_MEMORY_MASK;
        *reinterpret_cast<uint32_t *>(_ptr - SIZE_MARKER) = val | memory_state;
        _size = val;
      }

      char *end() const { return _ptr + _size; }
      ...
      bool is_alloc() const {
        return *reinterpret_cast<const uint32_t *>(_ptr - SIZE_MARKER) &
               ALLOC_MEMORY_MASK;
      }
    ...
    private:
      bool merge_contiguous(uint32_t needed_size, bool all_or_nothing) {
        ...
        while (possible_size < needed_size &&
               (_ptr + possible_size < _heap_end)) {
          const uint32_t next_mem_flag_size =
              *reinterpret_cast<const uint32_t *>(_ptr + possible_size);
          // if ALLOCed then done with contiguous free memory
          if (next_mem_flag_size & ALLOC_MEMORY_MASK)
            break;

          possible_size +=
              (next_mem_flag_size & ~ALLOC_MEMORY_MASK) + SIZE_MARKER;
        }
        ...
        const uint32_t new_size =
            possible_size < needed_size ? possible_size : needed_size;
        size(new_size);

        if (possible_size > needed_size) {
          const uint32_t freed_size = possible_size - needed_size - SIZE_MARKER;
          buffer_ptr freed_remainder(_ptr + needed_size + SIZE_MARKER,
                                     freed_size, _heap_end);
  ...
  // if sbrk is not called outside of this file, then this is the max times we
  // can call it
 ...
 * @endcode
 */
inline void *malloc(uint32_t size) { return memory_heap.malloc(size); }
 ...
 * @endcode
 */
inline void free(void *ptr) { return memory_heap.free(ptr); }
/// @} /// mathcppapi
}
