...
template <typename T> struct table_impl_obj {};

template <> struct table_impl_obj<char *> {

  static int32_t store(AccountName scope, TableName table, char *key,
                       uint32_t keylen, char *data, uint32_t datalen) {
    return store_str(scope, table, key, keylen, data, datalen);
  }

  static int32_t update(AccountName scope, TableName table, char *key,
                        uint32_t keylen, char *data, uint32_t datalen) {
    return update_str(scope, table, key, keylen, data, datalen);
  }

  static int32_t front(AccountName scope, AccountName code, TableName table,
                       char *data, uint32_t len) {
    return front_str(scope, code, table, data, len);
  }

  static int32_t back(AccountName scope, AccountName code, TableName table,
                      char *data, uint32_t len) {
    return back_str(scope, code, table, data, len);
  }

  static int32_t load(AccountName scope, AccountName code, TableName table,
                      char *key, uint32_t keylen, char *data,
                      uint32_t datalen) {
    return load_str(scope, code, table, key, keylen, data, datalen);
  }

  static int32_t next(AccountName scope, AccountName code, TableName table,
                      char *key, uint32_t keylen, char *data,
                      uint32_t datalen) {
    return next_str(scope, code, table, key, keylen, data, datalen);
  }

  static int32_t previous(AccountName scope, AccountName code, TableName table,
                          char *key, uint32_t keylen, char *data,
                          uint32_t datalen) {
    return previous_str(scope, code, table, key, keylen, data, datalen);
  }

  static int32_t lower_bound(AccountName scope, AccountName code,
                             TableName table, char *key, uint32_t keylen,
                             char *data, uint32_t datalen) {
    return lower_bound_str(scope, code, table, key, keylen, data, datalen);
  }

  static int32_t upper_bound(AccountName scope, AccountName code,
                             TableName table, char *key, uint32_t keylen,
                             char *data, uint32_t datalen) {
    return upper_bound_str(scope, code, table, key, keylen, data, datalen);
  }

  static int32_t remove(AccountName scope, TableName table, char *key,
                        uint32_t keylen) {
...
public:
  ...
  int32_t lower_bound(Primary key, uint32_t keylen, char *record,
                      uint32_t len) {
    return impl::lower_bound(scope, code, table, key, keylen, record, len);
  }

  int32_t upper_bound(Primary key, uint32_t keylen, char *record,
                      uint32_t len) {
...
template <int Primary, int Secondary> struct table_impl {};

template <> struct table_impl<sizeof(uint128_t), sizeof(uint128_t)> {

  /**
  *  @param scope - the account scope that will be read, must exist in the
  * transaction scopes list
  *  @param code  - identifies the code that controls write-access to the data
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  ...
  */
  static int32_t front_primary(uint64_t scope, uint64_t code, uint64_t table,
  ...
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t back_primary(uint64_t scope, uint64_t code, uint64_t table,
                              void *data, uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - location to copy the data; must be initialized with the
  * primary key.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t load_primary(uint64_t scope, uint64_t code, uint64_t table,
                              void *data, uint32_t len) {
  ...
  * query
  *  @param data  - location to copy the data; must be initialized with the
  * primary key to fetch next record.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t next_primary(uint64_t scope, uint64_t code, uint64_t table,
                              void *data, uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - location to copy the record; must be initialized with the
  * primary key to fetch previous record.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t previous_primary(uint64_t scope, uint64_t code, uint64_t table,
                                  void *data, uint32_t len) {
  ...
  * query
  *  @param data  - location to copy the record. must be initialized with the
  * primary key to fetch upper bound of.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t upper_bound_primary(uint64_t scope, uint64_t code,
                                     uint64_t table, void *data, uint32_t len) {
  ...
  * query
  *  @param data  - location to copy the record. must be initialized with the
  * primary key to fetch lower bound of.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t lower_bound_primary(uint64_t scope, uint64_t code,
                                     uint64_t table, void *data, uint32_t len) {
  ...
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t front_secondary(uint64_t scope, uint64_t code, uint64_t table,
                                 void *data, uint32_t len) {
  ...
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t back_secondary(uint64_t scope, uint64_t code, uint64_t table,
                                void *data, uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - location to copy the record based on secondary index; must
  * be initialized with secondary key.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t load_secondary(uint64_t scope, uint64_t code, uint64_t table,
                                void *data, uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - location to copy the next record based on secondary index;
  * must be initialized with a key.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t next_secondary(uint64_t scope, uint64_t code, uint64_t table,
                                void *data, uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - location to copy the previous record; must be initialized
  * with a key.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t previous_secondary(uint64_t scope, uint64_t code,
                                    uint64_t table, void *data, uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - location to copy the upper bound record; must be initialized
  * with a key to find the upper bound.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t upper_bound_secondary(uint64_t scope, uint64_t code,
                                       uint64_t table, void *data,
                                       uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - location to copy the lower bound record; must be initialized
  * with a key to find the lower bound.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t lower_bound_secondary(uint64_t scope, uint64_t code,
                                       uint64_t table, void *data,
                                       uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - must be initialized with the key (primary, secondary) of the
  * record to remove.
  *
  ...
  */
  static int32_t store(AccountName scope, TableName table, const void *data,
                       uint32_t len) {
  ...
  */
  static int32_t update(AccountName scope, TableName table, const void *data,
                        uint32_t len) {
    return update_i128i128(scope, table, data, len);
  }
};

template <> struct table_impl<sizeof(uint64_t), 0> {
  /**
  *  @param scope - the account scope that will be read, must exist in the
  * transaction scopes list
  *  @param code  - identifies the code that controls write-access to the data
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - location to copy the next record; must be initialized with a
  * key.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t next(uint64_t scope, uint64_t code, uint64_t table, void *data,
                      uint32_t len) {
  ...
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t previous(uint64_t scope, uint64_t code, uint64_t table,
                          void *data, uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - location to copy the lower bound; must be initialized with a
  * key.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t lower_bound(uint64_t scope, uint64_t code, uint64_t table,
                             void *data, uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - location to copy the upper bound; must be initialized with a
  * key.
  *  @param len - the maximum length of data to read
  *
  *  @return the number of bytes read or -1 if no record found
  */
  static int32_t upper_bound(uint64_t scope, uint64_t code, uint64_t table,
                             void *data, uint32_t len) {
  ...
  *  @param table - the ID/name of the table within the scope/code context to
  * query
  *  @param data  - must be initialized with the key of record to remove from
  * table.
  *
 ...
 *
 *  @tparam scope         - the default account name/scope that this table is
 * located within
 *  @tparam code          - the code account name which has write permission to
 * this table
 ...
 *
 *  The primary and secondary indices are sorted as N-bit unsigned integers from
 * lowest to highest.
 *
 *  @code
 *  struct Model {
 ...
 *  };
 *
 *  typedef Table<N(myscope), N(mycode), N(mytable), Model, uint64_t, uint64_t>
 * MyTable;
 ...
 *  ASSERT(res && query.secondary == 22 && query.value == N(second),
 * "previous");
 *
...
private:
  ...
  static_assert(sizeof(PrimaryType) + sizeof(SecondaryType) <= sizeof(Record),
...
public:
  ...
  */
  ...
  struct PrimaryIndex {
    /**
    *  @param r - reference to a record to store the front record based on
    * primary index.
    ...
    */
    static bool front(Record &r, uint64_t s = scope) {
      return impl::front_primary(s, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }

    /**
    *  @param r - reference to a record to store the back record based on
    * primary index.
    *  @param s - account scope. default is current scope of the class
    *
    *  @return true if successful read.
    */
    static bool back(Record &r, uint64_t s = scope) {
      return impl::back_primary(s, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }

    /**
    *  @param r - reference to a record to store next value; must be initialized
    * with current.
    *  @param s - account scope. default is current scope of the class
    *
    *  @return true if successful read.
    */
    static bool next(Record &r, uint64_t s = scope) {
      return impl::next_primary(s, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }

    /**
    *  @param r - reference to a record to store previous value; must be
    * initialized with current.
    *  @param s - account scope. default is current scope of the class
    *
    *  @return true if successful read.
    */
    static bool previous(Record &r, uint64_t s = scope) {
      return impl::previous_primary(s, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }

    /**
    *  @param p - reference to primary key to load; must be initialized with a
    * value;
    ...
    */
    static bool get(const PrimaryType &p, Record &r, uint64_t s = scope) {
      *reinterpret_cast<PrimaryType *>(&r) = p;
      return impl::load_primary(s, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }

    /**
    *  @param p - reference to primary key to get the lower bound of; must be
    ...
    */
    static bool lower_bound(const PrimaryType &p, Record &r) {
      return impl::lower_bound_primary(scope, code, table, &p & r,
                                       sizeof(Record)) == sizeof(Record);
    }

    /**
    *  @param p - reference to primary key to get the upper bound of; must be
    * initialized with a value;
    *  @param r - reference to a record to load the value to.
    *  @param s - account scope. default is current scope of the class
    *
    ...
    */
    static bool upper_bound(const PrimaryType &p, Record &r) {
      return impl::upper_bound_primary(scope, code, table, &p & r,
                                       sizeof(Record)) == sizeof(Record);
    }
  ...
  */
  ...
  struct SecondaryIndex {
    ...
    */
    static bool front(Record &r, uint64_t s = scope) {
      return impl::front_secondary(s, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }
    ...
    */
    static bool back(Record &r, uint64_t s = scope) {
      return impl::back_secondary(s, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }
    ...
    */
    static bool next(Record &r, uint64_t s = scope) {
      return impl::next_secondary(s, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }
    ...
    */
    static bool previous(Record &r, uint64_t s = scope) {
      return impl::previous_secondary(s, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }
    ...
    */
    static bool get(const SecondaryType &p, Record &r, uint64_t s = scope) {
      return impl::load_secondary(s, code, table, &p & r, sizeof(Record)) ==
             sizeof(Record);
    }

    /**
    *  @param p - reference to secondary key to get the lower bound of; must be
    * initialized with a value;
    *  @param r - reference to a record to load the value to.
    *  @param s - account scope. default is current scope of the class
    *
    ...
    */
    ...
    static bool lower_bound(const SecondaryType &p, Record &r,
                            uint64_t s = scope) {
      return impl::lower_bound_secondary(s, code, table, &p & r,
                                         sizeof(Record)) == sizeof(Record);
    }

    /**
     *  @param p - reference to secondary key to get the upper bound of; must be
     * initialized with a value;
     *  @param r - reference to a record to load the value to.
     *  @param s - account scope. default is current scope of the class
     *
    ...
    static bool upper_bound(const SecondaryType &p, Record &r,
                            uint64_t s = scope) {
      return impl::upper_bound_secondary(s, code, table, &p & r,
                                         sizeof(Record)) == sizeof(Record);
    }
 ...
 *
 *  @tparam scope - the default account name scope that this table is located
 within
 *  @tparam code  - the code account name which has write permission to this
 table
 *  @tparam table - a unique identifier (name) for this table
 *  @tparam Record - the type of data stored in each row
 *  @tparam PrimaryType - the type of the first field stored in @ref Record
 ...
 *  };
 *
 *  typedef Table<N(myscope), N(mycode), N(mytable), MyModel, uint128_t>
 MyTable;
 *
 *  MyModel a { 1, N(one) };
 *  MyModel b { 2, N(two) };
...
private:
  ...
  static_assert(sizeof(PrimaryType) <= sizeof(Record),
                "invalid template parameters");
...
public:
  ...
  struct PrimaryIndex {
    ...
    */
    static bool front(Record &r) {
      return impl::front_primary(scope, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }
    ...
    */
    static bool back(Record &r) {
      return impl::back_primary(scope, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }

    /**
    *  @param r - reference to store the next record. Must be initialized with a
    * key.
    *
    *  @return true if successfully retrieved the next record.
    */
    static bool next(Record &r) {
      return impl::next_primary(scope, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }

    /**
    *  @param r - reference to store previous record. Must be initialized with a
    * key.
    *
    *  @return true if successfully retrieved the previous record.
    */
    static bool previous(Record &r) {
      return impl::previous_primary(scope, code, table, &r, sizeof(Record)) ==
             sizeof(Record);
    }
  ...
  */
   ...
   * @param r - reference of a record to hold return value; must be initialized
   * to the primary key to be fetched.
   * @param s - scope; defaults to scope of the class.
   * @return true if get succeeds.
   */
   ...
   * @param r - the record to be updated including the updated values (cannot
   * update the index).
   * @param s - scope; defaults to scope of the class.
...
#define TABLE2(NAME, SCOPE, CODE, TABLE, TYPE, PRIMARY_NAME, PRIMARY_TYPE,     \
               SECONDARY_NAME, SECONDARY_TYPE)                                 \
  using NAME =                                                                 \
      Table<N(SCOPE), N(CODE), N(TABLE), TYPE, PRIMARY_TYPE, SECONDARY_TYPE>;  \
