...
public:
   ...
   */
  string(char *d, uint32_t s, bool copy) { assign(d, s, copy); }
  ...
  const bool is_own_memory() const { return own_memory; }

  // Get the ref count
  const uint32_t get_refcount() const { return *refcount; }
  ...
  string &operator+=(const string &str) {
    assert((size + str.size > size) && (size + str.size > str.size),
           "overflow");
  ...
  // The return value also represents the difference between the first character
  // that doesn't match of the two strings
  int32_t compare(const string &str) const {
    ...
    if (size == str.size) {
      ...
      result = memcmp(data, str.data, size);
      if (result == 0) {
        // String is equal up to size of the shorter string, return the
        // difference in byte of the next character
