...
/*
memory* memory_manager::next_active_heap()
{
...
void* memory_manager::malloc(uint32_t size)
{
   if (size == 0)
      return nullptr;
...
void* memory_manager::realloc(void* ptr, uint32_t size)
{
   if (size == 0)
      {
         free(ptr);
...
void memory_manager::adjust_to_mem_block(uint32_t& size)
{
   ...
   , _offset(0)
{
   ...
   void memory_manager::memory::init(char* const mem_heap, uint32_t size)
   {
      _heap_size = size;
      _heap = mem_heap;
   ...
   uint32_t memory_manager::memory::is_init() const
   {
      return _heap != nullptr;
   }

   uint32_t memory_manager::memory::is_in_heap(const char* const ptr) const
   {
   ...
   uint32_t memory_manager::memory::is_capacity_remaining() const
   {
      return _offset + _size_marker < _heap_size;
   }

   char* memory_manager::memory::malloc(uint32_t size)
   {
   ...
   char* memory_manager::memory::malloc_from_freed(uint32_t size)
   {
   ...
   char* memory_manager::memory::realloc_in_place(char* const ptr, uint32_t size, uint32_t* orig_ptr_size)
   {
      const char* const end_of_buffer = _heap + _heap_size;
   ...
   void memory_manager::memory::free(char* ptr)
   {
      buffer_ptr to_free(ptr, _heap + _heap_size);
      to_free.mark_free();
   }

   void memory_manager::memory::cleanup_remaining()
   {
   ...
   bool memory_manager::memory::expand_memory(char* exp_mem, uint32_t size)
   {
