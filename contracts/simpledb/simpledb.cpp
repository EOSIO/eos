...
void apply(uint64_t code, uint64_t action) {
  if (code == N(simpledb)) {
    if (action == N(insertkv1)) {
      // eosc push message simpledb insertkv1 '{"key":"a", "value":"aa"}' -S
      // simpledb
      ...
      uint32_t err = store_str(N(simpledb), N(keyvalue1), key, key_len, str,
                               ((uint32_t)*str) + 1);
    } else if (action == N(insertkv2)) {
      // eosc push message simpledb insertkv2 '{"key":"a",
      // "value":{"name":"aaa", "age":10}}' -S simpledb
      ...
      uint32_t err = store_str(N(simpledb), N(keyvalue2), key, key_len,
                               complex_data, complex_data_len);
