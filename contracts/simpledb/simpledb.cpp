   ...
   void apply( uint64_t code, uint64_t action ) {
      if( code == N(simpledb) ) {
         if( action == N(insertkv1) ) {
            ...
            eos::dump(kv1);
            Bytes bytes = eos::raw::pack(kv1.value);
            ...
            eos::dump(kv2);
            Bytes bytes = eos::raw::pack(kv2.value);
            ...
            auto tmp = eos::current_message<record1>();
            eos::print("Inserting record1\n");
            eos::dump(tmp);
            auto bytes = eos::raw::pack(tmp);
            ...
            auto tmp = eos::current_message<record2>();
            ...
            auto tmp = eos::current_message<record3>();
