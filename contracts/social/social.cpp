...
/**
 *  The purpose of this contract is to implement something like DPAY on ARISEN, this
...
void apply_social_vote() {
   ...
   if( context == vote.author ) {
      ...
      arisen_assert( Db::get( vote.postid, post ) > 0, "unable to find post" );
      ...
      arisen_assert( abs_vote <= vote_account.vote_power, "insufficient vote power" );
      post.votes += vote.vote_power;
