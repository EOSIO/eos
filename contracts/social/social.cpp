...
/**
 *  The purpose of this contract is to implement something like Steem on EOS,
 * this
 *  means this contract defines its own currency, allows people to create posts,
 * vote
 *  on posts, and stake their voting power.
 *
 *  Unlike Steem, the goal is to enable maximum parallelism and enable the
 * currency to
...
/**
 * When a user posts we create a record that tracks the total votes and the time
 * it
 * was created. A user can submit this action multiple times, but subsequent
 * calls do
 * nothing.
 *
...
/**
 * This action is called when a user casts a vote, it requires that this code is
 * executed
 * in the context of both the voter and the author. When executed in the
 * author's context it
...
void apply_social_vote() {
  ...
  disableContextCode(vote.author()); /// prevent the author's code from
                                     /// rejecting the potentially negative vote
  ...
  if (context == vote.author) {
    ...
    account.vote_power =
        max(account.social_power,
            account.vote_power +
                (account.social_power * (now() - last_vote)) / days(7));
