# ARISEN Testnet
To date, all work done to experiment with the ARISEN blockchain has been performed using a single instance of rsnd hosting all 21 block producers. While this is a perfectly valid solution for validating features of the blockchain, developing new contracts, or whatever, it does not scale. Nor does it expose the sort of issues raised when contract and block data must be shared across multiple instances. Providing the ability to scale involves deploying multiple rsnd nodes across many hosts and lining then into a peer-to-peer (p2p) network. Composing this network involves tailoring and distributing configuration files, coordinating starts and stops and other tasks.
...
Before getting into the details of the ARISEN testnet, lets clarify some terms. In this document I use the terms "host" and "machine" fairly interchangeably. A host generally boils down to a single IP address, although in practice it could have more.

The next term is "node." A node is an instance of the rsnd executable configured to serve as 0 or more producers. There is not a one-to-one mapping between nodes and hosts, a host may serve more than one node, but one node cannot span more than one host.
...
Finally there is the idea of distributed networks that involve remote hosts. These may be hosts on which you may not have direct access for starting and stopping rsnd instances, but with whom you may wish to collaborate for setting up a decentralized testnet.
...
Note that the Launcher will not push instances of rsnd to the remote hosts, you must prepare the various test network hosts separately.
...
#### Star network
![](https://github.com/arisenio/arisen/raw/master/star.png)
...
#### Mesh network
![](https://github.com/arisenio/arisen/raw/master/mesh.png)
...
To address the complexity implied by distributing multiple rsnd nodes across a LAN or a wider network, the launcher application was created.
...
launcher command line arguments:
  ...
  -d [ --delay ] arg (=0)               number of seconds to wait before starting the next node. Used to simulate a person keying in a series of individual rsnd startup command lines.
  ...
  -o [ --output ] arg                   save a copy of the generated topology
                                        in this file
  --skip-signature                      RSND does not require transaction
                                        signatures.
...
In this mode, the launcher does not activate any rsnd instances, it produces a file of the given filename. This file is a JSON formatted template that provides an easy means of
...
```
  "nodes": [[
      "testnet_0",{
        ...
        "filesize": 8192,
        "keys": [{
            "public_key": "RSN7HknuwTB85iRC3iL8Fkc13PXkRyLoQnKbUMh4iXYY43orcxpjS",
            "wif_private_key": "5HrpMSjfpEtWkaJALRBNPNysX7mv3juwAnY2bLK4A1ofMMuD9Qq"
          }
        ],
        "peers": [
          "testnet_1",
...
| nodes         | a collection of descriptors defining the rsnd instances used to assemble this testnet. The names used as keys in this collection are also aliases used within as placeholders for peer nodes.
...
| rsn_root_dir       | specifies the directory into which all rsnd artifacts are based. This is required for any hosts that are not the local host.
...
It is also necessary to provision the server by at least copying the rsnd executable, and the genesis.json files to their appropriate locations relative to some named ARISEN root directory. For example, I defined the ARISEN root to be `/home/jared/blockchain/arisen`. When run, the launcher will run through a variety of shell commands using ssh and finally using scp to copy a config.ini file to the appropriate data directory on the remote.
...
| :------------ | :----------
| config.ini    | The rsnd configuration file.
| rsnd.pid      | The process ID of the running rsnd instance.
| blockchain/*  | The blockchain backing store
| blocks/*      | The blockchain log store
| stderr.txt    | The cerr output from rsnd.
| stdout.txt    | The cout output from rsnd.
