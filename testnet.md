...
Hooking together multple eosd nodes into a peer-to-peer (p2p) network provides this extra level of scalablity. Doing so requires tailoring and distributing configuration files, coordinating starts and stops and other tasks.
...
Before getting into the details of the EOS testnet, lets clarify some terms. First, in this document, I use the terms "host" and "machine" fairly interchagably. A host generally boils down to a single IP address, although in practice it could have more.

Anyway, the next term is "node." A node is an instance of the eosd executable which is configured to serve as 0 or more producers. There is not a one-to-one mapping between nodes and hosts, a host may serve more than one node, but one node cannot span more than one host. 

The next term of distinction is "local network" which I use to refer to any group of nodes, whther on a single host or several, are all close in that access does not have to leave a secure netowrk environment. 

Finally there is the idea of distributed networks that involve remote hosts. These may be hosts on which you may not have direct access for starting and stopping eosd isntances, but with whom you may wish to collaborate for setting up a decentralized testnet.

### Localhost networks
Running a test network on a single machine is the quickest way to get started. As you will see below, this is the default mode for the Launcher application. You can set up a localhost network immediately by simply telling the launcher how many producing or non-producing nodes to activate, and perhaps what type of network topology to use.

The downside is that you need a lot of hardware when running many nodes on a single host. Also the multiple nodes will contend with each other in terms of CPU cycles, liiting true concurrency, and also localhost network performance is much different from iter-host peformance, even with very high speed lans.
...
In cases where a testnet spans multiple remote networks, a common launcher defined configuration file may be shared externally between distrubted operatores, each being responsible for launching his or her own local subset.
...
Based on a handful of commandline arguments the Launcher is able to compose per-node configuration files, distribute these files securely amongst the peer hosts, then start up the multple instances of eosd.
...
To support deploying distributed networks, the launcher will read more detailed configuation from a JSON file. You can use the launcher to create a default JSON file based on the command line options you supply. Edit that file to substute actual hostnames and other details 
...
Here is the current list of command line arguments recognied by the launcher.
...
|per-node elememnts | Description
|:-------- | :----------
...
filesize | sets the capacity in megabites for the size of the blockchain backing store file. 
...
producers | this list identifies which of the producers from the genesis.json file are held by this node. Note that the launcher uses a round-robin algorithm to spread the producer instances accoss the producing nodes.
