	...
	if [ "${OS_MIN}" -lt 12 ]; then
		echo "You must be running Mac OS 10.12.x or higher to install ARISEN."
		echo "Exiting now."
		exit 1
	fi

	if [ "${DISK_AVAIL}" -lt "$DISK_MIN" ]; then
		echo "You must have at least ${DISK_MIN}GB of available storage to install ARISEN."
		echo "Exiting now."
		exit 1
	fi
	...
	do
		...
		(( COUNT++ ))
	done < "${SOURCE_DIR}/scripts/arisen_build_dep"
	IFS="${var_ifs}"
