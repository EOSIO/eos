    ...
    if [ ! -e "/usr/local/lib/libmongocxx-static.a" ]; then
		...
		if [ "${STATUS}" -ne 200 ]; then
			rm -f "${TEMP_DIR}/mongo-c-driver-1.10.1.tar.gz" 2>/dev/null
		...
		then
			printf "\\tUnable to unarchive file %s/mongo-c-driver-1.10.1.tar.gz.\\n" "${TEMP_DIR}"
			printf "\\tExiting now.\\n\\n"
			exit 1;
		fi
		...
		then
			printf "\\tUnable to enter directory %s/mongo-c-driver-1.10.1.\\n" "${TEMP_DIR}"
			printf "\\tExiting now.\\n\\n"
			exit 1;
		fi
