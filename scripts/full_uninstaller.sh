      ...
      1 | [Yy]* )
         if [ -d "$HOME/opt/eosio" ] || [[ $1 == "force-new" ]]; then
            if [ $( uname ) == "Darwin" ]; then
               ...
               if [ $ANSWER != 1 ]; then read -p "Do you wish to uninstall and unlink all brew installed llvm@5 versions? (y/n) " ANSWER; fi
               case $ANSWER in
                  1 | [Yy]* )
                     brew uninstall llvm@5 --force
                     brew cleanup -s llvm@5
                  ;;
                  [Nn]* ) ;;
