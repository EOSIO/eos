...
if [ "${OS_VER}" -lt 6 ]; then
	printf "You must be running Mageia 6 or higher to install EOSIO.\\n"
	printf "Exiting now.\\n"
	exit 1;
fi

# procps-ng includes free command
if [[ -z "$( rpm -qi "procps-ng" 2>/dev/null | grep Name )" ]]; then dnf install -y procps-ng; fi
...
printf "\\nChecking Dnf installation...\\n"
if ! DNF=$( command -v dnf 2>/dev/null ); then
		printf "!! Dnf must be installed to compile EOS.IO !!\\n"
		printf "Exiting now.\\n"
		exit 1;
fi
printf " - Dnf installation found at %s.\\n" "${DNF}"


if [ $ANSWER != 1 ]; then read -p "Do you wish to update DNF repositories? (y/n) " ANSWER; fi
case $ANSWER in
	1 | [Yy]* )
		if ! sudo $DNF -y update; then
			printf " - DNF update failed.\\n"
			exit 1;
		else
			printf " - DNF update complete.\\n"
		fi
	;;
...
for (( i=0; i<${#DEP_ARRAY[@]}; i++ )); do
	pkg=$( rpm -qi --whatprovides "${DEP_ARRAY[$i]}" 2>/dev/null | grep Name )
	if [[ -z $pkg ]]; then
		DEP=$DEP" ${DEP_ARRAY[$i]} "
...
if [ "${COUNT}" -gt 1 ]; then
	...
	case $ANSWER in
		1 | [Yy]* )
			if ! sudo $DNF -y install ${DEP}; then
				printf " - DNF dependency installation failed!\\n"
				exit 1;
			else
				printf " - DNF dependencies installed successfully.\\n"
			fi
		;;
	...
	printf " - No required DNF dependencies to install.\\n"
fi
...
if [ "${BOOSTVERSION}" != "${BOOST_VERSION_MAJOR}0${BOOST_VERSION_MINOR}0${BOOST_VERSION_PATCH}" ]; then
        ...
        echo https://dl.bintray.com/boostorg/release/${BOOST_VERSION_MAJOR}.${BOOST_VERSION_MINOR}.${BOOST_VERSION_PATCH}/source/boost_$BOOST_VERSION.tar.bz2
