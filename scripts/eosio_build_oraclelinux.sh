...
( [[ $NAME == "Oracle Linux Server" ]] && [[ "$(echo ${VERSION} | sed 's/ .*//g')" < 7 ]] ) && echo " - You must be running Oracle Linux 7 or higher to install EOSIO." && exit 1
...
ensure-yum-packages "${REPO_ROOT}/scripts/eosio_build_oraclelinux7_deps"
