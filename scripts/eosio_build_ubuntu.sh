...
mongodconf
	...
	printf "\\n\\tChecking for LLVM with TEMP_DIRport.\\n"
	...
	if [ ! -d "${RDKAFKA_DIR}" ]; then
		...
		if ! git clone https://github.com/edenhill/librdkafka.git
		then
			printf "\\tUnable to clone librdkafka repo @git ://github.com/edenhill/librdkafka.\\n"
			printf "\\n\\tExiting now.\\n"
			exit 1;
		fi
	...
	if [ ! -d "${CPPKAFKA_DIR}" ]; then
		...
		if ! git clone https://github.com/mfontanini/cppkafka
		then
			printf "\\tUnable to clone cppkafka repo @git ://github.com/mfontanini/cppkafka.\\n"
			printf "\\n\\tExiting now.\\n"
			exit 1;
		fi
