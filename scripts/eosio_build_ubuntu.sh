...
# llvm-5.0 is installed into /usr/lib/llvm-5.0
# clang is necessary for building on ubuntu
DEP_ARRAY=(
	git llvm-5.0 clang-5.0 libclang-5.0-dev llvm-4.0 clang-4.0 \
	make automake libbz2-dev libssl-dev autotools-dev build-essential autoconf libtool pkg-config \
	doxygen graphviz libgmp3-dev libicu-dev python2.7 python2.7-dev python3 python3-dev \
	curl zlib1g-dev sudo ruby libusb-1.0-0-dev libcurl4-gnutls-dev
)
COUNT=1
DISPLAY=""
DEP=""
...
if [ ! -d $LLVM_ROOT ]; then
	ln -s /usr/lib/llvm-5.0 $LLVM_ROOT \
	|| exit 1
	printf " - LLVM successfully linked from /usr/lib/llvm-5.0 to ${LLVM_ROOT}\\n"
else
