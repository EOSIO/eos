...
if [[ -z "$TEST" ]]; then # run all serial tests
    ...
    if [[ "$TEST_COUNT" > '0' ]]; then
        ...
        perform "ctest -L 'nonparallelizable_tests' --output-on-failure -T 'Test'"
        EXIT_STATUS=$?
        printdt 'Done running non-parallelizable tests.'
    else
    ...
    if [[ "$TEST_COUNT" > '0' ]]; then
        ...
        # run tests
        set +e # defer ctest error handling to end
        perform "ctest -R '^$TEST$' -V -T 'Test' 2>&1 | tee 'ctest-output.log'"
        EXIT_STATUS=$?
        printdt "Done running $TEST."
        [[ "$EXIT_STATUS" == '0' ]] && printdt "$TEST PASSED" || printdt "$TEST FAILED - see ctest-output.log in the artifacts tab"
        printdt 'Late blocks:'
        perform "grep -n 'produced the following blocks late' 'ctest-output.log'"
    else
