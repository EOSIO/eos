...
if [[ -z "$TEST" ]]; then # run all serial tests
    ...
    if [[ $TEST_COUNT > 0 ]]; then
        echo "$TEST found."
        # run tests
        programs/nodeos/nodeos --max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on "*" --p2p-max-nodes-per-host 1 --contracts-console --plugin eosio::producer_api_plugin  --config-dir etc/eosio/node_bios --data-dir var/lib/node_bios --genesis-json etc/eosio/node_bios/genesis.json --genesis-timestamp
        set +e # defer ctest error handling to end
