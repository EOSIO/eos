...
function build-clang() {
    if $BUILD_CLANG; then
        ...
        if [[ ! -d $CLANG_ROOT ]]; then
            ...
            && rm -rf clang12 \
            && git clone --single-branch --branch $PINNED_COMPILER_BRANCH https://github.com/llvm/llvm-project clang12 \
            && cd clang12 \
            && mkdir build && cd build \
            ...
            && make -j${JOBS} \
            && make install"
            echo " - Clang 12 successfully installed @ ${CLANG_ROOT}"
            echo ""
        else
            echo " - Clang 12 found @ ${CLANG_ROOT}"
            echo ""
        fi
        export CXX=$CPP_COMP
        export CC=$CC_COMP
