...
mongodconf
	...
	if [ ! -e "/usr/local/lib/libmongocxx-static.a" ]; then
		...
		then
			printf "\\n\\tUnable to unarchive file  %s/mongo-c-driver-1.10.1.tar.gz.\\n" "${TEMP_DIR}"
			printf "\\n\\tExiting now.\\n"
			exit 1;
		fi
		...
		then
			printf "\\n\\tUnable to enter directory %s/mongo-c-driver-1.10.1.\\n" "${TEMP_DIR}"
			printf "\\n\\tExiting now.\\n"
			exit 1;
		fi
		...
		printf "\\tSuccessfully installed Mongo C/C++ drivers @ /usr/local/lib.\\n"
	else
