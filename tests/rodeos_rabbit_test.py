...
class Rodeos:
    ...
    def restart(self):
        self.rodeos = subprocess.Popen(['./programs/rodeos/rodeos', '--plugin', 'b1::streamer_plugin', '--stream-loggers', 'topic-a,topic-b', '--stream-rabbits-exchange', 'amqp://guest:guest@localhost/dedup-exchange', '--stream-rabbits-immediately',  '--rdb-database', os.path.join(self.rodeosDir,'rocksdb'), '--data-dir', os.path.join(self.rodeosDir,'data'),
