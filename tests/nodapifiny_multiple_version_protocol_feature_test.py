...
###############################################################
# nodapifiny_multiple_version_protocol_feature_test
#
# Test for verifying that older versions of nodapifiny can work with newer versions of nodapifiny.
#
###############################################################
...
def restartNode(node: Node, nodeId, chainArg=None, addSwapFlags=None, nodapifinyPath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(nodeId, chainArg, addSwapFlags=addSwapFlags,
                                      timeout=5, cachePopen=True, nodapifinyPath=nodapifinyPath)
...
try:
    ...
    assert cluster.launch(pnodes=4, totalNodes=4, prodCount=1, totalProducers=4,
                          extraNodapifinyArgs=" --plugin apifiny::producer_api_plugin ",
    ...
    # Finally, when we restart the 4th node with the version of nodapifiny that supports protocol feature,
    ...
    oldNode.relaunch(oldNodeId, chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, nodapifinyPath="programs/nodapifiny/nodapifiny")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, oldNodeId, chainArg="--replay", nodapifinyPath="programs/nodapifiny/nodapifiny")
