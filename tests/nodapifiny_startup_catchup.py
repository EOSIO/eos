...
try:
    ...
    for nodeNum in range(txnGenNodeNum, txnGenNodeNum+startedNonProdNodes):
        specificExtraNodapifinyArgs[nodeNum]="--plugin apifiny::txn_test_gen_plugin --txn-test-gen-account-prefix txntestacct"
    ...
    if cluster.launch(prodCount=prodCount, onlyBios=False, pnodes=pnodes, totalNodes=totalNodes, totalProducers=pnodes*prodCount,
                      useBiosBootFile=False, specificExtraNodapifinyArgs=specificExtraNodapifinyArgs, unstartedNodes=catchupCount, loadSystemContract=False) is False:
    ...
    txnGenNodes[0].txnGenCreateTestAccounts(cluster.apifinyAccount.name, cluster.apifinyAccount.activePrivateKey)
