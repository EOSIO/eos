...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, walletMgr=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="EOStest"):
        ...
        self.cmd = cmd
        if Utils.Debug:
            Utils.Print("new Node host=%s, port=%s, pid=%s, cmd=%s" %
                        (self.host, self.port, self.pid, self.cmd))
        ...
        # initial process is started by launcher, this will only be set on relaunch
        self.popenProc = None
        if self.enableMongo:
    ...
    def eosClientArgs(self):
        walletArgs = " " + \
            self.walletMgr.getWalletEndpointArgs() if self.walletMgr is not None else ""
    ...
    def validateTransaction(trans):
        ...
        def printTrans(trans, status):
            Utils.Print("ERROR: Valid transaction should be \"%s\" but it was \"%s\"." % (
                executed, status))
    ...
    def __printTransStructureError(trans, context):
        Utils.Print(
            "ERROR: Failure in expected transaction structure. Missing trans%s." % (context))
        Utils.Print("Transaction: %s" % (json.dumps(trans, indent=1)))
    ...
    class Context:
        ...
        def add(self, newKey):
            assert isinstance(newKey, str), print(
                "ERROR: Trying to use %s as a key" % (newKey))
            subSection = self.sections[-1]
            assert isinstance(subSection, dict), print(
                "ERROR: Calling \"add\" method when context is not a dictionary. %s in %s" % (self.__contextDesc(), self.__json()))
            assert newKey in subSection, print("ERROR: %s%s does not contain key \"%s\". %s" % (
                self.__contextDesc(), key, self.__json()))
        ...
        def index(self, i):
            assert isinstance(i, int), print(
                "ERROR: Trying to use \"%s\" as a list index" % (i))
            cur = self.getCurrent()
            assert isinstance(cur, list), print(
                "ERROR: Calling \"index\" method when context is not a list.  %s in %s" % (self.__contextDesc(), self.__json()))
            listLen = len(cur)
            assert i < listLen, print("ERROR: Index %s is beyond the size of the current list (%s).  %s in %s" % (
                i, listLen, self.__contextDesc(), self.__json()))
    ...
    def normalizeJsonObject(extJStr):
        ...
        tmpStr = re.sub(
            r'ISODate\("([\w|\-|\:|\.]+)"\)', r'"ISODate-\1"', tmpStr)
    ...
    def runMongoCmdReturnJson(cmd, subcommand, trace=False, exitOnError=False):
        ...
        if retId is not 0:
            errorMsg = "mongodb call failed. cmd=[ %s ] subcommand=\"%s\" - %s" % (
                ", ".join(cmd), subcommand, errs)
            if exitOnError:
    ...
    def getTransId(trans):
        ...
        assert "transaction_id" in trans, print("trans does not contain key %s. trans={%s}" % (
            "transaction_id", json.dumps(trans, indent=2, sort_keys=True)))
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            if Utils.Debug:
                Utils.Print("Validating account %s" % (account.name))
            ...
            try:
                ...
                Utils.Print("account validation failed. account: %s" %
                            (account.name))
                raise
    ...
    def getBlock(self, blockNum, silentErrors=False, exitOnError=False):
        ...
        if not self.enableMongo:
            ...
            subcommand = 'db.blocks.findOne( { "block_num": %d } )' % (
                blockNum)
            if Utils.Debug:
                Utils.Print("cmd: echo '%s' | %s" % (subcommand, cmd))
            start = time.perf_counter()
            try:
                block = Node.runMongoCmdReturnJson(
                    cmd.split(), subcommand, exitOnError=exitOnError)
                if Utils.Debug:
                ...
                if not silentErrors:
                    ...
                    errorMsg = "Exception during get db node get block.  cmd Duration: %.3f sec. %s" % (
                        end-start, msg)
                    if exitOnError:
                        Utils.cmdError(errorMsg)
                        Utils.errorExit(errorMsg)
    ...
    def getBlockByIdMdb(self, blockId, silentErrors=False):
        ...
        try:
            ...
            if not silentErrors:
                ...
                msg = ex.output.decode("utf-8")
                Utils.Print("ERROR: Exception during db get block by id.  cmd Duration: %.3f sec. %s" % (
                    end-start, msg))
            return None
    ...
    def isBlockPresent(self, blockNum, blockType=BlockType.head):
        ...
        try:
            ...
            Utils.Print("Failure in get info parsing %s block. %s" %
                        (blockType.type, info))
            raise
    ...
    def getTransaction(self, transId, silentErrors=False, exitOnError=False, delayedRetry=True):
        ...
        if not self.enableMongo:
            ...
            for i in range(0, (int(60/timeout) - 1)):
                trans = self.processCleosCmd(
                    cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnErrorForDelayed, exitMsg=msg)
                if trans is not None or not delayedRetry:
                    return trans
                if Utils.Debug:
                    Utils.Print(
                        "Could not find transaction with id %s, delay and retry" % (transId))
                time.sleep(timeout)
            ...
            for i in range(0, (int(60/timeout) - 1)):
                trans = self.getTransactionMdb(
                    transId, silentErrors=silentErrors, exitOnError=exitOnErrorForDelayed)
                if trans is not None or not delayedRetry:
                    return trans
                if Utils.Debug:
                    Utils.Print(
                        "Could not find transaction with id %s in mongodb, delay and retry" % (transId))
                time.sleep(timeout)
    ...
    def getTransactionMdb(self, transId, silentErrors=False, exitOnError=False):
        ...
        subcommand = 'db.transactions.findOne( { "trx_id": "%s" } )' % (
            transId)
        if Utils.Debug:
            Utils.Print("cmd: echo '%s' | %s" % (subcommand, cmd))
        start = time.perf_counter()
        try:
            trans = Node.runMongoCmdReturnJson(
                cmd.split(), subcommand, exitOnError=exitOnError)
            if Utils.Debug:
                end = time.perf_counter()
                Utils.Print("cmd Duration: %.3f sec" % (end-start))
            ...
            msg = ex.output.decode("utf-8")
            errorMsg = "Exception during get db node get trans in mongodb with transaction id=%s.  cmd Duration: %.3f sec.  %s" % (
                transId, end-start, msg)
            if exitOnError:
                Utils.cmdError("" % (errorMsg))
                Utils.errorExit(
                    "Failed to retrieve transaction in mongodb for transaction id=%s" % (transId))
        ...
        if transactions is not None:
            for trans in transactions:
                ...
                try:
                    ...
                    Utils.Print(
                        "transaction%s not found. Transaction: %s" % (key, trans))
    ...
    def getBlockIdByTransId(self, transId, delayedRetry=True):
        ...
        trans = self.getTransaction(
            transId, exitOnError=True, delayedRetry=delayedRetry)
        ...
        try:
            ...
            Utils.Print("transaction%s not found. Transaction: %s" %
                        (key, trans))
            return None
        ...
        try:
            ...
            Utils.Print("ERROR: Block info parsing failed. %s" %
                        (headBlockNum))
            raise

        if Utils.Debug:
            Utils.Print("Reference block num %d, Head block num: %d" %
                        (refBlockNum, headBlockNum))
        for blockNum in range(refBlockNum, headBlockNum+1):
            if self.isTransInBlock(str(transId), blockNum):
                if Utils.Debug:
                    Utils.Print("Found transaction %s in block %d" %
                                (transId, blockNum))
                return blockNum
    ...
    def getBlockIdByTransIdMdb(self, transId):
        ...
        try:
            ...
            Utils.Print(
                "transaction[ref_block_num] not found. Transaction: %s" % (trans))
            return None
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False, stakeNet=100, stakeCPU=100, buyRAM=10000, exitOnError=False):
        ...
        msg = "(creator account=%s, account=%s)" % (
            creatorAccount.name, account.name)
        trans = self.processCleosCmd(
            cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
        self.trackCmdTransaction(trans)
        transId = Node.getTransId(trans)

        if stakedDeposit > 0:
            # seems like account creation needs to be finalized before transfer can happen
            self.waitForTransInBlock(transId)
            trans = self.transferFunds(creatorAccount, account, Node.currencyIntToStr(
                stakedDeposit, CORE_SYMBOL), "init")
            transId = Node.getTransId(trans)
    ...
    def createAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False, exitOnError=False):
        ...
        if stakedDeposit > 0:
            ...
            self.waitForTransInBlock(transId)
            trans = self.transferFunds(creatorAccount, account, "%0.04f %s" % (
                stakedDeposit/10000, CORE_SYMBOL), "init")
            self.trackCmdTransaction(trans)
            transId = Node.getTransId(trans)
    ...
    def getEosAccountFromDb(self, name, exitOnError=False):
        ...
        try:
            ...
            if exitOnError:
                ...
                Utils.cmdError("Exception during get account from db for %s.  cmd Duration: %.3f sec.  %s" % (
                    name, end-start, msg))
                Utils.errorExit(
                    "Failed during get account from db for %s. %s" % (name, msg))

            Utils.Print(
                "ERROR: Exception during get account from db for %s. %s" % (name, msg))
            return None
    ...
    def getTableAccountBalance(self, contract, scope):
        ...
        try:
            ...
            print(
                "transaction[rows][0][balance] not found. Transaction: %s" % (trans))
            raise
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL, exitOnError=False):
        ...
        msg = "contract=%s, account=%s, symbol=%s" % (
        ...
        if not self.enableMongo:
            ...
            if ret is not None:
                ...
                if account_name is None:
                    Utils.Print(
                        "ERROR: Failed to verify account creation.", account.name)
                    return None
                return ret
        else:
    ...
    def waitForTransInBlock(self, transId, timeout=None):
        ...
        def lam(): return self.isTransInAnyBlock(transId)
    ...
    def waitForTransFinalization(self, transId, timeout=None):
        ...
        def lam(): return self.isTransFinalized(transId)
        ret = Utils.waitForBool(lam, timeout)
        return ret
    ...
    def waitForNextBlock(self, timeout=None, blockType=BlockType.head):
        ...
        def lam(): return self.getHeadBlockNum() > num
        ret = Utils.waitForBool(lam, timeout)
        return ret

    def waitForBlock(self, blockNum, timeout=None, blockType=BlockType.head, reportInterval=None):
        def lam(): return self.getBlockNum(blockType=blockType) > blockNum
        ...
        reporter = WaitReporter(
            self, reportInterval) if reportInterval is not None else None
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False, exitOnError=True):
        ...
        try:
            ...
            msg = ex.output.decode("utf-8")
            Utils.Print("ERROR: Exception during funds transfer.  cmd Duration: %.3f sec.  %s" % (
                end-start, msg))
            if exitOnError:
                Utils.cmdError("could not transfer \"%s\" from %s to %s" % (
                    amountStr, source, destination))
                Utils.errorExit("Failed to transfer \"%s\" from %s to %s" % (
                    amountStr, source, destination))
            return None
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        if len(currentBalances) != len(initialBalances):
            Utils.Print("ERROR: validateFunds> accounts length mismatch. Initial: %d, current: %d" % (
                len(initialBalances), len(currentBalances)))
            return False
    ...
    def getActionsMdb(self, account, pos=-1, offset=-1, exitOnError=False):
        ...
        cmd = "%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
        subcommand = 'db.action_traces.find({$or: [{"act.data.from":"%s"},{"act.data.to":"%s"}]}).sort({"_id":%d}).limit(%d)' % (
            account.name, account.name, pos, abs(offset))
        if Utils.Debug:
            Utils.Print("cmd: echo '%s' | %s" % (subcommand, cmd))
        start = time.perf_counter()
        try:
            actions = Node.runMongoCmdReturnJson(
                cmd.split(), subcommand, exitOnError=exitOnError)
            if Utils.Debug:
                end = time.perf_counter()
                Utils.Print("cmd Duration: %.3f sec" % (end-start))
            ...
            msg = ex.output.decode("utf-8")
            errorMsg = "Exception during get db actions.  cmd Duration: %.3f sec.  %s" % (
                end-start, msg)
            if exitOnError:
                Utils.cmdError(errorMsg)
                Utils.errorExit(errorMsg)
    ...
    def getAccountEosBalanceStr(self, scope):
        ...
        if Utils.Debug:
            Utils.Print("getNodeAccountEosBalance %s %s" % (scope, amount))
    ...
    def getAccountCodeHash(self, account):
        cmd = "%s %s get code %s" % (
            Utils.EosClientPath, self.eosClientArgs(), account)
        if Utils.Debug:
        ...
        try:
            ...
            msg = ex.output.decode("utf-8")
            Utils.Print("ERROR: Exception during code hash retrieval.  cmd Duration: %.3f sec.  %s" % (
                end-start, msg))
            return None
    ...
    def publishContract(self, account, contractDir, wasmFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd = "%s %s -v set contract -j %s %s" % (
            Utils.EosClientPath, self.eosClientArgs(), account, contractDir)
        ...
        if shouldFail:
            Utils.Print(
                "ERROR: The publish contract did not fail as expected.")
            return None
    ...
    def getTableRow(self, contract, scope, table, idx):
        if idx < 0:
            Utils.Print(
                "ERROR: Table index cannot be negative. idx: %d" % (idx))
            return None
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd = "%s %s push action -j %s %s" % (
            Utils.EosClientPath, self.eosClientArgs(), account, action)
        cmdArr = cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
        ...
        try:
            ...
            if not silentErrors:
                ...
                Utils.Print("ERROR: Exception during push message.  cmd Duration=%.3f sec.  %s" % (
                    end - start, msg))
            return (False, msg)
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False, exitOnError=False):
        ...
        cmd = "%s -j %s %s %s %s" % (cmdDesc, account,
                                     code, pType, requirement)
        trans = self.processCleosCmd(
            cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError)
        self.trackCmdTransaction(trans)
    ...
    def vote(self, account, producers, waitForTransBlock=False, exitOnError=False):
        ...
        msg = "account=%s, producers=[ %s ]" % (
            account.name, ", ".join(producers))
        trans = self.processCleosCmd(
            cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
        self.trackCmdTransaction(trans)
    ...
    def processCleosCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        ...
        try:
            ...
            if not silentErrors:
                ...
                msg = ex.output.decode("utf-8")
                errorMsg = "Exception during \"%s\". Exception message: %s.  cmd Duration=%.3f sec. %s" % (
                    cmdDesc, msg, end-start, exitMsg)
                if exitOnError:
                    Utils.cmdError(errorMsg)
                    Utils.errorExit(errorMsg)
                else:
    ...
    def killNodeOnProducer(self, producer, whereInSequence, blockType=BlockType.head, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        ...
        payload = "{ \"producer\":\"%s\", \"where_in_sequence\":%d, \"based_on_lib\":\"%s\" }" % (
            producer, whereInSequence, basedOnLib)
    ...
    def processCurlCmd(self, resource, command, payload, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        ...
        try:
            ...
            if Utils.Debug:
                ...
                Utils.Print("cmd Duration: %.3f sec" % (end-start))
                printReturn = json.dumps(
                    rtn) if returnType == ReturnType.json else rtn
                Utils.Print("cmd returned: %s" % (printReturn))
        except subprocess.CalledProcessError as ex:
            if not silentErrors:
                end = time.perf_counter()
                msg = ex.output.decode("utf-8")
                errorMsg = "Exception during \"%s\". %s.  cmd Duration=%.3f sec." % (
                    cmd, msg, end-start)
                if exitOnError:
                    Utils.cmdError(errorMsg)
                    Utils.errorExit(errorMsg)
    ...
    def waitForTransBlockIfNeeded(self, trans, waitForTransBlock, exitOnError=False):
        ...
        if not self.waitForTransInBlock(transId):
            if exitOnError:
                Utils.cmdError(
                    "transaction with id %s never made it to a block" % (transId))
                Utils.errorExit(
                    "Failed to find transaction with id %s in a block before timeout" % (transId))
            return None
        return trans

    def getInfo(self, silentErrors=False, exitOnError=False):
        cmdDesc = "get info"
        info = self.processCleosCmd(
            cmdDesc, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError)
        if info is None:
    ...
    def getBlockFromDb(self, idx):
        ...
        try:
            ...
            msg = ex.output.decode("utf-8")
            Utils.Print("ERROR: Exception during get db block.  cmd Duration: %.3f sec.  %s" % (
                end-start, msg))
            return None
    ...
    def kill(self, killSignal):
        if Utils.Debug:
            Utils.Print("Killing node: %s" % (self.cmd))
    ...
    def interruptAndVerifyExitStatus(self, timeout=15):
        if Utils.Debug:
            Utils.Print("terminating node: %s" % (self.cmd))
        assert self.popenProc is not None, "node: \"%s\" does not have a popenProc, this may be because it is only set after a relaunch." % (
            self.cmd)
        self.popenProc.send_signal(signal.SIGINT)
        try:
            outs, _ = self.popenProc.communicate(timeout=timeout)
            assert self.popenProc.returncode == 0, "Expected terminating \"%s\" to have an exit status of 0, but got %d" % (
                self.cmd, self.popenProc.returncode)
    ...
    def verifyAlive(self, silent=False):
        if not silent and Utils.Debug:
            Utils.Print("Checking if node(pid=%s) is alive(killed=%s): %s" % (
                self.pid, self.killed, self.cmd))
    ...
    def getBlockProducerByNum(self, blockNum, timeout=None, waitForBlock=True, exitOnError=True):
        ...
        if blockProducer is None and exitOnError:
            Utils.cmdError(
                "could not get producer for block number %s" % (blockNum))
    ...
    def getBlockProducer(self, timeout=None, waitForBlock=True, exitOnError=True, blockType=BlockType.head):
        ...
        block = self.getBlock(
            blockNum, exitOnError=exitOnError, blockType=blockType)
        blockProducer = block["producer"]
        if blockProducer is None and exitOnError:
            Utils.cmdError(
                "could not get producer for block number %s" % (blockNum))
    ...
    def getNextCleanProductionCycle(self, trans):
        ...
        Utils.Print("Searching for clean production cycle blockNum=%s ibn=%s  transId=%s  promoted bn=%s  ibn for schedule active=%s" % (
            blockNum, irreversibleBlockNum, transId, promotedBlockNum, ibnSchedActive))
    ...
    def relaunch(self, nodeId, chainArg=None, newChain=False, timeout=Utils.systemWaitTimeout, addOrSwapFlags=None, cachePopen=False, nodeosPath=None):
        ...
        assert(self.killed)
        assert isinstance(nodeId, int) or (isinstance(
            nodeId, str) and nodeId == "bios"), "Invalid Node ID is passed"

        if Utils.Debug:
            Utils.Print("Launching node process, Id: {}".format(nodeId))

        cmdArr = []
        splittedCmd = self.cmd.split()
        if nodeosPath:
            splittedCmd[0] = nodeosPath
        myCmd = " ".join(splittedCmd)
        toAddOrSwap = copy.deepcopy(
            addOrSwapFlags) if addOrSwapFlags is not None else {}
        if not newChain:
            skip = False
            swapValue = None
    ...
    def unstartedFile(nodeId):
        ...
        if not os.path.exists(startFile):
            Utils.errorExit(
                "Cannot find unstarted node since %s file does not exist" % startFile)
        return startFile
    ...
    def launchCmd(self, cmd, nodeId, cachePopen=False):
        ...
        with open(stdoutFile, 'w') as sout, open(stderrFile, 'w') as serr:
            ...
            self.pid = popen.pid
            if Utils.Debug:
                Utils.Print("start Node host=%s, port=%s, pid=%s, cmd=%s" % (
                    self.host, self.port, self.pid, self.cmd))
        ...
        if Utils.Debug:
            ...
            if transId in self.transCache.keys():
                replaceMsg = "replacing previous trans=\n%s" % json.dumps(
                    self.transCache[transId], indent=2, sort_keys=True)
            else:
                replaceMsg = ""
            Utils.Print("  cmd returned transaction id: %s, status: %s, (possible) block num: %s %s" % (
                transId, status, blockNum, replaceMsg))
    ...
    def reportStatus(self):
        ...
        Utils.Print(" hbn   : %s (%s)" %
                    (self.lastRetrievedHeadBlockNum, status))
    ...
    def scheduleProtocolFeatureActivations(self, featureDigests=[]):
        ...
        self.processCurlCmd(
            "producer", "schedule_protocol_feature_activations", json.dumps(param))
    ...
    def getSupportedProtocolFeatureDict(self, excludeDisabled=False, excludeUnactivatable=False):
        ...
        supportedProtocolFeatures = self.getSupportedProtocolFeatures(
            excludeDisabled, excludeUnactivatable)
            ...
            for spec in protocolFeature["specification"]:
                if (spec["name"] == "builtin_feature_codename"):
                    ...
                    if codename != "PREACTIVATE_FEATURE":
                        protocolFeatures.append(
                            protocolFeature["feature_digest"])
                    break
    ...
    def preactivateProtocolFeatures(self, featureDigests: list):
        for digest in featureDigests:
            ...
            if trans is None or not trans[0]:
                Utils.Print(
                    "ERROR: Failed to preactive digest {}".format(digest))
                return None
        self.waitForHeadToAdvance()
    ...
    def modifyBuiltinPFSubjRestrictions(self, nodeId, featureCodename, subjectiveRestriction={}):
        ...
        protocolFeatureJson["subjective_restrictions"].update(
