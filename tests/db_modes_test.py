...
def run_and_kill(extraCmd="", killsig=signal.SIGTERM, preCmd=""):
    ...
    # now start nodeos
    time.sleep(5)
    cmdArr = preCmd + nodeosCmd + \
        " --http-server-address=127.0.0.1:" + str(port) + extraCmd
    print("port %d is free now, start to run nodeosCmd with extra parameter \"%s\"" % (
        port, extraCmd))
...
for row in ulimits:
    if row.find("max locked memory") != -1:
        ...
        if row.find("unlimited") != -1:
            ...
            print(
                "skip default locked mode test because max locked memory is not unlimited")
        break
