...
# --dump-error-details <Upon error print etc/apifiny/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
try:
    ...
    if localTest and not dontLaunch:
        ...
        if cluster.launch(pnodes=prodCount, totalNodes=prodCount, prodCount=1, onlyBios=onlyBios,
                         ...
                         pfSetupPolicy=PFSetupPolicy.NONE, extraNodapifinyArgs=" --plugin apifiny::producer_api_plugin") is False:
            cmdError("launcher")
    ...
    retMap = node0.publishContract(cluster.apifinyAccount, contractDir, wasmFile, abiFile, True, shouldFail=True)
    ...
    time.sleep(0.6)
    retMap = node0.publishContract(cluster.apifinyAccount, contractDir, wasmFile, abiFile, True)
