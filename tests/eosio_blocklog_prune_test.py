...
try:
    ...
    assert cluster.launch(
        ...
        specificExtraNodeosArgs={
            0: "--plugin eosio::state_history_plugin --trace-history --disable-replay-opts --sync-fetch-span 200 --state-history-endpoint 127.0.0.1:8080 --plugin eosio::net_api_plugin ",
            1: "",
    ...
    def isBlockNumIrr(node):
        return node.getIrreversibleBlockNum() >= cfTrxBlockNum
    ...
    cluster.getBlockLog(0, blockLogAction=BlockLogAction.prune_transactions, extraArgs=" --block-num {} --transaction {}".format(trans_from_full["block_num"], cfTrxId), exitOnError=True)

    # try to prune the transaction where it doesn't belong
    try:
        cluster.getBlockLog(0, blockLogAction=BlockLogAction.prune_transactions, extraArgs=" --block-num {} --transaction {}".format(cfTrxBlockNum - 1, cfTrxId), throwException=True, silentErrors=True)
    except:
        ex = sys.exc_info()[0]
    ...
    #
    #  check if the cfd has been pruned from both validation node
    #
    trans = producerNode.processCleosCmd(cmd, cmd, silentErrors=False)
    ...
    # Check the producer node keep producing 
    ...
    for x in range(60):
        print("producer node head {}, lib {}".format(producerNode.getBlockNum(BlockType.head), producerNode.getBlockNum(BlockType.lib)))
        time.sleep(1)
