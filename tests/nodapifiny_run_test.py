...
# nodapifiny_run_test
#
# General test that tests a wide range of general use actions around nodapifiny and kapifinyd
#
###############################################################
...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.apifinyAccount)
    ...
    contractDir="unittests/contracts/apifiny.token"
    wasmFile="apifiny.token.wasm"
    abiFile="apifiny.token.abi"
    ...
    if not enableMongo:
        ...
        if abiName != "account" or abiActionName != "close" or abiType != "close":
            errorExit("FAILURE - get APIFINY account failed", raw=True)
