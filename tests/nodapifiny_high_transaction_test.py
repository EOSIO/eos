...
###############################################################
# nodapifiny_high_transaction_test
# 
...
try:
    ...
    accountsToCreate = [cluster.apifinyAccount]
    ...
    Print("Create new accounts via %s" % (cluster.apifinyAccount.name))
    # create accounts via apifiny as otherwise a bid is needed
    for account in accounts:
        trans = node.createInitializeAccount(account, cluster.apifinyAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
    ...
    Print("Transfer funds to new accounts via %s" % (cluster.apifinyAccount.name))
    for account in accounts:
        ...
        trans = node.transferFunds(cluster.apifinyAccount, account, transferAmount, "test transfer", waitForTransBlock=False, reportStatus=False, sign = True)
        checkTransIds.append(Node.getTransId(trans))
