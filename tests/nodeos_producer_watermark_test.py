...
def validBlockProducer(prodsActive, prodsSeen, blockNum, node):
    blockProducer = node.getBlockProducerByNum(blockNum)
    if blockProducer not in prodsActive:
        Utils.cmdError("unexpected block producer %s at blockNum=%s" %
                       (blockProducer, blockNum))
...
def setProds(sharedProdKey):
    ...
    trans = cluster.biosNode.pushMessage(
        "eosio", "setprods", setProdsStr, opts)
    if trans is None or not trans[0]:
        Utils.Print("ERROR: Failed to set producer with cmd %s" %
                    (setProdsStr))
...
def verifyProductionRounds(trans, node, prodsActive, rounds):
    ...
    Utils.Print(
        "Verify %s complete rounds of all producers producing" % (rounds))
    ...
    for i in range(0, rounds):
        ...
        for j in range(0, prodsSize):
            ...
            for k in range(0, 12):
                blockProducer = validBlockProducer(
                    prodsActive, prodsSeen, blockNum, node1)
                if lastBlockProducer != blockProducer:
                    if not reportFirstMissedBlock:
                        ...
                        for l in range(0, 36):
                            ...
                            printStr += "  "
                            newBlockNum += 1
                        Utils.Print("NOTE: expected blockNum %s (started from %s) to be produced by %s, but produded by %s: round=%s, prod slot=%s, prod num=%s - %s" % (
    ...
    if len(prodsSeenKeys) != prodsSize:
        Utils.cmdError("only saw %s producers of expected %d. At blockNum %s only the following producers were seen: %s" % (
