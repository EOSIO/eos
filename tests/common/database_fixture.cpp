...
testing_fixture::testing_fixture() {
  default_genesis_state.initial_timestamp =
      fc::time_point_sec(EOS_TESTING_GENESIS_TIMESTAMP);
  for (int i = 0; i < config::BlocksPerRound; ++i) {
    auto name = std::string("inita");
    name.back() += i;
    auto private_key = fc::ecc::private_key::regenerate(fc::sha256::hash(name));
    public_key_type public_key = private_key.get_public_key();
    default_genesis_state.initial_accounts.emplace_back(name, 0, 100000,
                                                        public_key, public_key);
    store_private_key(private_key);

    private_key =
        fc::ecc::private_key::regenerate(fc::sha256::hash(name + ".producer"));
...
fc::path testing_fixture::get_temp_dir(std::string id) {
  ...
  return named_temp_dirs.emplace(std::make_pair(id, fc::temp_directory()))
      .first->second.path();
}

const native_contract::genesis_state_type &
testing_fixture::genesis_state() const {
...
private_key_type
testing_fixture::get_private_key(const public_key_type &public_key) const {
  auto itr = key_ring.find(public_key);
  EOS_ASSERT(itr != key_ring.end(), missing_key_exception,
             "Private key corresponding to public key ${k} not known.",
             ("k", public_key));
  return itr->second;
}
...
testing_blockchain::testing_blockchain(chainbase::database &db,
                                       fork_database &fork_db,
                                       block_log &blocklog,
                                       chain_initializer_interface &initializer,
                                       testing_fixture &fixture)
    : chain_controller(
          db, fork_db, blocklog, initializer,
          native_contract::make_administrator(),
          ::eos::chain_plugin::DEFAULT_TRANSACTION_EXECUTION_TIME * 1000,
          ::eos::chain_plugin::
                  DEFAULT_RECEIVED_BLOCK_TRANSACTION_EXECUTION_TIME *
              1000,
          ::eos::chain_plugin::DEFAULT_CREATE_BLOCK_TRANSACTION_EXECUTION_TIME *
              1000),
      db(db), fixture(fixture) {}

void testing_blockchain::produce_blocks(uint32_t count,
                                        uint32_t blocks_to_miss) {
  if (count == 0)
    return;
  ...
  for (int i = 0; i < count; ++i) {
    ...
    generate_block(get_slot_time(slot), producer.owner, private_key,
                   block_schedule::in_single_thread,
                   chain_controller::created_block |
                       (skip_trx_sigs
...
types::Asset
testing_blockchain::get_liquid_balance(const types::AccountName &account) {
  return get_database()
      .get<BalanceObject, native::eos::byOwnerName>(account)
      .balance;
}

types::Asset
testing_blockchain::get_staked_balance(const types::AccountName &account) {
  return get_database()
      .get<StakedBalanceObject, native::eos::byOwnerName>(account)
      .stakedBalance;
}

types::Asset
testing_blockchain::get_unstaking_balance(const types::AccountName &account) {
  return get_database()
      .get<StakedBalanceObject, native::eos::byOwnerName>(account)
      .unstakingBalance;
}

std::set<types::AccountName>
testing_blockchain::get_approved_producers(const types::AccountName &account) {
  const auto &sbo =
      get_database().get<StakedBalanceObject, byOwnerName>(account);
...
types::PublicKey testing_blockchain::get_block_signing_key(
    const types::AccountName &producerName) {
  return get_database()
      .get<producer_object, by_owner>(producerName)
      .signing_key;
}
...
fc::optional<ProcessedTransaction>
testing_blockchain::push_transaction(SignedTransaction trx,
                                     uint32_t skip_flags) {
  if (skip_trx_sigs)
  ...
  return chain_controller::push_transaction(
      trx, skip_flags | chain_controller::pushed_transaction);
}
...
void testing_network::connect_blockchain(testing_blockchain &new_database) {
  ...
  // If the network isn't empty, sync the new database with one of the old ones.
  // The old ones are already in sync with
  // each other, so just grab one arbitrarily. The old databases are connected
  // to the propagation signals, so when one
  ...
  // The new database is now in sync with any old ones; go ahead and connect the
  // propagation signal.
  blockchains[&new_database] =
      new_database.applied_block.connect([this, &new_database](
          const signed_block &block) { propagate_block(block, new_database); });
}

void testing_network::disconnect_database(
    testing_blockchain &leaving_database) {
  blockchains.erase(&leaving_database);
}

void testing_network::disconnect_all() { blockchains.clear(); }
