...
/**
 * @file Contains support macros for the testcase helper macros. These macros
 * are implementation details, and thus
 * should not be used directly. Use their frontends instead.
 */

#define MKCHAIN1(name)                                                         \
  chainbase::database name##_db(                                               \
      get_temp_dir(), chainbase::database::read_write, TEST_DB_SIZE);          \
  block_log name##_log(get_temp_dir() / "blocklog");                           \
  fork_database name##_fdb;                                                    \
  native_contract::native_contract_chain_initializer name##_initializer(       \
      genesis_state());                                                        \
  testing_blockchain name(name##_db, name##_fdb, name##_log,                   \
                          name##_initializer, *this);                          \
  BOOST_TEST_CHECKPOINT("Created blockchain " << #name);
#define MKCHAIN2(name, id)                                                     \
  chainbase::database name##_db(                                               \
      get_temp_dir(#id), chainbase::database::read_write, TEST_DB_SIZE);       \
...
#define Complex_Authority_macro_Key(r, data, key_bubble)                       \
  data.keys.emplace_back(                                                      \
      BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2, 0, key_bubble), _public_key),        \
      BOOST_PP_TUPLE_ELEM(2, 1, key_bubble));
#define Complex_Authority_macro_Account(r, data, account_bubble)               \
  data.accounts.emplace_back(                                                  \
      types::AccountPermission{BOOST_PP_TUPLE_ELEM(3, 0, account_bubble),      \
...
#define MKACCT2(chain, name)                                                   \
  Make_Key(name) MKACCT_IMPL(                                                  \
      chain, name, inita, Key_Authority(name##_public_key),                    \
      Key_Authority(name##_public_key), Account_Authority(inita), Asset(100))
#define MKACCT3(chain, name, creator)                                          \
  Make_Key(name)                                                               \
      MKACCT_IMPL(chain, name, creator, Key_Authority(name##_public_key),      \
                  Key_Authority(name##_public_key),                            \
                  Account_Authority(creator), Asset(100))
#define MKACCT4(chain, name, creator, deposit)                                 \
  Make_Key(name) MKACCT_IMPL(                                                  \
      chain, name, creator, Key_Authority(name##_public_key),                  \
      Key_Authority(name##_public_key), Account_Authority(creator), deposit)
#define MKACCT5(chain, name, creator, deposit, owner)                          \
  Make_Key(name) MKACCT_IMPL(chain, name, creator, owner,                      \
                             Key_Authority(name##_public_key),                 \
                             Account_Authority(creator), deposit)
#define MKACCT6(chain, name, creator, deposit, owner, active)                  \
  MKACCT_IMPL(chain, name, creator, owner, active, Account_Authority(creator), \
              deposit)
...
#define SETCODE3(chain, acct, wast)                                            \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        trx, config::EosContractName,                                          \
        vector<types::AccountPermission>{{#acct, "active"}}, "setcode",        \
        handler);                                                              \
  }
...
#define SETAUTH5(chain, account, authname, parentname, auth)                   \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        trx, config::EosContractName,                                          \
        vector<types::AccountPermission>{{#account, "active"}}, "updateauth",  \
        types::updateauth{#account, authname, parentname, auth});              \
    trx.expiration = chain.head_block_time() + 100;                            \
    transaction_set_reference_block(trx, chain.head_block_id());               \
    chain.push_transaction(trx);                                               \
    BOOST_TEST_CHECKPOINT("Set " << #account << "'s " << authname              \
                                 << " authority.");                            \
  }
...
#define DELAUTH3(chain, account, authname)                                     \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        trx, config::EosContractName,                                          \
        vector<types::AccountPermission>{{#account, "active"}}, "deleteauth",  \
        types::deleteauth{#account, authname});                                \
    trx.expiration = chain.head_block_time() + 100;                            \
    transaction_set_reference_block(trx, chain.head_block_id());               \
    chain.push_transaction(trx);                                               \
    BOOST_TEST_CHECKPOINT("Deleted " << #account << "'s " << authname          \
                                     << " authority.");                        \
  }
...
#define LINKAUTH5(chain, account, authname, codeacct, messagetype)             \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        trx, config::EosContractName,                                          \
        vector<types::AccountPermission>{{#account, "active"}}, "linkauth",    \
        types::linkauth{#account, #codeacct, messagetype, authname});          \
    trx.expiration = chain.head_block_time() + 100;                            \
    transaction_set_reference_block(trx, chain.head_block_id());               \
    chain.push_transaction(trx);                                               \
    BOOST_TEST_CHECKPOINT("Link " << #codeacct << "::" << messagetype          \
                                  << " to " << #account << "'s " << authname   \
                                  << " authority.");                           \
  }
#define LINKAUTH4(chain, account, authname, codeacct)                          \
  LINKAUTH5(chain, account, authname, codeacct, "")
...
#define UNLINKAUTH4(chain, account, codeacct, messagetype)                     \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        trx, config::EosContractName,                                          \
        vector<types::AccountPermission>{{#account, "active"}}, "unlinkauth",  \
        types::unlinkauth{#account, #codeacct, messagetype});                  \
    trx.expiration = chain.head_block_time() + 100;                            \
    transaction_set_reference_block(trx, chain.head_block_id());               \
    chain.push_transaction(trx);                                               \
    BOOST_TEST_CHECKPOINT("Unlink " << #codeacct << "::" << messagetype        \
                                    << " from " << #account);                  \
  }
#define LINKAUTH3(chain, account, codeacct)                                    \
  LINKAUTH5(chain, account, codeacct, "")
...
#define XFER5(chain, sender, recipient, Amount, memo)                          \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        ...
        types::transfer{#sender, #recipient, Amount.amount, memo});            \
    trx.expiration = chain.head_block_time() + 100;                            \
    transaction_set_reference_block(trx, chain.head_block_id());               \
    chain.push_transaction(trx);                                               \
    BOOST_TEST_CHECKPOINT("Transferred " << Amount << " from " << #sender      \
                                         << " to " << #recipient);             \
  }
#define XFER4(chain, sender, recipient, amount)                                \
  XFER5(chain, sender, recipient, amount, "")
...
#define STAKE4(chain, sender, recipient, amount)                               \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        trx, config::EosContractName,                                          \
        vector<types::AccountPermission>{{#sender, "active"}}, "lock",         \
        types::lock{#sender, #recipient, amount});                             \
    trx.expiration = chain.head_block_time() + 100;                            \
    transaction_set_reference_block(trx, chain.head_block_id());               \
    chain.push_transaction(trx);                                               \
...
#define BEGIN_UNSTAKE3(chain, account, amount)                                 \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        trx, config::EosContractName,                                          \
        vector<types::AccountPermission>{{#account, "active"}}, "unlock",      \
        types::unlock{#account, amount});                                      \
    trx.expiration = chain.head_block_time() + 100;                            \
    transaction_set_reference_block(trx, chain.head_block_id());               \
    chain.push_transaction(trx);                                               \
...
#define FINISH_UNSTAKE3(chain, account, amount)                                \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        trx, config::EosContractName,                                          \
        vector<types::AccountPermission>{{#account, "active"}}, "claim",       \
        types::claim{#account, amount});                                       \
    trx.expiration = chain.head_block_time() + 100;                            \
    transaction_set_reference_block(trx, chain.head_block_id());               \
    chain.push_transaction(trx);                                               \
...
#define MKPDCR4(chain, owner, key, cfg)                                        \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        trx, config::EosContractName,                                          \
        vector<types::AccountPermission>{{#owner, "active"}}, "setproducer",   \
...
#define MKPDCR3(chain, owner, key)                                             \
  MKPDCR4(chain, owner, key, BlockchainConfiguration{});
...
#define APPDCR4(chain, voter, producer, approved)                              \
  {                                                                            \
    ...
    transaction_emplace_message(                                               \
        trx, config::EosContractName,                                          \
        vector<types::AccountPermission>{{#voter, "active"}}, "okproducer",    \
        types::okproducer{#voter, #producer, approved ? 1 : 0});               \
    trx.expiration = chain.head_block_time() + 100;                            \
    transaction_set_reference_block(trx, chain.head_block_id());               \
    chain.push_transaction(trx);                                               \
    BOOST_TEST_CHECKPOINT("Set producer approval from " << #voter << " for "   \
                                                        << #producer << " to " \
                                                        << approved);          \
  }
...
#define UPPDCR3(chain, owner, key)                                             \
  UPPDCR4(chain, owner, key, chain.get_producer(owner).configuration)
