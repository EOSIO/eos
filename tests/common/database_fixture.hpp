...
#define EOS_REQUIRE_THROW(expr, exc_type)                                      \
  {                                                                            \
    std::string req_throw_info = fc::json::to_string(                          \
        fc::mutable_variant_object()("source_file", __FILE__)(                 \
            "source_lineno", __LINE__)("expr", #expr)("exc_type", #exc_type)); \
    if (fc::enable_record_assert_trip)                                         \
      std::cout << "EOS_REQUIRE_THROW begin " << req_throw_info << std::endl;  \
    BOOST_REQUIRE_THROW(expr, exc_type);                                       \
    if (fc::enable_record_assert_trip)                                         \
      std::cout << "EOS_REQUIRE_THROW end " << req_throw_info << std::endl;    \
  }
  ...
  {                                                                            \
    std::string req_throw_info = fc::json::to_string(                          \
        fc::mutable_variant_object()("source_file", __FILE__)(                 \
            "source_lineno", __LINE__)("expr", #expr)("exc_type", #exc_type)); \
    if (fc::enable_record_assert_trip)                                         \
      std::cout << "EOS_CHECK_THROW begin " << req_throw_info << std::endl;    \
    BOOST_CHECK_THROW(expr, exc_type);                                         \
    if (fc::enable_record_assert_trip)                                         \
      std::cout << "EOS_CHECK_THROW end " << req_throw_info << std::endl;      \
  }

namespace eos {
namespace chain {
FC_DECLARE_EXCEPTION(testing_exception, 6000000, "test framework exception")
FC_DECLARE_DERIVED_EXCEPTION(missing_key_exception,
                             eos::chain::testing_exception, 6010000,
                             "key could not be found")

/**
 * @brief The testing_fixture class provides various services relevant to
...
public:
   ...
   *
   * @param id Identifier for the temporary directory. All requests for
   * directories with the same ID will receive the
   * same path. If id is empty, a unique directory will be returned.
   *
   * @return Path to the temporary directory
   *
   * This method makes it easy to get temporary directories for the duration of
   * a test. All returned directories will
   * be automatically deleted when the testing_fixture is destroyed.
   *
   * If multiple calls to this function are made with the same id, the same path
   * will be returned. Multiple calls with
   * distinct ids will not return the same path. If called with an empty id, a
   * unique path will be returned which will
...
/**
 * @brief The testing_blockchain class wraps chain_controller and eliminates
 * some of the boilerplate for common
 * operations on the blockchain during testing.
 *
 * testing_blockchains have an optional ID, which is passed to the constructor.
 * If two testing_blockchains are created
 * with the same ID, they will have the same data directory. If no ID, or an
 * empty ID, is provided, the database will
 * have a unique data directory which no subsequent testing_blockchain will be
 * assigned.
 *
 * testing_blockchain helps with producing blocks, or missing blocks, via the
 * @ref produce_blocks and @ref miss_blocks
 * methods. To produce N blocks, simply call produce_blocks(N); to miss N
 * blocks, call miss_blocks(N). Note that missing
 * blocks has no effect on the blockchain until the next block, following the
 * missed blocks, is produced.
 */
class testing_blockchain : public chain_controller {
public:
  testing_blockchain(chainbase::database &db, fork_database &fork_db,
                     block_log &blocklog,
                     chain_initializer_interface &initializer,
  ...
  /**
   * @brief Produce new blocks, adding them to the blockchain, optionally
   * following a gap of missed blocks
   * @param count Number of blocks to produce
   * @param blocks_to_miss Number of block intervals to miss a production before
   * producing the next block
   *
   * Creates and adds @ref count new blocks to the blockchain, after going @ref
   * blocks_to_miss intervals without
   * producing a block.
   */
   ...
   *
   * To sync the blockchains, all blocks from one blockchain which are unknown
   * to the other are pushed to the other,
   * then the same thing in reverse. Whichever blockchain has more blocks will
   * have its blocks sent to the other
   * first.
   *
  ...
  /// @brief Attempt to sign the provided transaction using the keys available
  /// to the testing_fixture
  void sign_transaction(SignedTransaction &trx) const;

  /// @brief Override push_transaction to apply testing policies
  /// If transactions are being held for review, transaction will be held after
  /// testing policies are applied
  fc::optional<ProcessedTransaction> push_transaction(SignedTransaction trx,
                                                      uint32_t skip_flags = 0);
  /// @brief Review and optionally push last held transaction
  /// @tparam F A callable with signature `bool f(SignedTransaction&,
  /// uint32_t&)`
  /// @param reviewer Callable which inspects and potentially alters the held
  /// transaction and skip flags, and returns
  ...
  fc::optional<ProcessedTransaction> review_transaction(F &&reviewer) {
    if (reviewer(review_storage.first, review_storage.second))
      return chain_controller::push_transaction(review_storage.first,
                                                review_storage.second);
    return {};
  }

  /// @brief Set whether testing_blockchain::push_transaction checks signatures
  /// by default
  /// @param skip_sigs If true, push_transaction will skip signature checks;
  /// otherwise, no changes will be made
  void set_skip_transaction_signature_checking(bool skip_sigs) {
    skip_trx_sigs = skip_sigs;
  }
  /// @brief Set whether testing_blockchain::push_transaction attempts to sign
  /// transactions or not
  void set_auto_sign_transactions(bool auto_sign) {
    auto_sign_trxs = auto_sign;
  }
  /// @brief Set whether testing_blockchain::push_transaction holds transactions
  /// for review or not
...
/**
 * @brief The testing_network class provides a simplistic virtual P2P network
 * connecting testing_blockchains together.
 *
 * A testing_blockchain may be connected to zero or more testing_networks at any
 * given time. When a new
 * testing_blockchain joins the network, it will be synced with all other
 * blockchains already in the network (blocks
 * known only to the new chain will be pushed to the prior network members and
 * vice versa, ignoring blocks not on the
 * main fork). After this, whenever any blockchain in the network gets a new
 * block, that block will be pushed to all
...
public:
  ...
  void propagate_block(const signed_block &block,
                       const testing_blockchain &skip_db);
