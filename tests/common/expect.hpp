...
template <typename Pred, typename Eval, typename T> struct expector {
  expector(Eval _eval, const T &_expected, const char *const _msg)
      : expected(_expected), eval(_eval), msg(_msg) {}

  template <typename Input> void operator()(const Input &input) {
    BOOST_TEST_INFO(msg);
...
template <typename Eval> auto expect(Eval &&eval, char const *const msg) {
...
#define EXPECT_3(P, E, T)                                                      \
  eos::test::expect<P>(E, T, "EXPECT<" #P ">(" #E "," #T ")")
