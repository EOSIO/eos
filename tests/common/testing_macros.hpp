 ...
 *
 * Creates and opens a testing_blockchain with the first argument as its name,
 * and, if present, the second argument as
 ...
 * @endcode
 */
#define Make_Blockchain(...)                                                   \
  BOOST_PP_OVERLOAD(MKCHAIN, __VA_ARGS__)(__VA_ARGS__)
/**
 ...
 * @endcode
 */
#define Make_Blockchains(...)                                                  \
  BOOST_PP_SEQ_FOR_EACH(MKCHAINS_MACRO, _, __VA_ARGS__)

/**
 * @brief Make_Network is a shorthand way to create a testing_network and
 * connect some testing_blockchains to it.
 *
 * Example usage:
 * @code{.cpp}
 ...
 * // The private key is generated off of the sha256 hash of
 * "a_key_private_key", so it should be unique from all
 * // other keys created with Make_Key in the same scope.
 * @endcode
 */
#ifdef HAVE_DATABASE_FIXTURE
#define Make_Key(name)                                                         \
  auto name##_private_key =                                                    \
      private_key_type::regenerate(fc::digest(#name "_private_key"));          \
...
/**
 * @brief Key_Authority is a shorthand way to create an inline Authority based
 * on a key
 *
 * Invoke Key_Authority passing the name of a public key in the current scope,
 * and Key_Authority will resolve inline to
 * an authority which can be satisfied by a signature generated by the
 * corresponding private key.
 */
#define Key_Authority(pubkey) (Authority{1, {{pubkey, 1}}, {}})
/**
 * @brief Account_Authority is a shorthand way to create an inline Authority
 * based on an account
 *
 * Invoke Account_Authority passing the name of an account, and
 * Account_Authority will resolve inline to an authority
 * which can be satisfied by the provided account's active authority.
 */
#define Account_Authority(account)                                             \
  (Authority{1, {}, {{{#account, "active"}, 1}}})
/**
 * @brief Complex_Authority is a shorthand way to create an arbitrary inline
 * @ref Authority
 *
 * Invoke Complex_Authority passing the weight threshold necessary to satisfy
 * the authority, a bubble list of keys and
 * weights, and a bubble list of accounts and weights.
 *
 * Key bubbles are structured as ((key_name, key_weight))
 * Account bubbles are structured as (("account_name", "account_authority",
 * weight))
 *
 * Example:
 * @code{.cpp}
 * // Create an authority which can be satisfied with a master key, or with any
 * three of:
 * // - key_1
 * // - key_2
 * // - key_3
 ...
 * Make_Key(key_2)
 * Make_Key(key_3)
 * auto auth = Complex_Authority(5, ((master_key, 5))((key_1, 2))((key_2,
 * 2))((key_3, 2)),
 *                               (("alice", "test_multisig", 2))(("bob",
 * "test_multisig", 2));
 * @endcode
 */
 ...
 *
 * Use Make_Account to create an account, including keys. The changes will be
 * applied via a transaction applied to the
 * provided blockchain object. The changes will not be incorporated into a
 * block; they will be left in the pending
 * state.
 *
 ...
 * // ...and also registers the account joe with owner and active authorities
 * satisfied by these keys, created by
 * // init0, with init0's active authority as joe's recovery authority, and
 * initially endowed with Asset(100)
 * @endcode
 *
 * You may specify a third argument for the creating account:
 * @code{.cpp}
 * // Same as MKACCT(chain, joe) except that sam will create joe's account
 * instead of init0
 * Make_Account(chain, joe, sam)
 * @endcode
 *
 * You may specify a fourth argument for the amount to transfer in account
 * creation:
 * @code{.cpp}
 * // Same as MKACCT(chain, joe, sam) except that sam will send joe ASSET(100)
 * during creation
 * Make_Account(chain, joe, sam, Asset(100))
 * @endcode
 *
 * You may specify a fifth argument, which will be used as the owner authority
 * (must be an Authority, NOT a key!).
 *
 * You may specify a sixth argument, which will be used as the active authority.
 * If six or more arguments are provided,
 * the default keypair will NOT be created or put into scope.
 *
 * You may specify a seventh argument, which will be used as the recovery
 * authority.
 */
 ...
 *
 * @code{.cpp}
 * // Add a new authority named "money" to account "alice" as a child of her
 * active authority
 * Authority newAuth = //...
 ...
 * @endcode
 */
#define Delete_Authority(...)                                                  \
  BOOST_PP_OVERLOAD(DELAUTH, __VA_ARGS__)(__VA_ARGS__)
/**
 ...
 * @endcode
 */
#define Link_Authority(...)                                                    \
  BOOST_PP_OVERLOAD(LINKAUTH, __VA_ARGS__)(__VA_ARGS__)
/**
 ...
 * @endcode
 */
#define Unlink_Authority(...)                                                  \
  BOOST_PP_OVERLOAD(UNLINKAUTH, __VA_ARGS__)(__VA_ARGS__)
 ...
 * @endcode
 *
 * The changes will be applied via a transaction applied to the provided
 * blockchain object. The changes will not be
 ...
 *
 * // Stake and transfer 10 EOS from alice to bob (alice pays liquid EOS and bob
 * receives stake)
 ...
 * @endcode
 *
 * This can also be used to cancel an unstaking in progress, by passing Asset(0)
 * as the amount.
 */
#define Begin_Unstake_Asset(...)                                               \
  BOOST_PP_OVERLOAD(BEGIN_UNSTAKE, __VA_ARGS__)(__VA_ARGS__)
 ...
 * @endcode
 */
#define Finish_Unstake_Asset(...)                                              \
  BOOST_PP_OVERLOAD(FINISH_UNSTAKE, __VA_ARGS__)(__VA_ARGS__)

/**
 * @brief Shorthand way to set voting proxy
 *
 * Use Set_Proxy to set what account a stakeholding account proxies its voting
 * power to
 * @code{.cpp}
  ...
  {                                                                            \
    ...
    if (std::string(#stakeholder) != std::string(#proxy))                      \
      transaction_emplace_message(                                             \
          ...
          vector<types::AccountPermission>{{#stakeholder, "active"}},          \
          "setproxy", types::setproxy{#stakeholder, #proxy});                  \
    else                                                                       \
      transaction_emplace_message(                                             \
          trx, config::EosContractName,                                        \
          vector<types::AccountPermission>{{#stakeholder, "active"}},          \
 ...
 * @code{.cpp}
 * // Create a block producer belonging to joe using signing_key as the block
 * signing key and config as the producer's
 ...
 *
 * // Create a block producer belonging to joe, using a new key as the block
 * signing key:
 ...
 * @endcode
 */
#define Approve_Producer(...)                                                  \
  BOOST_PP_OVERLOAD(APPDCR, __VA_ARGS__)(__VA_ARGS__)

/**
 * @brief Shorthand way to update a block producer
 *
 * @note Unlike with the Make_* macros, the Update_* macros take an expression
 * as the owner/name field, so be sure to
 * wrap names like this in quotes. You may also pass a normal C++ expression to
 * be evaulated here instead. The reason
 * for this discrepancy is that the Make_* macros add identifiers to the current
 * scope based on the owner/name field;
 * moreover, which can't be done with C++ expressions; however, the Update_*
 * macros do not add anything to the scope,
 * and it's more likely that these will be used in a loop or other context where
 * it is inconvenient to know the
 * owner/name at compile time.
 *
 * Use Update_Producer to update a block producer:
 * @code{.cpp}
 * // Update a block producer belonging to joe using signing_key as the new
 * block signing key, and config as the
