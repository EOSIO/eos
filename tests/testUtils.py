...
class Utils:
    ...
    def getChainStrategies():
        ...
        chainSyncStrategy = Utils.SyncStrategy(
            Utils.SyncReplayTag, 1, "--replay-blockchain")
        chainSyncStrategies[chainSyncStrategy.name] = chainSyncStrategy

        chainSyncStrategy = Utils.SyncStrategy(
            Utils.SyncResyncTag, 2, "--delete-all-blocks")
        chainSyncStrategies[chainSyncStrategy.name] = chainSyncStrategy

        chainSyncStrategy = Utils.SyncStrategy(
            Utils.SyncHardReplayTag, 3, "--hard-replay-blockchain")
        chainSyncStrategies[chainSyncStrategy.name] = chainSyncStrategy
    ...
    def checkOutput(cmd, ignoreError=False):
        ...
        if popen.returncode != 0 and not ignoreError:
            raise subprocess.CalledProcessError(
                returncode=popen.returncode, cmd=cmd, output=error)
        return output.decode("utf-8")

    @staticmethod
    def errorExit(msg="", raw=False, errorCode=1):
        if Utils.ShuttingDown:
            Utils.Print("ERROR:" if not raw else "",
                        " errorExit called during shutdown, ignoring.  msg=", msg)
            return
    ...
    def cmdError(name, cmdCode=0):
        msg = "FAILURE - %s%s" % (name, ("" if cmdCode ==
                                         0 else (" returned error code %d" % cmdCode)))
        Utils.Print(msg)
    ...
    def waitForBool(lam, timeout=None, sleepTime=3, reporter=None):
        def myLam(): return True if lam() else None
    ...
    def arePortsAvailable(ports):
        ...
        for port in ports:
            if Utils.Debug:
                Utils.Print("Checking if port %d is available." % (port))
            ...
            try:
                ...
                if e.errno == errno.EADDRINUSE:
                    ...
                    Utils.Print(
                        "ERROR: Unknown exception while trying to listen on port %d" % (port))
                    Utils.Print(e)
                return False
            finally:
                s.close()
    ...
    def pgrepCmd(serverName):
        ...
        try:
            pgrepHelp = re.search('-a', subprocess.Popen("pgrep --help 2>/dev/null",
                                                         shell=True, stdout=subprocess.PIPE).stdout.read().decode('utf-8'))
            # group() errors if -a is not found, so we don't need to do anything else special here.
            pgrepHelp.group(0)
            pgrepOpts = "-a"
    ...
    def getBlockLog(blockLogLocation, silentErrors=False, exitOnError=False):
        ...
        cmd = "%s --blocks-dir %s --as-json-array" % (
            Utils.EosBlockLogPath, blockLogLocation)
        if Utils.Debug:
