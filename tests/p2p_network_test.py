...
# apifiny should have the same key as defproducera
apifiny = copy.copy(defproduceraAccount)
apifiny.name = "apifiny"
...
wasmFile="apifiny.system.wasm"
abiFile="apifiny.system.abi"
Print("\nPush system contract %s %s" % (wasmFile, abiFile))
trans=node0.publishContract(apifiny, wasmFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish apifiny.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, apifiny)
