...
try:
    ...
    if localTest and not dontLaunch:
        ...
        producerAccounts = Cluster.createAccountKeys(1)
        producerAccount = producerAccounts[0]
        manualProducerArgs = {0 : { 'key': producerAccount, 'names': ['producera']}, 1 : { 'key': producerAccount, 'names': ['producera']} }
        if cluster.launch(totalNodes=2, prodCount=1, pnodes=2, onlyBios=onlyBios, dontBootstrap=dontBootstrap, specificExtraNodeosArgs=specificExtraNodeosArgs, manualProducerNodeConf=manualProducerArgs) is False:
            cmdError("launcher")
