...
programs/remnode/remnode --config-dir "$(pwd)/release-build-test/config" --data-dir "$(pwd)/release-build-test/data" 1>/dev/null 2>/dev/null &
sleep 10
kill $! # kill remnode gracefully, by PID
if [[ ! -f release-build-test/data/state/shared_memory.bin ]]; then
    echo 'ERROR: remnode state not found!'
    echo ''
