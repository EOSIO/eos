...
class Cluster(object):
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraNodeosArgs=None, useBiosBootFile=True, specificExtraNodeosArgs=None, activatePreactivateFeature=True):
        """Launch cluster.
        ...
        if onlyBios:
            ...
            if not biosNode.checkPulse():
                ...
                return False
            if activatePreactivateFeature: biosNode.activatePreactivateFeature()
        ...
        if onlyBios or not useBiosBootFile:
            self.biosNode=Cluster.bootstrap(totalNodes, prodCount, totalProducers, Cluster.__BiosHost, Cluster.__BiosPort, self.walletMgr, onlyBios, activatePreactivateFeature)
            if self.biosNode is None:
                Utils.Print("ERROR: Bootstrap failed.")
                return False
        else:
            self.useBiosBootFile=True
            self.biosNode=Cluster.bios_bootstrap(totalNodes, Cluster.__BiosHost, Cluster.__BiosPort, self.walletMgr, activatePreactivateFeature)
            if self.biosNode is None:
                Utils.Print("ERROR: Bootstrap failed.")
                return False
    ...
    @staticmethod
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, activatePreactivateFeature, silent=False):
    ...
    @staticmethod
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, activatePreactivateFeature, onlyBios=False):
