...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
        ...
        """
        Utils.Print("Cluster args: ", locals())
        self.accounts={}
        self.nodes={}
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        Utils.Print("nodeosArgs: %s" % nodeosArgs)
        ...
        if onlyBios or not useBiosBootFile:
            self.biosNode=Cluster.bootstrap(totalNodes, prodCount, totalProducers, Cluster.__BiosHost, Cluster.__BiosPort, self.walletMgr, onlyBios, enableMongo=self.enableMongo, mongoHost=self.mongoHost, mongoPort=self.mongoPort, mongoDb=self.mongoDb+"bios")
            if self.biosNode is None:
                Utils.Print("ERROR: Bootstrap failed.")
                return False
        else:
            self.useBiosBootFile=True
            self.biosNode=Cluster.bios_bootstrap(totalNodes, Cluster.__BiosHost, Cluster.__BiosPort, self.walletMgr, enableMongo=self.enableMongo, mongoHost=self.mongoHost, mongoPort=self.mongoPort, mongoDb=self.mongoDb+"bios")
            if self.biosNode is None:
                Utils.Print("ERROR: Bootstrap failed.")
                return False
        ...
        self.eosioAccount=self.defProducerAccounts["cyber"]
    ...
    @staticmethod
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False, enableMongo=False, mongoHost="", mongoPort="", mongoDb=""):
        """Bootstrap cluster using the bios_boot.sh script generated by eosio-launcher."""

        Utils.Print("Starting cluster bootstrap.")
        biosNode=Node(biosHost, biosPort, walletMgr=walletMgr, enableMongo=enableMongo, mongoHost=mongoHost, mongoPort=mongoPort, mongoDb=mongoDb)
        if not biosNode.checkPulse():
            Utils.Print("ERROR: Bios node doesn't appear to be running...")
            return None
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"cyber\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init cyber transfer")
            opts="--permission cyber@active"
            if name != "cyber":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from cyber.token to %s." % (name))
                    return None
    ...
    @staticmethod
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False, enableMongo=False, mongoHost="", mongoPort="", mongoDb=""):
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission cyber@active"
                    myTrans=biosNode.pushMessage("cyber", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("cyber", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but cyber).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "cyber"
        ...
        eosioTokenAccount.name="cyber.token"
        ...
        eosioRamAccount.name="cyber.ram"
        ...
        eosioRamfeeAccount.name="cyber.ramfee"
        ...
        Utils.Print("Verify cyber issue, Expected: %s" % (expectedAmount))
    ...
    def discoverLocalNodes(self, totalNodes, timeout=None):
        ...
        for i in range(0, totalNodes):
            ...
            if m is None:
                ...
                break
            instance=Node(self.host, self.port + i, pid=int(m.group(1)), cmd=m.group(2), walletMgr=self.walletMgr, enableMongo=self.enableMongo, mongoHost=self.mongoHost, mongoPort=self.mongoPort, mongoDb=("%stestnet_%02d" % (self.mongoDb, i)))
