...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="apifiny-ignition-wd/bootlog.txt"
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
        ...
        host: apifiny server host
        port: apifiny server port
        walletHost: apifiny wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures apifiny mongo plugin
        ...
        self.apifinyAccount=self.defProducerAccounts["apifiny"]= Account("apifiny")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodapifinyArgs=None, useBiosBootFile=True, specificExtraNodapifinyArgs=None, onlySetProds=False,
        ...
        extraNodapifinyArgs: string of arguments to pass through to each nodoes instance (via --nodapifiny flag on launcher)
        ...
        specificExtraNodapifinyArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodapifiny flags on launcher), example: { "5" : "--plugin apifiny::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the apifiny.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodapifiny default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        nodapifinyArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodapifinyArgs += " --plugin apifiny::wallet_api_plugin"
        if self.enableMongo:
            nodapifinyArgs += " --plugin apifiny::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodapifinyArgs is not None:
            assert(isinstance(extraNodapifinyArgs, str))
            nodapifinyArgs += extraNodapifinyArgs
        if Utils.Debug:
            nodapifinyArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodapifinyArgs += " --plugin apifiny::producer_api_plugin"

        if nodapifinyArgs:
            cmdArr.append("--nodapifiny")
            cmdArr.append(nodapifinyArgs)

        if specificExtraNodapifinyArgs is not None:
            assert(isinstance(specificExtraNodapifinyArgs, dict))
            for nodeNum,arg in specificExtraNodapifinyArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodapifiny")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodapifiny")
                cmdArr.append(path)
        ...
        self.apifinyAccount=self.defProducerAccounts["apifiny"]
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on apifiny server port %d" % (
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on apifiny node port: %d" % (node.port))
                return False
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.apifinyAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse apifiny private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by apifiny-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/apifiny.bios",
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/apifiny.bios"
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down apifiny cluster.")
            return None
        ...
        apifinyName="apifiny"
        apifinyKeys=producerKeys[apifinyName]
        apifinyAccount=Account(apifinyName)
        apifinyAccount.ownerPrivateKey=apifinyKeys["private"]
        apifinyAccount.ownerPublicKey=apifinyKeys["public"]
        apifinyAccount.activePrivateKey=apifinyKeys["private"]
        apifinyAccount.activePublicKey=apifinyKeys["public"]
        producerKeys.pop(apifinyName)

        if not self.walletMgr.importKey(apifinyAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (apifinyName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="apifiny.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"apifiny\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init apifiny transfer")
            opts="--permission apifiny@active"
            if name != "apifiny":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from apifiny.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        trans=biosNode.publishContract(apifinyAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, apifinyAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission apifiny@active"
                    myTrans=biosNode.pushMessage("apifiny", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("apifiny", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but apifiny).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "apifiny"
        ...
        apifinyTokenAccount=copy.deepcopy(apifinyAccount)
        apifinyTokenAccount.name="apifiny.token"
        trans=biosNode.createAccount(apifinyTokenAccount, apifinyAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (apifinyTokenAccount.name))
            return None

        apifinyRamAccount=copy.deepcopy(apifinyAccount)
        apifinyRamAccount.name="apifiny.ram"
        trans=biosNode.createAccount(apifinyRamAccount, apifinyAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (apifinyRamAccount.name))
            return None

        apifinyRamfeeAccount=copy.deepcopy(apifinyAccount)
        apifinyRamfeeAccount.name="apifiny.ramfee"
        trans=biosNode.createAccount(apifinyRamfeeAccount, apifinyAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (apifinyRamfeeAccount.name))
            return None

        apifinyStakeAccount=copy.deepcopy(apifinyAccount)
        apifinyStakeAccount.name="apifiny.stake"
        trans=biosNode.createAccount(apifinyStakeAccount, apifinyAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (apifinyStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(apifinyTokenAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=apifinyTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (apifinyAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to apifiny contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (apifinyAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (apifinyAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to apifiny contract.")
            return None
        ...
        Utils.Print("Verify apifiny issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountEosBalanceStr(apifinyAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (apifinyAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (apifinyAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (apifinyTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (apifinyAccount.name)
            trans=biosNode.pushMessage(apifinyAccount.name, action, data, opts)
    ...
    # Kills a percentange of Eos instances starting from the tail and update apifinyInstanceInfos state
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodapifiny instances. allInstances will kill all nodapifiny instances running on the system."""
        ...
        if allInstances:
            # ocassionally the launcher cannot kill the apifiny server
            cmd="pkill -9 %s" % (Utils.EosServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down apifiny cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodapifiny instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodapifiny instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
