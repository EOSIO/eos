...
class TokenType(EnumType):
    pass

addEnum(TokenType, "legacy")
addEnum(TokenType, "kv")
...
class Cluster(object):
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               ...
               tokenType=TokenType.legacy, manualProducerNodeConf={}):
        """Launch cluster.
        ...
        tokenType: indicate whether the eosio.token contract should be the legacy contract or the token.kv contract
        ...
        if not loadSystemContract or tokenType != TokenType.legacy:
            if useBiosBootFile: Utils.Print("NOTE: useBiosBootFile being set False because of other parameters not supported by bios_boot script")
            useBiosBootFile=False  #ensure we use Cluster.bootstrap

        if onlyBios or not useBiosBootFile:
            self.biosNode=self.bootstrap(biosNode, startedNodes, prodCount + sharedProducers, totalProducers, pfSetupPolicy, onlyBios, onlySetProds, loadSystemContract, manualProducerNodeConf, tokenType=tokenType)
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True, manualProducerNodeConf=[], tokenType=TokenType.legacy):
        ...
        if tokenType == TokenType.kv:
            ...
            assert biosNode.waitForLibToAdvance() is not None
            opts = "--permission eosio@active"
            biosNode.pushMessage(eosioAccount.name, "setkvparams", '[{"max_key_size":64, "max_value_size":1024, "max_iterators":128}]', opts)

            assert biosNode.waitForLibToAdvance() is not None

            Utils.Print("Loading kv_contract")
            contractDir="unittests/contracts/token.kv"
            trans=biosNode.publishContract(eosioAccount, contractDir, "kv_contract.wasm", "kv_contract.abi", waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return None
            Utils.Print("Loading kv_contract done")

            assert biosNode.waitForLibToAdvance() is not None

            biosNode.pushMessage(eosioAccount.name, "setkvparam", '["eosio.kvram"]', opts)
            ...
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(eosioKVTokenAccount, contractDir, "{}.wasm".format(contract), "{}.abi".format(contract), waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return None


        if not Cluster.publishTokenContract(biosNode, eosioTokenAccount):
            return None
    ...
    @staticmethod
    def publishTokenContract(biosNode, eosioTokenAccount):
        contract="eosio.token"
        contractDir="unittests/contracts/%s" % (contract)
