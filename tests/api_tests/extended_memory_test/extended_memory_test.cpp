...
/*******************************************************************************************************
 *
 * Current test is written assuming there is one intial "heap"(block of
 *contiguous memory) and that up
 * to 4 "heap"s will be created.  The first heap is assumed to be 8192 chars and
 *that the second through
...
void test_extended_memory() {
  ...
  assert(ptr3 + 20 < ptr4,
         "20 char buf should have been created after ptr3"); // test specific to
                                                             // implementation
                                                             // (can remove for
                                                             // refactor)
  ...
  assert(ptr4 + 20 < ptr1_realloc,
         "11 char buf should have been created after ptr4"); // test specific to
                                                             // implementation
                                                             // (can remove for
                                                             // refactor)
  ...
  assert(ptr1_realloc + 972 < ptr5, "972 char buf should have been created "
                                    "after ptr1_realloc"); // test specific to
                                                           // implementation
                                                           // (can remove for
                                                           // refactor)
  ...
  assert(ptr5 + 1020 < ptr6,
         "1020 char buf should have been created after ptr5"); // test specific
                                                               // to
                                                               // implementation
                                                               // (can remove
                                                               // for refactor)
...
void test_page_memory() {
  ...
  assert(reinterpret_cast<uint32_t>(prev) == _64K,
         "Should initially have 1 64K page allocated");

  prev = sbrk(1);

  assert(reinterpret_cast<uint32_t>(prev) == _64K,
         "Should still point to the end of 1st 64K page");

  prev = sbrk(2);

  assert(reinterpret_cast<uint32_t>(prev) == _64K + 8,
         "Should point to 8 past the end of 1st 64K page");

  prev = sbrk(_64K - 17);

  assert(reinterpret_cast<uint32_t>(prev) == _64K + 16,
         "Should point to 16 past the end of 1st 64K page");

  prev = sbrk(1);

  assert(reinterpret_cast<uint32_t>(prev) == 2 * _64K,
         "Should point to the end of 2nd 64K page");

  prev = sbrk(_64K);

  assert(reinterpret_cast<uint32_t>(prev) == 2 * _64K + 8,
         "Should point to 8 past the end of the 2nd 64K page");

  prev = sbrk(_64K - 15);

  assert(reinterpret_cast<uint32_t>(prev) == 3 * _64K + 8,
         "Should point to 8 past the end of the 3rd 64K page");

  prev = sbrk(2 * _64K - 1);

  assert(reinterpret_cast<uint32_t>(prev) == 4 * _64K,
         "Should point to the end of 4th 64K page");

  prev = sbrk(2 * _64K);

  assert(reinterpret_cast<uint32_t>(prev) == 6 * _64K,
         "Should point to the end of 6th 64K page");

  prev = sbrk(2 * _64K + 1);

  assert(reinterpret_cast<uint32_t>(prev) == 8 * _64K,
         "Should point to the end of 8th 64K page");

  prev = sbrk(6 * _64K - 15);

  assert(reinterpret_cast<uint32_t>(prev) == 10 * _64K + 8,
         "Should point to 8 past the end of 13th 64K page");

  prev = sbrk(0);

  assert(reinterpret_cast<uint32_t>(prev) == 16 * _64K,
         "Should point to the end of 16th 64K page");
}

void test_page_memory_exceeded() {
  auto prev = sbrk(15 * _64K);
  assert(reinterpret_cast<uint32_t>(prev) == _64K,
         "Should have allocated up to the 1M of memory limit");
  sbrk(1);
  assert(0, "Should have thrown exception for trying to allocate more than 1M "
            "of memory");
}
