...
void verify(const void *const ptr, const uint32_t val, const uint32_t size) {
  ...
  for (uint32_t i = 0; i < size; ++i) {
    assert(static_cast<uint32_t>(static_cast<unsigned char>(char_ptr[i])) ==
               val,
           "buf slot doesn't match");
  }
}
...
void print(const void *const ptr, const uint32_t size) {
  ...
  for (uint32_t i = 0; i < size; ++i) {
    ...
    eos::print("",
               static_cast<uint32_t>(static_cast<unsigned char>(char_ptr[i])),
               delim);
  }
  eos::print("}\n");
}
...
/*
 * malloc and realloc always allocate on 8 byte boundaries based off of total
 * allocation, so
 * if the requested size + the 2 byte header is not divisible by 8, then the
 * allocated space
...
void test_memory() {
  ...
  assert(ptr1 + 36 < ptr2,
         "20 char buf should have been created after ptr1"); // test specific to
                                                             // implementation
                                                             // (can remove for
                                                             // refactor)
  verify(ptr1, 0, 36);
  assert(ptr1[36] != 0, "should not have empty bytes following since block "
                        "allocated"); // test specific to implementation (can
                                      // remove for refactor)
  ...
  assert(ptr1_realloc == ptr1,
         "should have shrunk the reallocated 30 char buf");
  verify(ptr1, 0,
         14); // test specific to implementation (can remove for refactor)
  ...
  // 20 chars allocated (still)
  ptr1_realloc = (char *)eos::realloc(ptr1, 15);
  assert(ptr1_realloc != nullptr,
         "should have returned a reallocated 15 char buf");
  assert(ptr1_realloc == ptr1,
         "should have reallocated 15 char buf as the same buf");
  assert(ptr1[14] == 0x7e,
         "remaining 15 chars of buf should be untouched for unchanged buf");

  // same size as max allocated buffer -- test specific to implementation (can
  // remove for refactor)
  ptr1_realloc = (char *)eos::realloc(ptr1, 30);
  assert(ptr1_realloc != nullptr, "should have returned a 30 char buf");
  assert(ptr1_realloc == ptr1,
         "should have increased the buf back to orig max"); // test specific to
                                                            // implementation
                                                            // (can remove for
                                                            // refactor)
  assert(ptr1[14] == 0x7e,
         "remaining 15 chars of buf should be untouched for expanded buf");
  ...
  assert(ptr1_realloc == ptr1,
         "should have increased char buf to actual size"); // test specific to
                                                           // implementation
                                                           // (can remove for
                                                           // refactor)
  ...
  assert(ptr1_realloc != ptr1,
         "should have had to create new 37 char buf from 36 char buf");
  assert(ptr2 < ptr1_realloc,
         "should have been created after ptr2"); // test specific to
                                                 // implementation (can remove
                                                 // for refactor)
  assert(ptr1_realloc[14] == 0x7e,
         "orig 36 char buf's content should be copied");
  assert(ptr1_realloc[35] == 0x7f,
         "orig 36 char buf's content should be copied");

  // realloc with nullptr
  char *nullptr_realloc = (char *)eos::realloc(nullptr, 50);
  assert(nullptr_realloc != nullptr,
         "should have returned a 50 char buf and ignored nullptr");
  assert(ptr1_realloc < nullptr_realloc,
         "should have created after ptr1_realloc"); // test specific to
                                                    // implementation (can
                                                    // remove for refactor)

  // realloc with invalid ptr
  char *invalid_ptr_realloc = (char *)eos::realloc(nullptr_realloc + 4, 10);
  assert(invalid_ptr_realloc != nullptr,
         "should have returned a 10 char buf and ignored invalid ptr");
  assert(
      nullptr_realloc < invalid_ptr_realloc, "should have created "
                                             "invalid_ptr_realloc after "
                                             "nullptr_realloc"); // test
                                                                 // specific to
                                                                 // implementation
                                                                 // (can remove
                                                                 // for
...
// this test verifies that malloc can allocate 15 64K pages and treat them as
// one big heap space (if sbrk is not called in the mean time)
void test_memory_hunk() {
  // try to allocate the largest buffer we can, which is 15 contiguous 64K pages
  // (with the 4 char space for the ptr header)
  char *ptr1 = (char *)eos::malloc(15 * 64 * 1024 - 4);
  assert(ptr1 != nullptr, "should have allocated a ~983K char buf");
}

void test_memory_hunks() {
  // leave 784 bytes of initial buffer to allocate later (rounds up to nearest 8
  // byte boundary,
  ...
  // 96 * (10 * 1024 - 15) => 15 ~64K pages with 768 byte buffer left to
  // allocate
  for (int i = 0; i < 96; ++i) {
    ...
    if (last_ptr != nullptr) {
      // - 15 rounds to -8
      assert(last_ptr + 10 * 1024 - 8 == ptr2,
             "should allocate the very next ptr"); // test specific to
                                                   // implementation (can remove
                                                   // for refactor)
    }
  ...
  // try to allocate a buffer slightly larger than the remaining buffer| 765 + 4
  // rounds to 776
  char *ptr3 = (char *)eos::malloc(765);
  assert(ptr3 != nullptr, "should have allocated a 772 char buf");
  assert(ptr1 + 7408 == ptr3, "should allocate the very next ptr after ptr1 in "
                              "initial heap"); // test specific to
                                               // implementation (can remove for
                                               // refactor)
  ...
  assert(
      last_ptr + 10 * 1024 - 8 == ptr4,
      "should allocate the very next ptr after last_ptr at end of contiguous "
      "heap"); // test specific to implementation (can remove for refactor)

  // use up remaining 8 chars
  char *ptr5 = (char *)eos::malloc(4);
  assert(ptr5 != nullptr, "should have allocated a 4 char buf");
  assert(ptr3 + 776 == ptr5, "should allocate the very next ptr after ptr3 in "
                             "initial heap"); // test specific to implementation
                                              // (can remove for refactor)
...
void test_memory_hunks_disjoint() {
  ...
  // can only make 14 extra (64K) heaps for malloc, since calls to sbrk will eat
  // up part
  char *loop_ptr1[14];
  // 14 * (64 * 1024 - 28) => 14 ~64K pages with each page having 24 bytes left
  // to allocate
  for (int i = 0; i < 14; ++i) {
    // allocates a new heap for each request, since sbrk call doesn't allow
    // contiguous heaps to grow
  ...
  // the 15th extra heap is reduced in size because of the 14 * 8 bytes
  // allocated by sbrk calls
  // will leave 8 bytes to allocate later (verifying that we circle back in the
  // list
  ...
  for (int i = 0; i < 14; ++i) {
    ...
    assert(loop_ptr1[i] + 64 * 1024 - 24 == loop_ptr2[i],
           "loop_ptr2[i] should be very next pointer after loop_ptr1[i]");
  }

  // this shows that searching for free ptrs starts at the last loop to find
  // free memory, not at the begining
  char *ptr3 = (char *)eos::malloc(4);
  assert(ptr3 != nullptr, "should have allocated a 4 char buf");
  assert(loop_ptr2[13] + 16 == ptr3, "should allocate the very next ptr after "
                                     "loop_ptr2[13]"); // test specific to
                                                       // implementation (can
                                                       // remove for refactor)

  char *ptr4 = (char *)eos::malloc(4);
  assert(ptr4 != nullptr, "should have allocated a 4 char buf");
  assert(ptr2 + 65416 == ptr4, "should allocate the very next ptr after ptr2 "
                               "in last heap"); // test specific to
                                                // implementation (can remove
                                                // for refactor)

  char *ptr5 = (char *)eos::malloc(4);
  assert(ptr5 != nullptr, "should have allocated a 4 char buf");
  assert(ptr1 + 8184 == ptr5, "should allocate the very next ptr after ptr1 in "
                              "last heap"); // test specific to implementation
                                            // (can remove for refactor)
  ...
  for (int i = 0; i < 13; ++i) {
    ...
    assert(loop_ptr2[i] + 16 == loop_ptr3[i],
           "loop_ptr3[i] should be very next pointer after loop_ptr2[i]");
  }
  ...
  for (int i = 0; i < 64; ++i) {
    ...
    if (i == 0)
      assert(loop_ptr1[3] == slot3_ptr[0],
             "loop_ptr1[3] should be very next pointer after slot3_ptr[0]");
    else
      assert(slot3_ptr[i - 1] + 1024 == slot3_ptr[i],
             "slot3_ptr[i] should be very next pointer after slot3_ptr[i-1]");
  }
