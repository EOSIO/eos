...
void Set_Proxy_Owner(testing_blockchain &chain, AccountName proxy,
                     AccountName owner) {
  eos::chain::SignedTransaction trx;
  trx.scope = sort_names({proxy, owner});
  transaction_emplace_message(
      trx, "proxy", vector<types::AccountPermission>({}), "setowner", owner);
...
BOOST_FIXTURE_TEST_CASE(opaque_proxy, testing_fixture) {
  try {
    ...
    BOOST_CHECK_EQUAL(chain.fetch_block_by_number(12)
                          ->cycles.front()
                          .front()
                          .generated_input.size(),
                      1);
  }
  FC_LOG_AND_RETHROW()
}
