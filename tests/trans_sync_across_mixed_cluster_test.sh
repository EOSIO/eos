...
killAll()
{
  programs/apifiny-launcher/apifiny-launcher -k 15
}

cleanup()
{
    rm -rf etc/apifiny/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clapifiny/clapifiny get info)"
  verifyErrorCode "clapifiny get info"
...
# stand up nodapifiny cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodapifiny \"--plugin apifiny::wallet_api_plugin\" $launcherOpts
programs/apifiny-launcher/apifiny-launcher --nodapifiny "--plugin apifiny::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clapifiny/clapifiny --port $port2 get block 1)"
    verifyErrorCode "clapifiny get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clapifiny/clapifiny create key)"
verifyErrorCode "clapifiny create key"
...
PASSWORD_INITA="$(programs/clapifiny/clapifiny wallet create --name inita)"
verifyErrorCode "clapifiny wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clapifiny/clapifiny wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clapifiny wallet import"
programs/clapifiny/clapifiny wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clapifiny wallet import"
programs/clapifiny/clapifiny wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clapifiny wallet import"
programs/clapifiny/clapifiny wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clapifiny wallet import"
...
ACCOUNT_INFO="$(programs/clapifiny/clapifiny create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clapifiny create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clapifiny/clapifiny get account testera)"
verifyErrorCode "clapifiny get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clapifiny/clapifiny transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clapifiny transfer"
   ...
   TRANS_INFO="$(programs/clapifiny/clapifiny --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clapifiny get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
