 ...
 *  potentially take just .05 seconds with 10+ CPU cores. Future improvements
 * might
...
BOOST_AUTO_TEST_CASE(schedule_test) {
  ...
  auto rand_scope = []() { return rand() % 1000000 + rand() % 1000000; };
...
BOOST_FIXTURE_TEST_CASE(trx_variant, testing_fixture) {
  ...
  try {
    Make_Blockchain(chain) Name from("from"), to("to");
    uint64_t amount = 10;

    eos::chain::ProcessedTransaction trx;
    trx.scope = sort_names({from, to});
    transaction_emplace_message(
        trx, "eos", vector<types::AccountPermission>{{from, "active"}},
...
BOOST_FIXTURE_TEST_CASE(irrelevant_auth, testing_fixture) {
  try {
    ...
    transaction_emplace_message(
        ...
        vector<types::AccountPermission>{{"inita", "active"}}, "transfer",
        types::transfer{"inita", "joe", 50, ""});
    trx.expiration = chain.head_block_time() + 100;
    transaction_set_reference_block(trx, chain.head_block_id());
    ...
    trx.messages.front().authorization.emplace_back(
        types::AccountPermission{"initb", "active"});
    BOOST_CHECK_THROW(chain.push_transaction(
                          trx, chain_controller::skip_transaction_signatures),
                      tx_irrelevant_auth);
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_FIXTURE_TEST_CASE(produce_blocks, testing_fixture) {
  try {
    ...
    BOOST_CHECK_EQUAL(chain.head_block_num(),
                      chain.get_global_properties().active_producers.size() +
                          6);
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_FIXTURE_TEST_CASE(order_dependent_transactions, testing_fixture) {
  try {
    ...
    BOOST_CHECK_EQUAL(chain.fetch_block_by_number(11)
                          ->cycles.front()
                          .front()
                          .user_input.size(),
                      2);
...
BOOST_FIXTURE_TEST_CASE(missed_blocks, testing_fixture) {
  try {
    ...
    BOOST_CHECK_EQUAL(chain.head_block_time().to_iso_string(),
                      next_block_time.to_iso_string());
...
// Simple sanity test of test network: if databases aren't connected to the
// network, they don't sync to each other
...
BOOST_FIXTURE_TEST_CASE(simple_network, testing_fixture) {
  try {
    Make_Blockchains((chain1)(chain2)) Make_Network(net, (chain1)(chain2))
    ...
    BOOST_CHECK_EQUAL(chain1.head_block_id().str(),
                      chain2.head_block_id().str());
    chain2.produce_blocks(5);
...
// Test that two databases joining and leaving a network sync correctly after a
// fork
BOOST_FIXTURE_TEST_CASE(forked_network, testing_fixture) {
  try {
    Make_Blockchains((chain1)(chain2)) Make_Network(net)
...
BOOST_FIXTURE_TEST_CASE(rsf_missed_blocks, testing_fixture) {
  try {
    Make_Blockchain(chain) chain.produce_blocks();
...
BOOST_FIXTURE_TEST_CASE(restart_db, testing_fixture) {
  try {
    auto lag = EOS_PERCENT(config::BlocksPerRound,
                           config::IrreversibleThresholdPercent);
    {
...

    {
      ...
      // After restarting, we should have rewound to the last irreversible
      // block.
...
// Check that a db which is closed and reopened successfully syncs back with the
// network, including retrieving blocks
// that it missed while it was down
BOOST_FIXTURE_TEST_CASE(sleepy_db, testing_fixture) {
  try {
    Make_Blockchain(producer) Make_Network(net, (producer))
...

    {
      ...
      Make_Blockchain(sleepy, sleepy) net.connect_blockchain(sleepy);
    ...
    BOOST_CHECK_EQUAL(sleepy.head_block_id().str(),
                      producer.head_block_id().str());
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_FIXTURE_TEST_CASE(reindex, testing_fixture) {
  try {
    auto lag = EOS_PERCENT(config::BlocksPerRound,
                           config::IrreversibleThresholdPercent);
    {
      chainbase::database db(get_temp_dir(), chainbase::database::read_write,
                             TEST_DB_SIZE);
...
BOOST_FIXTURE_TEST_CASE(wipe, testing_fixture) {
  try {
    Make_Blockchains((chain1)(chain2)) Make_Network(net, (chain1)(chain2)) {
      ...
      Make_Blockchain(chain3) net.connect_blockchain(chain3);
...

    {
      ...
      Make_Blockchain(chain3) BOOST_CHECK_EQUAL(chain3.head_block_num(), 0);
...
BOOST_FIXTURE_TEST_CASE(irrelevant_sig_soft_check, testing_fixture) {
  try {
    ...
    BOOST_CHECK_THROW(chain.review_transaction([](SignedTransaction &trx,
                                                  auto) {
    ...
    // Now check that a second blockchain accepts the block with the oversigned
    // transaction
    Make_Blockchain(newchain);
    Make_Network(net, (chain)(newchain));
    BOOST_CHECK_NE((newchain_db.find<account_object, by_name>("alice")),
                   nullptr);
  }
  FC_LOG_AND_RETHROW()
}
