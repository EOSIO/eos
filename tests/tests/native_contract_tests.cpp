...
BOOST_FIXTURE_TEST_CASE(create_account, testing_fixture) {
  try {
    ...
    { // test in the pending state
      ...
      BOOST_CHECK_EQUAL(chain.get_liquid_balance("inita"),
                        Asset(100000 - 2000));

      const auto &joe_owner_authority =
          chain_db.get<permission_object, by_owner>(
              boost::make_tuple("joe", "owner"));
      ...
      BOOST_CHECK_EQUAL(string(joe_owner_authority.auth.keys[0].key),
                        string(joe_public_key));
      BOOST_CHECK_EQUAL(joe_owner_authority.auth.keys[0].weight, 1);

      const auto &joe_active_authority =
          chain_db.get<permission_object, by_owner>(
              boost::make_tuple("joe", "active"));
      ...
      BOOST_CHECK_EQUAL(string(joe_active_authority.auth.keys[0].key),
                        string(joe_public_key));
...
BOOST_FIXTURE_TEST_CASE(transfer, testing_fixture) {
  try {
    ...
    BOOST_REQUIRE_THROW(chain.push_transaction(trx),
                        transaction_exception); // no messages
    ...
    BOOST_REQUIRE_THROW(chain.push_transaction(trx),
                        transaction_exception); // not unique
...
BOOST_FIXTURE_TEST_CASE(producer_creation, testing_fixture) {
  try {
    Make_Blockchain(chain) Make_Account(chain, producer);
    chain.produce_blocks();
...
BOOST_FIXTURE_TEST_CASE(producer_voting_parameters, testing_fixture) {
  try {
    Make_Blockchain(chain) chain.produce_blocks(21);

    vector<BlockchainConfiguration> votes{
        {1024, 512, 4096, Asset(5000).amount, Asset(4000).amount,
         Asset(100).amount, 512, 6, 5000, 4, 4096, 65536},
        {10000, 100, 4096, Asset(3333).amount, Asset(27109).amount,
         Asset(10).amount, 100, 6, 5000, 4, 4096, 65536},
        {2048, 1500, 1000, Asset(5432).amount, Asset(2000).amount,
         Asset(50).amount, 1500, 6, 5000, 4, 4096, 65536},
        {100, 25, 1024, Asset(90000).amount, Asset(0).amount, Asset(433).amount,
         25, 6, 5000, 4, 4096, 65536},
        {1024, 1000, 100, Asset(10).amount, Asset(50).amount, Asset(200).amount,
         1000, 6, 5000, 4, 4096, 65536},
        {420, 400, 2710, Asset(27599).amount, Asset(1177).amount,
         Asset(27720).amount, 400, 6, 5000, 4, 4096, 65536},
        {271, 200, 66629, Asset(2666).amount, Asset(99991).amount,
         Asset(277).amount, 200, 6, 5000, 4, 4096, 65536},
        {1057, 1000, 2770, Asset(972).amount, Asset(302716).amount,
         Asset(578).amount, 1000, 6, 5000, 4, 4096, 65536},
        {9926, 27, 990, Asset(99999).amount, Asset(39651).amount,
         Asset(4402).amount, 27, 6, 5000, 4, 4096, 65536},
        {1005, 1000, 1917, Asset(937111).amount, Asset(2734).amount,
         Asset(1).amount, 1000, 6, 5000, 4, 4096, 65536},
        {80, 70, 5726, Asset(63920).amount, Asset(231561).amount,
         Asset(27100).amount, 70, 6, 5000, 4, 4096, 65536},
        {471617, 333333, 100, Asset(2666).amount, Asset(2650).amount,
         Asset(2772).amount, 33333, 6, 5000, 4, 4096, 65536},
        {2222, 1000, 100, Asset(33619).amount, Asset(1046).amount,
         Asset(10577).amount, 1000, 6, 5000, 4, 4096, 65536},
        {8, 7, 100, Asset(5757267).amount, Asset(2257).amount,
         Asset(2888).amount, 7, 6, 5000, 4, 4096, 65536},
        {2717, 2000, 57797, Asset(3366).amount, Asset(205).amount,
         Asset(4472).amount, 2000, 6, 5000, 4, 4096, 65536},
        {9997, 5000, 27700, Asset(29199).amount, Asset(100).amount,
         Asset(221).amount, 5000, 6, 5000, 4, 4096, 65536},
        {163900, 200, 882, Asset(100).amount, Asset(5720233).amount,
         Asset(105).amount, 200, 6, 5000, 4, 4096, 65536},
        {728, 80, 27100, Asset(28888).amount, Asset(6205).amount,
         Asset(5011).amount, 80, 6, 5000, 4, 4096, 65536},
        {91937, 44444, 652589, Asset(87612).amount, Asset(123).amount,
         Asset(2044).amount, 44444, 6, 5000, 4, 4096, 65536},
        {171, 96, 123456, Asset(8402).amount, Asset(321).amount,
         Asset(816).amount, 96, 6, 5000, 4, 4096, 65536},
        {17177, 6767, 654321, Asset(9926).amount, Asset(9264).amount,
         Asset(8196).amount, 6767, 6, 5000, 4, 4096, 65536},
    };
    BlockchainConfiguration medians = {1057,
                                       512,
                                       2770,
                                       Asset(9926).amount,
                                       Asset(2650).amount,
                                       ...
                                       4096,
                                       65536};
    // If this fails, the medians variable probably needs to be updated to have
    // the medians of the votes above
    BOOST_REQUIRE_EQUAL(BlockchainConfiguration::get_median_values(votes),
                        medians);

    for (int i = 0; i < votes.size(); ++i) {
      auto name = std::string("init") + char('a' + i);
      Update_Producer(chain, name, chain.get_producer(name).signing_key,
                      votes[i]);
    }
...
// Test producer votes on blockchain parameters in full blockchain context, with
// missed blocks
  ...
  try {
    ...
    vector<BlockchainConfiguration> votes{
        ...
        {80, 70, 5726, Asset(63920).amount, Asset(231561).amount,
         Asset(27100).amount, 70, 6, 5000, 4, 4096, 65536},
        {471617, 333333, 100, Asset(2666).amount, Asset(2650).amount,
         Asset(2772).amount, 333333, 6, 5000, 4, 4096, 65536},
        {2222, 1000, 100, Asset(33619).amount, Asset(1046).amount,
         Asset(10577).amount, 1000, 6, 5000, 4, 4096, 65536},
        {8, 7, 100, Asset(5757267).amount, Asset(2257).amount,
         Asset(2888).amount, 7, 6, 5000, 4, 4096, 65536},
...
// Test that if I create a producer and vote for him, he gets in on the next
// round (but not before)
BOOST_FIXTURE_TEST_CASE(producer_voting_1, testing_fixture) {
  try {
    Make_Blockchain(chain) Make_Account(chain, joe);
...

    {
      ...
      const auto &joeVotes = chain_db.get<native::eos::ProducerVotesObject,
                                          native::eos::byOwnerName>("joe");
    ...
    BOOST_REQUIRE(boost::find(gpo.active_producers, "joe") !=
...
// Same as producer_voting_1, except we first cast the vote for the producer,
// _then_ get a stake
...
BOOST_FIXTURE_TEST_CASE(producer_proxy_voting, testing_fixture) {
  try {
    ...
    Action setproxy = [](auto &chain) { Set_Proxy(chain, stakeholder, proxy); };
    Action stake = [](auto &chain) {
      Stake_Asset(chain, stakeholder, Asset(100).amount);
    };

    auto run = [this](std::vector<Action> actions) {
      Make_Blockchain(chain) Make_Account(chain, stakeholder);
      ...
      {
        BOOST_CHECK_EQUAL(
            chain.get_approved_producers("proxy").count("producer"), 1);
        BOOST_CHECK_EQUAL(chain.get_staked_balance("stakeholder"), Asset(100));
        const auto &producerVotes =
            chain_db.get<native::eos::ProducerVotesObject,
                         native::eos::byOwnerName>("producer");
        BOOST_CHECK_EQUAL(producerVotes.getVotes(),
                          chain.get_staked_balance("stakeholder"));
      }
      ...
      const auto &gpo = chain.get_global_properties();
      BOOST_REQUIRE(boost::find(gpo.active_producers, "producer") !=
                    gpo.active_producers.end());
      ...
      {
        BOOST_CHECK_EQUAL(
            chain.get_approved_producers("proxy").count("producer"), 0);
        const auto &producerVotes =
            chain_db.get<native::eos::ProducerVotesObject,
                         native::eos::byOwnerName>("producer");
    ...
    // Eh, I'm not going to try all legal permutations here... just several of
    // them
    /*
     ...
     // Make sure it throws if I try to proxy to an account before the account
     accepts proxying
     BOOST_CHECK_THROW(run({setproxy, approve, stake}),
...
BOOST_FIXTURE_TEST_CASE(auth_tests, testing_fixture) {
  try {
    Make_Blockchain(chain) Make_Account(chain, alice);
    chain.produce_blocks();

    BOOST_CHECK_THROW(Delete_Authority(chain, alice, "active"),
                      message_validate_exception);
    BOOST_CHECK_THROW(Delete_Authority(chain, alice, "owner"),
                      message_validate_exception);

    // Change owner permission
    Make_Key(new_owner_key);
    Set_Authority(chain, alice, "owner", "",
                  Key_Authority(new_owner_key_public_key));
    chain.produce_blocks();

    permission_object::id_type owner_id;
    {
      auto obj = chain_db.find<permission_object, by_owner>(
          boost::make_tuple("alice", "owner"));
      BOOST_CHECK_NE(obj, nullptr);
      BOOST_CHECK_EQUAL(obj->owner, "alice");
      BOOST_CHECK_EQUAL(obj->name, "owner");
      BOOST_CHECK_EQUAL(obj->parent, 0);
      owner_id = obj->id;
    ...
    Set_Authority(chain, alice, "active", "owner",
                  Key_Authority(new_active_key_public_key));
    chain.produce_blocks();

    {
      auto obj = chain_db.find<permission_object, by_owner>(
          boost::make_tuple("alice", "active"));
      BOOST_CHECK_NE(obj, nullptr);
      BOOST_CHECK_EQUAL(obj->owner, "alice");
    ...
    Make_Key(k1);
    Make_Key(k2);
    Set_Authority(chain, alice, "spending", "active",
                  Key_Authority(k1_public_key));

    // Ensure authority doesn't appear until next block
    BOOST_CHECK_EQUAL((chain_db.find<permission_object, by_owner>(
                          boost::make_tuple("alice", "spending"))),
                      nullptr);
    chain.produce_blocks();
    ...
    BOOST_CHECK_THROW(Set_Authority(chain, alice, "spending", "spending",
                                    Key_Authority(k1_public_key)),
                      message_validate_exception);
    BOOST_CHECK_THROW(Set_Authority(chain, alice, "spending", "owner",
                                    Key_Authority(k1_public_key)),
                      message_precondition_exception);
    Delete_Authority(chain, alice, "spending");
    BOOST_CHECK_NE((chain_db.find<permission_object, by_owner>(
                       boost::make_tuple("alice", "spending"))),
                   nullptr);
    chain.produce_blocks();
    ...
    Set_Authority(chain, alice, "trading", "active",
                  Key_Authority(k1_public_key));
    BOOST_CHECK_EQUAL((chain_db.find<permission_object, by_owner>(
                          boost::make_tuple("alice", "trading"))),
                      nullptr);
    chain.produce_blocks();
    Set_Authority(chain, alice, "spending", "trading",
                  Key_Authority(k2_public_key));
    BOOST_CHECK_EQUAL((chain_db.find<permission_object, by_owner>(
                          boost::make_tuple("alice", "spending"))),
                      nullptr);
    chain.produce_blocks();
    ...
    {
      auto trading = chain_db.find<permission_object, by_owner>(
          boost::make_tuple("alice", "trading"));
      auto spending = chain_db.find<permission_object, by_owner>(
          boost::make_tuple("alice", "spending"));
      ...
      auto tradingChecker =
          MakeAuthorityChecker(PermissionToAuthority, 2, {k1_public_key});
      auto spendingChecker =
          MakeAuthorityChecker(PermissionToAuthority, 2, {k2_public_key});
    ...
    BOOST_CHECK_THROW(Delete_Authority(chain, alice, "trading"),
                      message_precondition_exception);
    BOOST_CHECK_THROW(Set_Authority(chain, alice, "trading", "spending",
                                    Key_Authority(k1_public_key)),
                      message_precondition_exception);
    BOOST_CHECK_THROW(Set_Authority(chain, alice, "spending", "active",
                                    Key_Authority(k1_public_key)),
                      message_precondition_exception);
    Delete_Authority(chain, alice, "spending");
    BOOST_CHECK_NE((chain_db.find<permission_object, by_owner>(
...
BOOST_FIXTURE_TEST_CASE(auth_links, testing_fixture) {
  try {
    Make_Blockchain(chain);
    ...
    Set_Authority(chain, alice, "spending", "active",
                  Key_Authority(spending_public_key));
    chain.produce_blocks();
    Set_Authority(chain, alice, "scud", "spending",
                  Key_Authority(scud_public_key));
    chain.produce_blocks();
    Link_Authority(chain, alice, "spending", eos, "transfer");
    BOOST_CHECK_EQUAL((chain_db.find<permission_link_object, by_message_type>(
                          boost::make_tuple("alice", "eos", "transfer"))),
                      nullptr);
    chain.produce_blocks();

    {
      auto obj = chain_db.find<permission_link_object, by_message_type>(
          boost::make_tuple("alice", "eos", "transfer"));
      BOOST_CHECK_NE(obj, nullptr);
    ...
    // OK, set the above transfer's authorization level to scud and check that
    // it fails
    BOOST_CHECK_THROW(
    ...
    // Now we'll lock some funds, but we'll use the scud authority to do it.
    // First, this should fail, but back up the
    // transaction for later
    Stake_Asset(chain, alice, 100);
    BOOST_CHECK_THROW(chain.review_transaction([&chain, &backup](
                          SignedTransaction &trx, auto) {
      trx.messages.front().authorization = {{"alice", "scud"}};
    ...
    chain.produce_blocks();
    // And now the backed up transaction should succeed, because scud is
    // sufficient authority for all except "transfer"
    chain.chain_controller::push_transaction(
        backup, chain_controller::pushed_transaction);

    // But transfers with scud authority should still not work, because there's
    // an overriding link to spending
    Transfer_Asset(chain, alice, bob, Asset(10));
    BOOST_CHECK_THROW(
        chain.review_transaction([&chain](SignedTransaction &trx, auto) {
          trx.messages.front().authorization = {{"alice", "scud"}};
