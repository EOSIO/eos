 ...
 */
template <typename NEXT> struct shuffled_functor {
  shuffled_functor(NEXT &&_next) : next(_next){};

  block_schedule operator()(const vector<pending_transaction> &transactions,
...
template <typename NEXT> static shuffled_functor<NEXT> shuffled(NEXT &&next) {
  return shuffled_functor<NEXT>(next);
}

template <int NUM, int DEN, typename NEXT> struct lossy_functor {
  ...
  block_schedule operator()(const vector<pending_transaction> &transactions,
    ...
    std::copy_if(
        transactions.begin(), transactions.end(), copy.begin(),
        [&](const pending_transaction &trx) { return dist(rng) >= cutoff; });
...
protected:
  auto create_transactions(
      const std::initializer_list<test_transaction> &transactions) {
...
static void
null_global_property_object_constructor(const global_property_object &) {}

static chainbase::allocator<global_property_object>
    null_global_property_object_allocator(nullptr);

struct base_properties {
  base_properties()
      : properties(null_global_property_object_constructor,
...
struct default_properties : public base_properties {
  default_properties() { properties.configuration.maxBlockSize = 256 * 1024; }
};

struct small_block_properties : public base_properties {
  small_block_properties() { properties.configuration.maxBlockSize = 512; }
};
...
BOOST_FIXTURE_TEST_CASE(no_conflicts, default_fixture) {
  ...
  // in a single cycle
  schedule_and_validate(block_schedule::by_threading_conflicts,
                        {{0x1ULL, 0x2ULL},
                         {0x3ULL, 0x4ULL},
                         {0x5ULL, 0x6ULL},
                         {0x7ULL, 0x8ULL},
                         {0x9ULL, 0xAULL}},
                        EXPECT(schedule_is_valid), EXPECT(transaction_count, 5),
...
BOOST_FIXTURE_TEST_CASE(basic_cycle, default_fixture) {
  ...
  schedule_and_validate(
      ...
      {
          {0x1ULL, 0x2ULL}, {0x2ULL, 0x3ULL}, {0x3ULL, 0x1ULL},
      },
      EXPECT(schedule_is_valid), EXPECT(transaction_count, 3));
}
...
BOOST_FIXTURE_TEST_CASE(small_block, compose_fixture<small_block_properties>) {
  ...
  schedule_and_validate(
      block_schedule::by_threading_conflicts,
      {
          {0x1ULL, 0x2ULL},   {0x3ULL, 0x4ULL},   {0x5ULL, 0x6ULL},
          {0x7ULL, 0x8ULL},   {0x9ULL, 0xAULL},   {0xBULL, 0xCULL},
          {0xDULL, 0xEULL},   {0x11ULL, 0x12ULL}, {0x13ULL, 0x14ULL},
          {0x15ULL, 0x16ULL}, {0x17ULL, 0x18ULL}, {0x19ULL, 0x1AULL},
          {0x1BULL, 0x1CULL}, {0x1DULL, 0x1EULL}, {0x21ULL, 0x22ULL},
          {0x23ULL, 0x24ULL}, {0x25ULL, 0x26ULL}, {0x27ULL, 0x28ULL},
          {0x29ULL, 0x2AULL}, {0x2BULL, 0x2CULL}, {0x2DULL, 0x2EULL},
      },
...
BOOST_FIXTURE_TEST_CASE(no_conflicts_shuffled, default_fixture) {
  ...
  for (int i = 0; i < 3000; i++) {
    schedule_and_validate(
        ...
        EXPECT(schedule_is_valid), EXPECT(transaction_count, 21),
        EXPECT(cycle_count, 1));
  }
}
...
BOOST_FIXTURE_TEST_CASE(some_conflicts_shuffled, default_fixture) {
  ...
  for (int i = 0; i < 3000; i++) {
    schedule_and_validate(
        shuffled(block_schedule::by_threading_conflicts), {{0x1ULL, 0x2ULL},
                                                           {0x3ULL, 0x2ULL},
                                                           {0x5ULL, 0x1ULL},
                                                           {0x7ULL, 0x1ULL},
                                                           {0x1ULL, 0x7ULL},
                                                           ...
                                                           {0x27ULL, 0x21ULL},
                                                           {0x21ULL, 0x27ULL}},
        EXPECT(schedule_is_valid), EXPECT(transaction_count, 15),
        EXPECT(std::greater<uint>, cycle_count, 1));
  }
}
