...
BOOST_AUTO_TEST_CASE(wallet_manager_test) {
  try {
    ...
    if (fc::exists("test.wallet"))
      fc::remove("test.wallet");
    if (fc::exists("test2.wallet"))
...
#warning TODO: fix check commented out when keys was converted to map<public_key_type,string>
    ...
    fc::optional<fc::ecc::private_key> optional_private_key1 =
        utilities::wif_to_key(key1);
    fc::optional<fc::ecc::private_key> optional_private_key2 =
        utilities::wif_to_key(key2);
    fc::optional<fc::ecc::private_key> optional_private_key3 =
        utilities::wif_to_key(key3);
    ...
    trx = wm.sign_transaction(trx, {optional_private_key1->get_public_key(),
                                    optional_private_key2->get_public_key(),
                                    optional_private_key3->get_public_key()},
                              chain_id_type{});
    const auto &pks = trx.get_signature_keys(chain_id_type{});
    BOOST_CHECK_EQUAL(3, pks.size());
    BOOST_CHECK(find(pks.cbegin(), pks.cend(),
                     optional_private_key1->get_public_key()) != pks.cend());
    BOOST_CHECK(find(pks.cbegin(), pks.cend(),
                     optional_private_key2->get_public_key()) != pks.cend());
    BOOST_CHECK(find(pks.cbegin(), pks.cend(),
                     optional_private_key3->get_public_key()) != pks.cend());
