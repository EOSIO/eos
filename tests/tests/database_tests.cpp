...
BOOST_FIXTURE_TEST_CASE(undo_test, testing_fixture) {
  try {
    ...
    // Create an account
    db.create<account_object>([](account_object &a) { a.name = "billy"; });
...
// Test the block fetching methods on database, get_block_id_for_num,
// fetch_bock_by_id, and fetch_block_by_number
BOOST_FIXTURE_TEST_CASE(get_blocks, testing_fixture) {
  try {
    Make_Blockchain(chain) vector<block_id_type> block_ids;
    ...
    for (int i = 0; i < 20; ++i) {
      ...
      BOOST_CHECK_EQUAL(chain.fetch_block_by_number(i + 1)->id(),
                        block_ids.back());
    }
    ...
    BOOST_CHECK_THROW(chain.get_block_id_for_num(21),
                      eos::chain::unknown_block_exception);
