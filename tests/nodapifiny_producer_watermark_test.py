...
###############################################################
# nodapifiny_producer_watermark_test
...
def setProds(sharedProdKey):
    ...
    trans=cluster.biosNode.pushMessage("apifiny", "setprods", setProdsStr, opts)
...
try:
    ...
    while tries > 0:
        ...
        info = node.getInfo()
        if node.infoValid and node.lastRetrievedHeadBlockProducer != "apifiny":
            break
        time.sleep(1)
        tries = tries-1
    if tries == 0:
