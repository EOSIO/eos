...
#
# Tests restart scenarios for nodapifiny.  Uses "-c" flag to indicate "replay" (--replay-blockchain), "resync"
# (--delete-all-blocks), "hardReplay"(--hard-replay-blockchain), and "none" to indicate what kind of restart flag should
# be used. This is one of the only test that actually verify that nodapifiny terminates with a good exit status.
#
###############################################################
...
try:
    ...
    wallet=walletMgr.create(walletName, [cluster.apifinyAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    apifinyAccount=cluster.apifinyAccount
    ...
    if not cluster.createAccounts(apifinyAccount):
    ...
    Print("nodapifiny instances killed.")
    ...
    Print("nodapifiny instances relaunched.")
    ...
    if killEosInstances:
        ...
        assert atLeastOne, "Test is setup to verify that a cleanly interrupted nodapifiny exits with an exit status of 0, but this test may no longer be setup to do that"
