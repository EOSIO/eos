...
`Nodapifiny` is a command line interface (CLI) application. As such, it can be started manually from the command line or through an automated script. The behavior of `nodapifiny` is determined mainly by which plugins are loaded and which plugin options are used. The `nodapifiny` application features two main option categories: *nodapifiny-specific* options and *plugin-specific* options.

## Nodapifiny-specific Options

Nodapifiny-specific options are used mainly for housekeeping purposes, such as setting the directory where the blockchain data resides, specifying the name of the `nodapifiny` configuraton file, setting the name and path of the logging configuration file, etc. A sample output from running  `$ nodapifiny --help` is displayed below, showing the nodapifiny-specific options (Note: the plugin-specific options have been excluded for clarity):
...
Plugin-specific options control the behavior of the nodapifiny plugins. Every plugin-specific option has a unique name, so it can be specified in any order within the command line or `config.ini` file. When specifying one or more plugin-specific option(s), the applicable plugin(s) must also be enabled using the `--plugin` option or else the corresponding option(s) will be ignored. A sample output from running `$ nodapifiny --help` is displayed below, showing an excerpt from the plugin-specific options:
