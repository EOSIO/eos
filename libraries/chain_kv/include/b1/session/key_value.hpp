...
key_value make_kv(const void* key, size_t key_length, const void* value, size_t value_length, Allocator& a) {
    ...
    auto value_bytes = populate_bytes(reinterpret_cast<const void*>(value), value_length, chunk + 2 * sizeof(uint64_t) + 2 * sizeof(size_t) + key_length);
...
inline const shared_bytes& key_value::key() const {
    return m_key;
}

inline const shared_bytes& key_value::value() const {
    return m_value;
}
