...
BOOST_AUTO_TEST_CASE(make_bytes_test) 
{
  ...
  auto b1 = make_bytes(char_value, char_length, allocator);
  BOOST_REQUIRE(memcmp(b1.data(), reinterpret_cast<const void*>(char_value), char_length) == 0);
  BOOST_REQUIRE(b1.length() == char_length);

  auto b2 = make_bytes(reinterpret_cast<const void*>(char_value), char_length, allocator);
  BOOST_REQUIRE(memcmp(b1.data(), reinterpret_cast<const void*>(char_value), char_length) == 0);
  BOOST_REQUIRE(b2.length() == char_length);

  auto b3 = make_bytes(&int_value, 1, allocator);
  ...
  auto b4 = make_bytes(char_value, char_length);
