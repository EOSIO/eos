...
auto make_cache() -> cache<allocator>
{
  auto a = allocator::make();
  return b1::session::make_cache(std::move(a));
}
...
{
  auto cache1 = b1::session_tests::make_cache<boost_memory_allocator>();
  make_data_store(cache1, char_key_values, string_t{});
  verify_equal(cache1, char_key_values, string_t{});

  auto cache2 = b1::session_tests::make_cache<boost_memory_allocator>();
...
BOOST_AUTO_TEST_CASE(cache_rw_ds_test)
{
  ...
  auto cache3 = b1::session_tests::make_cache<boost_memory_allocator>();
  auto cache4 = b1::session_tests::make_cache<boost_memory_allocator>();
