...
rocks_data_store make_rocks_data_store(const std::string name = "testdb") {
   auto rocksdb = make_rocks_db(name);
   return eosio::session::make_rocks_data_store(std::move(rocksdb));
}
...
BOOST_AUTO_TEST_CASE(rocks_data_store_create_test) {
   {
      auto datastore1 = eosio::session_tests::make_rocks_data_store();
      make_data_store(datastore1, char_key_values, string_t{});
      verify_equal(datastore1, char_key_values, string_t{});
   }

   {
      auto datastore2 = eosio::session_tests::make_rocks_data_store();
...
BOOST_AUTO_TEST_CASE(rocks_data_store_rwd_test) {
   {
      auto datastore1 = eosio::session_tests::make_rocks_data_store();
      make_data_store(datastore1, char_key_values, string_t{});
      for (const auto& kv : char_batch_values) { verify_rwd(datastore1, kv); }
   }

   {
      auto datastore2 = eosio::session_tests::make_rocks_data_store();
      make_data_store(datastore2, int_key_values, int_t{});
      for (const auto& kv : int_batch_values) { verify_rwd(datastore2, kv); }
   }
}
...
BOOST_AUTO_TEST_CASE(rocks_data_store_rw_ds_test) {
   {
      auto datastore1 = eosio::session_tests::make_rocks_data_store();
      auto datastore2 = eosio::session_tests::make_rocks_data_store("testdb2");
      make_data_store(datastore1, char_key_values, string_t{});
      verify_read_from_datastore(datastore1, datastore2);
   }

   {
      auto datastore3 = eosio::session_tests::make_rocks_data_store();
      auto datastore4 = eosio::session_tests::make_rocks_data_store("testdb2");
...
BOOST_AUTO_TEST_CASE(rocks_data_store_iterator_test) {
   ...
   {
      auto datastore4 = eosio::session_tests::make_rocks_data_store();
