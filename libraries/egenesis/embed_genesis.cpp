...
fc::path get_path(const boost::program_options::variables_map &options,
...
void convert_to_c_array(const std::string &src, std::string &dest,
                        int width = 40) {
...
struct egenesis_info {
  ...
  int genesis_json_array_width, genesis_json_array_height;
...
void load_genesis(const boost::program_options::variables_map &options,
                  egenesis_info &info) {
  if (options.count("genesis-json")) {
    fc::path genesis_json_filename = get_path(options, "genesis-json");
    std::cerr << "embed_genesis:  Reading genesis from file "
              << genesis_json_filename.preferred_string() << "\n";
...
int main(int argc, char **argv) {
  try {
    ...
    boost::program_options::options_description cli_options(
        "EOS Chain Identifier");
    cli_options.add_options()("help,h", "Print this help message and exit.")(
        "genesis-json,g",
        boost::program_options::value<boost::filesystem::path>(),
        "File to read genesis state from")(
        "tmplsub,t",
        boost::program_options::value<std::vector<std::string>>()->composing(),
        "Given argument of form src.cpp.tmpl---dest.cpp, write dest.cpp "
        "expanding template invocations in src");

    boost::program_options::variables_map options;
    try {
      boost::program_options::store(
          boost::program_options::parse_command_line(argc, argv, cli_options),
          options);
    ...
    fc::mutable_variant_object template_context = fc::mutable_variant_object()(
        "generated_file_banner", generated_file_banner)("chain_id",
                                                        (*info.chain_id).str());
    if (info.genesis_json.valid()) {
      ...
      template_context["genesis_json_array_width"] =
          info.genesis_json_array_width;
      template_context["genesis_json_array_height"] =
          info.genesis_json_array_height;
    }

    for (const std::string &src_dest :
         options["tmplsub"].as<std::vector<std::string>>()) {
      std::cerr << "embed_genesis:  parsing tmplsub parameter \"" << src_dest
                << "\"\n";
      size_t pos = src_dest.find("---");
      if (pos == std::string::npos) {
        std::cerr << "embed_genesis:  could not parse tmplsub parameter:  "
                     "'---' not found\n";
        main_return = 1;
        continue;
      }
