...
chain::BlockchainConfiguration
native_contract_chain_initializer::get_chain_start_configuration() {
  return genesis.initial_configuration;
}

std::array<types::AccountName, config::BlocksPerRound>
native_contract_chain_initializer::get_chain_start_producers() {
  std::array<types::AccountName, config::BlocksPerRound> result;
  std::transform(genesis.initial_producers.begin(),
                 genesis.initial_producers.end(), result.begin(),
                 [](const auto &p) { return p.owner_name; });
  return result;
}

void native_contract_chain_initializer::register_types(
    chain_controller &chain, chainbase::database &db) {
  // Install the native contract's indexes; we can't do anything until our
...
#define SET_APP_HANDLER(contract, scope, action)                               \
  chain.set_apply_handler(                                                     \
      #contract, #scope, #action,                                              \
      &BOOST_PP_CAT(native::contract::apply_,                                  \
                    BOOST_PP_CAT(scope, BOOST_PP_CAT(_, action))))
...
types::Abi native_contract_chain_initializer::eos_contract_abi() {
  ...
  eos_abi.structs.push_back(
      eos::types::GetStruct<eos::types::transfer>::type());
  ...
  eos_abi.structs.push_back(
      eos::types::GetStruct<eos::types::okproducer>::type());
  eos_abi.structs.push_back(
      eos::types::GetStruct<eos::types::setproducer>::type());
  eos_abi.structs.push_back(
      eos::types::GetStruct<eos::types::setproxy>::type());
  eos_abi.structs.push_back(
      eos::types::GetStruct<eos::types::updateauth>::type());
  eos_abi.structs.push_back(
      eos::types::GetStruct<eos::types::linkauth>::type());
  eos_abi.structs.push_back(
      eos::types::GetStruct<eos::types::unlinkauth>::type());
  eos_abi.structs.push_back(
      eos::types::GetStruct<eos::types::deleteauth>::type());
  eos_abi.structs.push_back(
      eos::types::GetStruct<eos::types::newaccount>::type());

  return eos_abi;
}

std::vector<chain::Message>
native_contract_chain_initializer::prepare_database(chain_controller &chain,
  ...
  /// contracts to make them create themselves
  auto CreateNativeAccount = [this, &db](Name name, auto liquidBalance) {
    ...
    const auto &owner =
        db.create<permission_object>([&name](permission_object &p) {
          p.owner = name;
          p.name = "owner";
          p.auth.threshold = 1;
        });
    ...
    db.create<native::eos::StakedBalanceObject>(
        [&name](auto &sb) { sb.ownerName = name; });
  };
  ...
  for (const auto &acct : genesis.initial_accounts) {
    chain::Message message(
        ...
        "newaccount",
        types::newaccount(config::EosContractName, acct.name,
                          KeyAuthority(acct.owner_key),
                          KeyAuthority(acct.active_key),
                          KeyAuthority(acct.owner_key), acct.staking_balance));
    ...
    if (acct.liquid_balance > 0) {
      message = chain::Message(
          ...
          vector<types::AccountPermission>{{config::EosContractName, "active"}},
          "transfer",
          types::transfer(config::EosContractName, acct.name,
  ...
  auto CreateProducer =
      boost::adaptors::transformed([config = genesis.initial_configuration](
          const auto &p) {
        return chain::Message(
            config::EosContractName,
            vector<types::AccountPermission>{{p.owner_name, "active"}},
            "setproducer",
            types::setproducer(p.owner_name, p.block_signing_key, config));
      });
  boost::copy(genesis.initial_producers | CreateProducer,
              std::back_inserter(messages_to_process));

  // Create special accounts
  auto create_special_account = [this, &db](Name name, const auto &owner,
                                            const auto &active) {
    db.create<account_object>([this, &name](account_object &a) {
      a.name = name;
      a.creation_date = genesis.initial_timestamp;
    });
    const auto &owner_permission =
        db.create<permission_object>([&owner, &name](permission_object &p) {
    ...
    db.create<permission_object>(
        [&active, &owner_permission](permission_object &p) {
  ...
  for (auto &p : genesis.initial_producers) {
    active_producers_authority.accounts.push_back(
        {{p.owner_name, config::ActiveName}, 1});
  }

  // CreateNativeAccount(config::AnybodyAccountName, 0);
  create_special_account(config::NobodyAccountName, empty_authority,
                         empty_authority);
  create_special_account(config::ProducersAccountName, empty_authority,
