...
void ProducerVotesObject::updateVotes(ShareType deltaVotes,
                                      UInt128 currentRaceTime) {
...
void ProxyVoteObject::addProxySource(const AccountName &source,
                                     ShareType sourceStake,
                                     chainbase::database &db) const {
  db.modify(*this, [&source, sourceStake](ProxyVoteObject &pvo) {
    pvo.proxySources.insert(source);
    pvo.proxiedStake += sourceStake;
  });
  db.get<StakedBalanceObject, byOwnerName>(proxyTarget)
      .propagateVotes(sourceStake, db);
}

void ProxyVoteObject::removeProxySource(const AccountName &source,
                                        ShareType sourceStake,
                                        chainbase::database &db) const {
  db.modify(*this, [&source, sourceStake](ProxyVoteObject &pvo) {
...
void ProxyVoteObject::updateProxiedStake(ShareType stakeDelta,
                                         chainbase::database &db) const {
  db.modify(*this, [stakeDelta](ProxyVoteObject &pvo) {
    pvo.proxiedStake += stakeDelta;
  });
  db.get<StakedBalanceObject, byOwnerName>(proxyTarget)
      .propagateVotes(stakeDelta, db);
}
...
ProducerRound
ProducerScheduleObject::calculateNextRound(chainbase::database &db) const {
  // Create storage and machinery with nice names, for choosing the top-voted
  // producers
  ProducerRound round;
  auto FilterRetiredProducers =
      boost::adaptors::filtered([&db](const ProducerVotesObject &pvo) {
        return db.get<producer_object, by_owner>(pvo.ownerName).signing_key !=
               PublicKey();
      });
  auto ProducerObjectToName = boost::adaptors::transformed(
      [](const ProducerVotesObject &p) { return p.ownerName; });
  const auto &AllProducersByVotes =
      db.get_index<ProducerVotesMultiIndex, byVotes>();
  auto ActiveProducersByVotes = AllProducersByVotes | FilterRetiredProducers;

  FC_ASSERT(
      boost::distance(ActiveProducersByVotes) >= config::BlocksPerRound,
  ...
  auto runnerUpStorage =
      boost::copy_n(ActiveProducersByVotes | ProducerObjectToName,
                    config::VotedProducersPerRound, round.begin());

  // More machinery with nice names, this time for choosing runner-up producers
  auto VotedProducerRange =
      boost::make_iterator_range(round.begin(), runnerUpStorage);
  // Sort the voted producer names; we'll need to do it anyways, and it makes
  // searching faster if we do it now
  boost::sort(VotedProducerRange);
  auto FilterVotedProducers = boost::adaptors::filtered(
      [&VotedProducerRange](const ProducerVotesObject &pvo) {
        return !boost::binary_search(VotedProducerRange, pvo.ownerName);
      });
  const auto &AllProducersByFinishTime =
      db.get_index<ProducerVotesMultiIndex, byProjectedRaceFinishTime>();
  auto EligibleProducersByFinishTime =
      AllProducersByFinishTime | FilterRetiredProducers | FilterVotedProducers;

  auto runnerUpProducerCount =
      config::BlocksPerRound - config::VotedProducersPerRound;

  // Copy the front producers in the race into the round
  auto roundEnd =
      boost::copy_n(EligibleProducersByFinishTime | ProducerObjectToName,
                    runnerUpProducerCount, runnerUpStorage);

  FC_ASSERT(roundEnd == round.end(), "Round scheduling yielded an unexpected "
                                     "number of producers: got ${actual}, but "
                                     "expected ${expected}",
            ("actual", (int64_t)std::distance(round.begin(), roundEnd))(
                "expected", (int64_t)round.size()));
  ...
  // Machinery to update the virtual race tracking for the producers that
  // completed their lap
  auto lastRunnerUp = AllProducersByFinishTime.iterator_to(
      db.get<ProducerVotesObject, byOwnerName>(lastRunnerUpName));
  ...
  auto LapCompleters = boost::make_iterator_range(
      AllProducersByFinishTime.begin(), ++lastRunnerUp);

  // Start each producer that finished his lap on the next one, and update the
  // global race time.
  try {
    if (boost::distance(LapCompleters) < AllProducersByFinishTime.size() &&
        newRaceTime < std::numeric_limits<UInt128>::max()) {
      // ilog("Processed producer race. ${count} producers completed a lap at
      // virtual time ${time}",
      //     ("count", (int64_t)boost::distance(LapCompleters))("time",
      //     newRaceTime));
...
void ProducerScheduleObject::resetProducerRace(chainbase::database &db) const {
  auto ResetRace = [&db](const ProducerVotesObject &pvo) {
    db.modify(pvo, [](ProducerVotesObject &pvo) { pvo.startNewRaceLap(0); });
  };
  const auto &AllProducers = db.get_index<ProducerVotesMultiIndex, byVotes>();

  boost::for_each(AllProducers, ResetRace);
  db.modify(*this,
            [](ProducerScheduleObject &pso) { pso.currentRaceTime = 0; });
}
}
