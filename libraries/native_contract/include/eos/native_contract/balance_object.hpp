 ...
 */
class BalanceObject
    : public chainbase::object<chain::balance_object_type, BalanceObject> {
...
using BalanceMultiIndex = chainbase::shared_multi_index_container<
    BalanceObject,
    indexed_by<
        ordered_unique<tag<by_id>, member<BalanceObject, BalanceObject::id_type,
                                          &BalanceObject::id>>,
        ordered_unique<tag<byOwnerName>,
                       member<BalanceObject, types::AccountName,
                              &BalanceObject::ownerName>>>>;
}
} // namespace native::eos

CHAINBASE_SET_INDEX_TYPE(native::eos::BalanceObject,
                         native::eos::BalanceMultiIndex)
