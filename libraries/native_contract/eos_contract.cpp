...
void validate_authority_precondition(const apply_context &context,
                                     const Authority &auth) {
  for (const auto &a : auth.accounts) {
    context.db.get<account_object, by_name>(a.permission.account);
    context.db.find<permission_object, by_owner>(
        boost::make_tuple(a.permission.account, a.permission.permission));
  }
}

/**
 *  This method is called assuming precondition_system_newaccount succeeds and
 * proceeds to
 *  deduct the balance of the account creator by deposit, this deposit is
 * supposed to be
...
void apply_eos_newaccount(apply_context &context) {
  ...
  EOS_ASSERT(validate(create.owner), message_validate_exception,
             "Invalid owner authority");
  EOS_ASSERT(validate(create.active), message_validate_exception,
             "Invalid active authority");
  EOS_ASSERT(validate(create.recovery), message_validate_exception,
  ...
  EOS_ASSERT(
      existing_account == nullptr, message_precondition_exception,
  ...
  const auto &new_account =
      db.create<account_object>([&create, &db](account_object &a) {
        a.name = create.name;
        a.creation_date =
            db.get(dynamic_global_property_object::id_type()).time;
      });
  if (context.controller.is_applying_block()) {
    const auto &owner_permission = db.create<permission_object>(
        [&create, &new_account](permission_object &p) {
          p.name = "owner";
          p.parent = 0;
          p.owner = new_account.name;
          p.auth = std::move(create.owner);
        });
    db.create<permission_object>(
        [&create, &owner_permission](permission_object &p) {
          p.name = "active";
  ...
  const auto &creatorBalance =
      context.mutable_db.get<BalanceObject, byOwnerName>(create.creator);

  EOS_ASSERT(creatorBalance.balance >= create.deposit.amount,
             message_validate_exception, "Creator '${c}' has insufficient "
                                         "funds to make account creation "
                                         "deposit of ${a}",
  ...
  context.mutable_db.create<BalanceObject>([&create](BalanceObject &b) {
    ...
    b.balance =
        0; // create.deposit.amount; TODO: make sure we credit this in @staked
  });

  context.mutable_db.create<StakedBalanceObject>(
      [&create](StakedBalanceObject &sbo) {
...
void apply_eos_transfer(apply_context &context) {
  ...
  try {
    EOS_ASSERT(transfer.amount > 0, message_validate_exception,
  ...
  try {
    ...
    EOS_ASSERT(from.balance >= transfer.amount, message_precondition_exception,
               "Insufficient Funds", ("from.balance", from.balance)(
    ...
    db.modify(
        to, [&](BalanceObject &a) { a.balance += ShareType(transfer.amount); });
  }
  FC_CAPTURE_AND_RETHROW((transfer))
}
///@}
...
void apply_eos_lock(apply_context &context) {
  ...
  EOS_ASSERT(lock.amount > 0, message_validate_exception,
             "Locked amount must be positive");
  ...
  EOS_ASSERT(locker.balance >= lock.amount, message_precondition_exception,
             "Account ${a} lacks sufficient funds to lock ${amt} EOS",
             ("a", lock.from)("amt", lock.amount)("available", locker.balance));

  context.mutable_db.modify(
      locker, [&lock](BalanceObject &a) { a.balance -= lock.amount; });

  const auto &balance =
      context.db.get<StakedBalanceObject, byOwnerName>(lock.to);
...
void apply_eos_unlock(apply_context &context) {
  ...
  EOS_ASSERT(unlock.amount >= 0, message_validate_exception,
             "Unlock amount cannot be negative");

  const auto &balance =
      context.db.get<StakedBalanceObject, byOwnerName>(unlock.account);

  EOS_ASSERT(balance.stakedBalance >= unlock.amount,
...
/** TODO: consider whether a microsecond level local timestamp is sufficient to
 * detect code version changes*/
#warning TODO: update setcode message to include the hash, then validate it in validate
  ...
  apply_context init_context(context.mutable_controller, context.mutable_db,
                             context.trx, context.msg, msg.account);
...
void apply_eos_claim(apply_context &context) {
  ...
  EOS_ASSERT(claim.amount > 0, message_validate_exception,
             "Claim amount must be positive");

  context.require_authorization(claim.account);

  auto balance =
      context.db.find<StakedBalanceObject, byOwnerName>(claim.account);
  EOS_ASSERT(balance != nullptr, message_precondition_exception,
             "Could not find staked balance for ${name}",
             ("name", claim.account));
  auto balanceReleaseTime =
      balance->lastUnstakingTime + config::StakedBalanceCooldownSeconds;
  auto now = context.controller.head_block_time();
  EOS_ASSERT(now >= balanceReleaseTime, message_precondition_exception,
             "Cannot claim balance until ${releaseDate}",
             ("releaseDate", balanceReleaseTime));
  EOS_ASSERT(
      balance->unstakingBalance >= claim.amount, message_precondition_exception,
  ...
  const auto &stakedBalance =
      context.db.get<StakedBalanceObject, byOwnerName>(claim.account);
  stakedBalance.finishUnstakingTokens(claim.amount, context.mutable_db);

  const auto &liquidBalance =
      context.db.get<BalanceObject, byOwnerName>(claim.account);
  context.mutable_db.modify(
      liquidBalance, [&claim](BalanceObject &a) { a.balance += claim.amount; });
}
...
void apply_eos_setproducer(apply_context &context) {
  ...
  EOS_ASSERT(update.name.good(), message_validate_exception,
  ...
  if (producer) {
    EOS_ASSERT(producer->signing_key != update.key ||
                   producer->configuration != update.configuration,
...
void apply_eos_okproducer(apply_context &context) {
  ...
  EOS_ASSERT(approve.approve == 0 || approve.approve == 1,
             message_validate_exception,
  ...
  auto &db = context.mutable_db;
  const auto &producer =
      db.get<ProducerVotesObject, byOwnerName>(approve.producer);
  const auto &voter = db.get<StakedBalanceObject, byOwnerName>(approve.voter);

  EOS_ASSERT(voter.producerVotes.contains<ProducerSlate>(),
             message_precondition_exception, "Cannot approve producer; "
                                             "approving account '${name}' "
                                             "proxies its votes to '${proxy}'",
             ("name", voter.ownerName)("proxy",
                                       voter.producerVotes.get<AccountName>()));

  const auto &slate = voter.producerVotes.get<ProducerSlate>();

  EOS_ASSERT(
      slate.size < config::MaxProducerVotes, message_precondition_exception,
      "Cannot approve producer; approved producer count is already at maximum");
  if (approve.approve)
    EOS_ASSERT(!slate.contains(producer.ownerName),
               message_precondition_exception, "Cannot add approval to "
                                               "producer '${name}'; producer "
                                               "is already approved",
               ("name", producer.ownerName));
  else
    EOS_ASSERT(slate.contains(producer.ownerName),
               message_precondition_exception, "Cannot remove approval from "
                                               "producer '${name}'; producer "
                                               "is not approved",
               ("name", producer.ownerName));
  ...
  db.modify(producer,
            [ approve = approve.approve, totalVotingStake,
              &raceTime ](ProducerVotesObject & pvo) {
              if (approve)
  ...
  db.modify(voter, [&approve,
                    producer = producer.ownerName ](StakedBalanceObject & sbo) {
...
void apply_eos_setproxy(apply_context &context) {
  ...
  auto& db = context.mutable_db;
  const auto& proxy = db.get<ProxyVoteObject,
  byTargetName>(context.msg.recipient(svp.proxy));
  const auto& balance = db.get<StakedBalanceObject,
  byOwnerName>(context.msg.recipient(svp.stakeholder));


  auto proxy = db.find<ProxyVoteObject,
  byTargetName>(context.msg.recipient(svp.proxy));


  if (svp.proxy != svp.stakeholder) {
     // We are enabling proxying to svp.proxy
     proxy.addProxySource(context.msg.recipient(svp.stakeholder),
  balance.stakedBalance, db);
     db.modify(balance, [target = proxy.proxyTarget](StakedBalanceObject& sbo) {
  sbo.producerVotes = target; });
  } else {
     // We are disabling proxying to balance.producerVotes.get<AccountName>()
     proxy.removeProxySource(context.msg.recipient(svp.stakeholder),
  balance.stakedBalance, db);
     db.modify(balance, [](StakedBalanceObject& sbo) { sbo.producerVotes =
  ProducerSlate{}; });
  }
  */
}
...
void apply_eos_updateauth(apply_context &context) {
  ...
  EOS_ASSERT(!update.permission.empty(), message_validate_exception,
  ...
  if (update.permission == "active")
    EOS_ASSERT(update.parent == "owner", message_validate_exception,
               "Cannot change active authority's parent");
  if (update.permission == "owner")
    EOS_ASSERT(update.parent.empty(), message_validate_exception,
  ...
  auto permission = db.find<permission_object, by_owner>(
      boost::make_tuple(update.account, update.permission));

  permission_object::id_type parent_id = 0;
  if (update.permission != "owner") {
    auto &parent = db.get<permission_object, by_owner>(
        boost::make_tuple(update.account, update.parent));
    parent_id = parent.id;
  }
  ...
  if (permission) {
    ...
    if (context.controller.is_applying_block())
      // TODO/QUESTION: If we are updating an existing permission, should we
      // check if the message declared
    ...
    // TODO/QUESTION: If we are creating a new permission, should we check if
...
void apply_eos_deleteauth(apply_context &context) {
  ...
  EOS_ASSERT(remove.permission != "active", message_validate_exception,
             "Cannot delete active authority");
  EOS_ASSERT(remove.permission != "owner", message_validate_exception,
             "Cannot delete owner authority");

  auto &db = context.mutable_db;
  context.require_authorization(remove.account);
  const auto &permission = db.get<permission_object, by_owner>(
      boost::make_tuple(remove.account, remove.permission));

  // TODO/QUESTION: If we are deleting a permission, should we check if the
  // message declared
  // permission satisfies the permission we want to delete?

  // const auto& declaredAuthority = context.msg.authorization[0];
  // const auto& declaredPermission = db.get<permission_object,
  // by_owner>(boost::make_tuple(declaredAuthority.account,
  // declaredAuthority.permission));
  // const auto& index = _db.get_index<permission_index>().indices();
  // EOS_ASSERT(declaredPermission.satisfies(permission, index),
  ...
  { // Check for children
    ...
    EOS_ASSERT(range.first == range.second, message_precondition_exception,
               "Cannot delete an authority which has children. Delete the "
               "children first");
  }

  { // Check for links to this permission
    const auto &index =
        db.get_index<permission_link_index, by_permission_name>();
    auto range =
        index.equal_range(boost::make_tuple(remove.account, remove.permission));
    EOS_ASSERT(range.first == range.second, message_precondition_exception,
...
void apply_eos_linkauth(apply_context &context) {
  ...
  EOS_ASSERT(!requirement.requirement.empty(), message_validate_exception,
  ...
  auto linkKey = boost::make_tuple(requirement.account, requirement.code,
                                   requirement.type);
  auto link = db.find<permission_link_object, by_message_type>(linkKey);

  if (link) {
    EOS_ASSERT(link->required_permission != requirement.requirement,
               message_precondition_exception, "Attempting to update required "
                                               "authority, but new requirement "
                                               "is same as old");
    if (context.controller.is_applying_block())
      db.modify(*link, [requirement = requirement.requirement](
                           permission_link_object & link) {
        link.required_permission = requirement;
      });
  } else if (context.controller.is_applying_block()) {
    db.create<permission_link_object>(
        [&requirement](permission_link_object &link) {
...
void apply_eos_unlinkauth(apply_context &context) {
  ...
  auto link = db.find<permission_link_object, by_message_type>(linkKey);
  EOS_ASSERT(link != nullptr, message_precondition_exception,
