...
administrator::get_blockchain_configuration(const chainbase::database &db,
                                            const ProducerRound &round) {
  using boost::adaptors::transformed;
  using types::AccountName;
  using chain::producer_object;

  auto ProducerNameToConfiguration =
      transformed([&db](const AccountName &owner) {
        return db.get<producer_object, chain::by_owner>(owner).configuration;
      });

  auto votes_range = round | ProducerNameToConfiguration;

  return chain::BlockchainConfiguration::get_median_values(
      {votes_range.begin(), votes_range.end()});
}
}
