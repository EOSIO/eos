...
void state_history_traces_log::prune_traces(state_history_log::block_num_type        block_num,
   ...
   auto offset                   = state_history::trace_converter::prune_traces(entry_payload, version, ids);

   if (offset < entry_payload.size()) {
      auto bytes_to_write = entry_payload.size() - offset;
      ...
      update_log.write(entry_payload.data() + offset, bytes_to_write);
   }
}
