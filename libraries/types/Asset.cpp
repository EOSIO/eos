...
Asset operator*(const Asset &a, const Price &b) {
  if (a.symbol_name() == b.base.symbol_name()) {
    ...
    auto result =
        (UInt128(a.amount) * UInt128(b.quote.amount)) / UInt128(b.base.amount);
    ...
    auto result =
        (UInt128(a.amount) * UInt128(b.base.amount)) / UInt128(b.quote.amount);
    return Asset(Int64(result), b.base.symbol);
  }
  FC_THROW_EXCEPTION(fc::assert_exception, "invalid Asset * Price",
                     ("Asset", a)("Price", b));
}
...
Price Price::max(AssetSymbol base, AssetSymbol quote) {
  return Asset(ShareType(EOS_MAX_SHARE_SUPPLY), base) /
         Asset(ShareType(1), quote);
}
Price Price::min(AssetSymbol base, AssetSymbol quote) {
  return Asset(1, base) / Asset(EOS_MAX_SHARE_SUPPLY, quote);
}
