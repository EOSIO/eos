...
template <typename T> auto packUnpack() {
  return std::make_pair<AbiSerializer::unpack_function,
                        AbiSerializer::pack_function>(
...
void AbiSerializer::configureBuiltInTypes() {
  ...
  built_in_types.emplace("AccountPermissionWeight",
                         packUnpack<AccountPermissionWeight>());
  ...
  built_in_types.emplace("KeyPermissionWeight",
                         packUnpack<KeyPermissionWeight>());
  built_in_types.emplace("Authority", packUnpack<Authority>());
  built_in_types.emplace("BlockchainConfiguration",
                         packUnpack<BlockchainConfiguration>());
...
bool AbiSerializer::isType(const TypeName &rtype) const {
  ...
  if (typedefs.find(type) != typedefs.end())
    return isType(typedefs.find(type)->second);
  ...
  for (const auto &t : typedefs) {
    try {
      ...
      while (itr != typedefs.end()) {
        FC_ASSERT(find(types_seen.begin(), types_seen.end(), itr->second) ==
                      types_seen.end(),
                  "Circular reference in type ${type}", ("type", t.first));
    ...
    try {
      if (s.second.base != TypeName()) {
        ...
        while (current.base != TypeName()) {
          const auto &base = getStruct(
              current.base); //<-- force struct to inherit from another struct
          FC_ASSERT(find(types_seen.begin(), types_seen.end(), base.name) ==
                        types_seen.end(),
...
void AbiSerializer::binaryToVariant(const TypeName &type,
...
fc::variant
AbiSerializer::binaryToVariant(const TypeName &type,
                               fc::datastream<const char *> &stream) const {
...
fc::variant AbiSerializer::binaryToVariant(const TypeName &type,
                                           const Bytes &binary) const {
  fc::datastream<const char *> ds(binary.data(), binary.size());
  return binaryToVariant(type, ds);
}

void AbiSerializer::variantToBinary(const TypeName &type,
    ...
    if (btype != built_in_types.end()) {
      ...
      for (const auto &field : st.fields) {
        if (vo.contains(String(field.name).c_str())) {
          ...
          FC_ASSERT(!"missing field in variant object",
                    "Missing '${f}' in variant object", ("f", field.name));
        }
      }
    }
  }
...
Bytes AbiSerializer::variantToBinary(const TypeName &type,
                                     const fc::variant &var) const {
  if (!isType(type)) {
    return var.as<Bytes>();
  }
