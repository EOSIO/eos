...
#define EOS_SYMBOL                                                             \
  (int64_t(4) | (uint64_t('E') << 8) | (uint64_t('O') << 16) |                 \
   (uint64_t('S') << 24))
...
struct Asset {
  Asset(ShareType a = 0, AssetSymbol id = EOS_SYMBOL) : amount(a), symbol(id) {}
...
namespace fc {
inline void to_variant(const eos::types::Asset &var, fc::variant &vo) {
  vo = var.toString();
}
