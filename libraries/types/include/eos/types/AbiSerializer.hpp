...
/**
 *  Describes the binary representation message and table contents so that it
 * can
...
struct AbiSerializer {
  ...
  typedef std::function<fc::variant(fc::datastream<const char *> &, bool)>
      unpack_function;
  typedef std::function<void(const fc::variant &, fc::datastream<char *> &,
                             bool)>
      pack_function;
  ...
  fc::variant binaryToVariant(const TypeName &type,
                              fc::datastream<const char *> &binary) const;
  void variantToBinary(const TypeName &type, const fc::variant &var,
                       fc::datastream<char *> &ds) const;

private:
  void binaryToVariant(const TypeName &type,
