...
using FixedString32 = fc::fixed_string<fc::array<
    uint64_t, 4>>; // std::tuple<uint64_t,uint64_t,uint64_t,uint64_t>>;
...
struct Name {
  ...
  void set(const char *str) {
    try {
      ...
      FC_ASSERT(toString() == String(str), "name not properly normalized",
                ("name", String(str))("normalized", toString()));
    }
...
template <typename T> struct GetStruct {};

template <> struct GetStruct<Field> {
  static const Struct &type() {
    static Struct result = {
        "Field ", "", {{"name", "FieldName"}, {"type", "TypeName"}}};
    return result;
  }
};
template <> struct GetStruct<Struct> {
  static const Struct &type() {
    static Struct result = {
        "Struct ",
        "",
        {{"name", "TypeName"}, {"base", "TypeName"}, {"fields", "Field[]"}}};
    return result;
  }
};
...
void to_variant(const std::map<std::string, eos::types::Struct> &c,
                fc::variant &v);
void from_variant(const fc::variant &v,
                  std::map<std::string, eos::types::Struct> &check);
}
