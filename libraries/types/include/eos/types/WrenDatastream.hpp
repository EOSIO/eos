...
struct InputDatastream {
  ...
  InputDatastream(const std::string &data) : ds(nullptr, 0) {
    if (data.size()) {
  ...
  void read(char *data, uint32_t len) { ds.read(data, len); }
...
struct OutputDatastream {
  ...
  OutputDatastream(uint64_t bytes) : ds(nullptr, 0) {
    FC_ASSERT(bytes <= 1024 * 1024);
    if (bytes) {
      value.resize(bytes);
  ...
  void write(const char *d, uint32_t size) { ds.write(d, size); }
