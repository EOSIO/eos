...
using std::string;
namespace eos {
using types::SimpleSymbolTable;
}

bool isVector(const string &type) { return type.back() == ']'; }
...
string generate_wren(const eos::types::Struct &s,
                     eos::types::SimpleSymbolTable &symbols) {
  ...
  for (const auto &field : s.fields) {
    ss << "        _" << field.name << " = "
       << call_type_constructor(symbols.resolveTypedef(field.type)) << "\n";
  }
  ss << "    }\n";
  ss << "    unpack( stream ) { \n";
  if (s.base.size())
  ...
  for (const auto &field : s.fields) {
    ss << "        "
       << call_unpack(field.name, symbols.resolveTypedef(field.type)) << "\n";
  }
  ss << "    }\n";
  ss << "    pack( stream ) { \n";
  if (s.base.size())
  ...
  for (const auto &field : s.fields) {
    ss << "        "
       << call_pack(field.name, symbols.resolveTypedef(field.type)) << "\n";
  }
  ss << "    }\n";
  for (const auto &field : s.fields) {
...
void generate_hpp(eos::types::SimpleSymbolTable &ss, const char *outfile) {
  ...
  for (const auto &s : ss.order) {
    ...
    out << "        " << s << "() = default;\n        " << s << "(";
    {
      bool first = true;
      for (const auto &f :
           st.fields | boost::adaptors::transformed(arrays_to_vectors)) {
        if (first)
          first = false;
        else
          out << ", ";
  ...
  for (const auto &s : ss.order) {
    ...
    const auto &st = ss.structs[s];
    if (st.base.size()) {
      out << "FC_REFLECT_DERIVED( eos::types::" << s
          << ", (eos::types::" << st.base << "), ";
    } else {
...
int count_fields(eos::types::SimpleSymbolTable &ss,
                 const eos::types::Struct &st) {
  if (st.base.size())
...
int main(int argc, char **argv) {
  try {
    FC_ASSERT(argc > 2, "Usage: ${program} input path/to/out.hpp",
