...
bool AbstractSymbolTable::isValidTypeName(const TypeName &name,
                                          bool allowArray) const {
  std::string mutable_name = name;
  if (mutable_name.empty())
    return false;
...
bool AbstractSymbolTable::isValidFieldName(const FieldName &name) const {
  std::string mutable_name = name;
  if (mutable_name.empty() || !std::islower(mutable_name[0]))
    return false;
...
void AbstractSymbolTable::parse(std::istream &in) {
  ...
  auto finishStruct = [&]() {
    FC_ASSERT(current.fields.size() > 0,
              "A struct must specify at least one field");
  ...
  for (string line; std::getline(in, line);) {
    ...
    if (line_tokens[0] == "struct") {
      ...
      FC_ASSERT(false, "Invalid token ${t} on line ${n}",
                ("t", line_tokens[0])("n", linenum));
  }
...
void SimpleSymbolTable::addType(const Struct &s) {
  try {
    ...
    EOS_ASSERT(s.base.size() == 0 || isKnownType(s.base),
               unknown_type_exception, "Unknown base type name: ${name}",
               ("name", s.base));
...
void SimpleSymbolTable::addTypeDef(const TypeName &from, const TypeName &to) {
  try {
...
bool SimpleSymbolTable::isKnownType(const TypeName &n) const {
  try {
    String name(n);
    EOS_ASSERT(!name.empty(), invalid_type_name_exception,
               "Type name cannot be empty");
    if (name.size() > 2 && name.back() == ']') {
      FC_ASSERT(name[name.size() - 2] == '[');
      return isKnownType(name.substr(0, name.size() - 2));
    }
    return known.find(n) != known.end() || typedefs.find(n) != typedefs.end() ||
...
Struct eos::types::SimpleSymbolTable::getType(TypeName name) const {
  ...
  if (known.find(name) != known.end())
    FC_THROW_EXCEPTION(type_exception,
                       "Cannot get description of built-in type ${type}",
                       ("type", name));
  FC_THROW_EXCEPTION(unknown_type_exception, "Unknown type ${type}",
                     ("type", name));
}
