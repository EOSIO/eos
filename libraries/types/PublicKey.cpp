...
PublicKey::PublicKey(const fc::ecc::public_key_data &data) : key_data(data){};

PublicKey::PublicKey(const fc::ecc::public_key &pubkey) : key_data(pubkey){};
...
PublicKey::PublicKey(const std::string &base58str) {
  ...
  FC_ASSERT(fc::ripemd160::hash(key_data.data, key_data.size())._hash[0] ==
            bin_key.check);
}

PublicKey::operator fc::ecc::public_key_data() const { return key_data; };
