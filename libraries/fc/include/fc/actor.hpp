...
#define RPC_MEMBER_FUNCTOR(z, n, IS_CONST)                                     \
  template <typename R, typename C,                                            \
            typename P BOOST_PP_ENUM_TRAILING_PARAMS(n, typename A)>           \
  static std::function<fc::future<R>(BOOST_PP_ENUM_PARAMS(n, A))> functor(     \
      P p, R (C::*mem_func)(BOOST_PP_ENUM_PARAMS(n, A)) IS_CONST,              \
      fc::thread *c = 0) {                                                     \
    return [=](BOOST_PP_ENUM_BINARY_PARAMS(n, A, a)) -> fc::future<R> {        \
      return c->async(                                                         \
          [=]() { return (p->*mem_func)(BOOST_PP_ENUM_PARAMS(n, a)); });       \
    };                                                                         \
  }
...
#else // g++ has a bug that prevents lambdas and varidic templates from working
      // together (G++ Bug 41933)

  template <typename R, typename C, typename P, typename... Args>
  static std::function<fc::future<R>(Args...)>
  functor(P &&p, R (C::*mem_func)(Args...), fc::thread *c) {
    return [=](Args... args) -> fc::future<R> {
      c->async([=]() -> R { return p->*mem_func(fc::forward<Args>(args)...); })
    };
  }
  template <typename R, typename C, typename P, typename... Args>
  static std::function<fc::future<R>(Args...)>
  functor(P &&p, R (C::*mem_func)(Args...) const, fc::thread *c) {
    return [=](Args... args) -> fc::future<R> {
      c->async([=]() -> R { return p->*mem_func(fc::forward<Args>(args)...); })
    };
  }
...
template <typename ThisPtr> struct actor_vtable_visitor {
  template <typename U>
  actor_vtable_visitor(fc::thread *t, U &&u)
      : _thread(t), _this(fc::forward<U>(u)) {}
...
public:
  ...
  actor(InterfaceType *p, fc::thread *t = &fc::thread::current()) {
    ...
    this->_vtable->template visit<InterfaceType>(
        detail::actor_vtable_visitor<InterfaceType *>(t, p));
  }
};
