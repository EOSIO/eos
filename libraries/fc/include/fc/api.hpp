...
struct identity_member {
  ...
  template <typename R, typename C, typename P, typename... Args>
  static std::function<R(Args...)> functor(P &&p,
...
template <typename OtherType> struct vtable_copy_visitor {
...
template <typename Interface, typename Transform> class api;
...
public:
  ...
  template <typename T> api<T, identity_member> as();
};
...
public:
  ...
  template <typename T>
  api(const T &p) : _vtable(std::make_shared<vtable_type>()) {
    _data = std::make_shared<fc::any>(p);
    T &ptr = boost::any_cast<T &>(*_data);
    auto &pointed_at = *ptr;
    typedef typename std::remove_reference<decltype(pointed_at)>::type
        source_vtable_type;
  ...
  virtual api_id_type register_api(
      api_connection &conn) const override; // defined in api_connection.hpp
...
#define FC_API(CLASS, METHODS)                                                 \
  ...
  struct vtable<CLASS, Transform>                                              \
      : public std::enable_shared_from_this<vtable<CLASS, Transform>> {        \
