...
public:
  ...
  unhandled_exception(log_message &&m,
...
template <typename T> fc::exception_ptr copy_exception(T &&e) {
#if defined(_MSC_VER) && (_MSC_VER < 1700)
  return std::make_shared<unhandled_exception>(
      log_message(), std::copy_exception(fc::forward<T>(e)));
#else
  return std::make_shared<unhandled_exception>(
      log_message(), std::make_exception_ptr(fc::forward<T>(e)));
#endif
}
...
#define FC_DECLARE_DERIVED_EXCEPTION(TYPE, BASE, CODE, WHAT)                   \
  ...
  public:                                                                      \
    ...
    explicit TYPE(fc::log_message &&m, int64_t code,                           \
                  const std::string &name_value,                               \
                  const std::string &what_value)                               \
        : BASE(std::move(m), code, name_value, what_value) {}                  \
    explicit TYPE(fc::log_messages &&m, int64_t code,                          \
                  const std::string &name_value,                               \
                  const std::string &what_value)                               \
        : BASE(std::move(m), code, name_value, what_value) {}                  \
    explicit TYPE(const fc::log_messages &m, int64_t code,                     \
                  const std::string &name_value,                               \
                  const std::string &what_value)                               \
...
FC_DECLARE_EXCEPTION(invalid_operation_exception,
                     invalid_operation_exception_code, "Invalid Operation");
...
void record_assert_trip(const char *filename, uint32_t lineno,
                        const char *expr);
...
/**
 *  @brief Checks a condition and throws an assert_exception if the test is
 * FALSE
 */
...
// FC_INDIRECT_EXPAND workas around a bug in Visual C++ variadic macro
// processing that prevents it
// from separating __VA_ARGS__ into separate tokens
#define FC_INDIRECT_EXPAND(MACRO, ARGS) MACRO ARGS
#define FC_THROW(...)                                                          \
  FC_MULTILINE_MACRO_BEGIN                                                     \
  throw fc::exception(                                                         \
      FC_INDIRECT_EXPAND(FC_LOG_MESSAGE, (error, __VA_ARGS__)));               \
  FC_MULTILINE_MACRO_END
...
#define FC_LOG_AND_RETHROW()                                                   \
    ...
    fc::unhandled_exception e(FC_LOG_MESSAGE(warn, "rethrow"),                 \
...
#define FC_CAPTURE_LOG_AND_RETHROW(...)                                        \
    ...
    wdump(__VA_ARGS__);                                                        \
    FC_RETHROW_EXCEPTION(er, warn, "rethrow",                                  \
                         FC_FORMAT_ARG_PARAMS(__VA_ARGS__));                   \
  }                                                                            \
  catch (const std::exception &e) {                                            \
    fc::exception fce(                                                         \
        FC_LOG_MESSAGE(warn, "rethrow ${what}: ",                              \
                       FC_FORMAT_ARG_PARAMS(__VA_ARGS__)("what", e.what())),   \
        fc::std_exception_code, typeid(e).name(), e.what());                   \
    wlog("${details}", ("details", fce.to_detail_string()));                   \
    wdump(__VA_ARGS__);                                                        \
    throw fce;                                                                 \
 ...
 *  @brief  Catchs all exception's, std::exceptions, and ... and rethrows them
 * after
...
#define FC_RETHROW_EXCEPTIONS(LOG_LEVEL, FORMAT, ...)                          \
    ...
    fc::exception fce(FC_LOG_MESSAGE(LOG_LEVEL, "${what}: " FORMAT,            \
                                     __VA_ARGS__("what", e.what())),           \
                      fc::std_exception_code, typeid(e).name(), e.what());     \
    throw fce;                                                                 \
  }                                                                            \
  catch (...) {                                                                \
