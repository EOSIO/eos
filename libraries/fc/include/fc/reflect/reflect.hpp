 ...
 *
 *  The @ref FC_REFLECT(TYPE,MEMBERS) or
 * FC_STATIC_REFLECT_DERIVED(TYPE,BASES,MEMBERS) macro is used to specialize
 * this
 *  class for your type.
 */
...
#else
// Disable warning C4482: nonstandard extention used: enum
// 'enum_type::enum_value' used in qualified name
...
#define FC_REFLECT_VISIT_MEMBER(r, visitor, elem)                              \
  {                                                                            \
    ...
    visitor.TEMPLATE operator()<member_type, type, &type::elem>(               \
...
#define FC_REFLECT_MEMBER_COUNT(r, OP, elem) OP 1
...
#define FC_REFLECT_ENUM(ENUM, FIELDS)                                          \
  ...
  template <> struct reflector<ENUM> {                                         \
    ...
    static const char *to_string(ENUM elem) {                                  \
      ...
      default:                                                                 \
        fc::throw_bad_enum_cast(fc::to_string(int64_t(elem)).c_str(),          \
  ...
  template <> struct get_typename<ENUM> {                                      \
    static const char *name() { return BOOST_PP_STRINGIZE(ENUM); }             \
  };                                                                           \
  }

/*  Note: FC_REFLECT_ENUM previously defined this function, but I don't think it
 * ever
...
#define FC_REFLECT_DERIVED(TYPE, INHERITS, MEMBERS)                            \
  ...
  template <> struct reflector<TYPE> {                                         \
    ...
    enum member_count_enum {                                                   \
      local_member_count =                                                     \
          0 BOOST_PP_SEQ_FOR_EACH(FC_REFLECT_MEMBER_COUNT, +, MEMBERS),        \
      total_member_count = local_member_count BOOST_PP_SEQ_FOR_EACH(           \
          FC_REFLECT_BASE_MEMBER_COUNT, +, INHERITS)                           \
    };                                                                         \
