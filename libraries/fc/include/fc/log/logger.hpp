 ...
 *
 @code
   void my_class::func()
   {
      fc_dlog( my_class_logger, "Format four: ${arg}  five: ${five}",
 ("arg",4)("five",5) );
   }
 @endcode
 */
...
#define dlog(FORMAT, ...)                                                      \
  ...
  if ((fc::logger::get(DEFAULT_LOGGER)).is_enabled(fc::log_level::debug))      \
    (fc::logger::get(DEFAULT_LOGGER))                                          \
        .log(FC_LOG_MESSAGE(debug, FORMAT, __VA_ARGS__));                      \
  FC_MULTILINE_MACRO_END

/**
 * Sends the log message to a special 'user' log stream designed for messages
 * that
...
#define ilog(FORMAT, ...)                                                      \
  ...
  if ((fc::logger::get(DEFAULT_LOGGER)).is_enabled(fc::log_level::info))       \
    (fc::logger::get(DEFAULT_LOGGER))                                          \
        .log(FC_LOG_MESSAGE(info, FORMAT, __VA_ARGS__));                       \
  FC_MULTILINE_MACRO_END
...
#define wlog(FORMAT, ...)                                                      \
  ...
  if ((fc::logger::get(DEFAULT_LOGGER)).is_enabled(fc::log_level::warn))       \
    (fc::logger::get(DEFAULT_LOGGER))                                          \
        .log(FC_LOG_MESSAGE(warn, FORMAT, __VA_ARGS__));                       \
  FC_MULTILINE_MACRO_END
...
#define elog(FORMAT, ...)                                                      \
  ...
  if ((fc::logger::get(DEFAULT_LOGGER)).is_enabled(fc::log_level::error))      \
    (fc::logger::get(DEFAULT_LOGGER))                                          \
        .log(FC_LOG_MESSAGE(error, FORMAT, __VA_ARGS__));                      \
  FC_MULTILINE_MACRO_END
...
#define FC_FORMAT_ARGS(r, unused, base)                                        \
  BOOST_PP_LPAREN()                                                            \
  BOOST_PP_STRINGIZE(base), fc::variant(base) BOOST_PP_RPAREN()
...
#define wdump(SEQ) wlog(FC_FORMAT(SEQ), FC_FORMAT_ARG_PARAMS(SEQ))
#define edump(SEQ) elog(FC_FORMAT(SEQ), FC_FORMAT_ARG_PARAMS(SEQ))

// this disables all normal logging statements -- not something you'd normally
// want to do,
// but it's useful if you're benchmarking something and suspect logging is
// causing
// a slowdown.
