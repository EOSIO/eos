...
/**
 *  @brief provides information about where and when a log message was
 * generated.
...
public:
  log_context();
  log_context(log_level ll, const char *file, uint64_t line,
              const char *method);
  ~log_context();
...
/**
 *  @brief aggregates a message along with the context and associated
 * meta-information.
 *  @ingroup AthenaSerializable
 *
 *  @note log_message has reference semantics, all copies refer to the same log
 * message
...
public:
   ...
   */
  log_message(log_context ctx, std::string format,
 ...
 * @brief Automatically captures the File, Line, and Method names and passes
 * them to
 ...
 * @param FORMAT A const char* string containing zero or more references to keys
 * as "${key}"
