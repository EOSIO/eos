...
template <typename T> struct is_tuple { typedef fc::false_type type; };
...
#define TUPLE(z, n, unused)                                                    \
  ...
  struct tuple<BOOST_PP_ENUM_PARAMS(n, A)> {                                   \
    ...
    explicit tuple(BOOST_PP_ENUM(n, RREF_PARAMS, unused))                      \
        BOOST_PP_IF(n,                                                         \
                    :, BOOST_PP_EMPTY())                                       \
            BOOST_PP_ENUM(n, ILIST_PARAMS, unused) {}                          \
    tuple(const tuple &t) BOOST_PP_IF(n,                                       \
                                      :, BOOST_PP_EMPTY())                     \
        BOOST_PP_ENUM(n, ILIST_PARAMS_COPY, unused) {}                         \
    tuple(tuple &&t) BOOST_PP_IF(n,                                            \
                                 :, BOOST_PP_EMPTY())                          \
        BOOST_PP_ENUM(n, ILIST_PARAMS_COPY, unused) {}                         \
    tuple() {}                                                                 \
    template <typename V> void visit(V &&v) {                                  \
      BOOST_PP_REPEAT(n, VISIT_PARAMS, a)                                      \
    }                                                                          \
    template <typename V> void visit(V &&v) const {                            \
      BOOST_PP_REPEAT(n, VISIT_PARAMS, a)                                      \
    }                                                                          \
  ...
  template <BOOST_PP_ENUM_PARAMS(n, typename AA)>                              \
  tuple<BOOST_PP_ENUM_PARAMS(n, AA)> make_tuple(                               \
      BOOST_PP_ENUM(n, RREF_PARAMS, unused)) {                                 \
    return tuple<BOOST_PP_ENUM_PARAMS(n, AA)>(                                 \
        BOOST_PP_ENUM(n, FORWARD_PARAMS, unused));                             \
  }                                                                            \
