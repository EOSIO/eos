...
template <typename T>
void from_variant(const variant &v, std::unique_ptr<T> &s);

template <typename... T>
void to_variant(const static_variant<T...> &s, variant &v);
template <typename... T>
void from_variant(const variant &v, static_variant<T...> &s);
...
template <typename T> void to_variant(const std::deque<T> &var, variant &vo);
template <typename T> void from_variant(const variant &var, std::deque<T> &vo);

template <typename T> void to_variant(const fc::flat_set<T> &var, variant &vo);
template <typename T>
...
void to_variant(const microseconds &input_microseconds,
                variant &output_variant);
void from_variant(const variant &input_variant,
                  microseconds &output_microseconds);
...
/**
 * @brief stores null, int64, uint64, double, bool, string,
 * std::vector<variant>,
...
public:
  ...
  template <typename T> void as(T &v) const { from_variant(*this, v); }
  ...
  template <typename T> variant(const optional<T> &v) {
...
void from_variant(const variant &var, uint32_t &vo);
/** @ingroup Serializable */
template <typename T> void from_variant(const variant &var, optional<T> &vo) {
  if (var.is_null())
    vo = optional<T>();
  else {
    vo = T();
...
template <typename T> variant::variant(const T &val) {
  memset(this, 0, sizeof(*this));
  to_variant(val, *this);
}
#ifdef __APPLE__
...
template <typename T, typename... Args>
void to_variant(const boost::multi_index_container<T, Args...> &c, variant &v) {
...
template <typename T, typename... Args>
void from_variant(const variant &v,
                  boost::multi_index_container<T, Args...> &c) {
  const variants &vars = v.get_array();
  c.clear();
...
FC_REFLECT_ENUM(fc::variant::type_id,
                (null_type)(int64_type)(uint64_type)(double_type)(bool_type)(
                    string_type)(array_type)(object_type)(blob_type))
