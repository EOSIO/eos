...
public:
  ...
  friend bool operator>(const uint128 &l, const int64_t &r) {
    return uint128(r) < l;
  }
  friend bool operator>(const int64_t &l, const uint128 &r) {
    return r < uint128(l);
  }
  ...
  static void full_product(const uint128 &a, const uint128 &b,
                           uint128 &result_hi, uint128 &result_lo);
...
static_assert(sizeof(uint128) == 2 * sizeof(uint64_t),
...
namespace raw {
template <typename Stream> inline void pack(Stream &s, const uint128 &u) {
  s.write((char *)&u, sizeof(u));
}
template <typename Stream> inline void unpack(Stream &s, uint128 &u) {
...
namespace std {
template <> struct hash<fc::uint128> {
