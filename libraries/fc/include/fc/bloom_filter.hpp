...
public:
  bloom_filter()
      : salt_count_(0), table_size_(0), raw_table_size_(0),
        projected_element_count_(0), inserted_element_count_(0),
        random_seed_(0), desired_false_positive_probability_(0.0) {}
  ...
  inline bool operator==(const bloom_filter &f) const {
    if (this != &f) {
      return (salt_count_ == f.salt_count_) && (table_size_ == f.table_size_) &&
             ...
             (salt_ == f.salt_) &&
             std::equal(f.bit_table_.data(),
                        f.bit_table_.data() + raw_table_size_,
                        bit_table_.data());
    } else
      return true;
  }
  ...
  inline bloom_filter &operator=(const bloom_filter &f) {
    if (this != &f) {
      ...
      std::copy(f.bit_table_.data(), f.bit_table_.data() + raw_table_size_,
                bit_table_.data());
      salt_ = f.salt_;
    }
    return *this;
  ...
  template <typename T> inline bool contains(const T &t) const {
  ...
  inline std::size_t element_count() const { return inserted_element_count_; }
...
FC_REFLECT(fc::bloom_filter,
           (salt_)(bit_table_)(salt_count_)(table_size_)(raw_table_size_)(
               projected_element_count_)(inserted_element_count_)(random_seed_)(
               desired_false_positive_probability_))
FC_REFLECT(fc::bloom_parameters::optimal_parameters_t,
           (number_of_hashes)(table_size))
FC_REFLECT(fc::bloom_parameters,
           (minimum_size)(maximum_size)(minimum_number_of_hashes)(
