 ...
 * reference
 * to the api_connection, which is made available to from_variant() as a
 * parameter.
 *
...
public:
  ...
  std::weak_ptr<fc::api_connection> get_connection() { return _api_connection; }
...
private:
  ...
  template <typename R>
  R call_generic(const std::function<R()> &f, variants::const_iterator a0,
                 variants::const_iterator e) const {
    return f();
  }
  ...
  R call_generic(const std::function<R(std::function<Signature>, Args...)> &f,
    ...
    detail::callback_functor<Signature> arg0(get_connection(),
                                             a0->as<uint64_t>());
    return call_generic<R, Args...>(
  ...
  R call_generic(const std::function<R(Arg0, Args...)> &f,
    ...
    FC_ASSERT(a0 != e, "too few arguments passed to method");
    return call_generic<R, Args...>(
        this->bind_first_arg<R, Arg0, Args...>(
            f, a0->as<typename std::decay<Arg0>::type>()),
        a0 + 1, e);
  }

  struct api_visitor {
    api_visitor(generic_api &a, const std::weak_ptr<fc::api_connection> &s)
        : _api(a), _api_con(s) {}
...
public:
  ...
  virtual variant send_call(api_id_type api_id, string method_name,
                            variants args = variants()) = 0;
  virtual variant send_callback(uint64_t callback_id,
                                variants args = variants()) = 0;
  virtual void send_notice(uint64_t callback_id,
                           variants args = variants()) = 0;

  variant receive_call(api_id_type api_id, const string &method_name,
                       const variants &args = variants()) const {
    FC_ASSERT(_local_apis.size() > api_id);
    return _local_apis[api_id]->call(method_name, args);
  }
  variant receive_callback(uint64_t callback_id,
                           const variants &args = variants()) const {
...
private:
  ...
  struct api_visitor {
    ...
    static Result from_variant(const variant &v, Result *,
    ...
    template <typename T>
    static fc::variant
    convert_callbacks(const std::shared_ptr<fc::api_connection> &, const T &v) {
      return fc::variant(v);
    }

    template <typename Signature>
    static fc::variant
    convert_callbacks(const std::shared_ptr<fc::api_connection> &con,
 ...
 * to an api_connection.  The reason this is necessary is we have a goal of
 * being
...
api_id_type
api<Interface, Transform>::register_api(api_connection &conn) const {
...
operator()(Args... args) const {
  ...
  locked->send_callback(_callback_id, fc::variants{args...})
