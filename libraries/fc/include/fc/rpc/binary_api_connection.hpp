...
namespace detail {
template <typename Signature> class callback_functor {
public:
...
std::function<R(Args...)>
bind_first_arg(const std::function<R(Arg0, Args...)> &f, Arg0 a0) {
  return [=](Args... args) { return f(a0, args...); };
}
template <typename R>
R call_generic(const std::function<R()> &f, variants::const_iterator a0,
               variants::const_iterator e) {
  return f();
}

template <typename R, typename Arg0, typename... Args>
R call_generic(const std::function<R(Arg0, Args...)> &f,
               variants::const_iterator a0, variants::const_iterator e) {
  FC_ASSERT(a0 != e);
  return call_generic<R, Args...>(
      bind_first_arg<R, Arg0, Args...>(
          f, a0->as<typename std::decay<Arg0>::type>()),
      a0 + 1, e);
}

template <typename R, typename... Args>
std::function<variant(const fc::variants &)>
to_generic(const std::function<R(Args...)> &f) {
...
std::function<variant(const fc::variants &)>
to_generic(const std::function<void(Args...)> &f) {
  return [=](const variants &args) {
...
/**
 * If api<T> is returned from a remote method, the API is eagerly bound to
 * api<T> of
 * the correct type in api_visitor::from_variant().  This binding [1] needs a
 * reference
 * to the binary_api_connection, which is made available to from_variant() as a
 * parameter.
 *
 * However, in the case of a remote method which returns api_base which can
 * subsequently
 * be cast by the caller with as<T>, we need to keep track of the connection
 * because
...
public:
  ...
  vector<char> call(const string &name, const vector<char> &args) {
    ...
    FC_ASSERT(itr != _by_name.end(), "no method with name '${name}'",
...
private:
  ...
  template <typename R, typename Arg0, typename... Args>
  std::function<R(Args...)>
  bind_first_arg(const std::function<R(Arg0, Args...)> &f, Arg0 a0) const {
    return [=](Args... args) { return f(a0, args...); };
  }

  template <typename R>
  R call_generic(const std::function<R()> &f,
                 datastream<const char *> &ds) const {
    return f();
  }
  ...
  R call_generic(const std::function<R(std::function<Signature>, Args...)> &f,
    ...
    return call_generic<R, Args...>(
        this->bind_first_arg<R, std::function<Signature>, Args...>(
            f, std::function<Signature>(arg0)),
        ds);
  }
  template <typename R, typename Signature, typename... Args>
  R call_generic(
      const std::function<R(const std::function<Signature> &, Args...)> &f,
      fc::datastream<const char *> &ds) {
    uint64_t callback_id = 0;
    fc::raw::unpack(ds, callback_id);
    detail::callback_functor<Signature> arg0(get_connection(), callback_id);
    return call_generic<R, Args...>(
        this->bind_first_arg<R, const std::function<Signature> &, Args...>(
            f, arg0),
        ds);
  }
  ...
  R call_generic(const std::function<R(Arg0, Args...)> &f,
    ...
    fc::raw::unpack(ds, a0);
    return call_generic<R, Args...>(
        this->bind_first_arg<R, Arg0, Args...>(f, a0), ds);
  }

  struct api_visitor {
    api_visitor(generic_api &a,
                const std::weak_ptr<fc::binary_api_connection> &s)
        : api(a), _api_con(s) {}

    template <typename Interface, typename Adaptor, typename... Args>
    std::function<variant(const fc::variants &)>
    to_generic(const std::function<api<Interface, Adaptor>(Args...)> &f) const;

    template <typename Interface, typename Adaptor, typename... Args>
    std::function<variant(const fc::variants &)> to_generic(
        const std::function<fc::optional<api<Interface, Adaptor>>(Args...)> &f)
        const;

    template <typename... Args>
    std::function<variant(const fc::variants &)>
    to_generic(const std::function<fc::api_ptr(Args...)> &f) const;

    template <typename R, typename... Args>
    std::function<variant(const fc::variants &)>
    to_generic(const std::function<R(Args...)> &f) const;

    template <typename... Args>
    std::function<variant(const fc::variants &)>
    to_generic(const std::function<void(Args...)> &f) const;

    template <typename Result, typename... Args>
    void operator()(const char *name,
                    std::function<Result(Args...)> &memb) const {
...
class binary_api_connection
    : public std::enable_shared_from_this<fc::binary_api_connection> {
public:
  ...
  template <typename T> api<T> get_remote_api(api_id_type api_id = 0) {
    api<T> result;
  ...
  virtual result_type send_call(api_id_type api_id, string method_name,
                                params_type args = params_type()) = 0;
  virtual result_type send_callback(uint64_t callback_id,
                                    params_type args = params_type()) = 0;
  virtual void send_notice(uint64_t callback_id,
                           params_type args = params_type()) = 0;

  result_type receive_call(api_id_type api_id, const string &method_name,
                           const params_type &args = params_type()) const {
    FC_ASSERT(_local_apis.size() > api_id);
    return _local_apis[api_id]->call(method_name, args);
  }
  result_type receive_callback(uint64_t callback_id,
                               const params_type &args = params_type()) const {
    FC_ASSERT(_local_callbacks.size() > callback_id);
    return _local_callbacks[callback_id](args);
  }
  void receive_notice(uint64_t callback_id,
                      const params_type &args = params_type()) const {
    FC_ASSERT(_local_callbacks.size() > callback_id);
    _local_callbacks[callback_id](args);
  }

  template <typename Interface> api_id_type register_api(const Interface &a) {
    auto handle = a.get_handle();
    auto itr = _handle_to_id.find(handle);
    if (itr != _handle_to_id.end())
      return itr->second;

    _local_apis.push_back(
        std::unique_ptr<generic_api>(new generic_api(a, shared_from_this())));
  ...
  get_method_names(api_id_type local_api_id = 0) const {
    return _local_apis[local_api_id]->get_method_names();
  }
...
private:
  ...
  struct api_visitor {
    ...
    from_vector(const vector<char> &v,
                ...
                const std::shared_ptr<fc::binary_api_connection> &con) {
      return fc::api_ptr(
          new detail::any_api(fc::raw::unpack<uint64_t>(v), con));
    }

    template <typename T>
    static result_type
    convert_callbacks(const std::shared_ptr<fc::binary_api_connection> &,
                      const T &v) {
    ...
    template <typename Signature>
    static result_type
    convert_callbacks(const std::shared_ptr<fc::binary_api_connection> &con,
    ...
    template <typename... Args>
    void operator()(const char *name,
                    std::function<void(Args...)> &memb) const {
      auto con = _connection;
      auto api_id = _api_id;
      memb = [con, api_id, name](Args... args) {
        con->send_call(api_id, name, {convert_callbacks(con, args)...});
...
public:
  /** makes calls to the remote server */
  virtual result_type send_call(api_id_type api_id, string method_name,
                                params_type args = params_type()) override {
    FC_ASSERT(_remote_connection);
    return _remote_connection->receive_call(api_id, method_name,
                                            std::move(args));
  }
  virtual result_type send_callback(uint64_t callback_id,
                                    params_type args = params_type()) override {
  ...
  void
  set_remote_connection(const std::shared_ptr<fc::binary_api_connection> &rc) {
    FC_ASSERT(!_remote_connection);
...
template <typename Api>
generic_api::generic_api(const Api &a,
                         const std::shared_ptr<fc::binary_api_connection> &c)
...
template <typename Interface, typename Adaptor, typename... Args>
std::function<result_type(const params_type &)>
generic_api::api_visitor::to_generic(
...
std::function<result_type(const params_type &)>
generic_api::api_visitor::to_generic(
    const std::function<fc::optional<fc::api<Interface, Adaptor>>(Args...)> &f)
    const {
  auto api_con = _api_con;
  auto gapi = &api;
...
template <typename R, typename... Args>
std::function<result_type(const params_type &)>
generic_api::api_visitor::to_generic(const std::function<R(Args...)> &f) const {
 ...
 * to an binary_api_connection.  The reason this is necessary is we have a goal
 * of being
 ...
 * it a pure virtual method in the base class which is overridden by the
 * subclass's
 * implementation).
 */
template <typename Interface, typename Transform>
api_id_type
api<Interface, Transform>::register_api(binary_api_connection &conn) const {
  return conn.register_api(*this);
}

template <typename T> api<T> api_base::as() {
  ...
  std::shared_ptr<binary_api_connection> api_conn =
      maybe_any->_binary_api_connection.lock();
  FC_ASSERT(api_conn);
...
template <typename Signature>
template <typename... Args>
typename callback_functor<Signature>::result_type callback_functor<Signature>::
operator()(Args... args) const {
  std::shared_ptr<fc::binary_api_connection> locked =
      _binary_api_connection.lock();
  // TODO:  make new exception type for this instead of recycling eof_exception
  if (!locked)
    throw fc::eof_exception();

  /// TODO------------->>> pack args...
  locked->send_callback(_callback_id, fc::raw::pack(args...))
      .template as<result_type>();
}

template <typename... Args> class callback_functor<void(Args...)> {
public:
  ...
  void operator()(Args... args) const {
    std::shared_ptr<fc::binary_api_connection> locked =
        _binary_api_connection.lock();
    // TODO:  make new exception type for this instead of recycling
    // eof_exception
    if (!locked)
      throw fc::eof_exception();
