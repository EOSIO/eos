...
template <size_t N> class array<unsigned char, N> {
public:
...
/** provided for default 0 init */
template <size_t N> class array<char, N> {
public:
  typedef char T;
  array() { memset(data, 0, sizeof(data)); }
  /**
...
template <typename T, size_t N> struct get_typename<fc::array<T, N>> {
  static const char *name() {
    static std::string _name = std::string("fc::array<") +
...
namespace std {
template <typename T, size_t N> struct hash<fc::array<T, N>> {
