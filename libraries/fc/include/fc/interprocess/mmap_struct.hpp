 ...
 *  @note T must be POD
 */
template <typename T> class mmap_struct : public detail::mmap_struct_base {
public:
  mmap_struct() : _mapped_struct(nullptr) {}
  /**
   *  Create the file if it does not exist or is of the wrong size if create is
   * true, then maps
   *  the file to memory.
   *
   *  @throw an exception if the file does not exist or is the wrong size and
   * create is false
   */
