...
#ifdef _MSC_VER
#pragma warning(disable : 4482) // nonstandard extension used enum Name::Val,
                                // standard in C++11
...
template <typename T> typename fc::remove_reference<T>::type &&move(T &&t) {
  return static_cast<typename fc::remove_reference<T>::type &&>(t);
}

template <typename T, typename U> inline T &&forward(U &&u) {
...
template <typename T> void fc_swap(T &a, T &b) {
  T tmp = fc::move(a);
  a = fc::move(b);
  b = fc::move(tmp);
}
