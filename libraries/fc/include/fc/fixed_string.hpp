...
/**
 *  This class is designed to offer in-place memory allocation of a string up to
 * Length equal to
 *  sizeof(Storage).
 *
 *  The string will serialize the same way as std::string for variant and raw
 * formats
 *  The string will sort according to the comparison operators defined for
 * Storage, this enables effecient
 *  sorting.
 */
template <typename Storage = std::pair<uint64_t, uint64_t>> class fixed_string {
public:
