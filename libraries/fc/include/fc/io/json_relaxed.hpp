...
template <typename T, bool strict> variant variant_from_stream(T &in);

template <typename T> std::string tokenFromStream(T &in) {
  ...
  try {
      ...
      if (strict)
        ...
        while (true) {
          ...
          if (c == q) {
            ...
            FC_THROW_EXCEPTION(parse_error_exception,
                               "unexpected EOF in string '${token}'",
                               ("token", token.str()));
    ...
    while (true) {
      ...
      if (c == q) {
        ...
        FC_THROW_EXCEPTION(parse_error_exception,
                           "unexpected EOL in string '${token}'",
                           ("token", token.str()));
      else {
        in.get();
        token << c;
...
template <typename T, bool strict> std::string stringFromStream(T &in) {
  try {
    char c = in.peek(), c2;
...
fc::variant parseInt(const std::string &token, size_t start) {
  ...
  static const uint64_t INT64_MAX_PLUS_ONE =
  ...
  if (token[0] == '-') {
    if (val > INT64_MAX_PLUS_ONE)
      FC_THROW_EXCEPTION(parse_error_exception,
                         "negative integer literal overflow");
    // special cased to avoid trying to compute -INT64_MIN which is probably
...
template <bool strict> fc::variant parseNumberOrStr(const std::string &token) {
  try {
    // ilog( (token) );
    ...
    case '+':
      if (strict)
        FC_THROW_EXCEPTION(parse_error_exception,
                           "unary + not supported in strict mode");
      i++;
      break;
    case '-':
      i++;
    ...
    case '/':
      if (strict)
        FC_THROW_EXCEPTION(parse_error_exception,
                           "illegal character '{c}' parsing number", ("c", c));
      return fc::variant(token);
    default:
      FC_THROW_EXCEPTION(parse_error_exception,
                         "illegal character '{c}' in token", ("c", c));
    }
    size_t start = i - 1;
    ...
    while (true) {
      ...
      case 'E':
        ...
        case '/':
          if (strict)
            FC_THROW_EXCEPTION(parse_error_exception,
                               "illegal character '{c}' in number", ("c", c));
          return fc::variant(token);
        default:
          FC_THROW_EXCEPTION(parse_error_exception,
                             "illegal character '{c}' in token", ("c", c));
...
template <typename T, bool strict> variant_object objectFromStream(T &in) {
...
template <typename T, bool strict> variants arrayFromStream(T &in) {
  variants ar;
  try {
    if (in.peek() != '[')
...
template <typename T, bool strict> variant numberFromStream(T &in) {
  try {
    std::string token = tokenFromStream(in);
    variant result = json_relaxed::parseNumberOrStr<strict>(token);
    if (strict &&
        !(result.is_int64() || result.is_uint64() || result.is_double()))
...
template <typename T, bool strict> variant wordFromStream(T &in) {
  std::string token = tokenFromStream(in);
