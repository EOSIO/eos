...
using shared_string = bip::basic_string<
...
using UInt =
    number<cpp_int_backend<Size, Size, unsigned_magnitude, unchecked, void>>;
template <size_t Size>
using Int =
    number<cpp_int_backend<Size, Size, signed_magnitude, unchecked, void>>;
...
template <typename Stream> inline void pack(Stream &s, const fc::exception &e) {
  fc::raw::pack(s, e.code());
  fc::raw::pack(s, std::string(e.name()));
  fc::raw::pack(s, std::string(e.what()));
  fc::raw::pack(s, e.get_log());
}
template <typename Stream> inline void unpack(Stream &s, fc::exception &e) {
  int64_t code;
...
template <typename Stream> inline void unpack(Stream &s, fc::log_message &msg) {
  fc::variant vmsg;
  fc::raw::unpack(s, vmsg);
  msg = vmsg.as<log_message>();
}

template <typename Stream> inline void pack(Stream &s, const fc::path &tp) {
  fc::raw::pack(s, tp.generic_string());
}

template <typename Stream> inline void unpack(Stream &s, fc::path &tp) {
  std::string p;
  fc::raw::unpack(s, p);
  tp = p;
}
...
template <typename Stream> inline void unpack(Stream &s, fc::time_point &tp) {
  try {
    uint64_t usec;
...
inline void unpack(Stream &s, fc::array<T, N> &v) {
  ...
  FC_RETHROW_EXCEPTIONS(warn, "fc::array<type,length>",
                        ("type", fc::get_typename<T>::name())("length", N))
}
...
inline void unpack(Stream &s, std::shared_ptr<T> &v) {
  ...
  FC_RETHROW_EXCEPTIONS(warn, "std::shared_ptr<T>",
...
template <typename Stream, typename T>
inline void unpack(Stream &s, const T &vi) {
  T tmp;
  fc::raw::unpack(s, tmp);
  FC_ASSERT(vi == tmp);
}
...
template <typename Stream, typename T>
void pack(Stream &s, const fc::smart_ref<T> &v) {
  fc::raw::pack(s, *v);
}

template <typename Stream, typename T>
void unpack(Stream &s, fc::smart_ref<T> &v) {
  fc::raw::unpack(s, *v);
}
...
void unpack(Stream &s, fc::optional<T> &v) {
  ...
  FC_RETHROW_EXCEPTIONS(warn, "optional<${type}>",
                        ("type", fc::get_typename<T>::name()))
}
...
template <typename Stream, typename Class> struct pack_object_visitor {
  pack_object_visitor(const Class &_c, Stream &_s) : c(_c), s(_s) {}
...
template <typename Stream, typename Class> struct unpack_object_visitor {
  unpack_object_visitor(Class &_c, Stream &_s) : c(_c), s(_s) {}
  ...
  inline void operator()(const char *name) const {
    ...
    FC_RETHROW_EXCEPTIONS(warn, "Error unpacking field ${field}",
                          ("field", name))
  }
...
template <typename IsClass = fc::true_type> struct if_class {
  template <typename Stream, typename T>
...
template <> struct if_class<fc::false_type> {
  template <typename Stream, typename T>
  static inline void pack(Stream &s, const T &v) {
...
template <typename IsEnum = fc::false_type> struct if_enum {
  template <typename Stream, typename T>
  static inline void pack(Stream &s, const T &v) {
...
template <typename IsReflected = fc::false_type> struct if_reflected {
  template <typename Stream, typename T>
  static inline void pack(Stream &s, const T &v) {
...
template <> struct if_reflected<fc::true_type> {
  template <typename Stream, typename T>
  static inline void pack(Stream &s, const T &v) {
...
template <typename Stream, typename T> inline void pack(Stream &s, const T &v) {
  fc::raw::detail::if_reflected<typename fc::reflector<T>::is_defined>::pack(s,
                                                                             v);
}
template <typename Stream, typename T> inline void unpack(Stream &s, T &v) {
  try {
    fc::raw::detail::if_reflected<
        typename fc::reflector<T>::is_defined>::unpack(s, v);
  }
...
void pack(Stream &s, const boost::multiprecision::number<T> &n) {
  static_assert(
      sizeof(n) ==
          (std::numeric_limits<boost::multiprecision::number<T>>::digits + 1) /
              8,
      "unexpected padding");
