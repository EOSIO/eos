...
public:
  ...
  template <typename T> static sha256 hash(const T &t) {
    sha256::encoder e;
    fc::raw::pack(e, t);
    return e.result();
  }
  ...
  template <typename T> inline friend T &operator<<(T &ds, const sha256 &ep) {
    ds.write(ep.data(), sizeof(ep));
    return ds;
  }

  template <typename T> inline friend T &operator>>(T &ds, sha256 &ep) {
  ...
  uint32_t pop_count() const {
    return (uint32_t)(
        __builtin_popcountll(_hash[0]) + __builtin_popcountll(_hash[1]) +
   ...
   * - Low 24 bits of result are the 24 bits of input immediately after the most
   * significant 1 in the input.
   * - If above would require reading beyond the end of the input, zeros are
   * used instead.
   */
...
namespace std {
template <> struct hash<fc::sha256> {
