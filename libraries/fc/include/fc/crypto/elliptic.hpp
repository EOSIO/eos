...
typedef fc::array<char, 65>
    public_key_point_data; ///< the full non-compressed version of the ECC point
...
public:
  ...
  /**
   *  This method of generation enables creating a new private key in a
   * deterministic manner relative to
   *  an initial seed.   A public_key created from the seed can be multiplied by
   * the offset to calculate
   *  the new public key without having to know the private key.
   */
  static private_key
  generate_from_seed(const fc::sha256 &seed,
  ...
  //           bool              verify( const fc::sha256& digest, const
  //           signature& sig );
...
public:
  extended_public_key(const public_key &k, const sha256 &c, int child = 0,
...
public:
  extended_private_key(const private_key &k, const sha256 &c, int child = 0,
                       int parent_fp = 0, uint8_t depth = 0);
  ...
  static extended_private_key generate_master(const char *seed,
                                              uint32_t seed_len);
...
blind_factor_type blind_sum(const std::vector<blind_factor_type> &blinds,
                            uint32_t non_neg);
/**  verifies taht commnits + neg_commits + excess == 0 */
bool verify_sum(const std::vector<commitment_type> &commits,
                const std::vector<commitment_type> &neg_commits,
                int64_t excess);
bool verify_range(uint64_t &min_val, uint64_t &max_val,
                  const commitment_type &commit, const range_proof_type &proof);
...
namespace raw {
template <typename Stream> void unpack(Stream &s, fc::ecc::public_key &pk) {
  ecc::public_key_data ser;
  fc::raw::unpack(s, ser);
  pk = fc::ecc::public_key(ser);
}

template <typename Stream> void pack(Stream &s, const fc::ecc::public_key &pk) {
  fc::raw::pack(s, pk.serialize());
}

template <typename Stream> void unpack(Stream &s, fc::ecc::private_key &pk) {
  fc::sha256 sec;
  unpack(s, sec);
