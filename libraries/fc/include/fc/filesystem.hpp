...
namespace fc {
/**
 *  @brief wraps boost::filesystem::path to provide platform independent path
 * manipulation.
 *
 *  Most calls are simply a passthrough to boost::filesystem::path, however
 * exceptions are
 *  wrapped in an fc::error_report and the additional helper method
...
public:
  ...
  /** On windows, returns a path where all path separators are '\' suitable for
   * displaying
...
/** @return the home directory on Linux and OS X and the Profile directory on
 * Windows */
const path &home_path();

/** @return the home_path() on Linux, home_path()/Library/Application Support/
 * on OS X,
...
template <> struct get_typename<path> {
  static const char *name() { return "path"; }
};

/**
 * Class which creates a temporary directory inside an existing temporary
 * directory.
 */
...
public:
  ...
  temp_file(const fc::path &tempFolder = fc::temp_directory_path(),
            bool create = false);
};
...
// this code is known to work on linux and windows.  It may work correctly on
// mac,
// or it may need slight tweaks or extra includes.  It's disabled now to avoid
// giving
 ...
 *     return 1;
 *   }
 *   // do stuff here, file will be unlocked when instance_lock goes out of
 * scope
 * }
*/
