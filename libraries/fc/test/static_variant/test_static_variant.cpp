...
BOOST_AUTO_TEST_SUITE(static_variant_test_suite)
   ...
   BOOST_AUTO_TEST_CASE(get_if)
   {
     ...
     auto v1 = variant_type{"hello world"};
     BOOST_CHECK_EXCEPTION(fc::get_if<int32_t>(v1), fc::assert_exception, [](const auto& e) { return true; });
     auto result1 = fc::get_if<std::string>(v1);
     BOOST_REQUIRE(result1 == std::string{"hello world"});

     const auto v2 = variant_type{"hello world"};
     BOOST_CHECK_EXCEPTION(fc::get_if<int32_t>(v2), fc::assert_exception, [](const auto& e) { return true; });
