...
public:
  impl()
      : _parent(nullptr), _enabled(true), _additivity(false),
...
logger::logger(const string &name, const logger &parent) : my(new impl()) {
  my->_name = name;
  my->_parent = parent;
}
...
logger::logger(logger &&l) : my(fc::move(l.my)) {}
...
bool logger::is_enabled(log_level e) const { return e >= my->_level; }
