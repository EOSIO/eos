...
bool configure_logging(const logging_config &cfg) {
  try {
    ...
    static bool reg_file_appender =
        false; // appender::register_appender<file_appender>( "file" );
    get_logger_map().clear();
    get_appender_map().clear();

    // slog( "\n%s", fc::json::to_pretty_string(cfg).c_str() );
    for (size_t i = 0; i < cfg.appenders.size(); ++i) {
      appender::create(cfg.appenders[i].name, cfg.appenders[i].type,
                       cfg.appenders[i].args);
...
void set_thread_name(const string &name) { thread_name = name; }
