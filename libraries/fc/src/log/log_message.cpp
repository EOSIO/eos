...
public:
  log_message_impl(log_context &&ctx) : context(std::move(ctx)) {}
  log_message_impl() {}
...
log_context::log_context() : my(std::make_shared<detail::log_context_impl>()) {}
...
log_context::log_context(log_level ll, const char *file, uint64_t line,
  ...
  my->level = ll;
  my->file =
      fc::path(file).filename().generic_string(); // TODO truncate filename
  my->line = line;
  my->method = method;
...
fc::string log_context::to_string() const {
  return my->thread_name + "  " + my->file + ":" + fc::to_string(my->line) +
         " " + my->method;
}
...
void to_variant(const log_context &l, variant &v) { v = l.to_variant(); }

void from_variant(const variant &l, log_context &c) { c = log_context(l); }

void from_variant(const variant &l, log_message &c) { c = log_message(l); }
void to_variant(const log_message &m, variant &v) { v = m.to_variant(); }
...
void from_variant(const variant &v, log_level &e) {
  ...
  FC_RETHROW_EXCEPTIONS(
      error, "Expected 'all|debug|info|warn|error|off', but got '${variant}'",
      ("variant", v));
}
...
variant log_context::to_variant() const {
  ...
  o("level", variant(my->level))("file", my->file)("line", my->line)(
      "method", my->method)("hostname", my->hostname)(
...
log_message::log_message() : my(std::make_shared<detail::log_message_impl>()) {}

log_message::log_message(log_context ctx, std::string format,
                         variant_object args)
