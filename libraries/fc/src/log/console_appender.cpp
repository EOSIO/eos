...
console_appender::console_appender(const variant &args) : my(new impl) {
  configure(args.as<config>());
}

console_appender::console_appender(const config &cfg) : my(new impl) {
  configure(cfg);
}
console_appender::console_appender() : my(new impl) {}
...
void console_appender::configure(const config &console_appender_config) {
  try {
    ...
    for (auto itr = my->cfg.level_colors.begin();
         itr != my->cfg.level_colors.end(); ++itr)
...
void console_appender::log(const log_message &m) {
  ...
  // fc::string fmt_str = fc::format_string( cfg.format,
  // mutable_variant_object(m.get_context())( "message", message)  );
  std::stringstream file_line;
  file_line << m.get_context().get_file() << ":"
            << m.get_context().get_line_number() << " ";

  ///////////////
  std::stringstream line;
  line << (m.get_context().get_timestamp().time_since_epoch().count() %
           (1000ll * 1000ll * 60ll * 60)) /
              1000
       << "ms ";
  line << std::setw(10) << std::left
       << m.get_context().get_thread_name().substr(0, 9).c_str() << " "
  ...
  if (me.size()) {
    ...
    if (me[p] == ':')
      ++p;
    line << std::setw(20) << std::left
         << m.get_context().get_method().substr(p, 20).c_str() << " ";
  }
  line << "] ";
  fc::string message = fc::format_string(m.get_format(), m.get_data());
  line << message; //.c_str();
...
void console_appender::print(const std::string &text, color::type text_color) {
  ...
  if (isatty(fileno(out)))
    fprintf(out, "\r%s", get_console_color(text_color));
#endif
...
#else
  if (isatty(fileno(out)))
    fprintf(out, "\r%s", CONSOLE_DEFAULT);
#endif
