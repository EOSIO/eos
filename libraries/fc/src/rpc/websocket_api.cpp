...
websocket_api_connection::websocket_api_connection(
    fc::http::websocket_connection &c)
    : _connection(c) {
  ...
  _rpc_state.on_unhandled(
      [&](const std::string &method_name, const variants &args) {
...
variant websocket_api_connection::send_call(api_id_type api_id,
                                            string method_name,
...
variant
websocket_api_connection::send_callback(uint64_t callback_id,
                                        variants args /* = variants() */) {
  auto request =
      _rpc_state.start_remote_call("callback", {callback_id, std::move(args)});
  _connection.send_message(fc::json::to_string(request));
  return _rpc_state.wait_for_response(*request.id);
}

void websocket_api_connection::send_notice(uint64_t callback_id,
                                           variants args /* = variants() */) {
  fc::rpc::request req{
      optional<uint64_t>(), "notice", {callback_id, std::move(args)}};
  _connection.send_message(fc::json::to_string(req));
}

std::string
websocket_api_connection::on_message(const std::string &message,
  ...
  try {
    ...
    if (var_obj.contains("method")) {
      ...
      if (optexcept) {
        ...
        auto reply = fc::json::to_string(
            response(*call.id, error_object{1, optexcept->to_detail_string(),
                                            fc::variant(*optexcept)}));
        if (send_message)
          _connection.send_message(reply);
