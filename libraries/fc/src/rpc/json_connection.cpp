...
public:
  json_connection_impl(fc::buffered_istream_ptr &&in,
                       fc::buffered_ostream_ptr &&out)
      : _in(fc::move(in)), _out(fc::move(out)), _eof(false), _next_id(0),
        _logger("json_connection") {}
  ...
  boost::unordered_map<std::string, json_connection::named_param_method>
      _named_param_methods;
  ...
  void send_error(variant id, fc::exception &e) {
    ilog("send: {\"id\": ${i}, \"error\":{\"message\": "
         "${what},\"code\":0,\"data\":${data}}}",
      ...
      if (m != obj.end()) {
        ...
        try {
          ...
          if (p == obj.end()) {
            ...
            {
              FC_THROW_EXCEPTION(exception, "Invalid Method '${method}'",
            ...
            } else // invalid method / param combo
            {
              FC_THROW_EXCEPTION(exception,
                                 "Invalid method or params  '${method}'",
                                 ("method", m->value().as_string()));
            }
        ...
        if (exception_caught && i != obj.end())
          ...
          fc_wlog(_logger, "json rpc exception: ${exception}",
                  ("exception", except));
          ...
          } else if (e != obj.end()) // if error response
          {
            ...
            try {
              ...
              if (data != err.end()) {
                ...
                await->second->set_exception(
                    data->value().as<exception>().dynamic_copy_exception());
              } else
                await->second->set_exception(exception_ptr(new FC_EXCEPTION(
                    exception, "${error}", ("error", e->value()))));
            } catch (fc::exception &e) {
              elog("Error parsing exception: ${e}",
                   ("e", e.to_detail_string()));
              eptr = e.dynamic_copy_exception();
            }
            if (eptr)
              await->second->set_exception(eptr);
          } else // id found without error, result, nor method field
          {
            fc_wlog(_logger, "no error or result specified in '${message}'",
                    ("message", obj));
          }
        }
  ...
  void read_loop() {
    ...
    try {
      ...
      while (!_done.canceled()) {
        ...
        _handle_message_future =
            fc::async([=]() { handle_message(v.get_object()); },
                      "json_connection handle_message");
      }
      ...
      eptr = fc::exception_ptr(
          new FC_EXCEPTION(unhandled_exception, "json connection read error"));
    }
    if (eptr)
      close(eptr);
  }
...
json_connection::json_connection(fc::buffered_istream_ptr in,
                                 fc::buffered_ostream_ptr out)
    : my(new detail::json_connection_impl(fc::move(in), fc::move(out))) {}

json_connection::~json_connection() { close(); }
...
fc::future<void> json_connection::exec() {
  ...
  return my->_done =
             fc::async([=]() { my->read_loop(); }, "json_connection read_loop");
}

void json_connection::close() {
  try {
    if (my->_handle_message_future.valid() &&
        !my->_handle_message_future.ready())
...
void json_connection::set_on_disconnected_callback(
    std::function<void(exception_ptr)> callback) {
...
void json_connection::add_named_param_method(const fc::string &name,
                                             named_param_method m) {
  ilog("add named param method ${name}", ("name", name));
  my->_named_param_methods.emplace(
      std::pair<std::string, named_param_method>(name, fc::move(m)));
}
...
future<variant> json_connection::async_call(const fc::string &method,
                                            const variants &args) {
  auto id = my->_next_id++;
  my->_awaiting[id] = fc::promise<variant>::ptr(
...
future<variant>
json_connection::async_call(const fc::string &method, const variant &a1,
                            const variant &a2, const variant &a3,
                            const variant &a4, const variant &a5) {
  auto id = my->_next_id++;
  my->_awaiting[id] = fc::promise<variant>::ptr(
      new fc::promise<variant>("json_connection::async_call"));
...
json_connection::async_call(const fc::string &method, const variant &a1,
                            const variant &a2, const variant &a3,
                            const variant &a4, const variant &a5,
                            const variant &a6, const variant &a7) {
  auto id = my->_next_id++;
  my->_awaiting[id] = fc::promise<variant>::ptr(
      new fc::promise<variant>("json_connection::async_call"));
...
future<variant> json_connection::async_call(
    const fc::string &method, const variant &a1, const variant &a2,
    const variant &a3, const variant &a4, const variant &a5, const variant &a6,
    const variant &a7, const variant &a8) {
  auto id = my->_next_id++;
  my->_awaiting[id] = fc::promise<variant>::ptr(
      new fc::promise<variant>("json_connection::async_call"));
...
future<variant> json_connection::async_call(
    const fc::string &method, const variant &a1, const variant &a2,
    const variant &a3, const variant &a4, const variant &a5, const variant &a6,
    const variant &a7, const variant &a8, const variant &a9) {
  auto id = my->_next_id++;
  my->_awaiting[id] = fc::promise<variant>::ptr(
      new fc::promise<variant>("json_connection::async_call"));
...
future<variant> json_connection::async_call(const fc::string &method,
                                            mutable_variant_object named_args) {
...
logger json_connection::get_logger() const { return my->_logger; }

void json_connection::set_logger(const logger &l) { my->_logger = l; }
}
}
