...
void bstate::remove_method(const fc::string &name) { _methods.erase(name); }

result_type bstate::local_call(const string &method_name,
  ...
  FC_ASSERT(method_itr != _methods.end(), "Unknown Method: ${name}",
            ("name", method_name));
...
void bstate::handle_reply(const bresponse &bresponse) {
  ...
  FC_ASSERT(await != _awaiting.end(), "Unknown Response ID: ${id}",
            ("id", bresponse.id)("bresponse", bresponse));
  if (bresponse.result)
    await->second->set_value(*bresponse.result);
  else if (bresponse.error) {
    await->second->set_exception(std::make_exception_ptr(
        FC_EXCEPTION(exception, "${error}",
                     ("error", bresponse.error->message)("data", bresponse))));
  } else
    await->second->set_value(params_type());
  _awaiting.erase(await);
}

brequest bstate::start_remote_call(const string &method_name,
                                   params_type args) {
...
void bstate::close() {
  for (auto &item : _awaiting)
    item.second->set_exception(std::make_exception_ptr(
        FC_EXCEPTION(eof_exception, "connection closed")));
  _awaiting.clear();
}
void bstate::on_unhandled(
    const std::function<result_type(const string &, const params_type &)>
        &unhandled) {
