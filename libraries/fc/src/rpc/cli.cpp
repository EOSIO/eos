...
// I don't know exactly what version of readline we need.  I know the 4.2
// version that ships on some macs is
// missing some functions we require.  We're developing against 6.3, but
// probably anything in the 6.x
// series is fine
#if RL_VERSION_MAJOR < 6
#ifdef _MSC_VER
#pragma message(                                                               \
    "You have an old version of readline installed that might not support some of the features we need")
...
variant cli::send_call(api_id_type api_id, string method_name,
                       variants args /* = variants() */) {
  FC_ASSERT(false);
}

variant cli::send_callback(uint64_t callback_id,
                           variants args /* = variants() */) {
  FC_ASSERT(false);
}
...
void cli::run() {
  while (!_run_complete.canceled()) {
    try {
      ...
      auto result =
          receive_call(0, method, variants(args.begin() + 1, args.end()));
...
// so we just assume it's the same as the C stream...
  ...
  {
    std::cout << prompt;
    // sync_call( cin_thread, [&](){ std::getline( *input_stream, line ); },
    // "getline");
