...
void from_variant(const fc::variant &variant_to_convert,
                  fc::path &path_output) {
...
char path::separator_char = static_cast<char>(
    boost::filesystem::path("/").make_preferred().native()[0]);

path::path() {}
path::~path(){};
path::path(const boost::filesystem::path &p) : _p(p) {}

path::path(const char *p) : _p(p) {}
path::path(const fc::string &p) : _p(p.c_str()) {}

path::path(const std::wstring &p) : _p(p) {}

path::path(const path &p) : _p(p) {}

path::path(path &&p) : _p(std::move(p)) {}
...
path::operator boost::filesystem::path &() { return *_p; }
path::operator const boost::filesystem::path &() const { return *_p; }
fc::string path::generic_string() const { return _p->generic_string(); }

fc::string path::preferred_string() const {
  return boost::filesystem::path(*_p).make_preferred().string();
}

std::wstring path::wstring() const { return _p->wstring(); }

std::wstring path::generic_wstring() const { return _p->generic_wstring(); }
...
fc::string path::string() const { return _p->string(); }
fc::path path::filename() const { return _p->filename(); }
void path::replace_extension(const fc::path &e) { _p->replace_extension(e); }
fc::path path::extension() const { return _p->extension(); }
fc::path path::stem() const { return _p->stem(); }
fc::path path::parent_path() const { return _p->parent_path(); }
bool path::is_relative() const { return _p->is_relative(); }
bool path::is_absolute() const { return _p->is_absolute(); }

directory_iterator::directory_iterator(const fc::path &p) : _p(p) {}
...
void create_directories(const path &p) {
  try {
    ...
    FC_THROW("Unable to create directories ${path}",
             ("path", p)("inner", fc::except_str()));
  }
}
...
uint64_t directory_size(const path &p) {
  try {
    ...
    FC_THROW("Unable to calculate size of directory ${path}",
             ("path", p)("inner", fc::except_str()));
  }
}
...
#ifndef WIN32
  mode_t actual_perm =
      ((perm & 0400) ? S_IRUSR : 0) | ((perm & 0200) ? S_IWUSR : 0) |
      ((perm & 0100) ? S_IXUSR : 0)

      | ((perm & 0040) ? S_IRGRP : 0) | ((perm & 0020) ? S_IWGRP : 0) |
      ((perm & 0010) ? S_IXGRP : 0)

      | ((perm & 0004) ? S_IROTH : 0) | ((perm & 0002) ? S_IWOTH : 0) |
      ((perm & 0001) ? S_IXOTH : 0);
...
void rename(const path &f, const path &t) {
  try {
    boost::filesystem::rename(boost::filesystem::path(f),
                              boost::filesystem::path(t));
...
bool remove(const path &f) {
  try {
    ...
    FC_THROW("Unable to remove '${path}'",
             ("path", f)("exception", fc::except_str()));
  }
}
fc::path canonical(const fc::path &p) {
  try {
    ...
    FC_THROW("Unable to resolve path '${path}'",
             ("path", p)("exception", fc::except_str()));
  }
}
...
temp_file::temp_file(const fc::path &p, bool create)
  ...
  if (create) {
    std::ofstream ofs(_path->generic_string().c_str(),
...
void temp_file_base::release() { _path = fc::optional<fc::path>(); }
...
const fc::path &home_path() {
  static fc::path p = []() {
    ...
    BOOL success = GetUserProfileDirectoryW(access_token, user_profile_dir,
                                            &user_profile_dir_len);
...
simple_lock_file::impl::impl(const path &lock_file_path)
    :
      ...
      file_handle(-1),
#endif
      is_locked(false), lock_file_path(lock_file_path) {
}

simple_lock_file::impl::~impl() { unlock(); }

bool simple_lock_file::impl::try_lock() {
#ifdef _WIN32
  HANDLE fh =
      CreateFileA(lock_file_path.to_native_ansi_path().c_str(),
                  GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_ALWAYS, 0, NULL);
...
bool simple_lock_file::try_lock() { return my->try_lock(); }

void simple_lock_file::unlock() { my->unlock(); }
