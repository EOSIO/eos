...
variant_object::entry::entry(string k, variant v)
    : _key(fc::move(k)), _value(fc::move(v)) {}
variant_object::entry::entry(entry &&e)
    : _key(fc::move(e._key)), _value(fc::move(e._value)) {}
variant_object::entry::entry(const entry &e) : _key(e._key), _value(e._value) {}
variant_object::entry &variant_object::entry::
operator=(const variant_object::entry &e) {
  if (this != &e) {
    _key = e._key;
    _value = e._value;
  }
...
variant_object::entry &variant_object::entry::
operator=(variant_object::entry &&e) {
  fc_swap(_key, e._key);
...
void to_variant(const variant_object &var, variant &vo) { vo = variant(var); }
...
mutable_variant_object::iterator
mutable_variant_object::find(const string &key) const {
  return find(key.c_str());
}

mutable_variant_object::iterator
mutable_variant_object::find(const char *key) const {
  for (auto itr = begin(); itr != end(); ++itr) {
    if (itr->key() == key) {
      return itr;
    }
...
mutable_variant_object::iterator
mutable_variant_object::find(const string &key) {
  return find(key.c_str());
}
...
size_t mutable_variant_object::size() const { return _key_value->size(); }
...
mutable_variant_object::mutable_variant_object(
    const mutable_variant_object &obj)
    : _key_value(new std::vector<entry>(*obj._key_value)) {}
...
mutable_variant_object &mutable_variant_object::
operator=(const variant_object &obj) {
  *_key_value = *obj._key_value;
  return *this;
}

mutable_variant_object &mutable_variant_object::
operator=(mutable_variant_object &&obj) {
  if (this != &obj) {
    _key_value = fc::move(obj._key_value);
  }
  return *this;
...
mutable_variant_object &mutable_variant_object::
operator=(const mutable_variant_object &obj) {
  if (this != &obj) {
    *_key_value = *obj._key_value;
  }
  return *this;
...
void mutable_variant_object::reserve(size_t s) { _key_value->reserve(s); }
 ...
 */
mutable_variant_object &mutable_variant_object::operator()(string key,
                                                           variant var) {
  _key_value->push_back(entry(fc::move(key), fc::move(var)));
  return *this;
}
...
mutable_variant_object &mutable_variant_object::
operator()(const variant_object &vo) {
...
mutable_variant_object &mutable_variant_object::
operator()(const mutable_variant_object &mvo) {
