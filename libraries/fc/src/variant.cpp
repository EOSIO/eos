...
variant::variant() { set_variant_type(this, null_type); }

variant::variant(fc::nullptr_t) { set_variant_type(this, null_type); }
...
variant::variant(variant_object obj) {
  *reinterpret_cast<variant_object **>(this) =
      new variant_object(fc::move(obj));
...
variant::variant(const variant &v) {
  switch (v.get_type()) {
  case object_type:
    *reinterpret_cast<variant_object **>(this) = new variant_object(
...
bool variant::is_null() const { return get_type() == null_type; }

bool variant::is_string() const { return get_type() == string_type; }
bool variant::is_bool() const { return get_type() == bool_type; }
bool variant::is_double() const { return get_type() == double_type; }
bool variant::is_uint64() const { return get_type() == uint64_type; }
bool variant::is_int64() const { return get_type() == int64_type; }
...
bool variant::is_object() const { return get_type() == object_type; }

bool variant::is_array() const { return get_type() == array_type; }
bool variant::is_blob() const { return get_type() == blob_type; }
...
int64_t variant::as_int64() const {
  ...
  case null_type:
    return 0;
  default:
    FC_THROW_EXCEPTION(bad_cast_exception, "Invalid cast from ${type} to int64",
                       ("type", get_type()));
  }
}
  ...
  try {
    ...
    default:
      FC_THROW_EXCEPTION(bad_cast_exception,
                         "Invalid cast from ${type} to uint64",
                         ("type", get_type()));
    }
  }
...
double variant::as_double() const {
  ...
  default:
    FC_THROW_EXCEPTION(bad_cast_exception,
                       "Invalid cast from ${type} to double",
                       ("type", get_type()));
  }
}
...
bool variant::as_bool() const {
  ...
  case string_type: {
    ...
    if (s == "false")
      return false;
    FC_THROW_EXCEPTION(bad_cast_exception, "Cannot convert string to bool "
                                           "(only \"true\" or \"false\" can be "
                                           "converted)");
  }
  case double_type:
...
string variant::as_string() const {
  ...
  default:
    FC_THROW_EXCEPTION(bad_cast_exception,
                       "Invalid cast from ${type} to string",
                       ("type", get_type()));
  }
}
...
blob &variant::get_blob() {
  ...
  FC_THROW_EXCEPTION(bad_cast_exception, "Invalid cast from ${type} to Blob",
                     ("type", get_type()));
}
...
blob variant::as_blob() const {
  ...
  default:
    return blob(
        {std::vector<char>((char *)&_data, (char *)&_data + sizeof(_data))});
  }
}
...
/// @pre is_array()
size_t variant::size() const { return get_array().size(); }

const string &variant::get_string() const {
  if (get_type() == string_type)
    return **reinterpret_cast<const const_string_ptr *>(this);
  FC_THROW_EXCEPTION(bad_cast_exception,
                     "Invalid cast from type '${type}' to string",
                     ("type", get_type()));
}
...
void from_variant(const variant &var, int64_t &vo) { vo = var.as_int64(); }

void from_variant(const variant &var, uint64_t &vo) { vo = var.as_uint64(); }
