...
const secp256k1_context_t *_get_context() {
  static secp256k1_context_t *ctx = secp256k1_context_create(
      SECP256K1_CONTEXT_VERIFY | SECP256K1_CONTEXT_SIGN |
      SECP256K1_CONTEXT_RANGEPROOF | SECP256K1_CONTEXT_COMMIT);
  return ctx;
}
...
public:
  ...
  public_key_impl(const public_key_impl &cpy) BOOST_NOEXCEPT : _key(cpy._key) {
    _init_lib();
  }
...
fc::sha512 private_key::get_shared_secret(const public_key &other) const {
  ...
  FC_ASSERT(secp256k1_ec_pubkey_tweak_mul(
      detail::_get_context(), (unsigned char *)pub.begin(), pub.size(),
...
bool public_key::valid() const { return my->_key != empty_pub; }
...
public_key public_key::add(const fc::sha256 &digest) const {
  ...
  FC_ASSERT(secp256k1_ec_pubkey_tweak_add(
      detail::_get_context(), (unsigned char *)new_key.begin(), new_key.size(),
...
public_key_point_data public_key::serialize_ecc_point() const {
  ...
  FC_ASSERT(secp256k1_ec_pubkey_decompress(
      detail::_get_context(), (unsigned char *)dat.begin(), (int *)&pk_len));
...
public_key::public_key(const public_key_data &dat) { my->_key = dat; }
...
public_key::public_key(const compact_signature &c, const fc::sha256 &digest,
  ...
  unsigned int pk_len;
  FC_ASSERT(secp256k1_ecdsa_recover_compact(
      detail::_get_context(), (unsigned char *)digest.data(),
      (unsigned char *)c.begin() + 1, (unsigned char *)my->_key.begin(),
      (int *)&pk_len, 1, (*c.begin() - 27) & 3));
  FC_ASSERT(pk_len == my->_key.size());
}

extended_public_key::extended_public_key(const public_key &k,
                                         const fc::sha256 &c, int child,
...
extended_public_key extended_public_key::derive_normal_child(int i) const {
  ...
  FC_ASSERT(secp256k1_ec_pubkey_tweak_add(
                detail::_get_context(), (unsigned char *)key.begin(),
                key.size(), (unsigned char *)left.data()) > 0);
  // FIXME: check validity - if left + key == infinity then invalid
  extended_public_key result(key, detail::_right(l), i, fingerprint(),
                             depth + 1);
  return result;
}

static void to_bignum(const unsigned char *in, ssl_bignum &out,
                      unsigned int len) {
  if (*in & 0x80) {
...
static void from_bignum(const ssl_bignum &in, unsigned char *out,
                        unsigned int len) {
...
static void to_point(const public_key_data &in, ec_point &out) {
  ...
  ssl_bignum bn_x;
  to_bignum(x, bn_x);
  FC_ASSERT(EC_POINT_set_compressed_coordinates_GFp(curve, out, bn_x,
                                                    *in.begin() & 1, ctx) > 0);
}
...
static void from_point(const ec_point &in, public_key_data &out) {
  ...
  ssl_bignum bn_x;
  ssl_bignum bn_y;
  FC_ASSERT(EC_POINT_get_affine_coordinates_GFp(curve, in, bn_x, bn_y, ctx) >
            0);
  private_key_secret x;
  from_bignum(bn_x, x);
...
static public_key compute_k(const private_key_secret &a,
                            const private_key_secret &c, const public_key &p) {
  private_key_secret prod = a;
  FC_ASSERT(secp256k1_ec_privkey_tweak_mul(detail::_get_context(),
                                           (unsigned char *)prod.data(),
                                           (unsigned char *)c.data()) > 0);
  invert(prod, prod);
  public_key_data P = p.serialize();
  FC_ASSERT(secp256k1_ec_pubkey_tweak_mul(detail::_get_context(),
                                          (unsigned char *)P.begin(), P.size(),
                                          (unsigned char *)prod.data()));
...
static public_key
compute_t(const private_key_secret &a, const private_key_secret &b,
  ...
  FC_ASSERT(secp256k1_ec_pubkey_tweak_mul(
      detail::_get_context(), (unsigned char *)accu.begin(), accu.size(),
      (unsigned char *)prod.data()));
  // accu == prod * P == c^-1 * d * P
  ...
  to_point(q, point_q);
  bn_ctx ctx(BN_CTX_new());
  FC_ASSERT(EC_POINT_add(detail::get_curve(), point_accu, point_accu, point_q,
                         ctx) > 0);
...
extended_private_key::extended_private_key(const private_key &k,
                                           const sha256 &c, int child,
                                           int parent, uint8_t depth)
    : private_key(k), c(c), child_num(child), parent_fp(parent), depth(depth) {}

extended_private_key
extended_private_key::private_derive_rest(const fc::sha512 &hash, int i) const {
  fc::sha256 left = detail::_left(hash);
  FC_ASSERT(left < detail::get_curve_order());
  FC_ASSERT(secp256k1_ec_privkey_tweak_add(
                detail::_get_context(), (unsigned char *)left.data(),
                (unsigned char *)get_secret().data()) > 0);
  extended_private_key result(private_key::regenerate(left),
                              detail::_right(hash), i, fingerprint(),
                              depth + 1);
  return result;
}

public_key
extended_private_key::blind_public_key(const extended_public_key &bob,
                                       int i) const {
...
blinded_hash extended_private_key::blind_hash(const fc::sha256 &hash,
                                              int i) const {
  private_key_secret a = generate_a(i).get_secret();
  private_key_secret b = generate_b(i).get_secret();
  FC_ASSERT(secp256k1_ec_privkey_tweak_mul(detail::_get_context(),
  ...
  FC_ASSERT(secp256k1_ec_privkey_tweak_add(detail::_get_context(),
                                           (unsigned char *)a.data(),
...
private_key_secret
extended_private_key::compute_q(int i, const private_key_secret &p) const {
...
blind_signature extended_private_key::blind_sign(const blinded_hash &hash,
                                                 int i) const {
...
compact_signature
extended_private_key::unblind_signature(const extended_public_key &bob,
  ...
  for (int i = 0; i < 4; i++) {
    ...
    int pklen = 33;
    if (secp256k1_ecdsa_recover_compact(
            detail::_get_context(), (unsigned char *)hash.data(),
            (unsigned char *)result.begin() + 1, pubkey, &pklen, 1, i)) {
      if (!memcmp(t.begin(), pubkey, sizeof(pubkey))) {
        *result.begin() = 27 + 4 + i;
        return result;
        //                } else {
        //                    printf("Candidate: "); print( pubkey );
        //                    printf("\n");
      }
    }
  }
...
commitment_type blind(const blind_factor_type &blind, uint64_t value) {
  ...
  FC_ASSERT(secp256k1_pedersen_commit(detail::_get_context(),
                                      (unsigned char *)&result,
                                      (unsigned char *)&blind, value));
  return result;
}

blind_factor_type blind_sum(const std::vector<blind_factor_type> &blinds_in,
                            uint32_t non_neg) {
  ...
  FC_ASSERT(secp256k1_pedersen_blind_sum(
      detail::_get_context(), (unsigned char *)&result, blinds.data(),
      blinds_in.size(), non_neg));
  return result;
}

/**  verifies taht commnits + neg_commits + excess == 0 */
bool verify_sum(const std::vector<commitment_type> &commits_in,
  ...
  for (uint32_t i = 0; i < commits_in.size(); ++i)
    commits[i] = (const unsigned char *)&commits_in[i];
  std::vector<const unsigned char *> neg_commits(neg_commits_in.size());
  for (uint32_t i = 0; i < neg_commits_in.size(); ++i)
    neg_commits[i] = (const unsigned char *)&neg_commits_in[i];

  return secp256k1_pedersen_verify_tally(detail::_get_context(), commits.data(),
                                         commits.size(), neg_commits.data(),
                                         neg_commits.size(), excess);
}

bool verify_range(uint64_t &min_val, uint64_t &max_val,
                  const commitment_type &commit,
                  const std::vector<char> &proof) {
  return secp256k1_rangeproof_verify(detail::_get_context(), &min_val, &max_val,
...
std::vector<char> range_proof_sign(uint64_t min_value,
                                   ...
                                   int8_t base10_exp, uint8_t min_bits,
                                   uint64_t actual_value) {
  int proof_len = 5134;
  std::vector<char> proof(proof_len);

  FC_ASSERT(secp256k1_rangeproof_sign(
      detail::_get_context(), (unsigned char *)proof.data(), &proof_len,
      min_value, (const unsigned char *)&commit,
      (const unsigned char *)&commit_blind, (const unsigned char *)&nonce,
...
bool verify_range_proof_rewind(blind_factor_type &blind_out,
                               uint64_t &value_out, string &message_out,
                               const blind_factor_type &nonce,
  ...
  FC_ASSERT(secp256k1_rangeproof_rewind(
      detail::_get_context(), (unsigned char *)&blind_out, &value_out,
      (unsigned char *)msg, &mlen, (const unsigned char *)&nonce, &min_val,
      &max_val, (const unsigned char *)&commit,
...
range_proof_info range_get_info(const std::vector<char> &proof) {
  ...
  FC_ASSERT(secp256k1_rangeproof_info(
      detail::_get_context(), (int *)&result.exp, (int *)&result.mantissa,
      (uint64_t *)&result.min_value, (uint64_t *)&result.max_value,
      (const unsigned char *)proof.data(), (int)proof.size()));
