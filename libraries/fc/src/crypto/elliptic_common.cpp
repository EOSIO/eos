...
public_key public_key::from_base58(const std::string &b58) {
  ...
  FC_ASSERT(memcmp((char *)&check, data.data + sizeof(key), sizeof(check)) ==
            0);
...
private_key private_key::generate_from_seed(const fc::sha256 &seed,
                                            const fc::sha256 &offset) {
  ssl_bignum z;
...
static fc::string _to_base58(const extended_key_data &key) {
  ...
  fc::sha256 double_hash =
      fc::sha256::hash(fc::sha256::hash(key.begin(), key.size()));
...
extended_key_data extended_public_key::serialize_extended() const {
  ...
  *dest++ = depth;
  ...
  memcpy(dest, c.data(), c.data_size());
  dest += 32;
  public_key_data key = serialize();
...
extended_public_key
extended_public_key::deserialize(const extended_key_data &data) {
...
extended_public_key extended_public_key::from_base58(const fc::string &base58) {
  ...
  FC_ASSERT(detail::_get(&ptr) == BTC_EXT_PUB_MAGIC,
            "Invalid extended private key");
  uint8_t d = *ptr++;
  int fp = detail::_get(&ptr);
  int cn = detail::_get(&ptr);
  fc::sha256 chain;
  memcpy(chain.data(), ptr, chain.data_size());
  ptr += chain.data_size();
  public_key_data key;
...
public_key extended_public_key::generate_p(int i) const {
  return derive_normal_child(2 * i + 0);
}
public_key extended_public_key::generate_q(int i) const {
  return derive_normal_child(2 * i + 1);
}
...
extended_private_key extended_private_key::derive_normal_child(int i) const {
  const detail::chr37 data =
      detail::_derive_message(get_public_key().serialize(), i);
  hmac_sha512 mac;
...
extended_private_key::deserialize(const extended_key_data &data) {
  return from_base58(_to_base58(data));
}

private_key extended_private_key::generate_a(int i) const {
  return derive_hardened_child(4 * i + 0);
}
private_key extended_private_key::generate_b(int i) const {
  return derive_hardened_child(4 * i + 1);
}
private_key extended_private_key::generate_c(int i) const {
  return derive_hardened_child(4 * i + 2);
}
private_key extended_private_key::generate_d(int i) const {
  return derive_hardened_child(4 * i + 3);
}
...
extended_private_key
extended_private_key::from_base58(const fc::string &base58) {
  unsigned char buffer[78];
  unsigned char *ptr = buffer;
  _parse_extended_data(buffer, base58);
  FC_ASSERT(detail::_get(&ptr) == BTC_EXT_PRIV_MAGIC,
            "Invalid extended private key");
  uint8_t d = *ptr++;
  int fp = detail::_get(&ptr);
  int cn = detail::_get(&ptr);
...
extended_private_key
extended_private_key::generate_master(const fc::string &seed) {
  return generate_master(seed.c_str(), seed.size());
}

extended_private_key extended_private_key::generate_master(const char *seed,
                                                           uint32_t seed_len) {
  hmac_sha512 mac;
