...
public_key::public_key(const public_key &k) : my(k.my) {}

public_key::public_key(public_key &&k) : my(std::move(k.my)) {}
...
bool public_key::verify(const sha1 &digest,
...
bytes public_key::encrypt(const char *b, size_t l) const {
  ...
  int rtn =
      RSA_public_encrypt(l, (unsigned char *)b, (unsigned char *)out.data(),
  ...
  FC_THROW_EXCEPTION(
      exception, "openssl: ${message}",
      ("message", fc::string(ERR_error_string(ERR_get_error(), NULL))));
}

bytes public_key::encrypt(const bytes &in) const {
  FC_ASSERT(my && my->rsa);
  bytes out(RSA_size(my->rsa)); //, char(0) );
  int rtn = RSA_public_encrypt(in.size(), (unsigned char *)in.data(),
                               (unsigned char *)out.data(), my->rsa,
...
bytes public_key::decrypt(const bytes &in) const {
  FC_ASSERT(my && my->rsa);
  bytes out(RSA_size(my->rsa)); //, char(0) );
  int rtn = RSA_public_decrypt(in.size(), (unsigned char *)in.data(),
                               (unsigned char *)out.data(), my->rsa,
                               RSA_PKCS1_OAEP_PADDING);
  if (rtn >= 0) {
    out.resize(rtn);
...
private_key::private_key(const private_key &k) : my(k.my) {}
private_key::private_key(private_key &&k) : my(std::move(k.my)) {}
private_key::~private_key() {}
...
void private_key::sign(const sha1 &digest, array<char, 2048 / 8> &sig) const {
  ...
  uint32_t slen = 0;
  if (1 != RSA_sign(NID_sha1, (uint8_t *)&digest, 20, (unsigned char *)&sig,
                    &slen, my->rsa)) {
    FC_THROW_EXCEPTION(
        exception, "rsa sign failed with ${message}",
        ("message", fc::string(ERR_error_string(ERR_get_error(), NULL))));
  }
}
...
signature private_key::sign(const sha1 &digest) const {
  if (!my)
    FC_THROW_EXCEPTION(assert_exception, "!null");
  signature sig;
  sig.resize(RSA_size(my->rsa));

  uint32_t slen = 0;
  if (1 != RSA_sign(NID_sha1, (uint8_t *)digest.data(), 20,
...
signature private_key::sign(const sha256 &digest) const {
  ...
  uint32_t slen = 0;
  if (1 != RSA_sign(NID_sha256, (uint8_t *)digest.data(), 32,
                    (unsigned char *)sig.data(), &slen, my->rsa)) {
    FC_THROW_EXCEPTION(
        exception, "rsa sign failed with ${message}",
        ("message", fc::string(ERR_error_string(ERR_get_error(), NULL))));
...
bytes private_key::encrypt(const bytes &in) const {
  ...
  int rtn = RSA_private_encrypt(in.size(), (unsigned char *)in.data(),
                                (unsigned char *)out.data(), my->rsa,
                                RSA_PKCS1_OAEP_PADDING);
  if (rtn >= 0) {
    out.resize(rtn);
...
bytes private_key::decrypt(const char *in, size_t l) const {
  ...
  out.resize(RSA_size(my->rsa));
  int rtn =
      RSA_private_decrypt(l, (unsigned char *)in, (unsigned char *)out.data(),
                          my->rsa, RSA_PKCS1_OAEP_PADDING);
  if (rtn >= 0) {
    out.resize(rtn);
    return out;
...
bytes private_key::decrypt(const bytes &in) const {
  ...
  bytes out;
  out.resize(RSA_size(my->rsa));
  int rtn = RSA_private_decrypt(in.size(), (unsigned char *)in.data(),
                                (unsigned char *)out.data(), my->rsa,
                                RSA_PKCS1_OAEP_PADDING);
  if (rtn >= 0) {
    out.resize(rtn);
...
bytes private_key::serialize() const {
  ...
  if (e != 1) {
    BIO_free(mem);
    FC_THROW_EXCEPTION(
        exception, "Error writing private key, ${message}",
        ("message", fc::string(ERR_error_string(ERR_get_error(), NULL))));
  }
  char *dat;
  uint32_t l = BIO_get_mem_data(mem, &dat);
...
void to_variant(const public_key &bi, variant &v) { v = bi.serialize(); }
...
/** encodes the big int as base64 string, or a number */
void to_variant(const private_key &bi, variant &v) { v = bi.serialize(); }
