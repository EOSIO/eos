...
namespace ecc {
namespace detail {
void _init_lib() { static int init_o = init_openssl(); }

class private_key_impl {
public:
  private_key_impl() BOOST_NOEXCEPT { _init_lib(); }
  ...
  ~private_key_impl() BOOST_NOEXCEPT { free_key(); }
...
static void *ecies_key_derivation(const void *input, size_t ilen, void *output,
  ...
  *olen = SHA512_DIGEST_LENGTH;
  return (void *)SHA512((const unsigned char *)input, ilen,
...
fc::sha256 private_key::get_secret() const { return get_secret(my->_key); }

private_key::private_key(EC_KEY *k) { my->_key = k; }
...
fc::sha512 private_key::get_shared_secret(const public_key &other) const {
  ...
  fc::sha512 buf;
  ECDH_compute_key((unsigned char *)&buf, sizeof(buf),
                   EC_KEY_get0_public_key(other.my->_key), my->_key,
...
compact_signature private_key::sign_compact(const fc::sha256 &digest) const {
  try {
    ...
    // ECDSA_SIG *sig = ECDSA_do_sign((unsigned char*)&digest, sizeof(digest),
    // my->_key);
    public_key_data key_data;
    while (true) {
      ecdsa_sig sig =
          ECDSA_do_sign((unsigned char *)&digest, sizeof(digest), my->_key);
      ...
      if (nBitsR <= 256 && nBitsS <= 256) {
        ...
        for (int i = 0; i < 4; i++) {
          if (detail::public_key_impl::ECDSA_SIG_recover_key_GFp(
                  key, sig, (unsigned char *)&digest, sizeof(digest), i, 1) ==
              1) {
            unsigned char *buffer = (unsigned char *)key_data.begin();
            i2o_ECPublicKey(key,
                            &buffer); // FIXME: questionable memory handling
        ...
        // idump(
        // (result[0])(result[1])(result[2])(result[3])(result[3+lenR])(result[4+lenR])(bytes)(lenR)(lenS)
        // );
        if (lenR != 32) {
          free(result);
          continue;
  ...
  FC_RETHROW_EXCEPTIONS(warn, "sign ${digest}",
                        ("digest", digest)("private_key", *this));
}
}
}
