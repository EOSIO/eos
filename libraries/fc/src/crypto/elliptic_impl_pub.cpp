...
public_key_impl::public_key_impl() BOOST_NOEXCEPT { _init_lib(); }
...
public_key_impl::~public_key_impl() BOOST_NOEXCEPT { free_key(); }

public_key_impl &public_key_impl::
operator=(const public_key_impl &pk) BOOST_NOEXCEPT {
  if (pk._key == nullptr) {
    free_key();
...
public_key_impl &public_key_impl::
operator=(public_key_impl &&pk) BOOST_NOEXCEPT {
  if (this != &pk) {
    free_key();
    _key = pk._key;
    pk._key = nullptr;
...
bool public_key::valid() const { return my->_key != nullptr; }
...
//        return sig;
//    }
//    bool       public_key::verify( const fc::sha256& digest, const
//    fc::ecc::signature& sig )
//    {
//      return 1 == ECDSA_verify( 0, (unsigned char*)&digest, sizeof(digest),
//      (unsigned char*)&sig, sizeof(sig), my->_key );
//    }
...
public_key_data public_key::serialize() const {
  ...
  i2o_ECPublicKey(
      my->_key,
      (unsigned char **)&front); // FIXME: questionable memory handling
  return dat;
  /*
   EC_POINT* pub   = EC_KEY_get0_public_key( my->_key );
   EC_GROUP* group = EC_KEY_get0_group( my->_key );
   EC_POINT_get_affine_coordinates_GFp( group, pub, self.my->_pub_x.get(),
   self.my->_pub_y.get(), nullptr );
   */
}
...
public_key::public_key(const public_key_point_data &dat) {
  ...
  if (*front == 0) {
    ...
    my->_key =
        o2i_ECPublicKey(&my->_key, (const unsigned char **)&front, sizeof(dat));
    if (!my->_key) {
      FC_THROW_EXCEPTION(exception, "error decoding public key",
...
public_key::public_key(const public_key_data &dat) {
  ...
  if (*front == 0) {
    ...
    my->_key = EC_KEY_new_by_curve_name(NID_secp256k1);
    my->_key = o2i_ECPublicKey(&my->_key, (const unsigned char **)&front,
...
//    bool       private_key::verify( const fc::sha256& digest, const
//    fc::ecc::signature& sig )
//    {
//      return 1 == ECDSA_verify( 0, (unsigned char*)&digest, sizeof(digest),
//      (unsigned char*)&sig, sizeof(sig), my->_key );
...
public_key::public_key(const compact_signature &c, const fc::sha256 &digest,
  ...
  if (detail::public_key_impl::ECDSA_SIG_recover_key_GFp(
          my->_key, sig, (unsigned char *)&digest, sizeof(digest), nV - 27,
          0) == 1) {
    ECDSA_SIG_free(sig);
    return;
  }
