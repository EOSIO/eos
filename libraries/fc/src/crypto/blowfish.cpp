...
// Initialization with a fixed string which consists of the hexadecimal digits
...
void blowfish::start(unsigned char *ucKey, uint64_t keysize,
                     const sblock &roChain) {
  m_oChain0 = roChain;
  m_oChain = roChain;
  ...
  for (j = 0; j < 4; j++)
    for (int k = 0; k < 256;)
      encrypt(block), m_auiS[j][k++] = block.m_uil,
                      m_auiS[j][k++] = block.m_uir;
}
...
void blowfish::encrypt(unsigned char *buf, uint64_t n, int iMode) {
  ...
  if ((n == 0) || (n % 8 != 0))
    FC_THROW_EXCEPTION(
        exception, "invalid buffer length ${n}, not multiple of 8", ("n", n));
  sblock work;
...
// Returns false if n is multiple of 8
void blowfish::encrypt(const unsigned char *in, unsigned char *out, uint64_t n,
                       int iMode) {
  // Check the buffer's length - should be > 0 and multiple of 8
  if ((n == 0) || (n % 8 != 0))
    FC_THROW_EXCEPTION(
...
// Returns false if n is multiple of 8
void blowfish::decrypt(const unsigned char *in, unsigned char *out, uint64_t n,
                       int iMode) {
  // Check the buffer's length - should be > 0 and multiple of 8
  if ((n == 0) || (n % 8 != 0))
    FC_THROW_EXCEPTION(
