...
private_key_impl::private_key_impl() BOOST_NOEXCEPT { _init_lib(); }
...
operator=(const private_key_impl &pk) BOOST_NOEXCEPT {
  _key = pk._key;
  return *this;
}
}
...
fc::sha256 private_key::get_secret() const { return my->_key; }
...
public_key private_key::get_public_key() const {
  ...
  public_key_data pub;
  unsigned int pk_len;
  FC_ASSERT(secp256k1_ec_pubkey_create(
      detail::_get_context(), (unsigned char *)pub.begin(), (int *)&pk_len,
...
static int extended_nonce_function(unsigned char *nonce32,
                                   ...
                                   unsigned int attempt, const void *data) {
  unsigned int *extra = (unsigned int *)data;
  (*extra)++;
  return secp256k1_nonce_function_default(nonce32, msg32, key32, *extra,
                                          nullptr);
}
...
compact_signature private_key::sign_compact(const fc::sha256 &digest,
  ...
  do {
    FC_ASSERT(secp256k1_ecdsa_sign_compact(
        detail::_get_context(), (unsigned char *)digest.data(),
        (unsigned char *)result.begin() + 1, (unsigned char *)my->_key.data(),
