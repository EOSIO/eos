...
time_point time_point::now() {
  return time_point(
      microseconds(bch::duration_cast<bch::microseconds>(
                       bch::system_clock::now().time_since_epoch())
                       .count()));
}
...
time_point_sec::operator fc::string() const { return this->to_iso_string(); }
...
time_point_sec time_point_sec::from_iso_string(const fc::string &s) {
  try {
    ...
    if (s.size() >= 5 &&
        s.at(4) == '-') // http://en.wikipedia.org/wiki/ISO_8601
      pt = boost::date_time::parse_delimited_time<boost::posix_time::ptime>(
          s, 'T');
    else
  ...
  FC_RETHROW_EXCEPTIONS(
      warn, "unable to convert ISO-formatted string to fc::time_point_sec")
}
...
time_point time_point::from_iso_string(const fc::string &s) {
  ...
  FC_RETHROW_EXCEPTIONS(
      warn, "unable to convert ISO-formatted string to fc::time_point")
}

void to_variant(const fc::time_point &t, variant &v) { v = fc::string(t); }
void from_variant(const fc::variant &v, fc::time_point &t) {
  t = fc::time_point::from_iso_string(v.as_string());
}
void to_variant(const fc::time_point_sec &t, variant &v) { v = fc::string(t); }
...
string get_approximate_relative_time_string(
  ...
  int32_t seconds_ago =
      relative_to_time.sec_since_epoch() - event_time.sec_since_epoch();
  if (seconds_ago < 0) {
    ago = " in the future";
