...
real128::real128(const std::string &ratio_str) {
  ...
  if (digit >= 0 && digit <= 9) {
    ...
    *this = real128(int_part);
    ...
    // if the string doesn't look like "123.45" or ".45", this code isn't
    // designed to parse it correctly
    // in particular, we don't try to handle leading whitespace or '+'/'-'
...
void to_variant(const real128 &var, variant &vo) { vo = std::string(var); }
