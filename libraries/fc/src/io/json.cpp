...
variant_object objectFromStream(T &in) {
  ...
  try {
    ...
    while (in.peek() != '}') {
      ...
      if (in.peek() != ':') {
        FC_THROW_EXCEPTION(parse_error_exception,
                           "Expected ':' after key \"${key}\"", ("key", key));
      }
      in.get();
    ...
    FC_THROW_EXCEPTION(parse_error_exception, "Expected '}' after ${variant}",
                       ("variant", obj));
  } catch (const fc::eof_exception &e) {
    FC_THROW_EXCEPTION(parse_error_exception, "Unexpected EOF: ${e}",
...
variants arrayFromStream(T &in) {
  ...
  try {
    ...
    if (in.peek() != ']')
      FC_THROW_EXCEPTION(parse_error_exception,
                         "Expected ']' after parsing ${variant}",
                         ("variant", ar));
...
variant number_from_stream(T &in) {
  ...
  if (str == "-." ||
      str == ".") // check the obviously wrong things we could have encountered
    FC_THROW_EXCEPTION(
        parse_error_exception,
        "Can't parse token \"${token}\" as a JSON numeric constant",
        ("token", str));
  if (dot)
    return parser_type == json::legacy_parser_with_string_doubles
               ? variant(str)
...
/** the purpose of this check is to verify that we will not get a stack overflow
...
void check_string_depth(const string &utf8_str) {
  ...
  for (auto c : utf8_str) {
    ...
    FC_ASSERT(open_object < 100 && open_array < 100, "object graph too deep",
              ("object depth", open_object)("array depth", open_array));
  }
}
...
variant json::from_string(const std::string &utf8_str, parse_type ptype) {
  try {
    ...
    case legacy_parser_with_string_doubles:
      return variant_from_stream<std::stringstream,
                                 legacy_parser_with_string_doubles>(in);
    case strict_parser:
...
variants json::variants_from_string(const std::string &utf8_str,
                                    parse_type ptype) {
  try {
    check_string_depth(utf8_str);
    variants result;
...
void to_stream(T &os, const variant &v, json::output_formatting format) {
  ...
  case variant::int64_type: {
    ...
    if (format == json::stringify_large_ints_and_doubles && i > 0xffffffff)
      os << '"' << v.as_string() << '"';
    else
      os << i;
...
std::string pretty_print(const std::string &v, uint8_t indent) {
  ...
  for (uint32_t i = 0; i < v.size(); ++i) {
    ...
    case ',':
      ...
      break;
    case 'n':
      // If we're in quotes and see a \n, just print it literally but unset the
      // escape flag.
      if (quote && escape)
        escape = false;
...
std::string json::to_pretty_string(
    const variant &v,
    output_formatting format /* = stringify_large_ints_and_doubles */) {
  return pretty_print(to_string(v, format), 2);
}

void json::save_to_file(
    const variant &v, const fc::path &fi, bool pretty,
    output_formatting format /* = stringify_large_ints_and_doubles */) {
  if (pretty) {
...
variant json::from_file(const fc::path &p, parse_type ptype) {
  ...
  // auto tmp = std::make_shared<std::ifstream>( p.generic_string().c_str(),
  // std::ios::binary );
  ...
  case legacy_parser_with_string_doubles:
    return variant_from_stream<boost::filesystem::ifstream,
                               legacy_parser_with_string_doubles>(bi);
  case strict_parser:
    return json_relaxed::variant_from_stream<boost::filesystem::ifstream, true>(
        bi);
  case relaxed_parser:
    return json_relaxed::variant_from_stream<boost::filesystem::ifstream,
                                             false>(bi);
  default:
    FC_ASSERT(false, "Unknown JSON parser type {ptype}", ("ptype", ptype));
  }
...
variant json::from_stream( buffered_istream& in, parse_type ptype )
{
   switch( ptype )
   {
       ...
       case legacy_parser_with_string_doubles:
           return variant_from_stream<fc::buffered_istream,
legacy_parser_with_string_doubles>( in );
       case strict_parser:
       ...
       default:
           FC_ASSERT( false, "Unknown JSON parser type {ptype}", ("ptype",
ptype) );
   }
}
*/

std::ostream &json::to_stream(
    std::ostream &out, const variant &v,
    output_formatting format /* = stringify_large_ints_and_doubles */) {
  fc::to_stream(out, v, format);
  return out;
}
std::ostream &json::to_stream(
    std::ostream &out, const variants &v,
    output_formatting format /* = stringify_large_ints_and_doubles */) {
  fc::to_stream(out, v, format);
  return out;
}
std::ostream &json::to_stream(
    std::ostream &out, const variant_object &v,
    output_formatting format /* = stringify_large_ints_and_doubles */) {
  fc::to_stream(out, v, format);
  return out;
}
...
bool json::is_valid(const std::string &utf8_str, parse_type ptype) {
  ...
  case legacy_parser:
    ...
    break;
  case legacy_parser_with_string_doubles:
    variant_from_stream<std::stringstream, legacy_parser_with_string_doubles>(
        in);
    break;
  case strict_parser:
