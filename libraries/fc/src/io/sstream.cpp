...
public:
  impl(fc::string &s) : ss(s) { ss.exceptions(std::stringstream::badbit); }

  impl(const fc::string &s) : ss(s) {
...
stringstream::stringstream(fc::string &s) : my(s) {}
stringstream::stringstream(const fc::string &s) : my(s) {}
...
void stringstream::str(const fc::string &s) { my->ss.str(s); }

void stringstream::clear() { my->ss.clear(); }

bool stringstream::eof() const { return my->ss.eof(); }
...
size_t stringstream::writesome(const std::shared_ptr<const char> &buf,
...
size_t stringstream::readsome(const std::shared_ptr<char> &buf, size_t len,
                              size_t offset) {
...
istream& stringstream::read( fc::string&  v ) { my->ss >>
*reinterpret_cast<std::string*>(&v); return *this; }
