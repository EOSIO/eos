...
buffered_istream::buffered_istream(buffered_istream &&o) : my(fc::move(o.my)) {}
...
size_t buffered_istream::readsome(const std::shared_ptr<char> &buf, size_t len,
                                  size_t offset) {
...
#ifndef NDEBUG
  // This code was written with the assumption that you'd only be making one
  // call to readsome
  // at a time so it reuses _shared_read_buffer.  If you really need to make
  // concurrent calls to
  // readsome(), you'll need to prevent reusing _shared_read_buffer here
  struct check_buffer_in_use {
    bool &_buffer_in_use;
    check_buffer_in_use(bool &buffer_in_use) : _buffer_in_use(buffer_in_use) {
      assert(!_buffer_in_use);
      _buffer_in_use = true;
    }
    ~check_buffer_in_use() {
      assert(_buffer_in_use);
  ...
  if (!my->_shared_read_buffer)
    my->_shared_read_buffer.reset(new char[detail::minimum_read_size],
                                  [](char *p) { delete[] p; });
  size_t bytes_read = my->_istr->readsome(my->_shared_read_buffer,
                                          detail::minimum_read_size, 0);

  size_t bytes_to_deliver_immediately = std::min<size_t>(bytes_read, len);

  memcpy(buf.get() + offset, my->_shared_read_buffer.get(),
         bytes_to_deliver_immediately);
...
buffered_ostream::buffered_ostream(buffered_ostream &&o) : my(fc::move(o.my)) {}
...
size_t buffered_ostream::writesome(const std::shared_ptr<const char> &buf,
...
void buffered_ostream::flush() {
  ...
  // This code was written with the assumption that you'd only be making one
  // call to flush
  // at a time so it reuses _shared_write_buffer.  If you really need to make
  // concurrent calls to
  ...
  if (!my->_shared_write_buffer)
    my->_shared_write_buffer.reset(new char[write_buffer_size],
                                   [](char *p) { delete[] p; });

  while (size_t bytes_from_rdbuf = static_cast<size_t>(my->_rdbuf.sgetn(
             my->_shared_write_buffer.get(), write_buffer_size)))
