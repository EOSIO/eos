...
size_t string::find(const fc::string &str, size_t pos /* = 0 */) const {
  return my->find(str, pos);
}
size_t string::find(const char *s, size_t pos /* = 0 */) const {
  return my->find(s, pos);
}
...
size_t string::rfind(const fc::string &c, size_t p) const {
  return my->rfind(c, p);
}
size_t string::find_first_of(const fc::string &str,
                             size_t pos /* = 0 */) const {
  return my->find_first_of(str, pos);
}
size_t string::find_first_of(const char *s, size_t pos /* = 0 */) const {
  return my->find_first_of(s, pos);
}

fc::string &string::replace(size_t pos, size_t len, const string &str) {
  my->replace(pos, len, str);
  return *this;
}
fc::string &string::replace(size_t pos, size_t len, const char *s) {
  my->replace(pos, len, s);
  return *this;
}
...
fc::string string::substr(size_t start, size_t len) const {
  return my->substr(start, len);
}
...
fc::string to_string(double d) {
  // +2 is required to ensure that the double is rounded correctly when read
  // back in.  http://docs.oracle.com/cd/E19957-01/806-3568/ncg_goldberg.html
  std::stringstream ss;
