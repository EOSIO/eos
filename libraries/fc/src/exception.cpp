...
namespace fc {
FC_REGISTER_EXCEPTIONS((timeout_exception)(file_not_found_exception)(
    ...
    canceled_exception)(assert_exception)(eof_exception)(
    unknown_host_exception)(null_optional)(udt_exception)(aes_exception)(
    overflow_exception)(underflow_exception)(divide_by_zero_exception))
...
unhandled_exception::unhandled_exception(const exception &r) : exception(r) {}
unhandled_exception::unhandled_exception(log_messages m) : exception() {
  my->_elog = fc::move(m);
}
...
exception::exception(int64_t code, const std::string &name_value,
                     const std::string &what_value)
    : my(new detail::exception_impl()) {
  my->_code = code;
  my->_what = what_value;
...
exception::exception(exception &&c) : my(fc::move(c.my)) {}
...
void to_variant(const exception &e, variant &v) {
  v = mutable_variant_object("code", e.code())("name", e.name())(
      "message", e.what())("stack", e.get_log());
}
...
string exception::to_detail_string(log_level ll) const {
  ...
  for (auto itr = my->_elog.begin(); itr != my->_elog.end();) {
    ss << itr->get_message() << "\n"; // fc::format_string( itr->get_format(),
                                      // itr->get_data() ) <<"\n";
...
void record_assert_trip(const char *filename, uint32_t lineno,
                        const char *expr) {
  fc::mutable_variant_object assert_trip_info = fc::mutable_variant_object()(
      "source_file", filename)("source_lineno", lineno)("expr", expr);
