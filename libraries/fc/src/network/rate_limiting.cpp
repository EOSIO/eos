...
public:
  ...
  rate_limited_operation(size_t length, size_t offset,
                         promise<size_t>::ptr &&completion_promise)
      : length(length), offset(offset), permitted_length(0),
...
public:
  ...
  rate_limited_tcp_write_operation(boost::asio::ip::tcp::socket &socket,
      ...
      : rate_limited_operation(length, offset, std::move(completion_promise)),
        socket(socket), raw_buffer(buffer) {
    assert(false);
  }
  rate_limited_tcp_write_operation(boost::asio::ip::tcp::socket &socket,
                                   ...
                                   promise<size_t>::ptr completion_promise)
      : rate_limited_operation(length, offset, std::move(completion_promise)),
        socket(socket), raw_buffer(nullptr), shared_buffer(buffer) {}
  virtual void perform_operation() override {
    if (raw_buffer)
      asio::async_write_some(socket, raw_buffer, permitted_length,
                             completion_promise);
    else
      asio::async_write_some(socket, shared_buffer, permitted_length, offset,
                             completion_promise);
  }
};
...
public:
  ...
  virtual void perform_operation() override {
    if (raw_buffer)
      asio::async_read_some(socket, raw_buffer, permitted_length,
                            completion_promise);
    else
      asio::async_read_some(socket, shared_buffer, permitted_length, offset,
                            completion_promise);
  }
};
...
struct is_operation_shorter {
  ...
  bool operator()(const rate_limited_operation *lhs,
...
average_rate_meter::average_rate_meter(const microseconds &time_constant)
    : _average_rate(0.), _unaccounted_bytes(0),
      _last_update_time(time_point_sec::min()), _time_constant(time_constant) {}
...
void average_rate_meter::update_const(
    uint32_t bytes_transferred /* = 0 */) const {
  ...
  if (now <= _last_update_time)
    ...
    if (time_since_last_update > _time_constant)
      _average_rate = bytes_transferred /
                      (_time_constant.count() / (double)seconds(1).count());
    else {
      bytes_transferred += _unaccounted_bytes;
      double seconds_since_last_update =
          time_since_last_update.count() / (double)seconds(1).count();
      double rate_since_last_update =
          bytes_transferred / seconds_since_last_update;
      double alpha =
          time_since_last_update.count() / (double)_time_constant.count();
      _average_rate =
          rate_since_last_update * alpha + _average_rate * (1.0 - alpha);
    }
...
public:
  ...
  uint32_t _unused_read_tokens; // gets filled with tokens for unused bytes (if
                                // I'm allowed to read 200 bytes and I try to
                                // read 200 bytes, but can only read 50, tokens
                                // for the other 150 get returned here)
  ...
  rate_limiting_group_impl(uint32_t upload_bytes_per_second,
                           uint32_t download_bytes_per_second,
                           uint32_t burstiness_in_seconds = 1);
  ~rate_limiting_group_impl();

  virtual size_t readsome(boost::asio::ip::tcp::socket &socket, char *buffer,
                          size_t length) override;
  virtual size_t readsome(boost::asio::ip::tcp::socket &socket,
                          const std::shared_ptr<char> &buffer, size_t length,
                          size_t offset) override;
  template <typename BufferType>
  size_t readsome_impl(boost::asio::ip::tcp::socket &socket,
                       const BufferType &buffer, size_t length, size_t offset);
  virtual size_t writesome(boost::asio::ip::tcp::socket &socket,
                           const char *buffer, size_t length) override;
  virtual size_t writesome(boost::asio::ip::tcp::socket &socket,
                           const std::shared_ptr<const char> &buffer,
                           size_t length, size_t offset) override;
  template <typename BufferType>
  ...
  void process_pending_operations(
      time_point &last_iteration_start_time, uint32_t &limit_bytes_per_second,
      rate_limited_operation_list &operations_in_progress,
      rate_limited_operation_list &operations_for_next_iteration,
      uint32_t &tokens, uint32_t &unused_tokens);
};

rate_limiting_group_impl::rate_limiting_group_impl(
    uint32_t upload_bytes_per_second, uint32_t download_bytes_per_second,
    ...
    : _upload_bytes_per_second(upload_bytes_per_second),
      ...
      _granularity(milliseconds(50)), _read_tokens(_download_bytes_per_second),
      _unused_read_tokens(0), _write_tokens(_upload_bytes_per_second),
...
template <typename BufferType>
size_t
rate_limiting_group_impl::readsome_impl(boost::asio::ip::tcp::socket &socket,
                                        const BufferType &buffer, size_t length,
                                        size_t offset) {
  size_t bytes_read;
  if (_download_bytes_per_second) {
    promise<size_t>::ptr completion_promise(
        new promise<size_t>("rate_limiting_group_impl::readsome"));
    rate_limited_tcp_read_operation read_operation(socket, buffer, length,
                                                   offset, completion_promise);
    _read_operations_for_next_iteration.push_back(&read_operation);

    // launch the read processing loop it if isn't running, or signal it to
    // resume if it's paused.
    if (!_process_pending_reads_loop_complete.valid() ||
        _process_pending_reads_loop_complete.ready())
...
size_t
rate_limiting_group_impl::writesome(boost::asio::ip::tcp::socket &socket,
                                    const std::shared_ptr<const char> &buffer,
                                    size_t length, size_t offset) {
...
template <typename BufferType>
size_t
rate_limiting_group_impl::writesome_impl(boost::asio::ip::tcp::socket &socket,
                                         const BufferType &buffer,
                                         size_t length, size_t offset) {
  size_t bytes_written;
  if (_upload_bytes_per_second) {
    promise<size_t>::ptr completion_promise(
        new promise<size_t>("rate_limiting_group_impl::writesome"));
    rate_limited_tcp_write_operation write_operation(
        socket, buffer, length, offset, completion_promise);
    _write_operations_for_next_iteration.push_back(&write_operation);

    // launch the write processing loop it if isn't running, or signal it to
    // resume if it's paused.
    if (!_process_pending_writes_loop_complete.valid() ||
        _process_pending_writes_loop_complete.ready())
...
void rate_limiting_group_impl::process_pending_reads() {
  for (;;) {
    process_pending_operations(
        _last_read_iteration_time, _download_bytes_per_second,
...
void rate_limiting_group_impl::process_pending_writes() {
  for (;;) {
    process_pending_operations(
        _last_write_iteration_time, _upload_bytes_per_second,
...
void rate_limiting_group_impl::process_pending_operations(
  ...
  {
    microseconds time_since_last_iteration =
        this_iteration_start_time - last_iteration_start_time;
    ...
    tokens += (uint32_t)(
        (limit_bytes_per_second * time_since_last_iteration.count()) / 1000000);
    tokens += unused_tokens;
    unused_tokens = 0;
    tokens = std::min(tokens, limit_bytes_per_second * _burstiness_in_seconds);

    if (tokens) {
      // sort the pending reads/writes in order of the number of bytes they need
      // to write, smallest first
      ...
      std::sort(operations_sorted_by_length.begin(),
                operations_sorted_by_length.end(), is_operation_shorter());
      ...
      while (!operations_sorted_by_length.empty()) {
        ...
        uint32_t bytes_allocated_for_this_operation =
            std::min<size_t>(operations_sorted_by_length.back()->length,
                             bytes_permitted_for_this_operation);
        operations_sorted_by_length.back()->permitted_length =
  ...
  {
    // we shouldn't end up here often.  If the rate is unlimited, we should just
    // execute
    // the operation immediately without being queued up.  This should only be
    // hit if
...
void rate_limiting_group::set_actual_rate_time_constant(
...
void rate_limiting_group::set_download_limit(
    uint32_t download_bytes_per_second) {
...
void rate_limiting_group::remove_tcp_socket(
