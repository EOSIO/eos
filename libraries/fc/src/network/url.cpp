    ...
    if (pos != fc::string::npos) {
      try {
        ...
        FC_THROW_EXCEPTION(parse_error_exception,
                           "Unable to parse port field in url", ("url", s));
      }
...
#ifdef WIN32
    // On windows, a URL like file:///c:/autoexec.bat would result in _lpath =
    // c:/autoexec.bat
...
#else
    // On unix, a URL like file:///etc/rc.local would result in _lpath =
    // etc/rc.local
...
void to_variant(const url &u, fc::variant &v) { v = fc::string(u); }
void from_variant(const fc::variant &v, url &u) { u = url(v.as_string()); }
...
url::url(const fc::string &u) : my(std::make_shared<detail::url_impl>()) {
  my->parse(u);
}
...
url::url() : my(get_null_url()) {}

url::url(const url &u) : my(u.my) {}

url::url(url &&u) : my(fc::move(u.my)) { u.my = get_null_url(); }

url::url(const mutable_url &mu)
    : my(std::make_shared<detail::url_impl>(*mu.my)) {}
url::url(mutable_url &&mu) : my(fc::move(mu.my)) {}
...
string url::proto() const { return my->_proto; }
ostring url::host() const { return my->_host; }
ostring url::user() const { return my->_user; }
ostring url::pass() const { return my->_pass; }
opath url::path() const { return my->_path; }
ovariant_object url::args() const { return my->_args; }
fc::optional<uint16_t> url::port() const { return my->_port; }
}
