...
public:
  udt_epoll_service() : _epoll_thread("udt_epoll") {
    UDT::startup();
    check_udt_errors();
  ...
  void remove(int udt_socket_id) {
    {
      synchronized(_read_promises_mutex) auto read_itr =
          _read_promises.find(udt_socket_id);
    ...
    {
      synchronized(_write_promises_mutex) auto write_itr =
          _write_promises.find(udt_socket_id);
...
udt_socket::udt_socket() : _udt_socket_id(UDT::INVALID_SOCK) {}
...
void udt_socket::bind(const fc::ip::endpoint &local_endpoint) {
  try {
    ...
    if (UDT::ERROR ==
        UDT::bind(_udt_socket_id, (sockaddr *)&local_addr, sizeof(local_addr)))
      check_udt_errors();
  }
...
void udt_socket::connect_to(const ip::endpoint &remote_endpoint) {
  try {
    ...
    connect_thread
        .async(
            [&]() {
              if (UDT::ERROR == UDT::connect(_udt_socket_id,
                                             (sockaddr *)&serv_addr,
                                             sizeof(serv_addr)))
                check_udt_errors();
            },
...
ip::endpoint udt_socket::remote_endpoint() const {
  try {
    ...
    int error_code = UDT::getpeername(
        _udt_socket_id, (struct sockaddr *)&peer_addr, &peer_addr_size);
    if (error_code == UDT::ERROR)
      check_udt_errors();
    return ip::endpoint(ip::address(htonl(peer_addr.sin_addr.s_addr)),
...
ip::endpoint udt_socket::local_endpoint() const {
  try {
    ...
    int error_code = UDT::getsockname(
        _udt_socket_id, (struct sockaddr *)&sock_addr, &addr_size);
    if (error_code == UDT::ERROR)
      check_udt_errors();
    return ip::endpoint(ip::address(htonl(sock_addr.sin_addr.s_addr)),
...
size_t udt_socket::readsome(const std::shared_ptr<char> &buf, size_t len,
                            size_t offset) {
...
size_t udt_socket::writesome(const std::shared_ptr<const char> &buf, size_t len,
                             size_t offset) {
...
bool udt_socket::is_open() const { return _udt_socket_id != UDT::INVALID_SOCK; }

udt_server::udt_server() : _udt_socket_id(UDT::INVALID_SOCK) {
  _udt_socket_id = UDT::socket(AF_INET, SOCK_STREAM, 0);
  if (_udt_socket_id == UDT::INVALID_SOCK)
    check_udt_errors();
...
void udt_server::accept(udt_socket &s) {
  try {
    ...
    while (s._udt_socket_id == UDT::INVALID_SOCK) {
      s._udt_socket_id =
          UDT::accept(_udt_socket_id, (sockaddr *)&their_addr, &namelen);
      if (UDT::getlasterror().getErrorCode() == CUDTException::EASYNCRCV) {
        UDT::getlasterror().clear();
...
void udt_server::listen(const ip::endpoint &ep) {
  try {
    ...
    if (UDT::ERROR ==
        UDT::bind(_udt_socket_id, (sockaddr *)&my_addr, sizeof(my_addr)))
      check_udt_errors();
