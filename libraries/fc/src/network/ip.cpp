...
address::address(const fc::string &s) {
  ...
  FC_RETHROW_EXCEPTIONS(error, "Error parsing IP address ${address}",
                        ("address", s))
}
...
address::operator uint32_t() const { return _ip; }

endpoint::endpoint() : _port(0) {}
endpoint::endpoint(const address &a, uint16_t p) : _port(p), _ip(a) {}
...
endpoint endpoint::from_string(const string &endpoint_string) {
  try {
    ...
    ep._ip =
        boost::asio::ip::address_v4::from_string(endpoint_string.substr(0, pos))
            .to_ulong();
    ep._port = boost::lexical_cast<uint16_t>(
        endpoint_string.substr(pos + 1, endpoint_string.size()));
    return ep;
  }
...
bool address::is_private_address() const {
  ...
  if (_ip >= min10_ip._ip && _ip <= max10_ip._ip)
    return true;
  if (_ip >= min172_ip._ip && _ip <= max172_ip._ip)
    return true;
  if (_ip >= min192_ip._ip && _ip <= max192_ip._ip)
    return true;
  if (_ip >= min169_ip._ip && _ip <= max169_ip._ip)
    return true;
  return false;
}
...
void to_variant(const ip::endpoint &var, variant &vo) { vo = fc::string(var); }
void from_variant(const variant &var, ip::endpoint &vo) {
  vo = ip::endpoint::from_string(var.as_string());
}

void to_variant(const ip::address &var, variant &vo) { vo = fc::string(var); }
