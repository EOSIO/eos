...
public:
  ...
  ntp_impl()
      : _ntp_thread("ntp"), _request_interval_sec(60 * 60 /* 1 hr */),
  ...
  uint64_t fc_time_point_to_ntp_timestamp(const fc::time_point &fc_timestamp) {
    uint64_t microseconds_since_epoch =
        (uint64_t)fc_timestamp.time_since_epoch().count();
    ...
    uint32_t fractional_seconds = (uint32_t)(
        (((uint64_t)microseconds << 32) + (uint64_t(1) << 31)) / 1000000);
    uint64_t ntp_timestamp_net_order =
    ...
    for (auto item : _ntp_hosts) {
      try {
        ...
        for (auto ep : eps) {
          ...
          std::shared_ptr<char> send_buffer(new char[48],
                                            [](char *p) { delete[] p; });
          std::array<unsigned char, 48> packet_to_send{
              {010, 0, 0, 0, 0, 0, 0, 0, 0}};
          memcpy(send_buffer.get(), packet_to_send.data(),
                 packet_to_send.size());
          uint64_t *send_buf_as_64_array = (uint64_t *)send_buffer.get();
          send_buf_as_64_array[5] = fc_time_point_to_ntp_timestamp(
              fc::time_point::now()); // 5 = Transmit Timestamp
  ...
  // this task gets invoked every _retry_failed_request_interval_sec (currently
  // 5 min), and if
  // _request_interval_sec (currently 1 hour) has passed since the last
  // successful update,
  // it sends a new request
  void request_time_task() { request_now(); } // request_loop
  ...
  void read_loop() {
    ...
    std::shared_ptr<char> receive_buffer(new char[receive_buffer_size],
                                         [](char *p) { delete[] p; });
    uint64_t *recv_buf = (uint64_t *)receive_buffer.get();

    // if you start the read while loop here, the recieve_from call will throw
    // "invalid argument" on win32,
    ...
    while (true) {
      ...
      fc::time_point server_receive_time =
          ntp_timestamp_to_fc_time_point(recv_buf[4]);
      fc::time_point server_transmit_time =
          ntp_timestamp_to_fc_time_point(recv_buf[5]);
      ...
      fc::microseconds round_trip_delay(
          (receive_time - origin_time) -
          (server_transmit_time - server_receive_time));
      // wlog("origin_time = ${origin_time}, server_receive_time =
      // ${server_receive_time}, server_transmit_time = ${server_transmit_time},
      // receive_time = ${receive_time}",
      //     ("origin_time", origin_time)("server_receive_time",
      //     server_receive_time)("server_transmit_time",
      //     server_transmit_time)("receive_time", receive_time));
      // wlog("ntp offset: ${offset}, round_trip_delay ${delay}", ("offset",
      // offset)("delay", round_trip_delay));

      // if the reply we just received has occurred more than a second after our
      // last time request (it was more than a second ago since our last
      // request)
      if (round_trip_delay > fc::microseconds(300000)) {
        wlog("received stale ntp reply requested at ${request_time}, send a "
             "new time request",
      ...
      {
        if (offset < fc::seconds(60 * 60 * 24) &&
            offset > fc::seconds(-60 * 60 * 24)) {
          _last_ntp_delta_microseconds = offset.count();
          _last_ntp_delta_initialized = true;
          fc::microseconds ntp_delta_time =
              fc::microseconds(_last_ntp_delta_microseconds);
          _last_valid_ntp_reply_received_time = receive_time;
          wlog("ntp_delta_time updated to ${delta_time} us",
               ("delta_time", ntp_delta_time));
        } else
          elog("NTP time and local time vary by more than a day! "
               "ntp:${ntp_time} local:${local}",
               ("ntp_time", receive_time + offset)("local",
                                                   fc::time_point::now()));
      }
    }
...
ntp::ntp() : my(new detail::ntp_impl()) { my->start_read_loop(); }
...
void ntp::set_request_interval(uint32_t interval_sec) {
  ...
  my->_retry_failed_request_interval_sec =
      std::min(my->_retry_failed_request_interval_sec, interval_sec);
  my->reschedule();
}
...
optional<time_point> ntp::get_time() const {
  if (my->_last_ntp_delta_initialized)
    return fc::time_point::now() +
           fc::microseconds(my->_last_ntp_delta_microseconds);
