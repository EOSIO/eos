...
static std::string calc_sha1_base32_of_buffer(const std::string &buffer) {
  ...
  string sha1_result_base32 =
      to_base32((char *)&sha1_result, sizeof(sha1_result));
...
public:
  gntp_notifier_impl(
      const std::string &host_to_notify = "127.0.0.1", uint16_t port = 23053,
      const optional<std::string> &password = optional<std::string>());

  // there's no API to change these right now, it will always notify localhost
  // at the default GNTP port
  std::string hostname;
  uint16_t port;
  ...
  gntp_notification_type_list notification_types; // list of all notification
                                                  // types we're registered to
                                                  // send

  optional<boost::asio::ip::tcp::endpoint>
      endpoint; // cache the last endpoint we've connected to
...
gntp_notifier_impl::gntp_notifier_impl(
    const std::string &host_to_notify /* = "127.0.0.1" */,
    uint16_t port /* = 23053 */,
    const optional<std::string> &password /* = optional<std::string>() */)
    : hostname(host_to_notify), port(port), password(password),
      connection_failed(false), is_registered(false) {}
...
void gntp_notifier_impl::send_gntp_message(const std::string &message) {
  ...
  if (endpoint) {
    // we've successfully connected before, connect to the same endpoint that
    // worked last time
    try {
      asio::tcp::connect(*sock, *endpoint);
      connected = true;
    } catch (exception &er) {
      ilog("Failed to connect to GNTP service using an endpoint that "
           "previously worked: ${error_report}",
           ("error_report", er.to_detail_string()));
      sock->close();
      // clear the cached endpoint and fall through to the full connection
      // procedure
  ...
  if (!connected) {
    ...
    auto eps =
        asio::tcp::resolve(hostname, boost::lexical_cast<std::string>(port));
    if (eps.size() == 0)
  ...
  try {
    ...
    FC_RETHROW_EXCEPTION(
        er, warn, "Caught an exception while sending data to GNTP service");
  } catch (...) {
...
void gntp_notifier::add_notification_type(
...
void gntp_notifier::register_notifications() {
  ...
  if (my->application_icon) {
    message << "Application-Icon: x-growl-resource://"
            << my->application_icon->my->_sha1_hash << "\r\n";
    icons_used.insert(my->application_icon);
  }

  message << "Notifications-Count: " << my->notification_types.size() << "\r\n";
  for (const gntp_notification_type &notification_type :
    ...
    if (notification_type.icon) {
      message << "Notification-Icon: x-growl-resource://"
              << notification_type.icon->my->_sha1_hash << "\r\n";
      icons_used.insert(notification_type.icon);
    }
    message << "Notification-Enabled: "
            << (notification_type.enabled ? "True" : "False") << "\r\n";
  }
  if (!icons_used.empty()) {
    message << "\r\n";
...
gntp_guid gntp_notifier::send_notification(
    std::string name, std::string title, std::string text,
    const gntp_icon_ptr &icon,
    optional<gntp_guid> coalescingId /* = optional<gntp_guid>() */) {
  ...
  if (my->password) {
    ...
    message << " SHA256:" << boost::to_upper_copy(to_hex(keyhash.data(), 32))
            << "." << boost::to_upper_copy(to_hex(salt, sizeof(salt)));
  }
  message << "\r\n";
  message << "Application-Name: " << my->application_name << "\r\n";
  message << "Notification-Name: " << name << "\r\n";
  message << "Notification-ID: " << notification_id.str() << "\r\n";
  message << "Notification-Coalescing-ID: "
          << (coalescingId ? coalescingId->str() : notification_id.str())
          << "\r\n";
