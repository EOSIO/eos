...
namespace fc 
{
  namespace detail
  {
    ...
    {
    public:
      gntp_notifier_impl(const std::string& host_to_notify = "127.0.0.1", uint16_t port = 23053, const std::optional<std::string>& password = std::make_optional<std::string>());
    ...
    gntp_notifier_impl::gntp_notifier_impl(const std::string& host_to_notify /* = "127.0.0.1" */, uint16_t port /* = 23053 */,
                                           const std::optional<std::string>& password /* = std::make_optional<std::string>() */) :
    ...
    void gntp_notifier_impl::send_gntp_message(const std::string& message)
    {
      ...
      if (endpoint)
      {
        ...
        catch (exception& er)
        {
          ...
          endpoint = std::make_optional<boost::asio::ip::tcp::endpoint>();
        }
        catch (...)
        {
  ...
  gntp_guid gntp_notifier::send_notification(std::string name, std::string title, std::string text, 
                                             const gntp_icon_ptr& icon, std::optional<gntp_guid> coalescingId /* = std::make_optional<gntp_guid>() */)
  {
