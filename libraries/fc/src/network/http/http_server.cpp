...
public:
  impl(const fc::http::connection_ptr &c,
       const std::function<void()> &cont = std::function<void()>())
...
public:
  ...
  impl(const fc::ip::endpoint &p) {
    ...
    tcp_serv.listen(p);
    accept_complete =
        fc::async([this]() { this->accept_loop(); }, "http_server accept_loop");
  }
  ...
  ~impl() {
    ...
    for (fc::future<void> &request_in_progress : requests_in_progress) {
      try {
        ...
        wlog("Caught exception while canceling http request task: ${error}",
             ("error", e));
  ...
  void accept_loop() {
    while (!accept_complete.canceled()) {
      ...
      for (auto iter = requests_in_progress.begin();
           iter != requests_in_progress.end();)
        if (!iter->valid() || iter->ready())
          iter = requests_in_progress.erase(iter);
        else
          ++iter;
      requests_in_progress.emplace_back(
  ...
  void handle_connection(
      const http::connection_ptr &c,
      std::function<void(const http::request &, const server::response &s)>
          do_on_req) {
    try {
      http::server::response rep(
          fc::shared_ptr<response::impl>(new response::impl(c)));
      request req = c->read_request();
      if (do_on_req)
        do_on_req(req, rep);
      c->get_socket().close();
    } catch (fc::exception &e) {
      wlog("unable to read request ${1}",
           ("1", e.to_detail_string())); // fc::except_str().c_str());
    }
...
server::server(uint16_t port)
    : my(new impl(fc::ip::endpoint(fc::ip::address(), port))) {}
...
void server::listen(const fc::ip::endpoint &p) { my.reset(new impl(p)); }
...
server::response::response(const fc::shared_ptr<server::response::impl> &m)
    : my(m) {}

server::response &server::response::operator=(const server::response &s) {
  my = s.my;
  return *this;
}
server::response &server::response::operator=(server::response &&s) {
  fc_swap(my, s.my);
  return *this;
}

void server::response::add_header(const fc::string &key,
                                  const fc::string &val) const {
...
void server::on_request(
    const std::function<void(const http::request &, const server::response &s)>
        &cb) {
  my->on_req = cb;
}
}
