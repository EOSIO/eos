...
public:
  websocket_server_impl() : _server_thread(fc::thread::current()) {
      ...
      _server_thread
          .async([&]() {
            auto new_con = std::make_shared<websocket_connection_impl<
                websocket_server_type::connection_ptr>>(
    ...
    _server.set_message_handler(
        [&](connection_hdl hdl, websocket_server_type::message_ptr msg) {
          // ilog( "...waiting on server thread" );
          _server_thread
              .async([&]() {
  ...
  typedef std::map<connection_hdl, websocket_connection_ptr,
...
public:
  websocket_tls_server_impl(const string &server_pem,
      ...
      _server.set_tls_init_handler(
          [=](websocketpp::connection_hdl hdl) -> context_ptr {
            ...
            try {
              ...
              ctx->use_private_key_file(server_pem,
    ...
    _server.set_open_handler([&](connection_hdl hdl) {
      _server_thread
          .async([&]() {
            auto new_con = std::make_shared<websocket_connection_impl<
                websocket_tls_server_type::connection_ptr>>(
                _server.get_con_from_hdl(hdl));
            _on_connection(_connections[hdl] = new_con);
          })
          .get();
...
typedef websocket_tls_client_type::connection_ptr
    websocket_tls_client_connection_type;

class websocket_client_impl {
public:
  typedef websocket_client_type::message_ptr message_ptr;

  websocket_client_impl() : _client_thread(fc::thread::current()) {
    ...
    _client.set_fail_handler([=](connection_hdl hdl) {
      ...
      if (_connected /*&& !_connected->ready() REMOVED IN UPDATE */) {
        try {
          _connected->set_exception(std::make_exception_ptr(
              FC_EXCEPTION(exception, "${message}", ("message", message))));
        } catch (...) {
        }
      }
      if (_closed) {
...
public:
  ...
  websocket_tls_client_impl(const std::string &ca_filename)
    ...
    _client.set_close_handler([=](connection_hdl hdl) {
      if (_connection) {
        ...
        try {
          ...
          if (_closed)
            _closed->set_exception(
                std::current_exception()); // e.dynamic_copy_exception() );
        }
        if (_closed)
          _closed->set_value();
      }
    ...
    //
    // We need ca_filename to be copied into the closure, as the referenced
    // object might be destroyed by the caller by the time
    // tls_init_handler() is called.  According to [1], capture-by-value results
    // in the desired behavior (i.e. creation of
    // a copy which is stored in the closure) on standards compliant compilers,
    // but some compilers on some optimization levels
    // are buggy and are not standards compliant in this situation.  Also, keep
    // in mind this is the opinion of a single forum
  ...
  std::string get_host() const { return websocketpp::uri(_uri).get_host(); }
  ...
  void setup_peer_verify(context_ptr &ctx, const std::string &ca_filename) {
    ...
    ctx->set_verify_callback(
        boost::asio::ssl::rfc2818_verification(get_host()));
  }
...
void websocket_server::listen(uint16_t port) { my->_server.listen(port); }
...
void websocket_server::start_accept() { my->_server.start_accept(); }

websocket_tls_server::websocket_tls_server(const string &server_pem,
                                           const string &ssl_password)
    : my(new detail::websocket_tls_server_impl(server_pem, ssl_password)) {}
...
void websocket_tls_server::listen(uint16_t port) { my->_server.listen(port); }
void websocket_tls_server::listen(const fc::ip::endpoint &ep) {
  my->_server.listen(boost::asio::ip::tcp::endpoint(
      boost::asio::ip::address_v4(uint32_t(ep.get_address())), ep.port()));
}

void websocket_tls_server::start_accept() { my->_server.start_accept(); }

websocket_tls_client::websocket_tls_client(const std::string &ca_filename)
    : my(new detail::websocket_tls_client_impl(ca_filename)) {}
...
websocket_client::secure_connect(const std::string &uri) {
  try {
