...
public:
  impl() : _sock(fc::asio::default_io_service()), _io_hooks(this) {}
  ~impl() {
    if (_sock.is_open())
      try {
        _sock.close();
...
size_t tcp_socket::impl::readsome(boost::asio::ip::tcp::socket &socket,
                                  char *buffer, size_t length) {
  return (_read_in_progress = fc::asio::read_some(socket, buffer, length))
      .get();
}
size_t tcp_socket::impl::readsome(boost::asio::ip::tcp::socket &socket,
                                  const std::shared_ptr<char> &buffer,
                                  size_t length, size_t offset) {
  return (_read_in_progress =
              fc::asio::read_some(socket, buffer, length, offset))
      .get();
}
size_t tcp_socket::impl::writesome(boost::asio::ip::tcp::socket &socket,
                                   const char *buffer, size_t length) {
  return (_write_in_progress = fc::asio::write_some(socket, buffer, length))
      .get();
}
size_t tcp_socket::impl::writesome(boost::asio::ip::tcp::socket &socket,
                                   const std::shared_ptr<const char> &buffer,
                                   size_t length, size_t offset) {
  return (_write_in_progress =
              fc::asio::write_some(socket, buffer, length, offset))
      .get();
}

void tcp_socket::open() {
  my->_sock.open(
      boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), 0).protocol());
}

bool tcp_socket::is_open() const { return my->_sock.is_open(); }
...
bool tcp_socket::eof() const { return !my->_sock.is_open(); }
...
fc::ip::endpoint tcp_socket::local_endpoint() const {
  try {
    ...
    return fc::ip::endpoint(boost_local_endpoint.address().to_v4().to_ulong(),
...
void tcp_socket::connect_to(const fc::ip::endpoint &remote_endpoint) {
  fc::asio::tcp::connect(
      my->_sock, fc::asio::tcp::endpoint(
                     boost::asio::ip::address_v4(remote_endpoint.get_address()),
                     remote_endpoint.port()));
}

void tcp_socket::bind(const fc::ip::endpoint &local_endpoint) {
  try {
    my->_sock.bind(boost::asio::ip::tcp::endpoint(
        boost::asio::ip::address_v4(local_endpoint.get_address()),
        local_endpoint.port()));
  } catch (const std::exception &except) {
    elog("Exception binding outgoing connection to desired local endpoint "
         "${endpoint}: ${what}",
...
void tcp_socket::enable_keep_alives(const fc::microseconds &interval) {
  if (interval.count()) {
    ...
    keepalive_settings.keepalivetime =
        (ULONG)(interval.count() / fc::milliseconds(1).count());
    keepalive_settings.keepaliveinterval =
        (ULONG)(interval.count() / fc::milliseconds(1).count());

    DWORD dwBytesRet = 0;
    if (WSAIoctl(my->_sock.native(), SIO_KEEPALIVE_VALS, &keepalive_settings,
                 sizeof(keepalive_settings), NULL, 0, &dwBytesRet, NULL,
                 NULL) == SOCKET_ERROR)
...
#if !defined(__APPLE__) ||                                                     \
    defined(TCP_KEEPINTVL) // TCP_KEEPINTVL not defined before 10.9
...
public:
  impl() : _accept(fc::asio::default_io_service()) {
    _accept.open(boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), 0)
                     .protocol());
  }
...
tcp_server::tcp_server() : my(nullptr) {}
tcp_server::~tcp_server() { delete my; }
...
void tcp_server::listen(const fc::ip::endpoint &ep) {
  ...
  try {
    my->_accept.bind(boost::asio::ip::tcp::endpoint(
        boost::asio::ip::address_v4::from_string((string)ep.get_address()),
        ep.port()));
    my->_accept.listen();
  }
  FC_RETHROW_EXCEPTIONS(warn, "error listening on socket");
...
fc::ip::endpoint tcp_server::get_local_endpoint() const {
  ...
  return fc::ip::endpoint(
      my->_accept.local_endpoint().address().to_v4().to_ulong(),
