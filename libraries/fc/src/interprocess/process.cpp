...
class process::impl {
public:
  impl() : stat(fc::asio::default_io_service()) {}
  ...
  ~impl() {
    try {
      ...
      wlog("caught exception cleaning up process: ${except_str}",
           ("except_str", fc::except_str()));
    }
  }
...
process::process() : my(new process::impl()) {}
process::~process() {}

iprocess &process::exec(const fc::path &exe, std::vector<std::string> args,
  ...
  */
  my->child.reset(
      new bp::child(bp::create_child(exe.string(), fc::move(args), my->pctx)));
      ...
      [=](const boost::system::error_code &ec, int exit_code) {
        ...
        if (!ec) {
          ...
          p->set_exception(fc::exception_ptr(new fc::exception(FC_LOG_MESSAGE(
              error, "process exited with: ${message} ",
              ("message", boost::system::system_error(ec).what())))));
        }
      });
  if (opt & open_stdin)
    my->_in = std::make_shared<buffered_ostream>(
        std::make_shared<fc::asio::ostream<bp::pipe>>(my->_inp));
  if (opt & open_stdout)
    my->_out = std::make_shared<buffered_istream>(
        std::make_shared<fc::asio::istream<bp::pipe>>(my->_outp));
  if (opt & open_stderr)
    my->_err = std::make_shared<buffered_istream>(
        std::make_shared<fc::asio::istream<bp::pipe>>(my->_errp));
  my->_exited = p;
  return *this;
}
 ...
 */
void process::kill() { my->child->terminate(); }

/**
 *  @brief returns a stream that writes to the process' stdin
 */
fc::buffered_ostream_ptr process::in_stream() { return my->_in; }

/**
 *  @brief returns a stream that reads from the process' stdout
 */
fc::buffered_istream_ptr process::out_stream() { return my->_out; }
/**
 *  @brief returns a stream that reads from the process' stderr
 */
fc::buffered_istream_ptr process::err_stream() { return my->_err; }

int process::result(
    const microseconds &timeout /* = microseconds::maximum() */) {
