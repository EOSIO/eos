...
namespace enumivo { namespace testing {
  ...
  struct enumivo_assert_message_is {
     ...
     bool operator()( const enumivo_assert_message_exception& ex );
  ...
  /**
   * Utility predicate to check whether an enumivo_assert_code error code is equivalent to a given number
   */
  struct enumivo_assert_code_is {
     enumivo_assert_code_is( uint64_t error_code )
           : expected( "assertion failure with error code: " ) {
        expected.append( std::to_string(error_code) );
     }

     bool operator()( const enumivo_assert_code_exception& ex );
