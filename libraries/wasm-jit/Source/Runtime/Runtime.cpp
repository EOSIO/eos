...
// Returns a vector of strings, each element describing a frame of the call
// stack.
// If the frame is a JITed function, use the JIT's information about the
// function
// to describe it, otherwise fallback to whatever platform-specific symbol
// resolution
// is available.
std::vector<std::string>
describeCallStack(const Platform::CallStack &callStack) {
...
bool isA(ObjectInstance *object, const ObjectType &type) {
  ...
  case ObjectKind::function:
    return asFunctionType(type) == asFunction(object)->type;
  case ObjectKind::global:
    return asGlobalType(type) == asGlobal(object)->type;
  case ObjectKind::table:
    return isSubset(asTableType(type), asTable(object)->type);
  case ObjectKind::memory:
    return isSubset(asMemoryType(type), asMemory(object)->type);
  default:
    Errors::unreachable();
  }
}

[[noreturn]] void handleHardwareTrap(Platform::HardwareTrapType trapType,
                                     Platform::CallStack &&trapCallStack,
                                     Uptr trapOperand) {
  std::cerr << "handle hadrware trap \n";
  std::vector<std::string> callStackDescription =
      describeCallStack(trapCallStack);

  switch (trapType) {
  case Platform::HardwareTrapType::accessViolation: {
    // If the access violation occured in a Table's reserved pages, treat it as
    // an undefined table element runtime error.
    if (isAddressOwnedByTable(reinterpret_cast<U8 *>(trapOperand))) {
      throw Exception{Exception::Cause::undefinedTableElement,
                      callStackDescription};
    }
    // If the access violation occured in a Memory's reserved pages, treat it as
    // an access violation runtime error.
    else if (isAddressOwnedByMemory(reinterpret_cast<U8 *>(trapOperand))) {
      throw Exception{Exception::Cause::accessViolation, callStackDescription};
    } else {
      // If the access violation occured outside of a Table or Memory, treat it
      // as a bug (possibly a security hole)
      // rather than a runtime error in the WebAssembly code.
      Log::printf(Log::Category::error, "Access violation outside of table or "
                                        "memory reserved addresses. Call "
                                        "stack:\n");
  ...
  case Platform::HardwareTrapType::stackOverflow:
    throw Exception{Exception::Cause::stackOverflow, callStackDescription};
  case Platform::HardwareTrapType::intDivideByZeroOrOverflow:
    throw Exception{Exception::Cause::integerDivideByZeroOrIntegerOverflow,
                    callStackDescription};
  default:
    Errors::unreachable();
  };
...
Result invokeFunction(FunctionInstance *function,
                      const std::vector<Value> &parameters) {
  const FunctionType *functionType = function->type;

  // Check that the parameter types match the function, and copy them into a
  // memory block that stores each as a 64-bit value.
  if (parameters.size() != functionType->parameters.size()) {
    throw Exception{Exception::Cause::invokeSignatureMismatch};
  }

  U64 *thunkMemory = (U64 *)alloca(
      (functionType->parameters.size() + getArity(functionType->ret)) *
      sizeof(U64));
  for (Uptr parameterIndex = 0;
       parameterIndex < functionType->parameters.size(); ++parameterIndex) {
    if (functionType->parameters[parameterIndex] !=
  ...
  // Catch platform-specific runtime exceptions and turn them into
  // Runtime::Values.
  Result result;
