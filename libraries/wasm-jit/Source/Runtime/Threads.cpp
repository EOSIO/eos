...
// Keeps track of the entry and error functions used by a running
// WebAssembly-spawned thread.
...
// A call to openWaitList should be followed by a call to closeWaitList to avoid
// leaks.
...
// Closes a wait list, deleting it and removing it from the global map if it was
// the last reference.
...
template <typename Value>
static U32 waitOnAddress(Value *valuePointer, Value expectedValue,
                         F64 timeout) {
  const U64 endTime =
      getEndTimeFromTimeout(Platform::getMonotonicClock(), timeout);
  ...
  // Lock the wait list, and check that *valuePointer is still what the caller
  // expected it to be.
  lockMutex(waitList->mutex);
  if (atomicLoad(valuePointer) != expectedValue) {
    // If *valuePointer wasn't the expected value, unlock the wait list and
    // return.
    ...
    if (!threadWakeEvent) {
      threadWakeEvent = Platform::createEvent();
    }
  ...
  if (!Platform::waitForEvent(threadWakeEvent, endTime)) {
    // If the wait timed out, lock the wait list and check if the thread's wake
    // event is still in the wait list.
    Platform::Lock waitListLock(waitList->mutex);
    auto wakeEventIt = std::find(waitList->wakeEvents.begin(),
                                 waitList->wakeEvents.end(), threadWakeEvent);
    if (wakeEventIt != waitList->wakeEvents.end()) {
      // If the event was still on the wait list, remove it, and return the
      // "timed out" result.
      waitList->wakeEvents.erase(wakeEventIt);
      timedOut = true;
    } else {
      // In between the wait timing out and locking the wait list, some other
      // thread tried to wake this thread.
      // The event will now be signaled, so use an immediately expiring wait on
      // it to reset it.
      errorUnless(Platform::waitForEvent(threadWakeEvent,
...
static U32 wakeAddress(Uptr address, U32 numToWake) {
  ...
  WaitList *waitList = openWaitList(address);
  {
    ...
    waitList->wakeEvents.erase(waitList->wakeEvents.begin(),
                               waitList->wakeEvents.begin() + numToWake);
  }
  closeWaitList(address, waitList);
...
namespace Runtime {
DEFINE_INTRINSIC_FUNCTION1(wavmIntrinsics, misalignedAtomicTrap,
                           misalignedAtomicTrap, none, i32, address) {
  causeException(Exception::Cause::misalignedAtomicMemoryAccess);
}

DEFINE_INTRINSIC_FUNCTION1(wavmIntrinsics, isLockFree, isLockFree, i32, i32,
                           numBytes) {
...
DEFINE_INTRINSIC_FUNCTION3(wavmIntrinsics, wake, wake, i32, i32, addressOffset,
                           i32, numToWake, i64, memoryInstanceBits) {
  MemoryInstance *memoryInstance =
      reinterpret_cast<MemoryInstance *>(memoryInstanceBits);
  ...
  const Uptr address =
      reinterpret_cast<Uptr>(getMemoryBaseAddress(memoryInstance)) +
      addressOffset;
  return wakeAddress(address, numToWake);
}

DEFINE_INTRINSIC_FUNCTION4(wavmIntrinsics, wait, wait, i32, i32, addressOffset,
                           i32, expectedValue, f64, timeout, i64,
                           memoryInstanceBits) {
  MemoryInstance *memoryInstance =
      reinterpret_cast<MemoryInstance *>(memoryInstanceBits);

  // Validate that the address is within the memory's bounds and naturally
  // aligned.
  if (U32(addressOffset) > memoryInstance->endOffset) {
    causeException(Exception::Cause::accessViolation);
  }
  ...
  I32 *valuePointer = reinterpret_cast<I32 *>(
      getMemoryBaseAddress(memoryInstance) + addressOffset);
  return waitOnAddress(valuePointer, expectedValue, timeout);
}
DEFINE_INTRINSIC_FUNCTION4(wavmIntrinsics, wait, wait, i32, i32, addressOffset,
                           i64, expectedValue, f64, timeout, i64,
                           memoryInstanceBits) {
  MemoryInstance *memoryInstance =
      reinterpret_cast<MemoryInstance *>(memoryInstanceBits);
  ...
  I64 *valuePointer = reinterpret_cast<I64 *>(
      getMemoryBaseAddress(memoryInstance) + addressOffset);
  return waitOnAddress(valuePointer, expectedValue, timeout);
}

FunctionInstance *getFunctionFromTable(TableInstance *table,
                                       const FunctionType *expectedType,
                                       U32 elementIndex) {
  // Validate that the index is valid.
  if (elementIndex * sizeof(TableInstance::FunctionElement) >=
      table->endOffset) {
...
void callAndTurnHardwareTrapsIntoRuntimeExceptions(void (*function)(I32),
                                                   I32 argument) {
...
static void threadFunc(Thread *thread, I32 argument) {
  try {
    ...
    callAndTurnHardwareTrapsIntoRuntimeExceptions(
    ...
    for (auto calledFunction : exception.callStack) {
      Log::printf(Log::Category::error, "  %s\n", calledFunction.c_str());
    }
    Log::printf(Log::Category::error,
                "Passing exception on to thread error handler\n");
    ...
    try {
      ...
      // Log that the thread error function caused a runtime exception, and exit
      // with a fatal error.
      Log::printf(Log::Category::error,
                  "Runtime exception in thread error handler: %s\n",
                  describeExceptionCause(secondException.cause));
      for (auto calledFunction : secondException.callStack) {
        Log::printf(Log::Category::error, "  %s\n", calledFunction.c_str());
      }
...
DEFINE_INTRINSIC_FUNCTION4(wavmIntrinsics, launchThread, launchThread, none,
                           i32, entryFunctionIndex, i32, argument, i32,
                           errorFunctionIndex, i64, functionTablePointer) {
  TableInstance *defaultTable =
      reinterpret_cast<TableInstance *>(functionTablePointer);
  const FunctionType *functionType =
      FunctionType::get(ResultType::none, {ValueType::i32});

  // Create a thread object that will expose its entry and error functions to
  // the garbage collector as roots.
  Thread *thread = new Thread();
  thread->entryFunction =
      getFunctionFromTable(defaultTable, functionType, entryFunctionIndex);
  thread->errorFunction =
  ...
  std::thread stdThread([thread, argument]() { threadFunc(thread, argument); });
