...
Value evaluateInitializer(ModuleInstance *moduleInstance,
  ...
  case InitializerExpression::Type::get_global: {
    ...
    GlobalInstance *globalInstance =
        moduleInstance->globals[expression.globalIndex];
    return Runtime::Value(globalInstance->type.valueType,
...
ModuleInstance *instantiateModule(const IR::Module &module,
  ...
  errorUnless(moduleInstance->functions.size() ==
  ...
  for (Uptr importIndex = 0; importIndex < module.tables.imports.size();
       ++importIndex) {
    errorUnless(isA(moduleInstance->tables[importIndex],
                    module.tables.imports[importIndex].type));
  }
  errorUnless(moduleInstance->memories.size() ==
  ...
  for (Uptr importIndex = 0; importIndex < module.memories.imports.size();
    ...
    errorUnless(isA(moduleInstance->memories[importIndex],
                    module.memories.imports[importIndex].type));
  }
  ...
  for (Uptr importIndex = 0; importIndex < module.globals.imports.size();
    ...
    errorUnless(isA(moduleInstance->globals[importIndex],
                    module.globals.imports[importIndex].type));
  }
  ...
  // If any memory or table segment doesn't fit, throw an exception before
  // mutating any memory/table.
  ...
  for (auto &dataSegment : module.dataSegments) {
    ...
    errorUnless(baseOffsetValue.type == ValueType::i32);
    const U32 baseOffset = baseOffsetValue.i32;
    if (baseOffset + dataSegment.data.size() >
  ...
  for (const DataSegment &dataSegment : module.dataSegments) {
    ...
    assert(baseOffset + dataSegment.data.size() <=
           (memory->numPages << IR::numBytesPerPageLog2));

    memcpy(memory->baseAddress + baseOffset, dataSegment.data.data(),
  ...
  for (const GlobalDef &globalDef : module.globals.defs) {
    const Value initialValue =
        evaluateInitializer(moduleInstance, globalDef.initializer);
    errorUnless(initialValue.type == globalDef.type.valueType);
    moduleInstance->globals.push_back(
        new GlobalInstance(globalDef.type, initialValue));
  }
  ...
  for (Uptr functionDefIndex = 0;
    ...
    const DisassemblyNames::Function &functionNames =
        disassemblyNames.functions[functionIndex];
  ...
  for (const Export &exportIt : module.exports) {
    ...
    case ObjectKind::table:
      exportedObject = moduleInstance->tables[exportIt.index];
      break;
  ...
  for (const TableSegment &tableSegment : module.tableSegments) {
    ...
    for (Uptr index = 0; index < tableSegment.indices.size(); ++index) {
      ...
      setTableElement(table, baseOffset + index,
  ...
  if (module.startFunctionIndex != UINTPTR_MAX) {
    assert(moduleInstance->functions[module.startFunctionIndex]->type ==
           IR::FunctionType::get());
...
ModuleInstance::~ModuleInstance() { delete jitModule; }

MemoryInstance *getDefaultMemory(ModuleInstance *moduleInstance) {
  return moduleInstance->defaultMemory;
}
uint64_t getDefaultMemorySize(ModuleInstance *moduleInstance) {
  return moduleInstance->defaultMemory->numPages << IR::numBytesPerPageLog2;
}
TableInstance *getDefaultTable(ModuleInstance *moduleInstance) {
  return moduleInstance->defaultTable;
}

ObjectInstance *getInstanceExport(ModuleInstance *moduleInstance,
