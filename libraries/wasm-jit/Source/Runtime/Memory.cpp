...
namespace Runtime {
// Global lists of memories; used to query whether an address is reserved by one
// of them.
...
U8 *allocateVirtualPagesAligned(Uptr numBytes, Uptr alignmentBytes,
                                U8 *&outUnalignedBaseAddress,
                                Uptr &outUnalignedNumPlatformPages) {
  ...
  outUnalignedBaseAddress =
      Platform::allocateVirtualPages(outUnalignedNumPlatformPages);
  if (!outUnalignedBaseAddress) {
    return nullptr;
  } else {
    return (U8 *)((Uptr)(outUnalignedBaseAddress + alignmentBytes - 1) &
                  ~(alignmentBytes - 1));
  }
}
...
MemoryInstance *createMemory(MemoryType type) {
  ...
  // This allows eliding bounds checks on memory accesses, since a 32-bit index
  // + 32-bit offset will always be within the reserved address-space.
  // On a 32-bit runtime, allocate 1GB.
  const Uptr memoryMaxBytes =
      HAS_64BIT_ADDRESS_SPACE ? 8ull * 1024 * 1024 * 1024 : 0x40000000;

  // On a 64 bit runtime, align the instance memory base to a 4GB boundary, so
  // the lower 32-bits will all be zero. Maybe it will allow better code
  // generation?
  ...
  memory->baseAddress = allocateVirtualPagesAligned(
      memoryMaxBytes, alignmentBytes, memory->reservedBaseAddress,
      memory->reservedNumPlatformPages);
  ...
  if (growMemory(memory, type.size.min) == -1) {
    delete memory;
    return nullptr;
  }
...
MemoryInstance::~MemoryInstance() {
  ...
  if (reservedNumPlatformPages > 0) {
    Platform::freeVirtualPages(reservedBaseAddress, reservedNumPlatformPages);
  }
...
bool isAddressOwnedByMemory(U8 *address) {
  // Iterate over all memories and check if the address is within the reserved
  // address space for each.
  for (auto memory : memories) {
    U8 *startAddress = memory->reservedBaseAddress;
    U8 *endAddress =
        memory->reservedBaseAddress +
        (memory->reservedNumPlatformPages << Platform::getPageSizeLog2());
...
Iptr growMemory(MemoryInstance *memory, Uptr numNewPages) {
  ...
  if (numNewPages > 0) {
    // If the number of pages to grow would cause the memory's size to exceed
...
Iptr shrinkMemory(MemoryInstance *memory, Uptr numPagesToShrink) {
  ...
  if (numPagesToShrink > 0) {
    // If the number of pages to shrink would cause the memory's size to drop
...
U8 *getMemoryBaseAddress(MemoryInstance *memory) { return memory->baseAddress; }

U8 *getValidatedMemoryOffsetRange(MemoryInstance *memory, Uptr offset,
                                  Uptr numBytes) {
  // Validate that the range [offset..offset+numBytes) is contained by the
  // memory's reserved pages.
  U8 *address = memory->baseAddress + offset;
  if (!memory || address < memory->reservedBaseAddress ||
      address + numBytes < address ||
      address + numBytes >=
          memory->reservedBaseAddress + (memory->reservedNumPlatformPages
