...
namespace Runtime {
// Global lists of tables; used to query whether an address is reserved by one
// of them.
std::vector<TableInstance *> tables;

static Uptr getNumPlatformPages(Uptr numBytes) {
  return (numBytes + (Uptr(1) << Platform::getPageSizeLog2()) - 1) >>
...
TableInstance *createTable(TableType type) {
  ...
  // In 64-bit, allocate enough address-space to safely access 32-bit table
  // indices without bounds checking, or 16MB (4M elements) if the host is
  // 32-bit.
  const Uptr tableMaxBytes =
      HAS_64BIT_ADDRESS_SPACE ? (sizeof(TableInstance::FunctionElement) << 32)
                              : 16 * 1024 * 1024;

  // On a 64 bit runtime, align the table base to a 4GB boundary, so the lower
  // 32-bits will all be zero. Maybe it will allow better code generation?
  // Note that this reserves a full extra 4GB, but only uses (4GB-1 page) for
  // alignment, so there will always be a guard page at the end to
  // protect against unaligned loads/stores that straddle the end of the
  // address-space.
  const Uptr alignmentBytes = HAS_64BIT_ADDRESS_SPACE
                                  ? 4ull * 1024 * 1024 * 1024
                                  : (Uptr(1) << Platform::getPageSizeLog2());
  table->baseAddress =
      (TableInstance::FunctionElement *)allocateVirtualPagesAligned(
          tableMaxBytes, alignmentBytes, table->reservedBaseAddress,
          table->reservedNumPlatformPages);
  ...
  if (growTable(table, type.size.min) == -1) {
    delete table;
    return nullptr;
  }
...
TableInstance::~TableInstance() {
  ...
  if (elements.size() > 0) {
    Platform::decommitVirtualPages(
        (U8 *)baseAddress,
        getNumPlatformPages(elements.size() *
                            sizeof(TableInstance::FunctionElement)));
  }

  // Free the virtual address space.
  if (reservedNumPlatformPages > 0) {
    Platform::freeVirtualPages((U8 *)reservedBaseAddress,
...
bool isAddressOwnedByTable(U8 *address) {
  // Iterate over all tables and check if the address is within the reserved
  // address space for each.
  for (auto table : tables) {
    U8 *startAddress = (U8 *)table->reservedBaseAddress;
    U8 *endAddress =
        ((U8 *)table->reservedBaseAddress) +
        (table->reservedNumPlatformPages << Platform::getPageSizeLog2());
...
ObjectInstance *setTableElement(TableInstance *table, Uptr index,
                                ObjectInstance *newValue) {
  // Write the new table element to both the table's elements array and its
  // indirect function call data.
...
Iptr growTable(TableInstance *table, Uptr numNewElements) {
  ...
  if (numNewElements > 0) {
    // If the number of elements to grow would cause the table's size to exceed
    // its maximum, return -1.
    if (numNewElements > table->type.size.max ||
        table->elements.size() > table->type.size.max - numNewElements) {
      return -1;
    }

    // Try to commit pages for the new elements, and return -1 if the commit
    // fails.
    const Uptr previousNumPlatformPages = getNumPlatformPages(
        table->elements.size() * sizeof(TableInstance::FunctionElement));
    const Uptr newNumPlatformPages =
        getNumPlatformPages((table->elements.size() + numNewElements) *
                            sizeof(TableInstance::FunctionElement));
...
Iptr shrinkTable(TableInstance *table, Uptr numElementsToShrink) {
  ...
  if (numElementsToShrink > 0) {
    // If the number of elements to shrink would cause the tables's size to drop
    // below its minimum, return -1.
    ...
    // Decommit the pages that were shrunk off the end of the table's indirect
    // function call data.
    const Uptr previousNumPlatformPages = getNumPlatformPages(
        previousNumElements * sizeof(TableInstance::FunctionElement));
    const Uptr newNumPlatformPages = getNumPlatformPages(
        table->elements.size() * sizeof(TableInstance::FunctionElement));
    if (newNumPlatformPages != previousNumPlatformPages) {
      Platform::decommitVirtualPages(
          (U8 *)table->baseAddress +
              (newNumPlatformPages << Platform::getPageSizeLog2()),
