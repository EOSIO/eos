...
#ifdef _DEBUG
// This needs to be 1 to allow debuggers such as Visual Studio to place
// breakpoints and step through the JITed code.
...
// descriptive names.
struct JITSymbol {
  enum class Type { functionInstance, invokeThunk };
  Type type;
  union {
  ...
  JITSymbol(FunctionInstance *inFunctionInstance, Uptr inBaseAddress,
            Uptr inNumBytes, std::map<U32, U32> &&inOffsetToOpIndexMap)
      : type(Type::functionInstance), functionInstance(inFunctionInstance),
        baseAddress(inBaseAddress), numBytes(inNumBytes),
        offsetToOpIndexMap(inOffsetToOpIndexMap) {}

  JITSymbol(const FunctionType *inInvokeThunkType, Uptr inBaseAddress,
            Uptr inNumBytes, std::map<U32, U32> &&inOffsetToOpIndexMap)
      : type(Type::invokeThunk), invokeThunkType(inInvokeThunkType),
        baseAddress(inBaseAddress), numBytes(inNumBytes),
        offsetToOpIndexMap(inOffsetToOpIndexMap) {}
};
...
struct UnitMemoryManager : llvm::RTDyldMemoryManager {
  UnitMemoryManager()
      : imageBaseAddress(nullptr), isFinalized(false), codeSection({0}),
        readOnlySection({0}), readWriteSection({0}),
  ...
  virtual ~UnitMemoryManager() override {
    ...
    // Decommit the image pages, but leave them reserved to catch any references
    // to them that might erroneously remain.
  ...
  virtual void reserveAllocationSpace(uintptr_t numCodeBytes, U32 codeAlignment,
    ...
    codeSection.numPages =
        shrAndRoundUp(numCodeBytes, Platform::getPageSizeLog2());
    readOnlySection.numPages =
        shrAndRoundUp(numReadOnlyBytes, Platform::getPageSizeLog2());
    readWriteSection.numPages =
        shrAndRoundUp(numReadWriteBytes, Platform::getPageSizeLog2());
    numAllocatedImagePages = codeSection.numPages + readOnlySection.numPages +
                             readWriteSection.numPages;
    if (numAllocatedImagePages) {
      ...
      if (!imageBaseAddress ||
          !Platform::commitVirtualPages(imageBaseAddress,
      ...
      readOnlySection.baseAddress =
          ...
          (codeSection.numPages << Platform::getPageSizeLog2());
      readWriteSection.baseAddress =
          readOnlySection.baseAddress +
          (readOnlySection.numPages << Platform::getPageSizeLog2());
    }
  }
  virtual U8 *allocateCodeSection(uintptr_t numBytes, U32 alignment,
                                  U32 sectionID,
                                  llvm::StringRef sectionName) override {
    return allocateBytes((Uptr)numBytes, alignment, codeSection);
  }
  virtual U8 *allocateDataSection(uintptr_t numBytes, U32 alignment,
                                  U32 sectionID, llvm::StringRef SectionName,
                                  bool isReadOnly) override {
    return allocateBytes((Uptr)numBytes, alignment,
  ...
  virtual bool finalizeMemory(std::string *ErrMsg = nullptr) override {
    ...
    const Platform::MemoryAccess codeAccess =
        USE_WRITEABLE_JIT_CODE_PAGES ? Platform::MemoryAccess::ReadWriteExecute
                                     : Platform::MemoryAccess::Execute;
    if (codeSection.numPages &&
        !Platform::setVirtualPageAccess(codeSection.baseAddress,
                                        codeSection.numPages, codeAccess)) {
      return false;
    }
    if (readOnlySection.numPages &&
        !Platform::setVirtualPageAccess(readOnlySection.baseAddress,
                                        readOnlySection.numPages,
                                        Platform::MemoryAccess::ReadOnly)) {
      return false;
    }
    if (readWriteSection.numPages &&
        !Platform::setVirtualPageAccess(readWriteSection.baseAddress,
                                        readWriteSection.numPages,
  ...
  virtual void invalidateInstructionCache() {
    ...
    llvm::sys::Memory::InvalidateInstructionCache(
        imageBaseAddress,
        numAllocatedImagePages << Platform::getPageSizeLog2());
  }
...
private:
  ...
  U8 *allocateBytes(Uptr numBytes, Uptr alignment, Section &section) {
    ...
    section.numCommittedBytes = align(section.numCommittedBytes, alignment) +
                                align(numBytes, alignment);

    // Check that enough space was reserved in the section.
    if (section.numCommittedBytes >
        (section.numPages << Platform::getPageSizeLog2())) {
...
// Encapsulates the LLVM JIT compilation pipeline but allows subclasses to
// define how the resulting code is used.
struct JITUnit {
...
#ifdef _WIN32
    if (pdataCopy) {
      Platform::deregisterSEHUnwindInfo(reinterpret_cast<Uptr>(pdataCopy));
    }
#endif
  }
  ...
  virtual void notifySymbolLoaded(const char *name, Uptr baseAddress,
                                  Uptr numBytes,
                                  std::map<U32, U32> &&offsetToOpIndexMap) = 0;

private:
  // Functor that receives notifications when an object produced by the JIT is
  // loaded.
  struct NotifyLoadedFunctor {
    ...
    void operator()(
        ...
        const std::vector<std::unique_ptr<
            llvm::object::OwningBinary<llvm::object::ObjectFile>>> &objectSet,
  ...
  struct NotifyFinalizedFunctor {
    ...
    void operator()(const llvm::orc::ObjectLinkingLayerBase::ObjSetHandleT
                        &objectSetHandle);
  };
...
struct JITModule : JITUnit, JITModuleBase {
  ...
  ~JITModule() override {
    // Delete the module's symbols, and remove them from the global
  ...
  void notifySymbolLoaded(const char *name, Uptr baseAddress, Uptr numBytes,
    ...
    // Save the address range this function was loaded at for future
    // address->symbol lookups.
...
struct JITInvokeThunkUnit : JITUnit {
  ...
  JITInvokeThunkUnit(const FunctionType *inFunctionType)
...
// Used to override LLVM's default behavior of looking up unresolved symbols in
// DLL exports.
struct NullResolver : llvm::JITSymbolResolver {
  ...
  virtual llvm::JITSymbol
  findSymbolInLogicalDylib(const std::string &name) override;
};

NullResolver NullResolver::singleton;
llvm::JITSymbol NullResolver::findSymbol(const std::string &name) {
  // Allow __chkstk through: the LLVM X86 code generator adds calls to it when
  // allocating more than 4KB of stack space.
  if (name == "__chkstk") {
    void *addr = llvm::sys::DynamicLibrary::SearchForAddressOfSymbol(name);
    if (addr) {
      return llvm::JITSymbol(reinterpret_cast<Uptr>(addr),
                             llvm::JITSymbolFlags::None);
    }
  }

  Log::printf(Log::Category::error,
              "LLVM generated code referenced external symbol: %s\n",
              name.c_str());
  Errors::unreachable();
}
llvm::JITSymbol
...
void JITUnit::NotifyLoadedFunctor::operator()(
  ...
  for (Uptr objectIndex = 0; objectIndex < loadedObjects.size();
       ++objectIndex) {
    llvm::object::ObjectFile *object =
        objectSet[objectIndex].get()->getBinary();
    llvm::RuntimeDyld::LoadedObjectInfo *loadedObject =
        loadedObjects[objectIndex].get();

    // Make a copy of the loaded object info for use by the finalizer.
    jitUnit->loadedObjects.push_back({object, loadedObject});

#ifdef _WIN32
    // On Windows, look for .pdata and .xdata sections containing information
    // about how to unwind the stack.
    // This needs to be done before the below emitAndFinalize call, which will
    // incorrectly apply relocations to the unwind info.
    ...
    if (pdataSection.getObject()) {
      const Uptr imageBaseAddress =
          reinterpret_cast<Uptr>(jitUnit->memoryManager.getImageBaseAddress());
      const Uptr pdataSectionLoadAddress =
          (Uptr)loadedObject->getSectionLoadAddress(pdataSection);

      // The LLVM COFF dynamic loader doesn't handle the image-relative
      // relocations used by the pdata section,
      // and overwrites those values with o:
      ...
      // This works around that by making a copy of the pdata section and doing
      // the pdata relocations manually.
      jitUnit->pdataCopy = new U8[pdataSection.getSize()];
      memcpy(jitUnit->pdataCopy,
             reinterpret_cast<U8 *>(pdataSectionLoadAddress),
             pdataSection.getSize());

      for (auto pdataRelocIt : pdataSection.relocations()) {
        // Only handle type 3 (IMAGE_REL_AMD64_ADDR32NB).
        if (pdataRelocIt.getType() != 3) {
          Errors::unreachable();
        }

        const auto symbol = pdataRelocIt.getSymbol();
        const U64 symbolAddress = symbol->getAddress().get();
        const llvm::object::section_iterator symbolSection =
            symbol->getSection().get();
        U32 *valueToRelocate =
            (U32 *)(jitUnit->pdataCopy + pdataRelocIt.getOffset());
        const U64 relocatedValue64 =
            +(symbolAddress - symbolSection->getAddress()) +
            loadedObject->getSectionLoadAddress(*symbolSection) +
            *valueToRelocate - imageBaseAddress;
        if (relocatedValue64 > UINT32_MAX) {
          Errors::unreachable();
        }
      ...
      Platform::registerSEHUnwindInfo(
          imageBaseAddress, reinterpret_cast<Uptr>(jitUnit->pdataCopy),
          pdataSection.getSize());
    }
#endif
  }
...
void JITUnit::NotifyFinalizedFunctor::operator()(
    const llvm::orc::ObjectLinkingLayerBase::ObjSetHandleT &objectSetHandle) {
  for (Uptr objectIndex = 0; objectIndex < jitUnit->loadedObjects.size();
       ++objectIndex) {
    llvm::object::ObjectFile *object =
        jitUnit->loadedObjects[objectIndex].object;
    llvm::RuntimeDyld::LoadedObjectInfo *loadedObject =
        jitUnit->loadedObjects[objectIndex].loadedObject;

    // Create a DWARF context to interpret the debug information in this
    // compilation unit.
    auto dwarfContext =
        llvm::make_unique<llvm::DWARFContextInMemory>(*object, loadedObject);
    ...
    for (auto symbolSizePair : llvm::object::computeSymbolSizes(*object)) {
      ...
      if (symbol.getType() &&
          symbol.getType().get() == llvm::object::SymbolRef::ST_Function &&
          name && address) {
        ...
        if (symbolSection) {
          loadedAddress +=
              (Uptr)loadedObject->getSectionLoadAddress(*symbolSection.get());
        }

        // Get the DWARF line info for this symbol, which maps machine code
        // addresses to WebAssembly op indices.
        llvm::DILineInfoTable lineInfoTable =
            dwarfContext->getLineInfoForAddressRange(loadedAddress,
        ...
        jitUnit->notifySymbolLoaded(name->data(), loadedAddress,
                                    symbolSizePair.second,
                                    std::move(offsetToOpIndexMap));
      }
    }
...
void printModule(const llvm::Module *llvmModule, const char *filename) {
  ...
  llvm::raw_fd_ostream dumpFileStream(augmentedFilename, errorCode,
...
void JITUnit::compile(llvm::Module *llvmModule) {
  // Get a target machine object for this host, and set the module to use its
  // data layout.
  ...
  for (auto functionIt = llvmModule->begin(); functionIt != llvmModule->end();
       ++functionIt) {
    fpm->run(*functionIt);
  }
  delete fpm;
  ...
  if (shouldLogMetrics) {
    Timing::logRatePerSecond("Optimized LLVM module", optimizationTimer,
  ...
  handle = compileLayer->addModuleSet(std::vector<llvm::Module *>{llvmModule},
                                      &memoryManager, &NullResolver::singleton);
  compileLayer->emitAndFinalize(handle);

  if (shouldLogMetrics) {
    Timing::logRatePerSecond("Generated machine code", machineCodeTimer,
                             (F64)llvmModule->size(), "functions");
  }
...
void instantiateModule(const IR::Module &module,
                       ModuleInstance *moduleInstance) {
...
std::string getExternalFunctionName(ModuleInstance *moduleInstance,
                                    Uptr functionDefIndex) {
  assert(functionDefIndex < moduleInstance->functionDefs.size());
...
bool getFunctionIndexFromExternalName(const char *externalName,
...
bool describeInstructionPointer(Uptr ip, std::string &outDescription) {
  JITSymbol *symbol;
  {
    ...
    if (symbolIt == addressToSymbolMap.end()) {
      return false;
    }
    symbol = symbolIt->second;
  }
  if (ip < symbol->baseAddress ||
      ip >= symbol->baseAddress + symbol->numBytes) {
    return false;
  }
  ...
  case JITSymbol::Type::functionInstance:
    ...
    if (!outDescription.size()) {
      outDescription = "<unnamed function>";
    }
    break;
  case JITSymbol::Type::invokeThunk:
    outDescription =
        "<invoke thunk : " + asString(symbol->invokeThunkType) + ">";
    break;
  default:
    Errors::unreachable();
  };
  ...
  for (auto offsetMapIt : symbol->offsetToOpIndexMap) {
    if (offsetMapIt.first <= ipOffset) {
      opIndex = offsetMapIt.second;
    } else {
      break;
    }
  }
  if (opIndex >= 0) {
    outDescription += " (op " + std::to_string(opIndex) + ")";
  }
  return true;
}
...
InvokeFunctionPointer getInvokeThunk(const FunctionType *functionType) {
  ...
  // Load the function's arguments from an array of 64-bit values at an address
  // provided by the caller.
  std::vector<llvm::Value *> structArgLoads;
  for (Uptr parameterIndex = 0;
       parameterIndex < functionType->parameters.size(); ++parameterIndex) {
    structArgLoads.push_back(irBuilder.CreateLoad(irBuilder.CreatePointerCast(
        irBuilder.CreateInBoundsGEP(argBaseAddress,
                                    {emitLiteral((Uptr)parameterIndex)}),
  ...
  // If the function has a return value, write it to the end of the argument
  // array.
  ...
  {
    Platform::Lock addressToSymbolMapLock(addressToSymbolMapMutex);
    addressToSymbolMap[jitUnit->symbol->baseAddress +
                       jitUnit->symbol->numBytes] = jitUnit->symbol;
  }
...
void init() {
...
#ifdef __APPLE__
  // Didn't figure out exactly why, but this works around a problem with the
  // MacOS dynamic loader. Without it,
  ...
  typedZeroConstants[(Uptr)ValueType::v128] =
      llvm::ConstantVector::get({typedZeroConstants[(Uptr)ValueType::i64],
