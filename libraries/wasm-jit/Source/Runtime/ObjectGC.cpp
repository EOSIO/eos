...
void freeUnreferencedObjects(
    std::vector<ObjectInstance *> &&rootObjectReferences) {
  ...
  // Initialize the referencedObjects set from the rootObjectReferences and
  // intrinsic objects.
  ...
  const std::vector<ObjectInstance *> intrinsicObjects =
      Intrinsics::getAllIntrinsicObjects();
  ...
  // Scan the objects added to the referenced set so far: gather their child
  ...
  while (pendingScanObjects.size()) {
    ...
    case ObjectKind::table: {
      ...
      childReferences.insert(childReferences.end(), table->elements.begin(),
                             table->elements.end());
      break;
    }
    ...
    // Add the object's child references to the referenced set, and enqueue them
    // for scanning.
  ...
  // Iterate over all objects, and delete objects that weren't referenced
  // directly or indirectly by the root set.
  GCGlobals &gcGlobals = GCGlobals::get();
  auto objectIt = gcGlobals.allObjects.begin();
  while (objectIt != gcGlobals.allObjects.end()) {
    if (referencedObjects.count(*objectIt)) {
      ++objectIt;
    } else {
