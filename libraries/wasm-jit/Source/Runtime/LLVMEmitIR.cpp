...
struct EmitModuleContext {
  ...
  EmitModuleContext(const Module &inModule, ModuleInstance *inModuleInstance)
      : module(inModule), moduleInstance(inModuleInstance),
        llvmModule(new llvm::Module("", context)), diBuilder(*llvmModule) {
...
struct EmitFunctionContext {
  ...
  EmitFunctionContext(EmitModuleContext &inEmitModuleContext,
                      const Module &inModule, const FunctionDef &inFunctionDef,
                      FunctionInstance *inFunctionInstance,
                      llvm::Function *inLLVMFunction)
      : moduleContext(inEmitModuleContext), module(inModule),
        functionDef(inFunctionDef),
        functionType(inModule.types[inFunctionDef.type.index]),
        functionInstance(inFunctionInstance), llvmFunction(inLLVMFunction),
        irBuilder(context) {}
  ...
  llvm::Value *getTopValue() const { return stack.back(); }
  ...
  void logOperator(const std::string &operatorDescription) {
    if (ENABLE_LOGGING) {
      ...
      for (Uptr stackIndex = 0; stackIndex < controlStack.size();
           ++stackIndex) {
        ...
        case ControlContext::Type::function:
          ...
          break;
        case ControlContext::Type::block:
          controlStackString += "B";
          break;
        case ControlContext::Type::ifThen:
          controlStackString += "T";
          break;
        case ControlContext::Type::ifElse:
          controlStackString += "E";
          break;
        case ControlContext::Type::loop:
          controlStackString += "L";
          break;
        default:
          Errors::unreachable();
        };
  ...
  // Bounds checks and converts a memory operation I32 address operand to a LLVM
  // pointer.
  llvm::Value *coerceByteIndexToPointer(llvm::Value *byteIndex, U32 offset,
                                        llvm::Type *memoryType) {
    // On a 64 bit runtime, if the address is 32-bits, zext it to 64-bits.
    // This is crucial for security, as LLVM will otherwise implicitly sign
    // extend it to 64-bits in the GEP below,
    // interpreting it as a signed offset and allowing access to memory outside
    // the sandboxed memory range.
    // There are no 'far addresses' in a 32 bit runtime.
    llvm::Value *nativeByteIndex =
        sizeof(Uptr) == 4 ? byteIndex
                          : irBuilder.CreateZExt(byteIndex, llvmI64Type);
    llvm::Value *offsetByteIndex = nativeByteIndex;
    if (offset) {
      auto nativeOffset =
          sizeof(Uptr) == 4
              ? emitLiteral((U32)offset)
              : irBuilder.CreateZExt(emitLiteral((U32)offset), llvmI64Type);
      offsetByteIndex = irBuilder.CreateAdd(nativeByteIndex, nativeOffset);
    }

    // Mask the index to the address-space size.
    auto maskedByteIndex = irBuilder.CreateAnd(
        offsetByteIndex, moduleContext.defaultMemoryAddressMask);

    // Cast the pointer to the appropriate type.
    auto bytePointer = irBuilder.CreateInBoundsGEP(
        moduleContext.defaultMemoryBase, maskedByteIndex);
  ...
  llvm::Value *
  getLLVMIntrinsic(const std::initializer_list<llvm::Type *> &argTypes,
                   llvm::Intrinsic::ID id) {
    return llvm::Intrinsic::getDeclaration(
        moduleContext.llvmModule, id,
        llvm::ArrayRef<llvm::Type *>(argTypes.begin(), argTypes.end()));
  }

  // Emits a call to a WAVM intrinsic function.
  llvm::Value *
  emitRuntimeIntrinsic(const char *intrinsicName,
                       const FunctionType *intrinsicType,
                       const std::initializer_list<llvm::Value *> &args) {
    ObjectInstance *intrinsicObject =
        Intrinsics::find(intrinsicName, intrinsicType);
    ...
    auto intrinsicFunctionPointer =
        emitLiteralPointer(intrinsicFunction->nativeFunction,
                           asLLVMType(intrinsicType)->getPointerTo());
    return irBuilder.CreateCall(
        intrinsicFunctionPointer,
        llvm::ArrayRef<llvm::Value *>(args.begin(), args.end()));
  }

  // A helper function to emit a conditional call to a non-returning intrinsic
  // function.
  void emitConditionalTrapIntrinsic(
      llvm::Value *booleanCondition, const char *intrinsicName,
      const FunctionType *intrinsicType,
      const std::initializer_list<llvm::Value *> &args) {
    auto trueBlock = llvm::BasicBlock::Create(
        context, llvm::Twine(intrinsicName) + "Trap", llvmFunction);
    auto endBlock = llvm::BasicBlock::Create(
        context, llvm::Twine(intrinsicName) + "Skip", llvmFunction);

    irBuilder.CreateCondBr(booleanCondition, trueBlock, endBlock,
                           moduleContext.likelyFalseBranchWeights);
  ...
  void pushControlStack(ControlContext::Type type, ResultType resultType,
                        llvm::BasicBlock *endBlock, llvm::PHINode *endPHI,
                        llvm::BasicBlock *elseBlock = nullptr) {
    // The unreachable operator filtering should filter out any opcodes that
    // call pushControlStack.
    if (controlStack.size()) {
      errorUnless(controlStack.back().isReachable);
    }

    controlStack.push_back({type, endBlock, endPHI, elseBlock, resultType,
                            stack.size(), branchTargetStack.size(), true,
                            true});
  }
  ...
  void pushBranchTarget(ResultType branchArgumentType,
    ...
    branchTargetStack.push_back(
        {branchArgumentType, branchTargetBlock, branchTargetPHI});
  }
  ...
  void block(ControlStructureImm imm) {
    ...
    pushControlStack(ControlContext::Type::block, imm.resultType, endBlock,
                     endPHI);
  ...
  void loop(ControlStructureImm imm) {
    ...
    auto loopBodyBlock =
        llvm::BasicBlock::Create(context, "loopBody", llvmFunction);
    ...
    // Push a control context that ends at the end block/phi.
    pushControlStack(ControlContext::Type::loop, imm.resultType, endBlock,
                     endPHI);
  ...
  void if_(ControlStructureImm imm) {
    // Create a then block and else block for the if, and an end block+phi for
    // the if result.
    ...
    auto endBlock =
        llvm::BasicBlock::Create(context, "ifElseEnd", llvmFunction);
    auto endPHI = createPHI(endBlock, imm.resultType);
    ...
    // Push an ifThen control context that ultimately ends at the end block/phi,
    // but may
    // be terminated by an else operator that changes the control context to the
    // else block.
    pushControlStack(ControlContext::Type::ifThen, imm.resultType, endBlock,
                     endPHI, elseBlock);
  ...
  void else_(NoImm imm) {
    ...
    if (currentContext.isReachable) {
      // If the control context expects a result, take it from the operand stack
      // and add it to the
  ...
  void end(NoImm) {
    ...
    if (currentContext.isReachable) {
      // If the control context yields a result, take the top of the operand
      // stack and
    ...
    if (currentContext.elseBlock) {
      // If this is the end of an if without an else clause, create a dummy else
      // clause.
      currentContext.elseBlock->moveAfter(irBuilder.GetInsertBlock());
      irBuilder.SetInsertPoint(currentContext.elseBlock);
      irBuilder.CreateBr(currentContext.endBlock);
    ...
    if (currentContext.endPHI) {
      // If the control context yields a result, take the PHI that merges all
      // the control flow
      // to the end and push it onto the operand stack.
      if (currentContext.endPHI->getNumIncomingValues()) {
        push(currentContext.endPHI);
      } else {
        // If there weren't any incoming values for the end PHI, remove it and
        // push a dummy value.
    ...
    assert(currentContext.outerBranchTargetStackSize <=
  ...
  // This is called after unconditional control flow to indicate that operators
  // following it are unreachable until the control stack is popped.
  void enterUnreachable() {
    ...
    // Mark the current control context as unreachable: this will cause the
    // outer loop to stop dispatching operators to us
  ...
  void br_if(BranchImm imm) {
    ...
    if (target.argumentType != ResultType::none) {
      // Use the stack top as the branch argument (don't pop it) and add it to
    ...
    auto falseBlock =
        llvm::BasicBlock::Create(context, "br_ifElse", llvmFunction);

    // Emit a conditional branch to either the falseBlock or the target block.
    irBuilder.CreateCondBr(coerceI32ToBool(condition), target.block,
                           falseBlock);
  ...
  void br(BranchImm imm) {
    BranchTarget &target = getBranchTargetByDepth(imm.targetDepth);
    if (target.argumentType != ResultType::none) {
      // Pop the branch argument from the stack and add it to the target phi's
      // incoming values.
  ...
  void br_table(BranchTableImm imm) {
    ...
    // Look up the default branch target, and assume its argument type applies
    // to all targets.
    // (this is guaranteed by the validator)
    BranchTarget &defaultTarget =
        getBranchTargetByDepth(imm.defaultTargetDepth);
    const ResultType argumentType = defaultTarget.argumentType;
    llvm::Value *argument = nullptr;
    if (argumentType != ResultType::none) {
      // Pop the branch argument from the stack and add it to the default target
    ...
    const std::vector<U32> &targetDepths =
        functionDef.branchTables[imm.branchTableIndex];
    auto llvmSwitch = irBuilder.CreateSwitch(index, defaultTarget.block,
                                             (unsigned int)targetDepths.size());

    for (Uptr targetIndex = 0; targetIndex < targetDepths.size();
         ++targetIndex) {
      ...
      if (argumentType != ResultType::none) {
        // If this is the first case in the table for this branch target, add
  ...
  void return_(NoImm) {
    if (functionType->ret != ResultType::none) {
      // Pop the return value from the stack and add it to the return phi's
      // incoming values.
      llvm::Value *result = pop();
  ...
  void unreachable(NoImm) {
    // Call an intrinsic that causes a trap, and insert the LLVM unreachable
    // terminator.
  ...
  void select(NoImm) {
    ...
    auto falseValue = pop();
    auto trueValue = pop();
    push(irBuilder.CreateSelect(coerceI32ToBool(condition), trueValue,
                                falseValue));
  }
  ...
  void call(CallImm imm) {
    // Map the callee function index to either an imported function pointer or a
    // function in this module.
    llvm::Value *callee;
    const FunctionType *calleeType;
    if (imm.functionIndex < moduleContext.importedFunctionPointers.size()) {
      assert(imm.functionIndex <
             moduleContext.moduleInstance->functions.size());
      callee = moduleContext.importedFunctionPointers[imm.functionIndex];
      calleeType =
          moduleContext.moduleInstance->functions[imm.functionIndex]->type;
    } else {
      const Uptr calleeIndex =
          imm.functionIndex - moduleContext.importedFunctionPointers.size();
    ...
    auto llvmArgs = (llvm::Value **)alloca(sizeof(llvm::Value *) *
                                           calleeType->parameters.size());
    popMultiple(llvmArgs, calleeType->parameters.size());

    // Call the function.
    auto result = irBuilder.CreateCall(
        callee,
        llvm::ArrayRef<llvm::Value *>(llvmArgs, calleeType->parameters.size()));
  ...
  void call_indirect(CallIndirectImm imm) {
    ...
    auto functionPointerType =
        asLLVMType(calleeType)->getPointerTo()->getPointerTo();
    ...
    emitConditionalTrapIntrinsic(
        ...
        FunctionType::get(ResultType::none,
                          {ValueType::i32, ValueType::i64, ValueType::i64}),
        {tableElementIndex,
         irBuilder.CreatePtrToInt(llvmCalleeType, llvmI64Type),
         emitLiteral(reinterpret_cast<U64>(
             moduleContext.moduleInstance->defaultTable))});

    // Call the function loaded from the table.
    auto functionPointerPointer =
        irBuilder.CreateInBoundsGEP(moduleContext.defaultTablePointer,
                                    {functionIndexZExt, emitLiteral((U32)1)});
    auto functionPointer = irBuilder.CreateLoad(irBuilder.CreatePointerCast(
        functionPointerPointer, functionPointerType));
    auto result = irBuilder.CreateCall(
        functionPointer,
        llvm::ArrayRef<llvm::Value *>(llvmArgs, calleeType->parameters.size()));
  ...
  void set_global(GetOrSetVariableImm<true> imm) {
    assert(imm.variableIndex < moduleContext.globalPointers.size());
    auto value =
        irBuilder.CreateBitCast(pop(),
                                moduleContext.globalPointers[imm.variableIndex]
                                    ->getType()
                                    ->getPointerElementType());
    irBuilder.CreateStore(value,
                          moduleContext.globalPointers[imm.variableIndex]);
  }

  //
  // Memory size operators
  // These just call out to wavmIntrinsics.growMemory/currentMemory, passing a
  // pointer to the default memory for the module.
  //

  void grow_memory(MemoryImm) {
    auto deltaNumPages = pop();
    auto defaultMemoryObjectAsI64 = emitLiteral(
        reinterpret_cast<U64>(moduleContext.moduleInstance->defaultMemory));
...
#define EMIT_LOAD_OP(valueTypeId, name, llvmMemoryType, naturalAlignmentLog2,  \
                     conversionOp)                                             \
  void valueTypeId##_##name(LoadOrStoreImm<naturalAlignmentLog2> imm) {        \
    auto byteIndex = pop();                                                    \
    auto pointer =                                                             \
        coerceByteIndexToPointer(byteIndex, imm.offset, llvmMemoryType);       \
...
#define EMIT_STORE_OP(valueTypeId, name, llvmMemoryType, naturalAlignmentLog2, \
                      conversionOp)                                            \
  void valueTypeId##_##name(LoadOrStoreImm<naturalAlignmentLog2> imm) {        \
    auto value = pop();                                                        \
    auto byteIndex = pop();                                                    \
  ...
  EMIT_LOAD_OP(i32, load8_u, llvmI8Type, 0, irBuilder.CreateZExt) EMIT_LOAD_OP(
      i32, load16_s, llvmI16Type, 1,
      irBuilder.CreateSExt) EMIT_LOAD_OP(i32, load16_u, llvmI16Type, 1,
                                         irBuilder.CreateZExt)
      EMIT_LOAD_OP(i64, load8_s, llvmI8Type, 0,
                   irBuilder.CreateSExt) EMIT_LOAD_OP(i64, load8_u, llvmI8Type,
                                                      0, irBuilder.CreateZExt)
          EMIT_LOAD_OP(i64, load16_s, llvmI16Type, 1,
                       irBuilder.CreateSExt) EMIT_LOAD_OP(i64, load16_u,
                                                          llvmI16Type, 1,
                                                          irBuilder.CreateZExt)
              EMIT_LOAD_OP(
                  i64, load32_s, llvmI32Type, 2,
                  irBuilder.CreateSExt) EMIT_LOAD_OP(i64, load32_u, llvmI32Type,
                                                     2, irBuilder.CreateZExt)

                  EMIT_LOAD_OP(
                      i32, load, llvmI32Type, 2,
                      identityConversion) EMIT_LOAD_OP(i64, load, llvmI64Type,
                                                       3, identityConversion)
                      EMIT_LOAD_OP(
                          f32, load, llvmF32Type, 2,
                          identityConversion) EMIT_LOAD_OP(f64, load,
                                                           llvmF64Type, 3,
                                                           identityConversion)

                          EMIT_STORE_OP(
                              i32, store8, llvmI8Type, 0,
                              irBuilder
                                  .CreateTrunc) EMIT_STORE_OP(i64, store8,
                                                              llvmI8Type, 0,
                                                              irBuilder
                                                                  .CreateTrunc)
                              EMIT_STORE_OP(i32, store16, llvmI16Type, 1,
                                            irBuilder.CreateTrunc)
                                  EMIT_STORE_OP(i64, store16, llvmI16Type, 1,
                                                irBuilder.CreateTrunc)
                                      EMIT_STORE_OP(i32, store, llvmI32Type, 2,
                                                    irBuilder.CreateTrunc)
                                          EMIT_STORE_OP(i64, store32,
                                                        llvmI32Type, 2,
                                                        irBuilder.CreateTrunc)
                                              EMIT_STORE_OP(i64, store,
                                                            llvmI64Type, 3,
                                                            identityConversion)
                                                  EMIT_STORE_OP(
                                                      f32, store, llvmF32Type,
                                                      2, identityConversion)
                                                      EMIT_STORE_OP(
                                                          f64, store,
                                                          llvmF64Type, 3,
                                                          identityConversion)
...
#define EMIT_FP_UNARY_OP(name, emitCode)                                       \
  EMIT_UNARY_OP(f32, name, emitCode) EMIT_UNARY_OP(f64, name, emitCode)
      ...
      llvm::Value *emitSRem(ValueType type, llvm::Value *left,
                            llvm::Value *right) {
    // Trap if the dividend is zero.
    trapDivideByZero(type, right);

    // LLVM's srem has undefined behavior where WebAssembly's rem_s defines that
    // it should not trap if the corresponding
    // division would overflow a signed integer. To avoid this case, we just
    // branch around the srem if the INT_MAX%-1 case
    // that overflows is detected.
    auto preOverflowBlock = irBuilder.GetInsertBlock();
    auto noOverflowBlock =
        llvm::BasicBlock::Create(context, "sremNoOverflow", llvmFunction);
    ...
    auto noOverflow = irBuilder.CreateOr(
        irBuilder.CreateICmpNE(left,
                               type == ValueType::i32
                                   ? emitLiteral((U32)INT32_MIN)
                                   : emitLiteral((U64)INT64_MIN)),
        irBuilder.CreateICmpNE(right,
                               type == ValueType::i32 ? emitLiteral((U32)-1)
                                                      : emitLiteral((U64)-1)));
    irBuilder.CreateCondBr(noOverflow, noOverflowBlock, endBlock,
                           moduleContext.likelyTrueBranchWeights);
  ...
  llvm::Value *emitShiftCountMask(ValueType type, llvm::Value *shiftCount) {
    // LLVM's shifts have undefined behavior where WebAssembly specifies that
    // the shift count will wrap numbers
    // grather than the bit count of the operands. This matches x86's native
    // shift instructions, but explicitly mask
    // the shift count anyway to support other platforms, and ensure the
    // optimizer doesn't take advantage of the UB.
  ...
  llvm::Value *emitRotl(ValueType type, llvm::Value *left, llvm::Value *right) {
    auto bitWidthMinusRight = irBuilder.CreateSub(
        irBuilder.CreateZExt(emitLiteral(getTypeBitWidth(type)),
                             asLLVMType(type)),
        right);
  ...
  // Divides use trapDivideByZero to avoid the undefined behavior in LLVM's
  // division instructions.
  EMIT_INT_BINARY_OP(div_s, (trapDivideByZero(type, right),
                             irBuilder.CreateSDiv(left, right)))
  EMIT_INT_BINARY_OP(div_u, (trapDivideByZero(type, right),
                             irBuilder.CreateUDiv(left, right)))
  EMIT_INT_BINARY_OP(rem_u, (trapDivideByZero(type, right),
                             irBuilder.CreateURem(left, right)))
  EMIT_INT_BINARY_OP(rem_s, emitSRem(type, left, right))

  // Explicitly mask the shift amount operand to the word size to avoid LLVM's
  // undefined behavior.
  ...
  EMIT_INT_BINARY_OP(lt_s,
                     coerceBoolToI32(irBuilder.CreateICmpSLT(left, right)))
  EMIT_INT_BINARY_OP(lt_u,
                     coerceBoolToI32(irBuilder.CreateICmpULT(left, right)))
  EMIT_INT_BINARY_OP(le_s,
                     coerceBoolToI32(irBuilder.CreateICmpSLE(left, right)))
  EMIT_INT_BINARY_OP(le_u,
                     coerceBoolToI32(irBuilder.CreateICmpULE(left, right)))
  EMIT_INT_BINARY_OP(gt_s,
                     coerceBoolToI32(irBuilder.CreateICmpSGT(left, right)))
  EMIT_INT_BINARY_OP(gt_u,
                     coerceBoolToI32(irBuilder.CreateICmpUGT(left, right)))
  EMIT_INT_BINARY_OP(ge_s,
                     coerceBoolToI32(irBuilder.CreateICmpSGE(left, right)))
  EMIT_INT_BINARY_OP(ge_u,
                     coerceBoolToI32(irBuilder.CreateICmpUGE(left, right)))

  EMIT_INT_UNARY_OP(
      clz,
      irBuilder.CreateCall(
          getLLVMIntrinsic({operand->getType()}, llvm::Intrinsic::ctlz),
          llvm::ArrayRef<llvm::Value *>({operand, emitLiteral(false)})))
  EMIT_INT_UNARY_OP(
      ctz,
      irBuilder.CreateCall(
          getLLVMIntrinsic({operand->getType()}, llvm::Intrinsic::cttz),
          llvm::ArrayRef<llvm::Value *>({operand, emitLiteral(false)})))
  EMIT_INT_UNARY_OP(
      popcnt,
      irBuilder.CreateCall(getLLVMIntrinsic({operand->getType()},
                                            llvm::Intrinsic::ctpop),
                           llvm::ArrayRef<llvm::Value *>({operand})))
  EMIT_INT_UNARY_OP(eqz, coerceBoolToI32(irBuilder.CreateICmpEQ(
                             operand, typedZeroConstants[(Uptr)type])))
  ...
  EMIT_FP_BINARY_OP(
      copysign,
      irBuilder.CreateCall(getLLVMIntrinsic({left->getType()},
                                            llvm::Intrinsic::copysign),
                           llvm::ArrayRef<llvm::Value *>({left, right})))
  ...
  EMIT_FP_UNARY_OP(convert_s_i32,
                   irBuilder.CreateSIToFP(operand, asLLVMType(type)))
  EMIT_FP_UNARY_OP(convert_s_i64,
                   irBuilder.CreateSIToFP(operand, asLLVMType(type)))
  EMIT_FP_UNARY_OP(convert_u_i32,
                   irBuilder.CreateUIToFP(operand, asLLVMType(type)))
  EMIT_FP_UNARY_OP(convert_u_i64,
                   irBuilder.CreateUIToFP(operand, asLLVMType(type)))
  ...
  EMIT_UNARY_OP(f32, reinterpret_i32,
                irBuilder.CreateBitCast(operand, llvmF32Type))
  EMIT_UNARY_OP(f64, reinterpret_i64,
                irBuilder.CreateBitCast(operand, llvmF64Type))
  EMIT_UNARY_OP(i32, reinterpret_f32,
                irBuilder.CreateBitCast(operand, llvmI32Type))
  EMIT_UNARY_OP(i64, reinterpret_f64,
                irBuilder.CreateBitCast(operand, llvmI64Type))

  // These operations don't match LLVM's semantics exactly, so just call out to
  // C++ implementations.
  EMIT_FP_BINARY_OP(min,
                    emitRuntimeIntrinsic("wavmIntrinsics.floatMin",
                                         FunctionType::get(asResultType(type),
                                                           {type, type}),
                                         {left, right}))
  EMIT_FP_BINARY_OP(max,
                    emitRuntimeIntrinsic("wavmIntrinsics.floatMax",
                                         FunctionType::get(asResultType(type),
                                                           {type, type}),
                                         {left, right}))
  EMIT_FP_UNARY_OP(ceil,
                   emitRuntimeIntrinsic("wavmIntrinsics.floatCeil",
                                        FunctionType::get(asResultType(type),
                                                          {type}),
                                        {operand}))
  EMIT_FP_UNARY_OP(floor,
                   emitRuntimeIntrinsic("wavmIntrinsics.floatFloor",
                                        FunctionType::get(asResultType(type),
                                                          {type}),
                                        {operand}))
  EMIT_FP_UNARY_OP(trunc,
                   emitRuntimeIntrinsic("wavmIntrinsics.floatTrunc",
                                        FunctionType::get(asResultType(type),
                                                          {type}),
                                        {operand}))
  EMIT_FP_UNARY_OP(nearest,
                   emitRuntimeIntrinsic("wavmIntrinsics.floatNearest",
                                        FunctionType::get(asResultType(type),
                                                          {type}),
                                        {operand}))
  EMIT_INT_UNARY_OP(trunc_s_f32,
                    emitRuntimeIntrinsic("wavmIntrinsics.floatToSignedInt",
                                         FunctionType::get(asResultType(type),
                                                           {ValueType::f32}),
                                         {operand}))
  EMIT_INT_UNARY_OP(trunc_s_f64,
                    emitRuntimeIntrinsic("wavmIntrinsics.floatToSignedInt",
                                         FunctionType::get(asResultType(type),
                                                           {ValueType::f64}),
                                         {operand}))
  EMIT_INT_UNARY_OP(trunc_u_f32,
                    emitRuntimeIntrinsic("wavmIntrinsics.floatToUnsignedInt",
                                         FunctionType::get(asResultType(type),
                                                           {ValueType::f32}),
                                         {operand}))
  EMIT_INT_UNARY_OP(trunc_u_f64,
                    emitRuntimeIntrinsic("wavmIntrinsics.floatToUnsignedInt",
                                         FunctionType::get(asResultType(type),
                                                           {ValueType::f64}),
                                         {operand}))
  ...
  llvm::Value *emitAnyTrue(llvm::Value *boolVector) {
    ...
    for (Uptr laneIndex = 0; laneIndex < numLanes; ++laneIndex) {
      llvm::Value *scalar =
          irBuilder.CreateExtractElement(boolVector, laneIndex);
  ...
  llvm::Value *unimplemented() { Errors::unreachable(); }
...
#define EMIT_SIMD_FP_UNARY_OP(name, emitCode)                                  \
  ...
  EMIT_SIMD_UNARY_OP(f32x4_convert_s_i32x4, llvmI32x4Type,
                     irBuilder.CreateSIToFP(operand, llvmF32x4Type));
  EMIT_SIMD_UNARY_OP(f32x4_convert_u_i32x4, llvmI32x4Type,
                     irBuilder.CreateUIToFP(operand, llvmF32x4Type));
  EMIT_SIMD_UNARY_OP(f64x2_convert_s_i64x2, llvmI64x2Type,
                     irBuilder.CreateSIToFP(operand, llvmF64x2Type));
  EMIT_SIMD_UNARY_OP(f64x2_convert_u_i64x2, llvmI64x2Type,
...
#define EMIT_SIMD_EXTRACT_LANE_OP(name, llvmType, numLanes, coerceScalar)      \
  ...
  EMIT_SIMD_EXTRACT_LANE_OP(i8x16_extract_lane_s, llvmI8x16Type, 16,
                            irBuilder.CreateSExt(scalar, llvmI32Type))
  EMIT_SIMD_EXTRACT_LANE_OP(i8x16_extract_lane_u, llvmI8x16Type, 16,
                            irBuilder.CreateZExt(scalar, llvmI32Type))
  EMIT_SIMD_EXTRACT_LANE_OP(i16x8_extract_lane_s, llvmI16x8Type, 8,
                            irBuilder.CreateSExt(scalar, llvmI32Type))
  EMIT_SIMD_EXTRACT_LANE_OP(i16x8_extract_lane_u, llvmI16x8Type, 8,
                            irBuilder.CreateZExt(scalar, llvmI32Type))
  ...
  EMIT_SIMD_EXTRACT_LANE_OP(b8x16_extract_lane, llvmB8x16Type, 16,
                            coerceBoolToI32(scalar))
  EMIT_SIMD_EXTRACT_LANE_OP(b16x8_extract_lane, llvmB16x8Type, 8,
                            coerceBoolToI32(scalar))
  EMIT_SIMD_EXTRACT_LANE_OP(b32x4_extract_lane, llvmB32x4Type, 4,
                            coerceBoolToI32(scalar))
  EMIT_SIMD_EXTRACT_LANE_OP(b64x2_extract_lane, llvmB64x2Type, 2,
                            coerceBoolToI32(scalar))

#define EMIT_SIMD_REPLACE_LANE_OP(typePrefix, llvmType, numLanes,              \
                                  coerceScalar)                                \
  ...
  EMIT_SIMD_REPLACE_LANE_OP(i8x16, llvmI8x16Type, 16,
                            irBuilder.CreateTrunc(scalar, llvmI8Type))
  EMIT_SIMD_REPLACE_LANE_OP(i16x8, llvmI16x8Type, 8,
  ...
  void is_lock_free(NoImm) {
    ...
    push(emitRuntimeIntrinsic(
        ...
        FunctionType::get(ResultType::i32, {ValueType::i32}), {numBytes}));
  }
  void wake(AtomicLoadOrStoreImm<2>) {
    ...
    push(emitRuntimeIntrinsic(
        "wavmIntrinsics.wake",
        FunctionType::get(ResultType::i32,
  ...
  void launch_thread(LaunchThreadImm) {
    ...
    auto defaultTableAsI64 = emitLiteral(
        reinterpret_cast<U64>(moduleContext.moduleInstance->defaultTable));
    emitRuntimeIntrinsic(
        "wavmIntrinsics.launchThread",
        FunctionType::get(ResultType::none, {ValueType::i32, ValueType::i32,
  ...
  void trapIfMisalignedAtomic(llvm::Value *address, U32 naturalAlignmentLog2) {
    if (naturalAlignmentLog2 > 0) {
      emitConditionalTrapIntrinsic(
          ...
          FunctionType::get(ResultType::none, {ValueType::i32}), {address});
    }
  }

  EMIT_UNARY_OP(i32, extend_s_i8,
                irBuilder.CreateSExt(irBuilder.CreateTrunc(operand, llvmI8Type),
                                     llvmI32Type))
  EMIT_UNARY_OP(i32, extend_s_i16,
                irBuilder.CreateSExt(irBuilder.CreateTrunc(operand,
                                                           llvmI16Type),
                                     llvmI32Type))
  EMIT_UNARY_OP(i64, extend_s_i8,
                irBuilder.CreateSExt(irBuilder.CreateTrunc(operand, llvmI8Type),
                                     llvmI64Type))
  EMIT_UNARY_OP(i64, extend_s_i16,
                irBuilder.CreateSExt(irBuilder.CreateTrunc(operand,
                                                           llvmI16Type),
                                     llvmI64Type))

#define EMIT_ATOMIC_LOAD_OP(valueTypeId, name, llvmMemoryType,                 \
                            naturalAlignmentLog2, conversionOp)                \
...
#define EMIT_ATOMIC_STORE_OP(valueTypeId, name, llvmMemoryType,                \
                             naturalAlignmentLog2, conversionOp)               \
  void valueTypeId##_##name(AtomicLoadOrStoreImm<naturalAlignmentLog2> imm) {  \
    auto value = pop();                                                        \
    auto byteIndex = pop();                                                    \
  ...
  EMIT_ATOMIC_LOAD_OP(i32, atomic_load16_s, llvmI16Type, 1,
                      irBuilder.CreateSExt)
  EMIT_ATOMIC_LOAD_OP(i32, atomic_load16_u, llvmI16Type, 1,
                      irBuilder.CreateZExt)
  ...
  EMIT_ATOMIC_LOAD_OP(i64, atomic_load16_s, llvmI16Type, 1,
                      irBuilder.CreateSExt)
  EMIT_ATOMIC_LOAD_OP(i64, atomic_load16_u, llvmI16Type, 1,
                      irBuilder.CreateZExt)
  EMIT_ATOMIC_LOAD_OP(i64, atomic_load32_s, llvmI32Type, 2,
                      irBuilder.CreateSExt)
  EMIT_ATOMIC_LOAD_OP(i64, atomic_load32_u, llvmI32Type, 2,
                      irBuilder.CreateZExt)
  ...
  EMIT_ATOMIC_STORE_OP(i32, atomic_store16, llvmI16Type, 1,
                       irBuilder.CreateTrunc)
  EMIT_ATOMIC_STORE_OP(i64, atomic_store8, llvmI8Type, 0, irBuilder.CreateTrunc)
  EMIT_ATOMIC_STORE_OP(i64, atomic_store16, llvmI16Type, 1,
                       irBuilder.CreateTrunc)
  EMIT_ATOMIC_STORE_OP(i64, atomic_store32, llvmI32Type, 2,
                       irBuilder.CreateTrunc)

#define EMIT_ATOMIC_CMPXCHG(valueTypeId, name, llvmMemoryType,                 \
                            naturalAlignmentLog2, memoryToValueConversion,     \
  ...
  void valueTypeId##_##name(AtomicLoadOrStoreImm<naturalAlignmentLog2> imm) {  \
    ...
    auto atomicCmpXchg = irBuilder.CreateAtomicCmpXchg(                        \
        pointer, expectedValue, replacementValue,                              \
        llvm::AtomicOrdering::SequentiallyConsistent,                          \
        llvm::AtomicOrdering::SequentiallyConsistent);                         \
    atomicCmpXchg->setVolatile(true);                                          \
    push(memoryToValueConversion(atomicCmpXchg,                                \
                                 asLLVMType(ValueType::valueTypeId)));         \
  }

  EMIT_ATOMIC_CMPXCHG(i32, atomic_rmw8_u_cmpxchg, llvmI8Type, 0,
                      irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_CMPXCHG(i32, atomic_rmw16_u_cmpxchg, llvmI16Type, 1,
                      irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_CMPXCHG(i32, atomic_rmw_cmpxchg, llvmI32Type, 2,
                      identityConversion, identityConversion)

  EMIT_ATOMIC_CMPXCHG(i64, atomic_rmw8_u_cmpxchg, llvmI8Type, 0,
                      irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_CMPXCHG(i64, atomic_rmw16_u_cmpxchg, llvmI16Type, 1,
                      irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_CMPXCHG(i64, atomic_rmw32_u_cmpxchg, llvmI32Type, 2,
                      irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_CMPXCHG(i64, atomic_rmw_cmpxchg, llvmI64Type, 3,
                      identityConversion, identityConversion)

#define EMIT_ATOMIC_RMW(valueTypeId, name, rmwOpId, llvmMemoryType,            \
                        naturalAlignmentLog2, memoryToValueConversion,         \
                        valueToMemoryConversion)                               \
  void valueTypeId##_##name(AtomicLoadOrStoreImm<naturalAlignmentLog2> imm) {  \
    ...
    auto atomicRMW = irBuilder.CreateAtomicRMW(                                \
        llvm::AtomicRMWInst::BinOp::rmwOpId, pointer, value,                   \
        llvm::AtomicOrdering::SequentiallyConsistent);                         \
    atomicRMW->setVolatile(true);                                              \
    push(memoryToValueConversion(atomicRMW,                                    \
                                 asLLVMType(ValueType::valueTypeId)));         \
  }

  EMIT_ATOMIC_RMW(i32, atomic_rmw8_u_xchg, Xchg, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw16_u_xchg, Xchg, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw_xchg, Xchg, llvmI32Type, 2,
                  identityConversion, identityConversion)

  EMIT_ATOMIC_RMW(i64, atomic_rmw8_u_xchg, Xchg, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw16_u_xchg, Xchg, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw32_u_xchg, Xchg, llvmI16Type, 2,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw_xchg, Xchg, llvmI64Type, 3,
                  identityConversion, identityConversion)

  EMIT_ATOMIC_RMW(i32, atomic_rmw8_u_add, Add, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw16_u_add, Add, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw_add, Add, llvmI32Type, 2, identityConversion,
                  identityConversion)

  EMIT_ATOMIC_RMW(i64, atomic_rmw8_u_add, Add, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw16_u_add, Add, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw32_u_add, Add, llvmI32Type, 2,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw_add, Add, llvmI64Type, 3, identityConversion,
                  identityConversion)

  EMIT_ATOMIC_RMW(i32, atomic_rmw8_u_sub, Sub, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw16_u_sub, Sub, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw_sub, Sub, llvmI32Type, 2, identityConversion,
                  identityConversion)

  EMIT_ATOMIC_RMW(i64, atomic_rmw8_u_sub, Sub, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw16_u_sub, Sub, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw32_u_sub, Sub, llvmI32Type, 2,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw_sub, Sub, llvmI64Type, 3, identityConversion,
                  identityConversion)

  EMIT_ATOMIC_RMW(i32, atomic_rmw8_u_and, And, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw16_u_and, And, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw_and, And, llvmI32Type, 2, identityConversion,
                  identityConversion)

  EMIT_ATOMIC_RMW(i64, atomic_rmw8_u_and, And, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw16_u_and, And, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw32_u_and, And, llvmI32Type, 2,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw_and, And, llvmI64Type, 3, identityConversion,
                  identityConversion)

  EMIT_ATOMIC_RMW(i32, atomic_rmw8_u_or, Or, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw16_u_or, Or, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw_or, Or, llvmI32Type, 2, identityConversion,
                  identityConversion)

  EMIT_ATOMIC_RMW(i64, atomic_rmw8_u_or, Or, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw16_u_or, Or, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw32_u_or, Or, llvmI32Type, 2,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw_or, Or, llvmI64Type, 3, identityConversion,
                  identityConversion)

  EMIT_ATOMIC_RMW(i32, atomic_rmw8_u_xor, Xor, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw16_u_xor, Xor, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i32, atomic_rmw_xor, Xor, llvmI32Type, 2, identityConversion,
                  identityConversion)

  EMIT_ATOMIC_RMW(i64, atomic_rmw8_u_xor, Xor, llvmI8Type, 0,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw16_u_xor, Xor, llvmI16Type, 1,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw32_u_xor, Xor, llvmI32Type, 2,
                  irBuilder.CreateZExt, irBuilder.CreateTrunc)
  EMIT_ATOMIC_RMW(i64, atomic_rmw_xor, Xor, llvmI64Type, 3, identityConversion,
                  identityConversion)
#endif
};

// A do-nothing visitor used to decode past unreachable operators (but
// supporting logging, and passing the end operator through).
...
#undef VISIT_OP
  ...
  // Keep track of control structure nesting level in unreachable code, so we
  // know when we reach the end of the unreachable code.
  ...
  // If an else or end opcode would signal an end to the unreachable code, then
  // pass it through to the IR emitter.
  void else_(NoImm imm) {
...
void EmitFunctionContext::emit() {
  ...
  for (auto parameterType : functionType->parameters) {
    diFunctionParameterTypes.push_back(
        moduleContext.diValueTypes[(Uptr)parameterType]);
  }
  auto diFunctionType = moduleContext.diBuilder.createSubroutineType(
      moduleContext.diBuilder.getOrCreateTypeArray(diFunctionParameterTypes));
  diFunction = moduleContext.diBuilder.createFunction(
      moduleContext.diModuleScope, functionInstance->debugName,
      llvmFunction->getName(), moduleContext.diModuleScope, 0, diFunctionType,
      false, true, 0);
  llvmFunction->setSubprogram(diFunction);

  // Create the return basic block, and push the root control context for the
  // function.
  auto returnBlock = llvm::BasicBlock::Create(context, "return", llvmFunction);
  auto returnPHI = createPHI(returnBlock, functionType->ret);
  pushControlStack(ControlContext::Type::function, functionType->ret,
                   returnBlock, returnPHI);
  ...
  for (Uptr localIndex = 0;
       localIndex < functionType->parameters.size() +
    ...
    auto localType =
        localIndex < functionType->parameters.size()
            ...
            : functionDef
                  .nonParameterLocalTypes[localIndex -
                                          functionType->parameters.size()];
    auto localPointer =
        irBuilder.CreateAlloca(asLLVMType(localType), nullptr, "");
  ...
  while (decoder && controlStack.size()) {
    irBuilder.SetCurrentDebugLocation(
        llvm::DILocation::get(context, (unsigned int)opIndex++, 0, diFunction));
    if (ENABLE_LOGGING) {
...
llvm::Module *EmitModuleContext::emit() {
  ...
  if (moduleInstance->defaultMemory) {
    defaultMemoryBase = emitLiteralPointer(
        moduleInstance->defaultMemory->baseAddress, llvmI8PtrType);
    const Uptr defaultMemoryAddressMaskValue =
        Uptr(moduleInstance->defaultMemory->endOffset) - 1;
    defaultMemoryAddressMask = emitLiteral(defaultMemoryAddressMaskValue);
  } else {
    defaultMemoryBase = defaultMemoryAddressMask = nullptr;
  }

  // Set up the LLVM values used to access the global table.
  if (moduleInstance->defaultTable) {
    auto tableElementType =
        llvm::StructType::get(context, {llvmI8PtrType, llvmI8PtrType});
    defaultTablePointer =
        emitLiteralPointer(moduleInstance->defaultTable->baseAddress,
                           tableElementType->getPointerTo());
    defaultTableEndOffset =
        emitLiteral((Uptr)moduleInstance->defaultTable->endOffset);
  } else {
    defaultTablePointer = defaultTableEndOffset = nullptr;
  }

  // Create LLVM pointer constants for the module's imported functions.
  for (Uptr functionIndex = 0; functionIndex < module.functions.imports.size();
       ++functionIndex) {
    const FunctionInstance *functionInstance =
        moduleInstance->functions[functionIndex];
    importedFunctionPointers.push_back(
        emitLiteralPointer(functionInstance->nativeFunction,
                           asLLVMType(functionInstance->type)->getPointerTo()));
  }

  // Create LLVM pointer constants for the module's globals.
  for (auto global : moduleInstance->globals) {
    globalPointers.push_back(emitLiteralPointer(
        &global->value, asLLVMType(global->type.valueType)->getPointerTo()));
  }

  // Create the LLVM functions.
  functionDefs.resize(module.functions.defs.size());
  for (Uptr functionDefIndex = 0;
       functionDefIndex < module.functions.defs.size(); ++functionDefIndex) {
    auto llvmFunctionType = asLLVMType(
        module.types[module.functions.defs[functionDefIndex].type.index]);
    auto externalName =
        getExternalFunctionName(moduleInstance, functionDefIndex);
    functionDefs[functionDefIndex] = llvm::Function::Create(
        llvmFunctionType, llvm::Function::ExternalLinkage, externalName,
        llvmModule);
  }
  ...
  for (Uptr functionDefIndex = 0;
       functionDefIndex < module.functions.defs.size(); ++functionDefIndex) {
    EmitFunctionContext(*this, module, module.functions.defs[functionDefIndex],
  ...
  Timing::logRatePerSecond("Emitted LLVM IR", emitTimer,
                           (F64)llvmModule->size(), "functions");
