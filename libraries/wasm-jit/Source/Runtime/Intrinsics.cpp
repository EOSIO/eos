...
std::string getDecoratedName(const std::string &name,
...
Function::Function(const char *inName, const IR::FunctionType *type,
                   void *nativeFunction)
    : name(inName) {
...
Function::~Function() {
  {
    Platform::Lock Lock(Singleton::get().mutex);
    Singleton::get().functionMap.erase(Singleton::get().functionMap.find(
        getDecoratedName(name, function->type)));
  }
  delete function;
}
...
Global::~Global() {
  {
    Platform::Lock Lock(Singleton::get().mutex);
    Singleton::get().variableMap.erase(Singleton::get().variableMap.find(
        getDecoratedName(name, global->type)));
  }
  delete global;
}
...
Table::~Table() {
  {
    Platform::Lock Lock(Singleton::get().mutex);
    Singleton::get().tableMap.erase(
        Singleton::get().tableMap.find(getDecoratedName(name, table->type)));
  }
  delete table;
}
...
Memory::~Memory() {
  {
    Platform::Lock Lock(Singleton::get().mutex);
    Singleton::get().memoryMap.erase(
        Singleton::get().memoryMap.find(getDecoratedName(name, memory->type)));
  }
  delete memory;
}

Runtime::ObjectInstance *find(const std::string &name,
                              const IR::ObjectType &type) {
  ...
  case IR::ObjectKind::function: {
    ...
    result = keyValue == Singleton::get().functionMap.end()
                 ? nullptr
  ...
  case IR::ObjectKind::table: {
    ...
    result = keyValue == Singleton::get().tableMap.end()
                 ? nullptr
                 : asObject((Runtime::TableInstance *)*keyValue->second);
    break;
  }
  case IR::ObjectKind::memory: {
    auto keyValue = Singleton::get().memoryMap.find(decoratedName);
    result = keyValue == Singleton::get().memoryMap.end()
                 ? nullptr
                 : asObject((Runtime::MemoryInstance *)*keyValue->second);
    break;
  }
  case IR::ObjectKind::global: {
    auto keyValue = Singleton::get().variableMap.find(decoratedName);
    result = keyValue == Singleton::get().variableMap.end()
                 ? nullptr
                 : asObject(keyValue->second->global);
    break;
  }
  default:
    Errors::unreachable();
...
std::vector<Runtime::ObjectInstance *> getAllIntrinsicObjects() {
  ...
  for (auto mapIt : Singleton::get().tableMap) {
    result.push_back((Runtime::TableInstance *)*mapIt.second);
  }
  for (auto mapIt : Singleton::get().memoryMap) {
    result.push_back((Runtime::MemoryInstance *)*mapIt.second);
  }
  for (auto mapIt : Singleton::get().variableMap) {
