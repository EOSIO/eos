...
U8 *allocateVirtualPages(Uptr numPages) {
  ...
  auto result =
      mmap(nullptr, numBytes, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
bool commitVirtualPages(U8 *baseVirtualAddress, Uptr numPages,
  ...
  return mprotect(baseVirtualAddress, numPages << getPageSizeLog2(),
                  memoryAccessAsPOSIXFlag(access)) == 0;
}
...
void initThread() {
  if (!signalStack) {
    // Allocate a stack to use when handling signals, so stack overflow can be
    // handled safely.
    ...
    // Get the stack address from pthreads, but use getrlimit to find the
    // maximum size of the stack instead of the current.
    struct rlimit stackLimit;
    getrlimit(RLIMIT_STACK, &stackLimit);
#ifdef __linux__
    // Linux uses pthread_getattr_np/pthread_attr_getstack, and returns a
    // pointer to the minimum address of the stack.
    pthread_attr_t threadAttributes;
    memset(&threadAttributes, 0, sizeof(threadAttributes));
    pthread_getattr_np(pthread_self(), &threadAttributes);
    Uptr stackSize;
    pthread_attr_getstack(&threadAttributes, (void **)&stackMinAddr,
                          &stackSize);
    pthread_attr_destroy(&threadAttributes);
    stackMaxAddr = stackMinAddr + stackSize;
    stackMinAddr = stackMaxAddr - stackLimit.rlim_cur;
#else
    // MacOS uses pthread_getstackaddr_np, and returns a pointer to the maximum
    // address of the stack.
    stackMaxAddr = (U8 *)pthread_get_stackaddr_np(pthread_self());
    stackMinAddr = stackMaxAddr - stackLimit.rlim_cur;
#endif

    // Include an extra page below the stack's usable address range to
    // distinguish stack overflows from general SIGSEGV.
...
void signalHandler(int signalNumber, siginfo_t *signalInfo, void *) {
  ...
  switch (signalNumber) {
  case SIGFPE:
    if (signalInfo->si_code != FPE_INTDIV &&
        signalInfo->si_code != FPE_INTOVF) {
  ...
  case SIGSEGV:
  case SIGBUS:
    signalType = signalInfo->si_addr >= stackMinAddr &&
                         signalInfo->si_addr < stackMaxAddr
  ...
  // Capture the execution context, omitting this function and the function that
  // called it,
  // so the top of the callstack is the function that triggered the signal.
  *signalCallStack = captureCallStack(2);

  // If the signal occurred outside of a catchHardwareTraps call, just treat it
  // as a fatal error.
  if (!isCatchingSignals) {
    switch (signalNumber) {
    case SIGFPE:
      Errors::fatalf("unhandled SIGFPE\n");
    case SIGSEGV:
      Errors::fatalf("unhandled SIGSEGV\n");
    case SIGBUS:
      Errors::fatalf("unhandled SIGBUS\n");
    default:
      Errors::unreachable();
    };
  }
...
CallStack captureCallStack(Uptr numOmittedFramesFromTop) {
  ...
  // Copy the return pointers into the stack frames of the resulting
  // ExecutionContext.
  // Skip the first numOmittedFramesFromTop+1 frames, which correspond to this
  // function
  // and others that the caller would like to omit.
  CallStack result;
  for (Iptr index = numOmittedFramesFromTop + 1; index < numCallStackEntries;
       ++index) {
...
U64 getMonotonicClock() {
  ...
  return U64(monotonicClock.tv_sec) * 1000000 +
         U64(monotonicClock.tv_nsec) / 1000;
#endif
}
...
#ifndef __APPLE__
  errorUnless(
      !pthread_condattr_setclock(&conditionVariableAttr, CLOCK_MONOTONIC));
#endif
...
bool waitForEvent(Event *event, U64 untilTime) {
  ...
  if (untilTime == UINT64_MAX) {
    ...
    result = pthread_cond_timedwait(&event->conditionVariable, &event->mutex,
                                    &untilTimeSpec);
  }
