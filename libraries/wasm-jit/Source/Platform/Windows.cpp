...
Uptr getPageSizeLog2() {
  static Uptr preferredVirtualPageSizeLog2 =
      internalGetPreferredVirtualPageSizeLog2();
...
U32 memoryAccessAsWin32Flag(MemoryAccess access) {
  ...
  case MemoryAccess::ReadWriteExecute:
...
bool commitVirtualPages(U8 *baseVirtualAddress, Uptr numPages,
                        MemoryAccess access) {
  errorUnless(isPageAligned(baseVirtualAddress));
  return baseVirtualAddress ==
         VirtualAlloc(baseVirtualAddress, numPages << getPageSizeLog2(),
                      MEM_COMMIT, memoryAccessAsWin32Flag(access));
}

bool setVirtualPageAccess(U8 *baseVirtualAddress, Uptr numPages,
                          MemoryAccess access) {
  errorUnless(isPageAligned(baseVirtualAddress));
  DWORD oldProtection = 0;
  return VirtualProtect(baseVirtualAddress, numPages << getPageSizeLog2(),
                        memoryAccessAsWin32Flag(access), &oldProtection) != 0;
}

void decommitVirtualPages(U8 *baseVirtualAddress, Uptr numPages) {
  errorUnless(isPageAligned(baseVirtualAddress));
  auto result = VirtualFree(baseVirtualAddress, numPages << getPageSizeLog2(),
                            MEM_DECOMMIT);
  if (baseVirtualAddress && !result) {
    Errors::fatal("VirtualFree(MEM_DECOMMIT) failed");
  }
}

void freeVirtualPages(U8 *baseVirtualAddress, Uptr numPages) {
  errorUnless(isPageAligned(baseVirtualAddress));
  auto result = VirtualFree(baseVirtualAddress,
                            0 /*numPages << getPageSizeLog2()*/, MEM_RELEASE);
  if (baseVirtualAddress && !result) {
    Errors::fatal("VirtualFree(MEM_RELEASE) failed");
  }
}
...
bool describeInstructionPointer(Uptr ip, std::string &outDescription) {
  ...
  // Allocate up a SYMBOL_INFO struct to receive information about the symbol
  // for this instruction pointer.
  ...
  // Call DbgHelp::SymFromAddr to try to find any debug symbol containing this
  // address.
  if (!dbgHelp->symFromAddr(GetCurrentProcess(), ip, nullptr, symbolInfo)) {
    return false;
  } else {
...
void registerSEHUnwindInfo(Uptr imageLoadAddress, Uptr pdataAddress,
                           Uptr pdataNumBytes) {
  ...
  if (!RtlAddFunctionTable(reinterpret_cast<RUNTIME_FUNCTION *>(pdataAddress),
                           numFunctions, imageLoadAddress)) {
...
CallStack unwindStack(const CONTEXT &immutableContext) {
  ...
  // Unwind the stack until there isn't a valid instruction pointer, which
  // signals we've reached the base.
  CallStack callStack;
  while (context.Rip) {
...
CallStack captureCallStack(Uptr numOmittedFramesFromTop) {
  ...
  const Uptr numOmittedFrames =
      std::min(result.stackFrames.size(), numOmittedFramesFromTop + 1);
  result.stackFrames.erase(result.stackFrames.begin(),
                           result.stackFrames.begin() + numOmittedFrames);

  return result;
}

THREAD_LOCAL bool isReentrantException = false;
LONG CALLBACK sehFilterFunction(EXCEPTION_POINTERS *exceptionPointers,
                                HardwareTrapType &outType, Uptr &outTrapOperand,
                                CallStack &outCallStack) {
  if (isReentrantException) {
    Errors::fatal("reentrant exception");
  } else {
    ...
    case EXCEPTION_STACK_OVERFLOW:
      outType = HardwareTrapType::stackOverflow;
      break;
    case STATUS_INTEGER_DIVIDE_BY_ZERO:
      outType = HardwareTrapType::intDivideByZeroOrOverflow;
      break;
...
void initThread() {
  if (!isThreadInitialized) {
    ...
    // Ensure that there's enough space left on the stack in the case of a stack
    // overflow to prepare the stack trace.
    ULONG stackOverflowReserveBytes = 32768;
    SetThreadStackGuarantee(&stackOverflowReserveBytes);
  }
}

HardwareTrapType catchHardwareTraps(CallStack &outTrapCallStack,
                                    Uptr &outTrapOperand,
  ...
  __try {
    thunk();
  } __except (sehFilterFunction(GetExceptionInformation(), result,
                                outTrapOperand, outTrapCallStack)) {
    isReentrantException = false;

    // After a stack overflow, the stack will be left in a damaged state. Let
    // the CRT repair it.
    if (result == HardwareTrapType::stackOverflow) {
      _resetstkoflw();
    }
  }
  return result;
...
void lockMutex(Mutex *mutex) { EnterCriticalSection(&mutex->criticalSection); }
...
bool waitForEvent(Event *event, U64 untilTime) {
  ...
  while (true) {
    const U64 timeoutMicroseconds =
        currentTime > untilTime ? 0 : (untilTime - currentTime);
    const U64 timeoutMilliseconds64 = timeoutMicroseconds / 1000;
    const U32 timeoutMilliseconds32 = timeoutMilliseconds64 > UINT32_MAX
                                          ? (UINT32_MAX - 1)
                                          : U32(timeoutMilliseconds64);

    const U32 waitResult = WaitForSingleObject(reinterpret_cast<HANDLE>(event),
                                               timeoutMilliseconds32);
