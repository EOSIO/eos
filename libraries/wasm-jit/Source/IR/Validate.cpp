...
#define VALIDATE_INDEX(index, arraySize)                                       \
  VALIDATE_UNLESS("invalid index: ", index >= arraySize)

void validate(ValueType valueType) {
  if (valueType == ValueType::any || valueType > ValueType::max) {
    throw ValidationException("invalid value type (" +
                              std::to_string((Uptr)valueType) + ")");
  }
}

void validate(ResultType returnType) {
  if (returnType > ResultType::max) {
    throw ValidationException("invalid return type (" +
                              std::to_string((Uptr)returnType) + ")");
  }
}

void validate(ObjectKind kind) {
  if (kind > ObjectKind::max) {
    throw ValidationException("invalid external kind (" +
                              std::to_string((Uptr)kind) + ")");
  }
}
...
void validate(TableElementType type) {
  if (type != TableElementType::anyfunc) {
    throw ValidationException("invalid table element type (" +
                              std::to_string((Uptr)type) + ")");
  }
}
...
void validate(GlobalType type) { validate(type.valueType); }
...
void validateType(Type expectedType, Type actualType, const char *context) {
  if (expectedType != actualType) {
    throw ValidationException(std::string("type mismatch: expected ") +
                              asString(expectedType) + " but got " +
                              asString(actualType) + " in " + context);
  }
}

void validateOperandType(ValueType expectedType, ValueType actualType,
                         const char *context) {
  // Handle polymorphic values popped off the operand stack after unconditional
  // branches.
  if (expectedType != actualType && expectedType != ValueType::any &&
    ...
    throw ValidationException(
        ...
        " but got " + asString(actualType) + " in " + context + " operand");
  }
}

ValueType validateGlobalIndex(const Module &module, Uptr globalIndex,
                              bool mustBeMutable, bool mustBeImmutable,
                              bool mustBeImport, const char *context) {
  ...
  if (mustBeMutable && !globalType.isMutable) {
    ...
    throw ValidationException("global variable initializer expression may only "
                              "access imported globals");
  } else if (mustBeImmutable && globalType.isMutable) {
    throw ValidationException("global variable initializer expression may only "
                              "access immutable globals");
  }
  return globalType.valueType;
}

const FunctionType *validateFunctionIndex(const Module &module,
                                          Uptr functionIndex) {
...
void validateInitializer(const Module &module,
                         ...
                         ValueType expectedType, const char *context) {
  ...
  case InitializerExpression::Type::get_global: {
    const ValueType globalValueType =
        validateGlobalIndex(module, expression.globalIndex, false, true, true,
...
struct FunctionValidationContext {
  FunctionValidationContext(const Module &inModule,
    ...
    locals.reserve(functionType->parameters.size() +
                   functionDef.nonParameterLocalTypes.size());
    locals = functionType->parameters;
    locals.insert(locals.end(), functionDef.nonParameterLocalTypes.begin(),
  ...
  void unknown(Opcode opcode) {
    throw ValidationException("Unknown opcode: " +
                              std::to_string((Uptr)opcode));
  }
  void block(ControlStructureImm imm) {
    validate(imm.resultType);
    pushControlStack(ControlContext::Type::block, imm.resultType,
                     imm.resultType);
  }
  void loop(ControlStructureImm imm) {
    validate(imm.resultType);
    pushControlStack(ControlContext::Type::loop, ResultType::none,
                     imm.resultType);
  }
  void if_(ControlStructureImm imm) {
    validate(imm.resultType);
    popAndValidateOperand("if condition", ValueType::i32);
    pushControlStack(ControlContext::Type::ifThen, imm.resultType,
                     imm.resultType);
  }
  ...
  void br(BranchImm imm) {
    popAndValidateResultType(
        "br argument",
        getBranchTargetByDepth(imm.targetDepth).branchArgumentType);
    enterUnreachable();
  }
  ...
  void get_global(GetOrSetVariableImm<true> imm) {
    pushOperand(validateGlobalIndex(module, imm.variableIndex, false, false,
                                    false, "get_global"));
  }
  void set_global(GetOrSetVariableImm<true> imm) {
    popAndValidateOperand("set_global",
                          validateGlobalIndex(module, imm.variableIndex, true,
  ...
  void call(CallImm imm) {
    ...
    popAndValidateOperands("call arguments", calleeType->parameters.data(),
                           (Uptr)calleeType->parameters.size());
  ...
  void call_indirect(CallIndirectImm imm) {
    ...
    popAndValidateOperands("call_indirect arguments",
                           calleeType->parameters.data(),
                           (Uptr)calleeType->parameters.size());
    pushOperand(calleeType->ret);
  }
  ...
  void validateImm(MemoryImm) {
    VALIDATE_UNLESS("current_memory and grow_memory are only valid if there is "
                    "a default memory",
                    module.memories.size() == 0);
  }
  ...
  void validateImm(LaunchThreadImm) {
    VALIDATE_UNLESS("launch_thread is only valid if there is a default table",
                    module.tables.size() == 0);
  }
  template <Uptr naturalAlignmentLog2>
...
#define BINARY(operandTypeId, resultTypeId)                                    \
  popAndValidateOperands(operatorName, ValueType::operandTypeId,               \
                         ValueType::operandTypeId);                            \
  pushOperand(ResultType::resultTypeId)
...
#define VECTORSELECT(vectorTypeId)                                             \
  popAndValidateOperands(operatorName, ValueType::vectorTypeId,                \
                         ValueType::vectorTypeId, ValueType::vectorTypeId);    \
  pushOperand(ValueType::vectorTypeId);
#define REPLACELANE(scalarTypeId, vectorTypeId)                                \
  popAndValidateOperands(operatorName, ValueType::vectorTypeId,                \
                         ValueType::scalarTypeId);                             \
  pushOperand(ValueType::vectorTypeId);
#endif

#if ENABLE_THREADING_PROTOTYPE
#define LAUNCHTHREAD                                                           \
  popAndValidateOperands(operatorName, ValueType::i32, ValueType::i32,         \
                         ValueType::i32);
...
#define ATOMICRMW(valueTypeId)                                                 \
  popAndValidateOperands(operatorName, ValueType::i32,                         \
                         ValueType::valueTypeId);                              \
  pushOperand(ValueType::valueTypeId);
#endif
...
private:
  ...
  void pushControlStack(ControlContext::Type type,
                        ResultType branchArgumentType, ResultType resultType) {
    controlStack.push_back(
        {type, stack.size(), branchArgumentType, resultType, true});
  }

  void popControlStack(bool isElse = false) {
    VALIDATE_UNLESS("stack was not empty at end of control structure: ",
                    stack.size() > controlStack.back().outerStackSize);
  ...
  void popAndValidateOperands(const char *context,
                              const ValueType *expectedTypes, Uptr num) {
    for (Uptr operandIndexFromEnd = 0; operandIndexFromEnd < num;
         ++operandIndexFromEnd) {
  ...
  template <Uptr num>
  void popAndValidateOperands(const char *context,
                              const ValueType (&expectedTypes)[num]) {
  ...
  void popAndValidateOperand(const char *context,
...
void validateDefinitions(const Module &module) {
  ...
  for (auto &globalImport : module.globals.imports) {
    ...
    VALIDATE_UNLESS("mutable globals cannot be imported: ",
                    globalImport.type.isMutable);
  }
  ...
  for (auto &globalDef : module.globals.defs) {
    ...
    validateInitializer(module, globalDef.initializer, globalDef.type.valueType,
  ...
  for (auto &exportIt : module.exports) {
    ...
    case ObjectKind::global:
      validateGlobalIndex(module, exportIt.index, false, true, false,
                          "exported global index");
      break;
    default:
      throw ValidationException("unknown export kind");
    };
  ...
  for (auto &dataSegment : module.dataSegments) {
    ...
    validateInitializer(module, dataSegment.baseOffset, ValueType::i32,
  ...
  for (auto &tableSegment : module.tableSegments) {
    ...
    validateInitializer(module, tableSegment.baseOffset, ValueType::i32,
                        "table segment base initializer");
...
CodeValidationStream::CodeValidationStream(const Module &module,
                                           const FunctionDef &functionDef) {
