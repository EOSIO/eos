...
void getDisassemblyNames(const Module &module, DisassemblyNames &outNames) {
  ...
  for (const auto &functionImport : module.functions.imports) {
    ...
    functionNames.locals.resize(
        module.types[functionImport.type.index]->parameters.size());
  ...
  for (Uptr functionDefIndex = 0;
    ...
    functionNames.locals.insert(
        ...
        module.types[functionDef.type.index]->parameters.size() +
  ...
  outNames.tables.insert(
      outNames.tables.end(),
      module.tables.defs.size() + module.tables.imports.size(), "");
  outNames.memories.insert(
      outNames.memories.end(),
      module.memories.defs.size() + module.memories.imports.size(), "");
  outNames.globals.insert(
      outNames.globals.end(),
      module.globals.defs.size() + module.globals.imports.size(), "");
  ...
  if (findUserSection(module, "name", userSectionIndex)) {
    try {
      ...
      MemoryInputStream stream(nameSection.data.data(),
                               nameSection.data.size());
      ...
      numFunctionNames =
          std::min(numFunctionNames, (Uptr)outNames.functions.size());

      for (Uptr functionIndex = 0; functionIndex < numFunctionNames;
           ++functionIndex) {
        DisassemblyNames::Function &functionNames =
        ...
        for (Uptr localIndex = 0; localIndex < numLocalNames; ++localIndex) {
          ...
          if (localIndex < functionNames.locals.size()) {
            functionNames.locals[localIndex] = std::move(localName);
          }
        }
      }
    } catch (FatalSerializationException exception) {
      Log::printf(Log::Category::debug, "FatalSerializationException while "
                                        "deserializing WASM user name section: "
                                        "%s\n",
...
void setDisassemblyNames(Module &module, const DisassemblyNames &names) {
  // Replace an existing name section if one is present, or create a new
  // section.
  Uptr userSectionIndex = 0;
  if (!findUserSection(module, "name", userSectionIndex)) {
  ...
  for (Uptr functionIndex = 0; functionIndex < names.functions.size();
       ++functionIndex) {
