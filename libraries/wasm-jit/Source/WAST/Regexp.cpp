...
struct Lit : Node {
  ...
  Lit(const NFA::CharSet &inCharSet)
      : Node(NodeType::lit), charSet(inCharSet) {}
};
...
static Node *parse(const char *string) {
  ...
  if (*nextChar != 0) {
    Errors::fatalf("failed to parse entire regexp");
  }
  return node;
}

static void createNFA(NFA::Builder *nfaBuilder, Node *node,
  ...
  case NodeType::oneOrMore: {
    ...
    createNFA(nfaBuilder, oneOrMore->child, intermediateState,
              intermediateState);
...
namespace Regexp {
void addToNFA(const char *regexpString, NFA::Builder *nfaBuilder,
              NFA::StateIndex initialState, NFA::StateIndex finalState) {
