...
std::string expandIndentation(std::string &&inString,
  ...
  Uptr indentDepth = 0;
  while (next < end) {
    // Absorb INDENT_STRING and DEDENT_STRING, but keep track of the indentation
    // depth,
...
struct NameScope {
  ...
  void map(std::string &name) {
    ...
    if (mapIt == nameToCountMap.end()) {
      ...
      name = name + std::to_string(mapIt->second++);
    }
    name = sigil + name;
  }
...
struct ModulePrintContext {
  ...
  ModulePrintContext(const Module &inModule, std::string &inString)
    ...
    // Start with the names from the module's user name section, but make sure
    // they are unique, and add the "$" sigil.
    IR::getDisassemblyNames(module, names);
    NameScope globalNameScope('$');
    for (auto &name : names.types) {
      globalNameScope.map(name);
    }
    for (auto &name : names.tables) {
      globalNameScope.map(name);
    }
    for (auto &name : names.memories) {
      globalNameScope.map(name);
    }
    for (auto &name : names.globals) {
      globalNameScope.map(name);
    }
...
struct FunctionPrintContext {
  ...
  FunctionPrintContext(ModulePrintContext &inModuleContext,
                       Uptr functionDefIndex)
      : moduleContext(inModuleContext), module(inModuleContext.module),
  ...
  void br_table(BranchTableImm imm) {
    ...
    for (Uptr targetIndex = 0; targetIndex < targetDepths.size();
         ++targetIndex) {
      if (targetIndex % numTargetsPerLine == 0) {
        string += '\n';
      } else {
        string += ' ';
      }
...
private:
  ...
  std::string getBranchTargetId(Uptr depth) {
    const ControlContext &controlContext =
        controlStack[controlStack.size() - depth - 1];
    return controlContext.type == ControlContext::Type::function
               ? std::to_string(depth)
...
template <typename Type>
void printImport(std::string &string, const Module &module,
                 const Import<Type> &import, Uptr importIndex, const char *name,
                 const char *typeTag) {
  string += '\n';
...
void ModulePrintContext::printModule() {
  ...
  for (Uptr importIndex = 0; importIndex < module.functions.imports.size();
       ++importIndex) {
    printImport(string, module, module.functions.imports[importIndex],
                importIndex, names.functions[importIndex].name.c_str(), "func");
  }
  for (Uptr importIndex = 0; importIndex < module.tables.imports.size();
       ++importIndex) {
    printImport(string, module, module.tables.imports[importIndex], importIndex,
                names.tables[importIndex].c_str(), "table");
  }
  for (Uptr importIndex = 0; importIndex < module.memories.imports.size();
       ++importIndex) {
    printImport(string, module, module.memories.imports[importIndex],
                importIndex, names.memories[importIndex].c_str(), "memory");
  }
  for (Uptr importIndex = 0; importIndex < module.globals.imports.size();
       ++importIndex) {
    printImport(string, module, module.globals.imports[importIndex],
                importIndex, names.globals[importIndex].c_str(), "global");
  }
  ...
  for (auto export_ : module.exports) {
    ...
    case ObjectKind::table:
      string += "table " + names.tables[export_.index];
      break;
    case ObjectKind::memory:
      string += "memory " + names.memories[export_.index];
      break;
    case ObjectKind::global:
      string += "global " + names.globals[export_.index];
      break;
    default:
      Errors::unreachable();
    };
  ...
  for (Uptr memoryDefIndex = 0; memoryDefIndex < module.memories.defs.size();
       ++memoryDefIndex) {
  ...
  for (Uptr tableDefIndex = 0; tableDefIndex < module.tables.defs.size();
       ++tableDefIndex) {
  ...
  for (Uptr globalDefIndex = 0; globalDefIndex < module.globals.defs.size();
       ++globalDefIndex) {
  ...
  for (auto tableSegment : module.tableSegments) {
    ...
    for (Uptr elementIndex = 0; elementIndex < tableSegment.indices.size();
         ++elementIndex) {
  ...
  for (auto dataSegment : module.dataSegments) {
    ...
    for (Uptr offset = 0; offset < dataSegment.data.size();
  ...
  for (Uptr functionDefIndex = 0;
    ...
    for (Uptr localIndex = 0;
         localIndex < functionDef.nonParameterLocalTypes.size(); ++localIndex) {
      string += '\n';
      ScopedTagPrinter localTag(string, "local");
      string += ' ';
      string += functionContext
                    .localNames[functionType->parameters.size() + localIndex];
      string += ' ';
  ...
  for (const auto &userSection : module.userSections) {
    if (userSection.name != "name") {
      ...
      string += "\" ";
      enum { numBytesPerLine = 64 };
      for (Uptr offset = 0; offset < userSection.data.size();
           offset += numBytesPerLine) {
        string += "\n\"";
        string += escapeString(
