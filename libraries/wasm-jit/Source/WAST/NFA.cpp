...
void addEdge(Builder *builder, StateIndex initialState,
             const CharSet &predicate, StateIndex nextState) {
  CharSet &transitionPredicate =
      builder->nfaStates[initialState].nextStateToPredicateMap[nextState];
  transitionPredicate = transitionPredicate | predicate;
}

void addEpsilonEdge(Builder *builder, StateIndex initialState,
                    StateIndex nextState) {
  addUnique(builder->nfaStates[initialState].epsilonNextStates, nextState);
}

StateIndex getNonTerminalEdge(Builder *builder, StateIndex initialState,
                              char c) {
  for (const auto &nextStateToPredicatePair :
       builder->nfaStates[initialState].nextStateToPredicateMap) {
    if (nextStateToPredicatePair.first >= 0 &&
        nextStateToPredicatePair.second.contains((U8)c)) {
...
struct DFAState {
  ...
  DFAState() {
    for (Uptr charIndex = 0; charIndex < 256; ++charIndex) {
      nextStateByChar[charIndex] =
          unmatchedCharacterTerminal | edgeDoesntConsumeInputFlag;
    }
  }
};
...
std::vector<DFAState> convertToDFA(Builder *builder) {
  ...
  while (pendingDFAStates.size()) {
    ...
    const StateSet currentStateSet =
        dfaStateToNFAStateSetMap[currentDFAStateIndex];

    // Expand the set of current states to include all states reachable by
    // epsilon transitions from the current states.
    StateSet epsilonClosureCurrentStateSet = currentStateSet;
    for (Uptr scanIndex = 0; scanIndex < epsilonClosureCurrentStateSet.size();
         ++scanIndex) {
      StateIndex scanState = epsilonClosureCurrentStateSet[scanIndex];
      if (scanState >= 0) {
        for (auto epsilonNextState :
             builder->nfaStates[scanState].epsilonNextStates) {
    ...
    StateIndex currentTerminalState =
        unmatchedCharacterTerminal | edgeDoesntConsumeInputFlag;
    ...
    // Build a compact index of the states following all states in the current
    // set.
    ...
    for (auto stateIndex : nonTerminalCurrentStateSet) {
      ...
      for (auto transition : nfaState.nextStateToPredicateMap) {
        if (!stateIndexToLocalStateIndexMap.count(transition.first)) {
          stateIndexToLocalStateIndexMap.emplace(
              transition.first,
    ...
    typedef DenseStaticIntSet<StateIndex, numSupportedLocalStates>
        LocalStateSet;
    ...
    // Combine the [nextState][char] transition maps for current states and
    // transpose to [char][nextState]
    // After building the compact index of referenced states, the nextState set
    // can be represented as a 64-bit mask.
        ...
        for (Uptr charIndex = 0; charIndex < 256; ++charIndex) {
          if (transition.second.contains((char)charIndex)) {
            charToLocalStateSet[charIndex].add(
                stateIndexToLocalStateIndexMap.at(transition.first));
          }
        }
      }
    }
    ...
    // For each unique local state set that follows this state set, find or
    // create a corresponding DFA state.
    ...
    for (auto localNextStateSet : uniqueLocalNextStateSets) {
      ...
      StateSet nextStateSet;
      {
        ...
        while (true) {
          const StateIndex localStateIndex =
              residualLocalStateSet.getSmallestMember();
          if (localStateIndex == numSupportedLocalStates) {
            break;
          }

          nextStateSet.push_back(
              localStateIndexToStateIndexMap.at(localStateIndex));
          residualLocalStateSet.remove(localStateIndex);
        };
      }

      if (nextStateSet.size() == 1 && *nextStateSet.begin() < 0) {
        localStateSetToDFAStateIndexMap.insert(
            std::make_pair(localNextStateSet, *nextStateSet.begin()));
      } else {
        ...
        if (nextDFAStateIt != nfaStateSetToDFAStateMap.end()) {
          localStateSetToDFAStateIndexMap.emplace(localNextStateSet,
                                                  nextDFAStateIt->second);
        } else {
          // If no corresponding DFA state existing yet, create a new one and
          // add it to the queue
          // of pending states to process.
          nextDFAStateIndex = (StateIndex)dfaStates.size();
          localStateSetToDFAStateIndexMap.emplace(localNextStateSet,
                                                  nextDFAStateIndex);
          nfaStateSetToDFAStateMap.insert(
              std::make_pair(nextStateSet, nextDFAStateIndex));
    ...
    maxDFANextStates =
        std::max(maxDFANextStates, (Uptr)uniqueLocalNextStateSets.size());
  };
  ...
  Log::printf(Log::Category::metrics,
              "  maximum number of states following a NFA state set: %u\n",
              maxLocalStates);
  Log::printf(Log::Category::metrics,
              "  maximum number of states following a DFA state: %u\n",
              maxDFANextStates);
...
struct StateTransitionsByChar {
  ...
  StateTransitionsByChar(StateTransitionsByChar &&inMove)
      : c(inMove.c), nextStateByInitialState(inMove.nextStateByInitialState),
  ...
  bool operator<(const StateTransitionsByChar &right) const {
    ...
    return memcmp(nextStateByInitialState, right.nextStateByInitialState,
                  sizeof(StateIndex) * numStates) < 0;
  }
  bool operator!=(const StateTransitionsByChar &right) const {
    assert(numStates == right.numStates);
    return memcmp(nextStateByInitialState, right.nextStateByInitialState,
...
Machine::Machine(Builder *builder) {
  ...
  for (Uptr charIndex = 0; charIndex < 256; ++charIndex) {
    stateTransitionsByChar.push_back(
        StateTransitionsByChar((U8)charIndex, dfaStates.size()));
  ...
  // Build a minimal set of character equivalence classes that have the same
  // transition across all states.
  ...
  for (Uptr charIndex = 1; charIndex < stateTransitionsByChar.size();
       ++charIndex) {
    if (stateTransitionsByChar[charIndex] !=
        stateTransitionsByChar[charIndex - 1]) {
      representativeCharsByClass[numClasses] =
          stateTransitionsByChar[charIndex].c;
      ++numClasses;
    }
    characterToClassMap[stateTransitionsByChar[charIndex].c] =
        U8(numClasses - 1);
  }
      ...
      stateAndOffsetToNextStateMap[stateIndex + classIndex * numStates] =
          InternalStateIndex(
              dfaStates[stateIndex]
                  .nextStateByChar[representativeCharsByClass[classIndex]]);
    }
  }

  // Build a map from character index to offset into [charClass][initialState]
  // transition map.
  assert((numClasses - 1) * (numStates - 1) <= UINT32_MAX);
  for (Uptr charIndex = 0; charIndex < 256; ++charIndex) {
    charToOffsetMap[charIndex] =
        U32(numStates * characterToClassMap[charIndex]);
  }
...
std::string getGraphEdgeCharLabel(Uptr charIndex) {
  ...
  case '-':
    return "\\-";
  default:
    return std::string(1, (char)charIndex);
  };
}
...
std::string dumpNFAGraphViz(const Builder *builder) {
  ...
  for (Uptr stateIndex = 0; stateIndex < builder->nfaStates.size();
       ++stateIndex) {
    const NFAState &nfaState = builder->nfaStates[stateIndex];
    ...
    for (auto epsilonNextState : nfaState.epsilonNextStates) {
      std::string nextStateName =
          epsilonNextState < 0 ? "terminal" + std::to_string(-epsilonNextState)
...
std::string Machine::dumpDFAGraphViz() const {
...

  {
    ...
    for (auto transitionPair : transitions) {
      if ((transitionPair.first & ~edgeDoesntConsumeInputFlag) !=
        ...
        std::string nextStateName =
            transitionPair.first < 0
                ? "terminal" + std::to_string(-(transitionPair.first &
                                                ~edgeDoesntConsumeInputFlag))
                : "state" + std::to_string(transitionPair.first);
        result +=
            "start" + std::to_string(startIndex) + " -> " + nextStateName +
            "[label=\"" +
            (transitionPair.first < 0 &&
                     (transitionPair.first & edgeDoesntConsumeInputFlag) != 0
                 ? "&epsilon; "
                 : "") +
            escapeString(edgeLabel) + "\"];\n";

        if (transitionPair.first < 0) {
          terminalStates.emplace(
              StateIndex(transitionPair.first & ~edgeDoesntConsumeInputFlag));
        }
      ...
      if ((transitionPair.first & ~edgeDoesntConsumeInputFlag) !=
        ...
        std::string nextStateName =
            transitionPair.first < 0
                ...
                : "state" + std::to_string(transitionPair.first);
        result +=
            "state" + std::to_string(stateIndex) + " -> " + nextStateName +
            "[label=\"" +
            (transitionPair.first < 0 &&
                     (transitionPair.first & edgeDoesntConsumeInputFlag) != 0
                 ? "&epsilon; "
  ...
  for (auto terminalState : terminalStates) {
    result += "terminal" +
              std::to_string(-(terminalState & ~edgeDoesntConsumeInputFlag)) +
              "[shape=octagon label=\"" +
