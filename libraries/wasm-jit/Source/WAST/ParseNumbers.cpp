...
// #define strtod and dtoa to avoid conflicting with the C standard library
// versions
// This is complicated by dtoa.c including stdlib.h, so we need to also include
// stdlib.h
...
// Parses an unsigned integer from hexits, starting with "0x", and advancing
// nextChar past the parsed hexits.
// be called for input that's already been accepted by the lexer as a
// hexadecimal integer.
static U64 parseHexUnsignedInt(const char *&nextChar, ParseState &state,
                               U64 maxValue) {
...
// Parses an unsigned integer from digits, advancing nextChar past the parsed
// digits.
// Assumes it will only be called for input that's already been accepted by the
// lexer as a decimal integer.
static U64 parseDecimalUnsignedInt(const char *&nextChar, ParseState &state,
                                   U64 maxValue, const char *context) {
  U64 result = 0;
...
Float parseNaN(const char *&nextChar, ParseState &state) {
  ...
  assert(nextChar[0] == 'n' && nextChar[1] == 'a' && nextChar[2] == 'n');
  nextChar += 3;
  ...
  if (*nextChar == ':') {
    ...
    const U64 significandBits =
        parseHexUnsignedInt(nextChar, state, FloatComponents::maxSignificand);
    resultComponents.bits.significand =
        typename FloatComponents::Bits(significandBits);
  } else {
    // If the NaN's significand isn't specified, just set the top bit.
    resultComponents.bits.significand =
        typename FloatComponents::Bits(1)
...
// Assumes it will only be called for input that's already been accepted by the
// lexer as a literal infinity.
template <typename Float> Float parseInfinity(const char *nextChar) {
  // Floating point infinite is represented by max exponent with a zero
  // significand.
  typedef typename Floats::FloatComponents<Float> FloatComponents;
  FloatComponents resultComponents;
  resultComponents.bits.sign = parseSign(nextChar) ? 1 : 0;
...
// Parses a decimal floating point literal, advancing nextChar past the parsed
// characters.
// Assumes it will only be called for input that's already been accepted by the
// lexer as a decimal float literal.
template <typename Float>
Float parseDecimalFloat(const char *&nextChar, ParseState &state) {
  ...
  if (f64 < std::numeric_limits<Float>::lowest() ||
      f64 > std::numeric_limits<Float>::max()) {
...
// Parses a hexadecimal floating point literal, advancing nextChar past the
// parsed characters.
// Assumes it will only be called for input that's already been accepted by the
// lexer as a hexadecimal float literal.
template <typename Float>
Float parseHexFloat(const char *&nextChar, ParseState &state) {
  ...
  // Parse hexits into a 64-bit fixed point number, keeping track of where the
  // point is in exponent.
  U64 fixedPoI64 = 0;
  ...
  while (true) {
    ...
    if (tryParseHexit(nextChar, hexit)) {
      // Once there are too many hexits to accumulate in the 64-bit fixed point
      // number, ignore
      // the hexits, but continue to update exponent so we get an accurate but
      // imprecise number.
  ...
  if (*nextChar == 'p' || *nextChar == 'P') {
    ...
    const U64 userExponent = parseDecimalUnsignedInt(
        nextChar, state, U64(-INT32_MIN), "float literal exponent");
    exponent =
        isExponentNegative ? exponent - userExponent : exponent + userExponent;
  }
  ...
  if (!fixedPoI64) {
    ...
    if (exponentWithImplicitLeadingOne > FloatComponents::maxNormalExponent) {
      ...
      parseErrorf(state, state.nextToken,
                  "hexadecimal float literal is out of range");
    } else if (exponentWithImplicitLeadingOne <
               FloatComponents::minNormalExponent) {
      // Denormals are encoded as if their exponent is minNormalExponent, but
      // with the significand shifted down to include the leading 1 that is
      // implicit for
      ...
      resultComponents.bits.exponent = 0;
      resultComponents.bits.significand =
          FloatBits(fixedPoI64 >> (64 - FloatComponents::numSignificandBits));
    } else {
      // Encode a normal floating point value.
      assert(exponentWithImplicitLeadingOne >=
             FloatComponents::minNormalExponent);
      assert(exponentWithImplicitLeadingOne <=
             FloatComponents::maxNormalExponent);
      resultComponents.bits.exponent = FloatBits(
          exponentWithImplicitLeadingOne + FloatComponents::exponentBias);
      resultComponents.bits.significand =
          FloatBits(fixedPoI64 >> (63 - FloatComponents::numSignificandBits));
    }
  }
...
// Tries to parse an numeric literal token as an integer, advancing
// state.nextToken.
// Returns true if it matched a token.
template <typename UnsignedInt>
bool tryParseInt(ParseState &state, UnsignedInt &outUnsignedInt,
                 I64 minSignedValue, U64 maxUnsignedValue) {
  ...
  case t_decimalInt:
    ...
    u64 = parseDecimalUnsignedInt(
        nextChar, state, isNegative ? U64(-minSignedValue) : maxUnsignedValue,
        "int literal");
    break;
  case t_hexInt:
    isNegative = parseSign(nextChar);
...
// Tries to parse a numeric literal literal token as a float, advancing
// state.nextToken.
// Returns true if it matched a token.
template <typename Float>
bool tryParseFloat(ParseState &state, Float &outFloat) {
  ...
  case t_decimalInt:
  case t_decimalFloat:
    outFloat = parseDecimalFloat<Float>(nextChar, state);
    break;
  case t_hexInt:
  case t_hexFloat:
    outFloat = parseHexFloat<Float>(nextChar, state);
    break;
  case t_floatNaN:
    outFloat = parseNaN<Float>(nextChar, state);
    break;
  case t_floatInf:
    outFloat = parseInfinity<Float>(nextChar);
    break;
  default:
