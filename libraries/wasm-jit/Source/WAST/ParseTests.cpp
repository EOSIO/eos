...
static void parseTestScriptModule(ParseState &state, IR::Module &outModule,
                                  std::vector<UnresolvedError> &outErrors,
                                  std::string &outInternalModuleName) {
  ModuleParseState moduleState(state.string, state.lineInfo, outErrors,
                               state.nextToken, outModule);

  outInternalModuleName = parseOptionalNameAsString(moduleState);

  if (moduleState.nextToken->type == t_quote ||
      moduleState.nextToken->type == t_binary) {
    ...
    if (quoteToken->type == t_quote) {
      ...
      parseModule(moduleQuotedString.c_str(), moduleQuotedString.size(),
                  outModule, quotedErrors);
      ...
      try {
        ...
        parseErrorf(moduleState, quoteToken,
    ...
    try {
      ...
      outErrors.insert(outErrors.end(),
                       std::make_move_iterator(moduleState.errors.begin()),
                       std::make_move_iterator(moduleState.errors.end()));
...
static Action *parseAction(ParseState &state) {
  ...
  parseParenthesized(state, [&] {
    TextFileLocus locus = calcLocusFromOffset(state.string, state.lineInfo,
                                              state.nextToken->begin);
    ...
    case t_get: {
      ...
      result = new GetAction(std::move(locus), std::move(nameString),
                             std::move(exportName));
      break;
    }
    case t_invoke: {
      ...
      result = new InvokeAction(std::move(locus), std::move(nameString),
                                std::move(exportName), std::move(arguments));
      break;
    }
    case t_module: {
      ++state.nextToken;
      ...
      result = new ModuleAction(std::move(locus), std::move(internalModuleName),
                                module);
      break;
    }
    default:
  ...
  if (state.nextToken[0].type == t_leftParenthesis &&
    ...
    parseParenthesized(state, [&] {
      ...
      case t_register: {
        ...
        result = new RegisterCommand(std::move(locus), std::move(moduleName),
                                     std::move(nameString));
        break;
      }
      case t_assert_return: {
        ...
        Runtime::Result expectedReturn =
            state.nextToken->type == t_leftParenthesis
                ? parseConstExpression(state)
                : Runtime::Result();
        result =
            new AssertReturnCommand(std::move(locus), action, expectedReturn);
        break;
      }
      case t_assert_return_canonical_nan:
      case t_assert_return_arithmetic_nan: {
        const Command::Type commandType =
            state.nextToken->type == t_assert_return_canonical_nan
        ...
        Action *action = parseAction(state);
        result =
            new AssertReturnNaNCommand(commandType, std::move(locus), action);
        break;
      }
      ...
      case t_assert_trap: {
        ...
        Runtime::Exception::Cause expectedCause =
            Runtime::Exception::Cause::unknown;
        if (!strcmp(expectedErrorMessage.c_str(),
                    "out of bounds memory access")) {
          expectedCause = Runtime::Exception::Cause::accessViolation;
        } else if (!strcmp(expectedErrorMessage.c_str(),
                           "call stack exhausted")) {
          expectedCause = Runtime::Exception::Cause::stackOverflow;
          ...
          expectedCause = Runtime::Exception::Cause::invalidFloatOperation;
        } else if (!strcmp(expectedErrorMessage.c_str(),
                           "unreachable executed")) {
          expectedCause = Runtime::Exception::Cause::reachedUnreachable;
        } else if (!strcmp(expectedErrorMessage.c_str(), "unreachable")) {
          expectedCause = Runtime::Exception::Cause::reachedUnreachable;
        } else if (!strcmp(expectedErrorMessage.c_str(),
                           "indirect call signature mismatch")) {
          expectedCause =
              Runtime::Exception::Cause::indirectCallSignatureMismatch;
        } else if (!strcmp(expectedErrorMessage.c_str(), "indirect call")) {
          expectedCause =
              Runtime::Exception::Cause::indirectCallSignatureMismatch;
        } else if (!strcmp(expectedErrorMessage.c_str(), "undefined element")) {
          expectedCause = Runtime::Exception::Cause::undefinedTableElement;
        } else if (!strcmp(expectedErrorMessage.c_str(), "undefined")) {
          expectedCause = Runtime::Exception::Cause::undefinedTableElement;
        } else if (!strcmp(expectedErrorMessage.c_str(), "uninitialized")) {
          expectedCause = Runtime::Exception::Cause::undefinedTableElement;
        } else if (!strcmp(expectedErrorMessage.c_str(),
      ...
      case t_assert_malformed: {
        const Command::Type commandType =
            state.nextToken->type == t_assert_invalid
        ...
        parseParenthesized(state, [&] {
          ...
          parseTestScriptModule(state, module, moduleErrors,
                                internalModuleName);
        });
        ...
        result = new AssertInvalidOrMalformedCommand(
            commandType, std::move(locus), moduleErrors.size() != 0);
        break;
      };
      default:
...
namespace WAST {
void parseTestCommands(const char *string, Uptr stringLength,
