...
static bool tryParseSizeConstraints(ParseState &state, U64 maxMax,
  ...
  if (!tryParseI64(state, outSizeConstraints.min)) {
    ...
    if (!tryParseI64(state, outSizeConstraints.max)) {
      outSizeConstraints.max = UINT64_MAX;
    } else {
      // Validate that the maximum size is within the limit, and that the size
      // contraints is not disjoint.
      if (outSizeConstraints.max > maxMax) {
        parseErrorf(state, state.nextToken - 1,
...
static GlobalType parseGlobalType(ParseState &state) {
  ...
  result.isMutable = tryParseParenthesizedTagged(
      state, t_mut, [&] { result.valueType = parseValueType(state); });
  if (!result.isMutable) {
...
parseInitializerExpression(ModuleParseState &state) {
  InitializerExpression result;
  parseParenthesized(state, [&] {
    switch (state.nextToken->type) {
    case t_i32_const: {
      ++state.nextToken;
      result = (I32)parseI32(state);
      break;
    }
    case t_i64_const: {
      ++state.nextToken;
      result = (I64)parseI64(state);
      break;
    }
    case t_f32_const: {
      ++state.nextToken;
    ...
    case t_get_global: {
      ...
      result.globalIndex =
          parseAndResolveNameOrIndexRef(state, state.globalNameToIndexMap,
                                        state.module.globals.size(), "global");
      break;
    }
    default:
...
static void errorIfFollowsDefinitions(ModuleParseState &state) {
  if (state.module.functions.defs.size() || state.module.tables.defs.size() ||
    ...
    parseErrorf(state, state.nextToken,
...
static void
createImport(ParseState &state, Name name, std::string &&moduleName,
             std::string &&exportName, NameToIndexMap &nameToIndexMap,
             IndexSpace<Def, Type> &indexSpace,
             std::vector<DisassemblyName> &disassemblyNameArray, Type type) {
  bindName(state, nameToIndexMap, name, indexSpace.size());
  disassemblyNameArray.push_back({name.getString()});
  indexSpace.imports.push_back(
      {type, std::move(moduleName), std::move(exportName)});
}
...
static void parseImport(ModuleParseState &state) {
  ...
  parseParenthesized(state, [&] {
    ...
    // Parse the import type and create the import in the appropriate name/index
    // spaces.
    switch (importKindToken->type) {
    case t_func: {
      NameToIndexMap localNameToIndexMap;
      std::vector<std::string> localDissassemblyNames;
      const IndexedFunctionType functionType = parseFunctionTypeRef(
          state, localNameToIndexMap, localDissassemblyNames);
      createImport(state, name, std::move(moduleName), std::move(exportName),
                   state.functionNameToIndexMap, state.module.functions,
                   state.disassemblyNames.functions, functionType);
      state.disassemblyNames.functions.back().locals = localDissassemblyNames;
      break;
    }
    case t_table: {
      const SizeConstraints sizeConstraints =
          parseSizeConstraints(state, UINT32_MAX);
      ...
      createImport(state, name, std::move(moduleName), std::move(exportName),
                   state.tableNameToIndexMap, state.module.tables,
                   state.disassemblyNames.tables,
                   {elementType, isShared, sizeConstraints});
      break;
    }
    case t_memory: {
      const SizeConstraints sizeConstraints =
          parseSizeConstraints(state, IR::maxMemoryPages);
      const bool isShared = parseOptionalSharedDeclaration(state);
      createImport(state, name, std::move(moduleName), std::move(exportName),
                   state.memoryNameToIndexMap, state.module.memories,
                   state.disassemblyNames.memories,
    ...
    case t_global: {
      ...
      createImport(state, name, std::move(moduleName), std::move(exportName),
                   state.globalNameToIndexMap, state.module.globals,
                   state.disassemblyNames.globals, globalType);
      break;
    }
    default:
      Errors::unreachable();
...
static void parseExport(ModuleParseState &state) {
  ...
  parseParenthesized(state, [&] {
    ...
    switch (state.nextToken->type) {
    case t_func:
      exportKind = ObjectKind::function;
      break;
    case t_table:
      exportKind = ObjectKind::table;
      break;
    case t_memory:
      exportKind = ObjectKind::memory;
      break;
    case t_global:
      exportKind = ObjectKind::global;
      break;
    default:
    ...
    state.postDeclarationCallbacks.push_back([=](ModuleParseState &state) {
      ...
      case ObjectKind::function:
        exportedObjectIndex =
            resolveRef(state, state.functionNameToIndexMap,
                       state.module.functions.size(), exportRef);
        break;
      case ObjectKind::table:
        exportedObjectIndex = resolveRef(state, state.tableNameToIndexMap,
                                         state.module.tables.size(), exportRef);
        break;
      case ObjectKind::memory:
        exportedObjectIndex =
            resolveRef(state, state.memoryNameToIndexMap,
                       state.module.memories.size(), exportRef);
        break;
      case ObjectKind::global:
        exportedObjectIndex =
            resolveRef(state, state.globalNameToIndexMap,
                       state.module.globals.size(), exportRef);
        break;
      default:
        Errors::unreachable();
      }
...
static void parseType(ModuleParseState &state) {
  ...
  parseParenthesized(state, [&] {
    ...
    const FunctionType *functionType = parseFunctionType(
        state, parameterNameToIndexMap, localDisassemblyNames);
...
static void parseData(ModuleParseState &state) {
  ...
  // Enqueue a callback that is called after all declarations are parsed to
  // resolve the memory to put the data segment in.
  state.postDeclarationCallbacks.push_back([=](ModuleParseState &state) {
    if (!state.module.memories.size()) {
      parseErrorf(state, firstToken, "data segments aren't allowed in modules "
                                     "without any memory declarations");
    } else {
      state.module.dataSegments[dataSegmentIndex].memoryIndex =
          hasMemoryRef ? resolveRef(state, state.memoryNameToIndexMap,
                                    state.module.memories.size(), memoryRef)
                       : 0;
    }
  });
}

static Uptr parseElemSegmentBody(ModuleParseState &state, Reference tableRef,
                                 InitializerExpression baseIndex,
                                 const Token *elemToken) {
  // Allocate the elementReferences array on the heap so it doesn't need to be
  // copied for the post-declaration callback.
  ...
  state.module.tableSegments.push_back(
      {UINTPTR_MAX, baseIndex, std::vector<Uptr>()});

  // Enqueue a callback that is called after all declarations are parsed to
  // resolve the table elements' references.
  state.postDeclarationCallbacks.push_back([tableRef, tableSegmentIndex,
                                            elementReferences, elemToken](
      ModuleParseState &state) {
    if (!state.module.tables.size()) {
      parseErrorf(state, elemToken, "data segments aren't allowed in modules "
                                    "without any memory declarations");
    } else {
      TableSegment &tableSegment =
          state.module.tableSegments[tableSegmentIndex];
      tableSegment.tableIndex =
          tableRef ? resolveRef(state, state.tableNameToIndexMap,
                                state.module.tables.size(), tableRef)
                   : 0;

      tableSegment.indices.resize(elementReferences->size());
      for (Uptr elementIndex = 0; elementIndex < elementReferences->size();
           ++elementIndex) {
...
static void
parseObjectDefOrImport(ModuleParseState &state, NameToIndexMap &nameToIndexMap,
                       IR::IndexSpace<Def, Type> &indexSpace,
                       std::vector<DisassemblyName> &disassemblyNameArray,
                       TokenType declarationTag, IR::ObjectKind kind,
                       ParseImport parseImportFunc, ParseDef parseDefFunc) {
  ...
  while (true) {
    const bool isExport = tryParseParenthesizedTagged(state, t_export, [&] {
      state.module.exports.push_back(
          {parseUTF8String(state), kind, indexSpace.size()});
    });
    if (!isExport) {
      break;
    }
  ...
  if (isImport) {
    ...
    createImport(state, name, std::move(importModuleName),
                 std::move(exportName), nameToIndexMap, indexSpace,
                 disassemblyNameArray, importType);
  } else {
...
static void parseFunc(ModuleParseState &state) {
  parseObjectDefOrImport(
      state, state.functionNameToIndexMap, state.module.functions,
      state.disassemblyNames.functions, t_func, ObjectKind::function,
      [&](ModuleParseState &state) {
        NameToIndexMap localNameToIndexMap;
        std::vector<std::string> localDisassemblyNames;
        return parseFunctionTypeRef(state, localNameToIndexMap,
...
static void parseTable(ModuleParseState &state) {
  parseObjectDefOrImport(
      state, state.tableNameToIndexMap, state.module.tables,
      state.disassemblyNames.tables, t_table, ObjectKind::table,
      ...
      [](ModuleParseState &state, const Token *) {
        ...
        const bool hasSizeConstraints =
            tryParseSizeConstraints(state, UINT32_MAX, sizeConstraints);

        const TableElementType elementType = TableElementType::anyfunc;
        require(state, t_anyfunc);

        // If we couldn't parse an explicit size constraints, the table
        // definition must contain an table segment that implicitly defines the
        // size.
        if (!hasSizeConstraints) {
          parseParenthesized(state, [&] {
            ...
            const Uptr numElements = parseElemSegmentBody(
                state, Reference(U32(tableIndex)),
                InitializerExpression((I32)0), state.nextToken - 1);
...
static void parseMemory(ModuleParseState &state) {
  parseObjectDefOrImport(
      state, state.memoryNameToIndexMap, state.module.memories,
      state.disassemblyNames.memories, t_memory, ObjectKind::memory,
      ...
      [](ModuleParseState &state, const Token *) {
        SizeConstraints sizeConstraints;
        if (!tryParseSizeConstraints(state, IR::maxMemoryPages,
                                     sizeConstraints)) {
          std::string dataString;
          ...
          std::vector<U8> dataVector((const U8 *)dataString.data(),
                                     (const U8 *)dataString.data() +
                                         dataString.size());
          sizeConstraints.min = sizeConstraints.max =
              (dataVector.size() + IR::numBytesPerPage - 1) /
              IR::numBytesPerPage;
          state.module.dataSegments.push_back({state.module.memories.size(),
...
static void parseGlobal(ModuleParseState &state) {
  parseObjectDefOrImport(state, state.globalNameToIndexMap,
                         state.module.globals, state.disassemblyNames.globals,
                         t_global, ObjectKind::global,
...
static void parseStart(ModuleParseState &state) {
  ...
  state.postDeclarationCallbacks.push_back(
      [functionRef](ModuleParseState &state) {
        state.module.startFunctionIndex =
            resolveRef(state, state.functionNameToIndexMap,
                       state.module.functions.size(), functionRef);
      });
}
...
void parseModuleBody(ModuleParseState &state) {
  ...
  // Validate the module's definitions (excluding function code, which is
  // validated as it is parsed).
  if (!state.errors.size()) {
    try {
      IR::validateDefinitions(state.module);
    } catch (ValidationException validationException) {
      parseErrorf(state, firstToken, "validation exception: %s",
                  validationException.message.c_str());
    }
  }

  // Set the module's disassembly names.
  assert(state.module.functions.size() ==
         state.disassemblyNames.functions.size());
  assert(state.module.tables.size() == state.disassemblyNames.tables.size());
  assert(state.module.memories.size() ==
         state.disassemblyNames.memories.size());
...
bool parseModule(const char *string, Uptr stringLength, IR::Module &outModule,
  ...
  for (auto &unresolvedError : unresolvedErrors) {
    TextFileLocus locus =
        calcLocusFromOffset(state.string, lineInfo, unresolvedError.charOffset);
  ...
  Timing::logRatePerSecond("lexed and parsed WAST", timer,
                           stringLength / 1024.0 / 1024.0, "MB");
