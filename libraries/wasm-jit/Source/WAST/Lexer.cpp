...
// This ENUM_TOKENS must come before the literalTokenPairs definition that
// redefines VISIT_OPERATOR_TOKEN.
...
static NFA::StateIndex
createTokenSeparatorPeekState(NFA::Builder *builder,
  ...
  NFA::addEdge(builder, separatorState, tokenSeparatorCharSet,
               finalState | NFA::edgeDoesntConsumeInputFlag);
  return separatorState;
}

static void addLiteralToNFA(const char *string, NFA::Builder *builder,
                            NFA::StateIndex initialState,
                            NFA::StateIndex finalState) {
  // Add the literal to the NFA, one character at a time, reusing existing
  // states that are reachable by the same string.
  for (const char *nextChar = string; *nextChar; ++nextChar) {
    NFA::StateIndex nextState =
        NFA::getNonTerminalEdge(builder, initialState, *nextChar);
...
StaticData::StaticData() {
  static const std::pair<TokenType, const char *> regexpTokenPairs[] = {
      ...
      {t_string, "\"([^\"\n\\\\]*(\\\\([^0-9a-fA-Fu]|[0-9a-fA-F][0-9a-fA-F]|"
                 "u\\{[0-9a-fA-F]+})))*\""},
...
#define VISIT_OPERATOR_TOKEN(_, name, nameString, ...)                         \
  std::make_tuple(t_##name, nameString, false),
...
#undef VISIT_OPERATOR_TOKEN
  ...
  for (auto literalTokenTuple : literalTokenTuples) {
    ...
    NFA::StateIndex finalState =
        NFA::maximumTerminalStateIndex - (NFA::StateIndex)tokenType;
    if (!isTokenSeparator) {
      finalState = createTokenSeparatorPeekState(nfaBuilder, finalState);
    }
...
Token *lex(const char *string, Uptr stringLength, LineInfo *&outLineInfo) {
  ...
  if (stringLength > UINT32_MAX) {
    Errors::fatalf("cannot lex strings with more than %u characters",
                   UINT32_MAX);
  }

  // Allocate enough memory up front for a token and newline for each character
  // in the input string.
  ...
  *nextLineStart++ = 0;
  ...
  doneSkippingWhitespace:
    ...
    // Once we reach a non-whitespace, non-comment character, feed characters
    // into the NFA until it reaches a terminal state.
    ...
    if (terminalState != NFA::unmatchedCharacterTerminal) {
      nextToken->type = TokenType(NFA::maximumTerminalStateIndex -
      ...
      if (nextToken->begin < stringLength) {
        ...
        while (!isRecoveryPointChar(*nextChar)) {
          ++nextChar;
        }
      } else {
        break;
  ...
  // Emit an extra line start for the end of the file, so you can find the end
  // of a line with lineStarts[line + 1].
  *nextLineStart++ = U32(nextChar - string) + 1;
  ...
  Timing::logRatePerSecond("lexed WAST file", timer,
...
void freeTokens(Token *tokens) { free(tokens); }
...
TextFileLocus calcLocusFromOffset(const char *string, const LineInfo *lineInfo,
                                  Uptr charOffset) {
  ...
  for (U32 index = lineInfo->lineStarts[result.newlines]; index < charOffset;
       ++index) {
    if (string[index] == '\t') {
      ++result.tabs;
    } else {
  ...
  result.sourceLine =
      std::string(string + lineStartOffset, lineEndOffset - lineStartOffset);
