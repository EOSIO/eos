...
struct FunctionParseState : ParseState {
  ...
  FunctionParseState(const ModuleParseState &inModuleState,
                     NameToIndexMap *inLocalNameToIndexMap,
                     const Token *firstBodyToken, FunctionDef &inFunctionDef)
      : ParseState(inModuleState.string, inModuleState.lineInfo,
                   inModuleState.errors, firstBodyToken),
        moduleState(inModuleState), functionDef(inFunctionDef),
        localNameToIndexMap(inLocalNameToIndexMap),
        numLocals(inFunctionDef.nonParameterLocalTypes.size() +
                  inModuleState.module.types[inFunctionDef.type.index]
                      ->parameters.size()),
        branchTargetDepth(0), operationEncoder(codeByteStream),
        validatingCodeStream(inModuleState.module, functionDef,
                             operationEncoder) {}
};
  ...
  ~ScopedBranchTarget() {
    ...
    if (name) {
      ...
      if (previousBranchTargetIndex == UINT32_MAX) {
        ...
        // If hte name was previously bound to an outer branch target, restore
        // it.
...
static bool tryParseAndResolveBranchTargetRef(FunctionParseState &state,
                                              U32 &outTargetDepth) {
  ...
  if (tryParseNameOrIndexRef(state, branchTargetRef)) {
    ...
    case Reference::Type::index:
      outTargetDepth = branchTargetRef.index;
      break;
    case Reference::Type::name: {
      auto nameToIndexMapIt =
          state.branchTargetNameToIndexMap.find(branchTargetRef.name);
...
static void parseAndValidateRedundantBranchTargetName(
    ParseState &state, Name branchTargetName, const char *context,
    const char *redundantContext) {
  Name redundantName;
  if (tryParseName(state, redundantName) && branchTargetName != redundantName) {
    parseErrorf(state, state.nextToken - 1, "%s label doesn't match %s label",
                redundantContext, context);
  }
}
...
static void parseImm(FunctionParseState &state,
                     GetOrSetVariableImm<isGlobal> &outImm) {
  outImm.variableIndex = parseAndResolveNameOrIndexRef(
      state,
      isGlobal ? state.moduleState.globalNameToIndexMap
               : *state.localNameToIndexMap,
...
static void parseImm(FunctionParseState &state, CallImm &outImm) {
  outImm.functionIndex = parseAndResolveNameOrIndexRef(
      state, state.moduleState.functionNameToIndexMap,
      state.moduleState.module.functions.size(), "function");
}

static void parseImm(FunctionParseState &state, CallIndirectImm &outImm) {
  outImm.type.index = parseAndResolveNameOrIndexRef(
      state, state.moduleState.typeNameToIndexMap,
      state.moduleState.module.types.size(), "type");
}
  ...
  if (state.nextToken->type == t_align) {
    ...
    if (alignment > naturalAlignment) {
      parseErrorf(state, state.nextToken,
                  "alignment must be <= natural alignment");
...
static void parseImm(FunctionParseState &state,
  ...
  if (u64 > numLanes) {
    parseErrorf(state, state.nextToken - 1,
                "lane index must be in the range 0..%u", numLanes);
  }
...
static void parseImm(FunctionParseState &state, ShuffleImm<numLanes> &outImm) {
  parseParenthesized(state, [&] {
    for (Uptr laneIndex = 0; laneIndex < numLanes; ++laneIndex) {
      ...
      if (u64 >= numLanes * 2) {
        parseErrorf(state, state.nextToken - 1,
                    "lane index must be in the range 0..%u", numLanes * 2);
      }
...
static void parseControlImm(FunctionParseState &state,
...
static void parseBlock(FunctionParseState &state, bool isExpr) {
  ...
  if (!isExpr) {
    require(state, t_end);
    parseAndValidateRedundantBranchTargetName(state, branchTargetName, "block",
                                              "end");
  }
}
...
static void parseLoop(FunctionParseState &state, bool isExpr) {
  ...
  if (!isExpr) {
    require(state, t_end);
    parseAndValidateRedundantBranchTargetName(state, branchTargetName, "loop",
                                              "end");
  }
}
    ...
    try {
      ...
      case t_if_: {
        ...
        if (state.nextToken->type == t_else_) {
          ++state.nextToken;
          parseAndValidateRedundantBranchTargetName(state, branchTargetName,
                                                    "if", "else");
        ...
        require(state, t_end);
        parseAndValidateRedundantBranchTargetName(state, branchTargetName, "if",
                                                  "end");
...
FunctionDef parseFunctionDef(ModuleParseState &state, const Token *funcToken) {
  ...
  if (state.nextToken[0].type == t_leftParenthesis &&
    ...
    referencedFunctionType = parseFunctionTypeRef(state, *localNameToIndexMap,
                                                  *localDisassemblyNames);
  }

  // Parse the explicit function parameters and result type.
  const FunctionType *directFunctionType =
      parseFunctionType(state, *localNameToIndexMap, *localDisassemblyNames);
  const bool hasNoDirectType = directFunctionType == FunctionType::get();

  // Validate that if the function definition has both a type reference, and
  // explicit parameter/result type declarations, that they match.
  ...
  if (referencedFunctionType.index != UINT32_MAX && hasNoDirectType) {
    ...
    if (referencedFunctionType.index != UINT32_MAX &&
        state.module.types[referencedFunctionType.index] !=
            directFunctionType) {
      parseErrorf(
          state, typeReferenceToken, "referenced function type (%s) does not "
                                     "match declared parameters and results "
                                     "(%s)",
  ...
  while (tryParseParenthesizedTagged(state, t_local, [&] {
    ...
    if (tryParseName(state, localName)) {
      bindName(state, *localNameToIndexMap, localName,
               directFunctionType->parameters.size() +
  ...
  // Defer parsing the body of the function until after all declarations have
  // been parsed.
  ...
  state.postDeclarationCallbacks.push_back(
      [functionIndex, functionDefIndex, firstBodyToken, localNameToIndexMap,
       localDisassemblyNames](ModuleParseState &state) {
        FunctionParseState functionState(
            state, localNameToIndexMap, firstBodyToken,
            state.module.functions.defs[functionDefIndex]);
        try {
        ...
        state.module.functions.defs[functionDefIndex].code =
            std::move(functionState.codeByteStream.getBytes());
        state.disassemblyNames.functions[functionIndex].locals =
