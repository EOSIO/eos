...
// These serialization functions need to be declared in the IR namespace for the
// array serializer in the Serialization namespace to find them.
namespace IR {
template <typename Stream> void serialize(Stream &stream, ValueType &type) {
  ...
  if (Stream::isInput) {
    type = (ValueType)-encodedValueType;
  }
}
...
void serialize(Stream &stream, SizeConstraints &sizeConstraints, bool hasMax) {
  ...
  if (hasMax) {
    serializeVarUInt32(stream, sizeConstraints.max);
...
void serialize(Stream &stream, TableType &tableType) {
  ...
  Uptr flags = 0;
  if (!Stream::isInput && tableType.size.max != UINT64_MAX) {
    flags |= 1;
  }
#if ENABLE_THREADING_PROTOTYPE
  if (!Stream::isInput && tableType.isShared) {
    flags |= 2;
  }
  serializeVarUInt32(stream, flags);
  if (Stream::isInput) {
    tableType.isShared = (flags & 2) != 0;
  }
#else
  serializeVarUInt32(stream, flags);
#endif
...
void serialize(Stream &stream, MemoryType &memoryType) {
  Uptr flags = 0;
  if (!Stream::isInput && memoryType.size.max != UINT64_MAX) {
    flags |= 1;
  }
#if ENABLE_THREADING_PROTOTYPE
  if (!Stream::isInput && memoryType.isShared) {
    flags |= 2;
  }
  serializeVarUInt32(stream, flags);
  if (Stream::isInput) {
    memoryType.isShared = (flags & 2) != 0;
  }
#else
  serializeVarUInt32(stream, flags);
#endif
...
void serialize(Stream &stream, InitializerExpression &initializer) {
  ...
  case InitializerExpression::Type::f32_const:
    serialize(stream, initializer.f32);
    break;
  case InitializerExpression::Type::f64_const:
    serialize(stream, initializer.f64);
    break;
  case InitializerExpression::Type::get_global:
    serializeVarUInt32(stream, initializer.globalIndex);
    break;
  default:
    throw FatalSerializationException("invalid initializer expression opcode");
  }
...
void serialize(Stream &stream, TableSegment &tableSegment) {
  ...
  serializeArray(stream, tableSegment.indices,
                 [](Stream &stream, Uptr &functionIndex) {
...
public:
  ChecktimeInjection() : typeSlot(-1) {}

  void addCall(const Module &module,
               Serialization::OutputStream &inByteStream) {
    OpcodeAndImm<CallImm> *encodedOperator =
        (OpcodeAndImm<CallImm> *)inByteStream.advance(
            sizeof(OpcodeAndImm<CallImm>));
  ...
  void setTypeSlot(const Module &module, ResultType returnType,
                   const std::vector<ValueType> &parameterTypes) {
  ...
  void addImport(Module &module) {
    ...
    module.functions.imports.push_back(
        {{functionTypeIndex}, std::move(u8"env"), std::move(u8"checktime")});
  }

  void conditionallyAddCall(Opcode opcode, const ControlStructureImm &imm,
                            const Module &module,
                            Serialization::OutputStream &inByteStream) {
    switch (opcode) {
    case Opcode::loop:
  ...
  void conditionallyAddCall(Opcode, const Imm &, const Module &,
                            Serialization::OutputStream &) {}

  void adjustExportIndex(Module &module) {
    // all function exports need to have their index increased to account for
    // inserted definition
  ...
  template <typename Imm> void adjustCallIndex(const Module &, Imm &) {}
...
void serialize(Stream &stream, ControlStructureImm &imm, const FunctionDef &) {
  I8 encodedResultType =
      imm.resultType == ResultType::none ? -64 : -(I8)imm.resultType;
  serializeVarInt7(stream, encodedResultType);
  if (Stream::isInput) {
    imm.resultType = encodedResultType == -64 ? ResultType::none
...
void serialize(InputStream &stream, BranchTableImm &imm,
               FunctionDef &functionDef) {
  std::vector<U32> branchTable;
  serializeArray(stream, branchTable,
                 [](InputStream &stream, U32 &targetDepth) {
...
void serialize(Stream &stream, GetOrSetVariableImm<isGlobal> &imm,
               const FunctionDef &) {
...
void serialize(Stream &stream, LoadOrStoreImm<naturalAlignmentLog2> &imm,
               const FunctionDef &) {
...
void serialize(Stream &stream, LaneIndexImm<numLanes> &imm,
               const FunctionDef &) {
...
template <typename Stream, Uptr naturalAlignmentLog2>
void serialize(Stream &stream, AtomicLoadOrStoreImm<naturalAlignmentLog2> &imm,
               const FunctionDef &) {
  serializeVarUInt7(stream, imm.alignmentLog2);
  serializeVarUInt32(stream, imm.offset);
}
...
void serializeSection(OutputStream &stream, SectionType type,
...
void serializeSection(InputStream &stream, SectionType expectedType,
  ...
  MemoryInputStream sectionStream(stream.advance(numSectionBytes),
                                  numSectionBytes);
  serializeSectionBody(sectionStream);
...
void serialize(OutputStream &stream, UserSection &userSection) {
  serializeConstant(stream, "expected user section (section ID 0)",
                    (U8)SectionType::user);
  ArrayOutputStream sectionStream;
  Serialization::serialize(sectionStream, userSection.name);
  serializeBytes(sectionStream, userSection.data.data(),
                 userSection.data.size());
  std::vector<U8> sectionBytes = sectionStream.getBytes();
...
void serialize(InputStream &stream, UserSection &userSection) {
  ...
  Serialization::serializeBytes(sectionStream, userSection.data.data(),
                                userSection.data.size());
...
template <typename Injection> struct WasmSerializationImpl {
  Injection injection;

  void serializeFunctionBody(OutputStream &sectionStream, Module &module,
                             FunctionDef &functionDef) {
    ArrayOutputStream bodyStream;

    // Convert the function's local types into LocalSets: runs of locals of the
    // same type.
    LocalSet *localSets = (LocalSet *)alloca(
        sizeof(LocalSet) * functionDef.nonParameterLocalTypes.size());
    Uptr numLocalSets = 0;
    if (functionDef.nonParameterLocalTypes.size()) {
      ...
      for (auto localType : functionDef.nonParameterLocalTypes) {
        if (localSets[numLocalSets].type != localType) {
          if (localSets[numLocalSets].type != ValueType::any) {
            ++numLocalSets;
          }
    ...
    while (irDecoderStream) {
      irDecoderStream.decodeOp(wasmOpEncoderStream);
    };
  ...
  void serializeFunctionBody(InputStream &sectionStream, Module &module,
                             FunctionDef &functionDef) {
    Uptr numBodyBytes = 0;
    serializeVarUInt32(sectionStream, numBodyBytes);

    MemoryInputStream bodyStream(sectionStream.advance(numBodyBytes),
                                 numBodyBytes);

    // Deserialize local sets and unpack them into a linear array of local
    // types.
    Uptr numLocalSets = 0;
    serializeVarUInt32(bodyStream, numLocalSets);
    for (Uptr setIndex = 0; setIndex < numLocalSets; ++setIndex) {
    ...
    // Deserialize the function code, validate it, and re-encode it in the IR
    // format.
...
#undef VISIT_OPCODE
  ...
  void serializeTypeSection(Stream &moduleStream, Module &module) {
    ...
    serializeSection(
        moduleStream, SectionType::type,
        [&module, &localInjection](Stream &sectionStream) {
          serializeArray(
              sectionStream, module.types,
              [&module, &localInjection](Stream &stream,
                ...
                if (Stream::isInput) {
                  ...
                  localInjection.setTypeSlot(module, returnType,
                                             parameterTypes);
                } else {
                  serialize(stream, const_cast<std::vector<ValueType> &>(
                                        functionType->parameters));
                  serialize(stream,
                            const_cast<ResultType &>(functionType->ret));
                }
              });
        });
  ...
  void serializeImportSection(Stream &moduleStream, Module &module) {
    Injection &localInjection = injection;
    serializeSection(
        moduleStream, SectionType::import,
        [&module, &localInjection](Stream &sectionStream) {
          Uptr size =
            ...
            for (Uptr index = 0; index < size; ++index) {
              ...
              case ObjectKind::function: {
                ...
                if (functionTypeIndex >= module.types.size()) {
                  throw FatalSerializationException(
                      "invalid import function type index");
                }
                module.functions.imports.push_back({{functionTypeIndex},
                                                    std::move(moduleName),
                                                    std::move(exportName)});
                break;
              }
              ...
              default:
                throw FatalSerializationException("invalid ObjectKind");
              }
            }
  ...
  void serializeFunctionSection(Stream &moduleStream, Module &module) {
    serializeSection(
        moduleStream, SectionType::functionDeclarations,
        [&module](Stream &sectionStream) {
          ...
          if (Stream::isInput) {
            ...
            // try to get a serialization exception before making a huge
            // allocation for malformed input.
            module.functions.defs.clear();
            for (Uptr functionIndex = 0; functionIndex < numFunctions;
                 ++functionIndex) {
              U32 functionTypeIndex = 0;
              serializeVarUInt32(sectionStream, functionTypeIndex);
              if (functionTypeIndex >= module.types.size()) {
  ...
  void serializeTableSection(Stream &moduleStream, Module &module) {
    serializeSection(moduleStream, SectionType::table,
                     [&module](Stream &sectionStream) {
  ...
  void serializeMemorySection(Stream &moduleStream, Module &module) {
    serializeSection(moduleStream, SectionType::memory,
                     [&module](Stream &sectionStream) {
  ...
  void serializeGlobalSection(Stream &moduleStream, Module &module) {
    serializeSection(moduleStream, SectionType::global,
                     [&module](Stream &sectionStream) {
  ...
  void serializeExportSection(Stream &moduleStream, Module &module) {
    serializeSection(moduleStream, SectionType::export_,
                     [&module](Stream &sectionStream) {
  ...
  void serializeStartSection(Stream &moduleStream, Module &module) {
    serializeSection(
        moduleStream, SectionType::start, [&module](Stream &sectionStream) {
  ...
  void serializeElementSection(Stream &moduleStream, Module &module) {
    serializeSection(moduleStream, SectionType::elem,
                     [&module](Stream &sectionStream) {
  ...
  void serializeCodeSection(Stream &moduleStream, Module &module) {
    serializeSection(
        moduleStream, SectionType::functionDefinitions,
        [&module, this](Stream &sectionStream) {
          Uptr numFunctionBodies = module.functions.defs.size();
          serializeVarUInt32(sectionStream, numFunctionBodies);
          if (Stream::isInput &&
              numFunctionBodies != module.functions.defs.size()) {
            throw FatalSerializationException(
  ...
  void serializeDataSection(Stream &moduleStream, Module &module) {
    serializeSection(moduleStream, SectionType::data,
                     [&module](Stream &sectionStream) {
  ...
  void serializeModule(OutputStream &moduleStream, Module &module) {
    ...
    if (module.tables.defs.size() > 0) {
      serializeTableSection(moduleStream, module);
    }
    if (module.memories.defs.size() > 0) {
      serializeMemorySection(moduleStream, module);
    }
    if (module.globals.defs.size() > 0) {
    ...
    if (module.tableSegments.size() > 0) {
    ...
    for (auto &userSection : module.userSections) {
  ...
  void serializeModule(InputStream &moduleStream, Module &module) {
    ...
    while (moduleStream.capacity()) {
      const SectionType sectionType =
          *(SectionType *)moduleStream.peek(sizeof(SectionType));
      if (sectionType != SectionType::user) {
        if (sectionType > lastKnownSectionType) {
          lastKnownSectionType = sectionType;
        } else {
          throw FatalSerializationException(
              "incorrect order for known section");
        }
      }
      switch (sectionType) {
...
void serializeWithInjection(Serialization::InputStream &stream,
                            Module &module) {
