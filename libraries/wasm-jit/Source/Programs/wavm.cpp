...
void showHelp() {
  std::cerr << "Usage: wavm [switches] [programfile] [--] [arguments]"
            << std::endl;
  std::cerr << "  in.wast|in.wasm\t\tSpecify program file (.wast/.wasm)"
            << std::endl;
  std::cerr << "  -f|--function name\t\tSpecify function name to run in module "
               "rather than main"
            << std::endl;
  std::cerr << "  -c|--check\t\t\tExit after checking that the program is valid"
            << std::endl;
  std::cerr << "  -d|--debug\t\t\tWrite additional debug information to stdout"
            << std::endl;
...
struct RootResolver : Resolver {
  ...
  bool resolve(const std::string &moduleName, const std::string &exportName,
    ...
    if (namedResolverIt != moduleNameToResolverMap.end()) {
      return namedResolverIt->second->resolve(moduleName, exportName, type,
                                              outObject);
    }

    // Finally, stub in missing function imports.
    if (type.kind == ObjectKind::function) {
      // Generate a function body that just uses the unreachable op to fault if
      // called.
      ...
      stubModule.functions.defs.push_back(
          {{0}, {}, std::move(codeStream.getBytes()), {}});
      ...
      Log::printf(Log::Category::error,
                  "Generated stub for missing function import %s.%s : %s\n",
      ...
      Log::printf(Log::Category::error,
                  "Generated stub for missing memory import %s.%s : %s\n",
                  moduleName.c_str(), exportName.c_str(),
                  asString(type).c_str());
      return true;
      ...
      Log::printf(Log::Category::error,
                  "Generated stub for missing table import %s.%s : %s\n",
                  moduleName.c_str(), exportName.c_str(),
                  asString(type).c_str());
      return true;
    } else if (type.kind == ObjectKind::global) {
      outObject = asObject(Runtime::createGlobal(
          asGlobalType(type), Runtime::Value(asGlobalType(type).valueType,
                                             Runtime::UntaggedValue())));
      Log::printf(Log::Category::error,
                  "Generated stub for missing global import %s.%s : %s\n",
                  moduleName.c_str(), exportName.c_str(),
                  asString(type).c_str());
      return true;
    }
...
int mainBody(const char *filename, const char *functionName, bool onlyCheck,
             char **args) {
  Module module;
  if (filename) {
    if (!loadModule(filename, module)) {
      return EXIT_FAILURE;
    }
  } else {
    showHelp();
  ...
  if (!linkResult.success) {
    ...
    for (auto &missingImport : linkResult.missingImports) {
      std::cerr << "Missing import: module=\"" << missingImport.moduleName
                << "\" export=\"" << missingImport.exportName << "\" type=\""
                << asString(missingImport.type) << "\"" << std::endl;
    }
    return EXIT_FAILURE;
  }
  ModuleInstance *moduleInstance =
      instantiateModule(module, std::move(linkResult.resolvedImports));
  if (!moduleInstance) {
    return EXIT_FAILURE;
  }
  ...
  if (!functionName) {
    functionInstance =
        asFunctionNullable(getInstanceExport(moduleInstance, "main"));
    if (!functionInstance) {
      functionInstance =
          asFunctionNullable(getInstanceExport(moduleInstance, "_main"));
    }
    if (!functionInstance) {
    ...
    if (!functionInstance) {
      std::cerr << "Module does not export '" << functionName << "'"
                << std::endl;
      return EXIT_FAILURE;
    }
  }
  ...
  if (!functionName) {
    if (functionType->parameters.size() == 2) {
      ...
      if (!defaultMemory) {
        std::cerr << "Module does not declare a default memory object to put "
                     "arguments in."
                  << std::endl;
        return EXIT_FAILURE;
      }
      ...
      std::cerr << "WebAssembly function requires "
    ...
    for (U32 i = 0; args[i]; ++i) {
      ...
      case ValueType::i32:
        value = (U32)atoi(args[i]);
        break;
      case ValueType::i64:
        value = (U64)atol(args[i]);
        break;
      case ValueType::f32:
        value = (F32)atof(args[i]);
        break;
      case ValueType::f64:
        value = atof(args[i]);
        break;
  ...
  if (functionName) {
    ...
    return EXIT_SUCCESS;
  } else if (functionResult.type == ResultType::i32) {
