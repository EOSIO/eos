...
struct TestScriptState {
  ...
  TestScriptState()
      : hasInstantiatedModule(false), lastModuleInstance(nullptr) {}
};
...
void testErrorf(TestScriptState &state, const TextFileLocus &locus,
                const char *messageFormat, ...) {
  va_list messageArguments;
  va_start(messageArguments, messageFormat);
  char messageBuffer[1024];
  int numPrintedChars = std::vsnprintf(messageBuffer, sizeof(messageBuffer),
...
void collectGarbage(TestScriptState &state) {
  ...
  for (auto &mapIt : state.moduleInternalNameToInstanceMap) {
    rootObjects.push_back(asObject(mapIt.second));
  }
  for (auto &mapIt : state.moduleNameToInstanceMap) {
    rootObjects.push_back(asObject(mapIt.second));
  }
...
ModuleInstance *
getModuleContextByInternalName(TestScriptState &state,
                               const TextFileLocus &locus, const char *context,
  ...
  if (internalName.size()) {
    ...
    if (mapIt == state.moduleInternalNameToInstanceMap.end()) {
      testErrorf(state, locus, "unknown %s module name: %s", context,
                 internalName.c_str());
      return nullptr;
    }
...
bool processAction(TestScriptState &state, Action *action, Result &outResult) {
  ...
  case ActionType::_module: {
    ...
    if (linkResult.success) {
      ...
      state.lastModuleInstance = instantiateModule(
          *moduleAction->module, std::move(linkResult.resolvedImports));
    } else {
      // Create an error for each import that couldn't be linked.
      for (auto &missingImport : linkResult.missingImports) {
        testErrorf(state, moduleAction->locus,
    ...
    if (moduleAction->internalModuleName.size()) {
      state.moduleInternalNameToInstanceMap[moduleAction->internalModuleName] =
  ...
  case ActionType::invoke: {
    ...
    // A null module instance at this point indicates a module that failed to
    // link or instantiate, so don't produce further errors.
    if (moduleInstance) {
      // Find the named export in the module instance.
      auto functionInstance = asFunctionNullable(
          getInstanceExport(moduleInstance, invokeAction->exportName));
      if (!functionInstance) {
        testErrorf(state, invokeAction->locus,
  ...
  case ActionType::get: {
    ...
    // A null module instance at this point indicates a module that failed to
    // link or instantiate, so just return without further errors.
    if (!moduleInstance) {
      return false;
    }

    // Find the named export in the module instance.
    auto globalInstance = asGlobalNullable(
        getInstanceExport(moduleInstance, getAction->exportName));
    if (!globalInstance) {
      testErrorf(state, getAction->locus,
...
// Tests whether a float is a "canonical" NaN, which just means that it's a NaN
// only the MSB of its significand set.
template <typename Float>
bool isCanonicalOrArithmeticNaN(Float value, bool requireCanonical) {
  Floats::FloatComponents<Float> components;
  components.value = value;
  return components.bits.exponent ==
             Floats::FloatComponents<Float>::maxExponentBits &&
         (!requireCanonical ||
...
void processCommand(TestScriptState &state, const Command *command) {
  try {
    ...
    case Command::_register: {
      ...
      // Look up a module by internal name, and bind the result to the public
      // name.
      ModuleInstance *moduleInstance = getModuleContextByInternalName(
          state, registerCommand->locus, "register",
          registerCommand->internalModuleName);
      state.moduleNameToInstanceMap[registerCommand->moduleName] =
          moduleInstance;
      break;
    }
    case Command::action: {
    ...
    case Command::assert_return: {
      ...
      // Execute the action and do a bitwise comparison of the result to the
      // expected result.
      Result actionResult;
    ...
    case Command::assert_return_arithmetic_nan: {
      ...
      // Execute the action and check that the result is a NaN of the expected
      // type.
      Result actionResult;
      if (processAction(state, assertCommand->action.get(), actionResult)) {
        ...
        const bool isError =
            actionResult.type == ResultType::f32
                ? !isCanonicalOrArithmeticNaN(actionResult.f32,
                                              requireCanonicalNaN)
                : actionResult.type == ResultType::f64
                      ? !isCanonicalOrArithmeticNaN(actionResult.f64,
                                                    requireCanonicalNaN)
                      : true;
        if (isError) {
    ...
    case Command::assert_trap: {
      ...
      try {
        Result actionResult;
        if (processAction(state, assertCommand->action.get(), actionResult)) {
          testErrorf(state, assertCommand->locus, "expected trap but got %s",
                     asString(actionResult).c_str());
        }
      } catch (Runtime::Exception exception) {
        if (exception.cause != assertCommand->expectedCause) {
          testErrorf(state, assertCommand->action->locus,
    ...
    case Command::assert_malformed: {
      ...
      if (!assertCommand->wasInvalidOrMalformed) {
        testErrorf(state, assertCommand->locus, "module was %s",
                   assertCommand->type == Command::assert_invalid
                       ? "valid"
                       : "well formed");
      }
      break;
    ...
    case Command::assert_unlinkable: {
      ...
      try {
        ...
        if (linkResult.success) {
          instantiateModule(*assertCommand->moduleAction->module,
                            std::move(linkResult.resolvedImports));
          testErrorf(state, assertCommand->locus, "module was linkable");
        }
      } catch (Runtime::Exception) {
        // If the instantiation throws an exception, the assert_unlinkable
        // succeeds.
      }
      break;
    }
    ...
    testErrorf(state, command->locus, "unexpected trap: %s",
               describeExceptionCause(exception.cause));
  }
}
...
DEFINE_INTRINSIC_FUNCTION2(spectest, spectest_print, print, none, f64, a, f64,
                           b) {
  std::cout << a << " : f64" << std::endl << b << " : f64" << std::endl;
}
DEFINE_INTRINSIC_FUNCTION2(spectest, spectest_print, print, none, i32, a, f32,
                           b) {
  std::cout << a << " : i32" << std::endl << b << " : f32" << std::endl;
}
DEFINE_INTRINSIC_FUNCTION2(spectest, spectest_print, print, none, i64, a, f64,
                           b) {
  std::cout << a << " : i64" << std::endl << b << " : f64" << std::endl;
}
...
DEFINE_INTRINSIC_TABLE(spectest, spectest_table, table,
                       TableType(TableElementType::anyfunc, false,
                                 SizeConstraints{10, 20}))
DEFINE_INTRINSIC_MEMORY(spectest, spectest_memory, memory,
...
int commandMain(int argc, char **argv) {
  ...
  WAST::parseTestCommands(testScriptString.c_str(), testScriptString.size(),
                          testCommands, testScriptState.errors);
  ...
  if (testScriptState.errors.size()) {
    ...
    for (auto &error : testScriptState.errors) {
      std::cerr << filename << ":" << error.locus.describe() << ": "
