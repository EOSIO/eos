   ...
   struct generic_table : public table {
        ...
        const value_type& emplace( Constructor&& c ) {
         ...
         int64_t revision()const {
            return _stack.size() ? _stack.back().revision : 0;
         }

         void start_revision( int64_t new_revision ) {
            ilog( "table starting new revision ${r}", ("r", new_revision) );
            uint64_t prior_revision = revision();
            FC_ASSERT( new_revision > prior_revision, "attempting to create a new revision with older number" );
            _stack.emplace_back( _indices.get_allocator() );
            auto& newrev = _stack.back();
            newrev.old_next_id = _next_id;
            newrev.revision  = new_revision;
         }

        /**
         * Discards all undo history prior to revision
         */
        void commit( int64_t rev ) {
           while( _stack.size() && _stack[0].revision <= rev )
           {
              _stack.pop_front();
           }
        }
