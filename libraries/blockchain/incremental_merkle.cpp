 ...
 */
static int clz_power_2(uint64_t value) {
   int lz = 64;
 ...
 */
static int calcluate_max_depth(uint64_t node_count) {
   if (node_count == 0) {
      return 0;
   }
...
incremental_merkle::incremental_merkle()
: _node_count(0)
{}
 ...
 */
void incremental_merkle::append(const digest_type &digest) {
   auto current_depth = calcluate_max_depth(_node_count + 1) - 1;
   auto index = _node_count;
   auto top = digest;
   bool partial = false;
   auto updated_active_nodes = vector<digest_type>();
   ...
   while (current_depth > 0) {
      if (!(index & 0x1)) {
         ...
         // to the "left" value
         top = digest_type::hash(std::make_pair(top, top));
         partial = true;
      } else {
         ...
         // calculate the node
         top = digest_type::hash(std::make_pair(left_value, top));
      }
   ...
   std::swap(_active_nodes, updated_active_nodes);
...
/**
 * Return the root of the current merkle tree
 *
 * Note: by convention this is stored as the last entry in the active nodes
 * unless the tree is empty, in which case it is not well defined.
 *
 * @return the root of the current merkle tree
 */
digest_type incremental_merkle::get_root() const {
   if (_node_count > 0) {
