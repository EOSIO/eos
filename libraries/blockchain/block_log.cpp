...
namespace eosio { namespace blockchain {
   ...
   namespace detail {
      class block_log_impl {
         public:
            optional<signed_block_header>   head;
   ...
   void block_log::open(const fc::path& data_dir) {
      ...
      if (log_size) {
         ...
         auto h = read_head();
         if( h )
            my->head = signed_block_header(*read_head());
   ...
   uint64_t block_log::append( const vector<char>& data ) {
      try {
         ...
         fc::raw::unpack( data, header );

         FC_ASSERT(my->index_stream.tellp() == sizeof(uint64_t) * (header.block_num() - 1),
                   "Append to index file occuring at wrong position.",
                   ("position", (uint64_t) my->index_stream.tellp())
                   ("expected", (header.block_num() - 1) * sizeof(uint64_t)));
         ...
         my->head    = header;
         my->head_id = header.id();

         return pos;
      }
      FC_LOG_AND_RETHROW()
   }
   uint64_t block_log::append(const packed_block& b) {
      return block_log::append( fc::raw::pack( b ) );
   }
   ...
   std::pair<packed_block, uint64_t> block_log::read_block(uint64_t pos)const {
      my->check_block_read();

      my->block_stream.seekg(pos);
      std::pair<packed_block,uint64_t> result;
      fc::raw::unpack(my->block_stream, result.first);
      result.second = uint64_t(my->block_stream.tellg()) + 8;
      return result;
   }

   optional<packed_block> block_log::read_block_by_num(uint32_t block_num)const {
      try {
         optional<packed_block> b;
   ...
   optional<packed_block> block_log::read_head()const {
      my->check_block_read();
   ...
   const optional<signed_block_header>& block_log::head()const {
      return my->head;
   }
