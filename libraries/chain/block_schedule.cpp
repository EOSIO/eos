...
struct schedule_entry {
  schedule_entry(uint _cycle, uint _thread,
                 const pending_transaction &_transaction)
      : cycle(_cycle), thread(_thread), transaction(_transaction) {}
...
static block_schedule from_entries(vector<schedule_entry> &entries) {
  // sort in reverse to save allocations, this should put the highest thread
  // index
  // for the highest cycle index first meaning the naive resize in the loop
  // below
  ...
  for (const auto &entry : entries) {
    ...
    thread.transactions.emplace(thread.transactions.begin(),
...
template <typename Container> auto initialize_ref_vector(const Container &c) {
...
block_schedule block_schedule::by_threading_conflicts(
  ...
  uint HASH_SIZE =
      std::max<uint>(4096, next_power_of_two(transactions.size() / 8));
