...
shared_ptr<fork_item> fork_database::push_block(const signed_block &b) {
  ...
  try {
    ...
    wlog("Head: ${num}, ${id}",
         ("num", _head->data.block_num())("id", _head->data.id()));
    throw;
...
void fork_database::_push_block(const item_ptr &item) {
  ...
  if (_head && item->previous_id() != block_id_type()) {
    ...
    EOS_ASSERT(itr != index.end(), unlinkable_block_exception,
  ...
  if (!_head)
    ...
    //      ilog( "min block in fork DB ${n}, max_size: ${m}",
    //      ("n",min_num)("m",_max_size) );
...
fork_database::fetch_branch_from(block_id_type first,
