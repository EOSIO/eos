         ...
         if constexpr (std::is_same_v<value_t, kv_object>) {
            snapshot->write_section<value_t>([this]( auto& section ){
               ...
               if (db.get<kv_db_config_object>().using_rocksdb_for_disk) {
                  ...
                  chain_kv::append_key(prefix, kvdisk_id.to_uint64_t());
                  it->Seek(chain_kv::to_slice(rocksdb_contract_kv_prefix));
                  while(it->Valid()) {
                     auto key = it->key();
               ...
               snapshot->read_section<value_t>([this, &batch, &prefix]( auto& section ) {
                  ...
                  while (more) {
                     ...
                     if(move_to_rocks) {
                        ...
                        batch.Put(chain_kv::to_slice(chain_kv::create_full_key(prefix, move_to_rocks->contract.to_uint64_t(), key)), value);
