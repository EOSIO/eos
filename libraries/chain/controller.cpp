...
*/
                    ...
                    const forked_branch_callback& forked_branch_cb, const trx_meta_cache_lookup& trx_lookup )
   {
      ...
      try {
         ...
         if( conf.terminate_at_block > 0 &&
            ( (conf.read_mode != db_read_mode::IRREVERSIBLE && conf.terminate_at_block < b->block_num() ) ||
              ( conf.read_mode == db_read_mode::IRREVERSIBLE && conf.terminate_at_block <= self.last_irreversible_block_num() ) )  ) {
