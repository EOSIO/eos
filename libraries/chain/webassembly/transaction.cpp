...
namespace eosio { namespace chain { namespace webassembly {
   void interface::send_inline( legacy_array_ptr<char> data ) {
      ...
      EOS_ASSERT( data.ref().size() < context.control.get_global_properties().configuration.max_inline_action_size, inline_action_too_big,
                 "inline action too big" );

      action act;
      fc::raw::unpack<action>(data.ref().data(), data.ref().size(), act);
   ...
   void interface::send_deferred( legacy_ptr<const uint128_t> sender_id, account_name payer, legacy_array_ptr<char> data, uint32_t replace_existing) {
      transaction trx;
      fc::raw::unpack<transaction>(data.ref().data(), data.ref().size(), trx);
      context.schedule_deferred_transaction(sender_id.ref(), payer, std::move(trx), replace_existing);
   }

   bool interface::cancel_deferred( legacy_ptr<const uint128_t> val ) {
      return context.cancel_deferred_transaction( val.ref() );
   }
