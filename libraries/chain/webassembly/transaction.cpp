...
namespace eosio { namespace chain { namespace webassembly {
   void interface::send_inline( legacy_span<char> data ) {
   ...
   void interface::send_context_free_inline( legacy_span<char> data ) {
      //TODO: Why is this limit even needed? And why is it not consistently checked on actions in input or deferred transactions
      EOS_ASSERT( data.size() < context.control.get_global_properties().configuration.max_inline_action_size, inline_action_too_big,
                "inline action too big" );
   ...
   void interface::send_deferred( legacy_ptr<const uint128_t> sender_id, account_name payer, legacy_span<char> data, uint32_t replace_existing) {
      transaction trx;
