...
#include <apifiny/chain/webassembly/apifiny-vm-oc/ipc_protocol.hpp>
...
#include <apifiny/chain/webassembly/apifiny-vm-oc/compile_trampoline.hpp>
...
struct compile_monitor_session {
   compile_monitor_session(boost::asio::io_context& context, local::datagram_protocol::socket&& n, wrapped_fd&& c, wrapped_fd& t) :
      _ctx(context),
      _nodapifiny_instance_socket(std::move(n)),
   ...
   void read_message_from_nodapifiny() {
      _nodapifiny_instance_socket.async_wait(local::datagram_protocol::socket::wait_read, [this](auto ec) {
         if(ec) {
            connection_dead_signal();
            return;
         }
         auto [success, message, fds] = read_message_with_fds(_nodapifiny_instance_socket);
         if(!success) {
            connection_dead_signal();
            return;
         }
   ...
   void kick_compile_off(const code_tuple& code_id, wrapped_fd&& wasm_code) {
      ...
      apifinyvmoc_message trampoline_compile_request = compile_wasm_message{code_id};
      if(write_message_with_fds(_trampoline_socket, trampoline_compile_request, fds_pass_to_trampoline) == false) {
         ...
         write_message_with_fds(_nodapifiny_instance_socket, reply);
         return;
      }
...
private:
   ...
   local::datagram_protocol::socket _nodapifiny_instance_socket;
...
struct compile_monitor {
   compile_monitor(boost::asio::io_context& ctx, local::datagram_protocol::socket&& n, wrapped_fd&& t) : _nodapifiny_socket(std::move(n)), _trampoline_socket(std::move(t)) {
      //the only duty of compile_monitor is to create a compile_monitor_session when a code_cache instance
      // in nodapifiny wants one
      wait_for_new_incomming_session(ctx);
   }

   void wait_for_new_incomming_session(boost::asio::io_context& ctx) {
      _nodapifiny_socket.async_wait(boost::asio::local::datagram_protocol::socket::wait_read, [this, &ctx](auto ec) {
         if(ec) {
            ctx.stop();
            return;
         }
         auto [success, message, fds] = read_message_with_fds(_nodapifiny_socket);
         if(!success) {   //failure reading indicates that nodapifiny has shut down
            ctx.stop();
            return;
         }
         ...
         try {
            ...
            write_message_with_fds(_nodapifiny_socket, initalize_response_message());
         }
         catch(const fc::exception& e) {
            write_message_with_fds(_nodapifiny_socket, initalize_response_message{e.what()});
         }
         catch(...) {
            write_message_with_fds(_nodapifiny_socket, initalize_response_message{"Failed to create compile process"});
         }
   ...
   local::datagram_protocol::socket _nodapifiny_socket;
   wrapped_fd _trampoline_socket;

   std::list<compile_monitor_session> _compile_sessions;
};

void launch_compile_monitor(int nodapifiny_fd) {
...

   {
      ...
      boost::asio::local::datagram_protocol::socket nodapifiny_socket(ctx);
      nodapifiny_socket.assign(boost::asio::local::datagram_protocol(), nodapifiny_fd);
