...
block_id_type signed_block_header::id() const {
  ...
  result._hash[0] +=
      fc::endian_reverse_u32(block_num()); // store the block num in the ID, 160
                                           // bits is plenty for the hash
  return result;
}

fc::ecc::public_key signed_block_header::signee() const {
  return fc::ecc::public_key(producer_signature, digest(),
                             true /*enforce canonical*/);
}
...
bool signed_block_header::validate_signee(
    const fc::ecc::public_key &expected_signee) const {
...
/**
 *  Suggest moving thread::merkle_digest code to return vector of ids which get
 * added to ids above and then calculating root over all
 */
