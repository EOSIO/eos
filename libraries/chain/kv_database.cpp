...
namespace eosio { namespace chain {
   ...
   kv_database::kv_database(chainbase::database& chain_db)
       : backing_store(backing_store_type::CHAINBASE), db(chain_db) {}

   void kv_database::check_backing_store_setting(bool clean_startup) {
   ...
   void kv_database::destroy(const fc::path& p) {
   ...
   void kv_database::set_revision(uint64_t revision) {
      db.set_revision(revision);
   }

   int64_t kv_database::revision() {
      return db.revision();
   }

   void kv_database::undo() {
      db.undo();
   }

   void kv_database::commit(int64_t revision) {
      db.commit(revision);
   }

   void kv_database::flush() {
   }

   std::unique_ptr<kv_context> kv_database::create_kv_context(name receiver, kv_resource_manager resource_manager,
   ...
   std::unique_ptr<db_context> kv_database::create_db_context(apply_context& context, name receiver) {
      return backing_store::create_db_chainbase_context(context, receiver);
   }

   void kv_database::add_to_snapshot(
   ...
   void kv_database::read_from_snapshot(const snapshot_reader_ptr& snapshot,
                                              uint32_t blog_start,
                                              uint32_t blog_end,
   ...
   void kv_database::add_contract_tables_to_snapshot(const snapshot_writer_ptr& snapshot) const {
   ...
   void kv_database::read_contract_tables_from_snapshot(const snapshot_reader_ptr& snapshot) {
