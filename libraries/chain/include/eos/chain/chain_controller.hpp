...
public:
  chain_controller(database &database, fork_database &fork_db,
   ...
   *  block have been applied but before the get_applied_operations() are
   * cleared.
   *
   ...
   */
  ///@{
  void set_apply_handler(const AccountName &contract, const AccountName &scope,
                         const ActionName &action, apply_handler v);
  //@}
  ...
  enum validation_steps {
    ...
    skip_tapos_check = 1 << 5,     ///< used while reindexing -- note this skips
                                   ///expiration check as well
    skip_authority_check = 1 << 6, ///< used while reindexing -- disables any
                                   ///checking of authority on transactions
    skip_merkle_check = 1 << 7,    ///< used while reindexing
    skip_assert_evaluation = 1 << 8,        ///< used while reindexing
    skip_undo_history_check = 1 << 9,       ///< used while reindexing
    skip_producer_schedule_check = 1 << 10, ///< used while reindexing
    skip_validate = 1 << 11,     ///< used prior to checkpoint, skips validate()
                                 ///call on transaction
    skip_scope_check = 1 << 12,  ///< used to skip checks for proper scope
    skip_output_check = 1 << 13, ///< used to skip checks for outputs in block
                                 ///exactly matching those created from apply
    pushed_transaction = 1 << 14, ///< used to indicate that the origination of
                                  ///the call was from a push_transaction, to
                                  ///determine time allotment
    created_block = 1 << 15, ///< used to indicate that the origination of the
                             ///call was for creating a block, to determine time
                             ///allotment
    received_block = 1 << 16 ///< used to indicate that the origination of the
                             ///call was for a received block, to determine time
                             ///allotment
  };
  ...
  get_recent_transaction(const transaction_id_type &trx_id) const;
  std::vector<block_id_type>
  get_block_ids_on_fork(block_id_type head_of_fork) const;
  const GeneratedTransaction &
  get_generated_transaction(const generated_transaction_id_type &id) const;

  /**
   *  This method will convert a variant to a SignedTransaction using a
   * contract's ABI to
  ...
  /**
   * This method will convert a signed transaction into a human-friendly variant
   * that can be
   * converted to JSON.
   */
   ...
   */
  vector<char> message_to_binary(Name code, Name type,
                                 const fc::variant &obj) const;
  fc::variant message_from_binary(Name code, Name type,
                                  const vector<char> &bin) const;
  ...
  ProcessedTransaction push_transaction(const SignedTransaction &trx,
   ...
   * @return Subset of candidateKeys whose private keys should be used to sign
   * transaction
   * @throws fc::exception if candidateKeys does not contain all required keys
   */
  flat_set<public_key_type>
  get_required_keys(const SignedTransaction &trx,
  ...
  signed_block generate_block(
      ...
      uint32_t skip = skip_nothing);
  signed_block
  _generate_block(fc::time_point_sec when, const AccountName &producer,
                  const fc::ecc::private_key &block_signing_private_key,
                  block_schedule::factory scheduler);

  template <typename Function>
  auto with_skip_flags(uint64_t flags, Function &&f)
      -> decltype((*((Function *)nullptr))()) {
  ...
  const deque<SignedTransaction> &pending() const {
...
private:
  ...
  template <typename Function>
  auto with_applying_block(Function &&f)
      -> decltype((*((Function *)nullptr))()) {
    auto on_exit =
        fc::make_scoped_exit([this]() { _currently_applying_block = false; });
    _currently_applying_block = true;
    return f();
  }

  void
  check_transaction_authorization(const SignedTransaction &trx,
                                  bool allow_unused_signatures = false) const;

  template <typename T>
  void check_transaction_output(const T &expected, const T &actual,
                                const path_cons_list &path) const;

  template <typename T> typename T::Processed apply_transaction(const T &trx);

  template <typename T>
  typename T::Processed process_transaction(const T &trx, int depth,
   ...
   */
  template <typename T> void validate_transaction(const T &trx) const {
    try {
      EOS_ASSERT(trx.messages.size() > 0, transaction_exception,
  ...
  /**
   * @brief Find the lowest authority level required for @ref authorizer_account
   * to authorize a message of the
   * specified type
   * @param authorizer_account The account authorizing the message
   * @param code_account The account which publishes the contract that handles
   * the message
   * @param type The type of message
   * @return
   */
  const permission_object &
  lookup_minimum_permission(types::AccountName authorizer_account,
                            types::AccountName code_account,
                            types::FuncName type) const;

  void process_message(const Transaction &trx, AccountName code,
                       const Message &message, MessageOutput &output,
                       apply_context *parent_context = nullptr);
  void apply_message(apply_context &c);

  bool should_check_for_duplicate_transactions() const {
    return !(_skip_flags & skip_transaction_dupe_check);
  }
  ...
  validate_block_header(uint32_t skip, const signed_block &next_block) const;
  const producer_object &
  _validate_block_header(const signed_block &next_block) const;
  void create_block_summary(const signed_block &next_block);

  void update_global_properties(const signed_block &b);
  void update_global_dynamic_data(const signed_block &b);
  void update_signing_producer(const producer_object &signing_producer,
