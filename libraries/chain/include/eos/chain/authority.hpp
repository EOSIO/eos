...
struct shared_authority {
  ...
  shared_authority &operator=(const Authority &a) {
    ...
    accounts = decltype(accounts)(a.accounts.begin(), a.accounts.end(),
                                  accounts.get_allocator());
  ...
  Authority to_authority() const {
    ...
    for (const auto &k : keys) {
      auth.keys.emplace_back(k);
    }
    for (const auto &a : accounts) {
      auth.accounts.emplace_back(a);
    }
    return auth;
  }
};

inline bool operator<(const types::AccountPermission &a,
                      const types::AccountPermission &b) {
  return std::tie(a.account, a.permission) < std::tie(b.account, b.permission);
}

/**
 * Makes sure all keys are unique and sorted and all account permissions are
 * unique and sorted and that authority can
 * be satisfied
 */
inline bool validate(const types::Authority &auth) {
  ...
  for (const auto &a : auth.accounts) {
    if (!pa)
      pa = &a;
    else if (pa->permission < a.permission)
      return false;
