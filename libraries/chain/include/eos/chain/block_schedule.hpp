...
namespace eos {
namespace chain {
using pending_transaction =
    static_variant<std::reference_wrapper<const SignedTransaction>,
                   std::reference_wrapper<const GeneratedTransaction>>;
...
struct block_schedule {
  typedef block_schedule (*factory)(const vector<pending_transaction> &,
  ...
  /**
   * A greedy scheduler that attempts to make short threads to resolve scope
   * contention before
   * falling back on cycles
   * @return the block scheduler
   */
  static block_schedule
  by_threading_conflicts(const vector<pending_transaction> &transactions,
                         const global_property_object &properties);

  /**
   * A greedy scheduler that attempts uses future cycles to resolve scope
   * contention
   * @return the block scheduler
   */
  static block_schedule
  by_cycling_conflicts(const vector<pending_transaction> &transactions,
                       const global_property_object &properties);
   ...
   */
  static block_schedule
  in_single_thread(const vector<pending_transaction> &transactions,
                   const global_property_object &properties);
};
