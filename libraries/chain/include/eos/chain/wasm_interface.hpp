 ...
 *
 * EOS uses the wasm-jit library to evaluate web assembly code. This library
 * relies
 * upon a singlton interface which means there can be only one instance. This
 * interface
 * is designed to wrap that singlton interface and potentially make it
 * thread-local state
 * in the future.
 */
class wasm_interface {
