...
template <typename T> struct eos_extension_pack_count_visitor {
...
template <typename Stream, typename T> struct eos_extension_pack_read_visitor {
  eos_extension_pack_read_visitor(Stream &s, const T &v)
      : stream(s), value(v) {}
...
template <typename Stream, typename T> struct eos_extension_unpack_visitor {
...
template <typename T> struct eos_extension_from_variant_visitor {
  ...
  void operator()(const char *name) const {
    ...
    if (it != vo.end()) {
      ...
      assert(count_left > 0); // x.find(k) returns true for n distinct values of
                              // k only if x.size() >= n
      --count_left;
    }
  }
...
template <typename T> struct eos_extension_to_variant_visitor {
