...
public:
  apply_context(chain_controller &con, chainbase::database &db,
                const chain::Transaction &t, const chain::Message &m,
                const types::AccountName &code)
      : controller(con), db(db), trx(t), msg(m), code(code),
        mutable_controller(con), mutable_db(db),
        used_authorizations(msg.authorization.size(), false),
        next_pending_transaction_serial(0) {}

  template <typename ObjectType>
  int32_t store_record(Name scope, Name code, Name table,
                       typename ObjectType::key_type *keys, char *value,
                       uint32_t valuelen) {
    require_scope(scope);
    ...
    if (obj) {
      // wlog( "modify" );
      mutable_db.modify(*obj,
                        [&](auto &o) { o.value.assign(value, valuelen); });
      return 0;
    } else {
      // wlog( "new" );
  ...
  template <typename ObjectType>
  int32_t update_record(Name scope, Name code, Name table,
                        typename ObjectType::key_type *keys, char *value,
                        uint32_t valuelen) {
    require_scope(scope);
  ...
  template <typename ObjectType>
  int32_t remove_record(Name scope, Name code, Name table,
                        typename ObjectType::key_type *keys, char *value,
                        uint32_t valuelen) {
    require_scope(scope);
  ...
  int32_t load_record(Name scope, Name code, Name table,
                      typename IndexType::value_type::key_type *keys,
                      char *value, uint32_t valuelen) {
    require_scope(scope);

    const auto &idx = db.get_index<IndexType, Scope>();
    auto tuple = load_record_tuple<typename IndexType::value_type, Scope>::get(
        scope, code, table, keys);
    auto itr = idx.lower_bound(tuple);

    if (itr == idx.end() || itr->scope != scope || itr->code != code ||
        itr->table != table ||
        !load_record_compare<typename IndexType::value_type, Scope>::compare(
            *itr, keys))
      return -1;
  ...
  template <typename IndexType, typename Scope>
  int32_t front_record(Name scope, Name code, Name table,
                       typename IndexType::value_type::key_type *keys,
                       char *value, uint32_t valuelen) {
    require_scope(scope);

    const auto &idx = db.get_index<IndexType, Scope>();
    auto tuple = front_record_tuple<typename IndexType::value_type>::get(
        scope, code, table);
  ...
  template <typename IndexType, typename Scope>
  int32_t back_record(Name scope, Name code, Name table,
                      typename IndexType::value_type::key_type *keys,
                      char *value, uint32_t valuelen) {
    require_scope(scope);

    const auto &idx = db.get_index<IndexType, Scope>();
    auto tuple = boost::make_tuple(AccountName(scope), AccountName(code),
                                   AccountName(uint64_t(table) + 1));
    auto itr = idx.lower_bound(tuple);

    if (std::distance(idx.begin(), itr) == 0)
      return -1;

    --itr;

    if (itr->scope != scope || itr->code != code || itr->table != table)
      return -1;
  ...
  template <typename IndexType, typename Scope>
  int32_t next_record(Name scope, Name code, Name table,
                      typename IndexType::value_type::key_type *keys,
                      char *value, uint32_t valuelen) {
    require_scope(scope);
    ...
    auto tuple = next_record_tuple<typename IndexType::value_type>::get(
        scope, code, table, keys);
  ...
  template <typename IndexType, typename Scope>
  int32_t previous_record(Name scope, Name code, Name table,
                          typename IndexType::value_type::key_type *keys,
                          char *value, uint32_t valuelen) {
    require_scope(scope);
    ...
    if (itr == idx.end() || itr == idx.begin() || itr->scope != scope ||
        itr->code != code || itr->table != table ||
        !key_helper<typename IndexType::value_type>::compare(*itr, keys))
      return -1;
  ...
  template <typename IndexType, typename Scope>
  int32_t lower_bound_record(Name scope, Name code, Name table,
                             typename IndexType::value_type::key_type *keys,
                             char *value, uint32_t valuelen) {
    require_scope(scope);

    const auto &idx = db.get_index<IndexType, Scope>();
    auto tuple = lower_bound_tuple<typename IndexType::value_type, Scope>::get(
        scope, code, table, keys);
    auto itr = idx.lower_bound(tuple);
  ...
  template <typename IndexType, typename Scope>
  int32_t upper_bound_record(Name scope, Name code, Name table,
                             typename IndexType::value_type::key_type *keys,
                             char *value, uint32_t valuelen) {
    require_scope(scope);

    const auto &idx = db.get_index<IndexType, Scope>();
    auto tuple = upper_bound_tuple<typename IndexType::value_type, Scope>::get(
        scope, code, table, keys);
   ...
   *
   * This method will check that @ref account is listed in the message's
   * declared authorizations, and marks the
   * authorization as used. Note that all authorizations on a message must be
   * used, or the message is invalid.
   *
  ...
  ///< Parallel to msg.authorization; tracks which permissions have been used
  ...
  struct pending_transaction : public types::Transaction {
    ...
    pending_transaction(const handle_type &_handle,
                        const apply_context &_context, const UInt16 &block_num,
                        const UInt32 &block_ref, const Time &expiration)
        : types::Transaction(
              block_num, block_ref, expiration, vector<types::AccountName>(),
              vector<types::AccountName>(), vector<types::Message>()),
          handle(_handle), context(_context) {}
  ...
  pending_transaction &
  get_pending_transaction(pending_transaction::handle_type handle);
  ...
  struct pending_message : public types::Message {
    ...
    pending_message(const handle_type &_handle, const AccountName &code,
  ...
  pending_message &create_pending_message(const AccountName &code,
                                          const FuncName &type,
                                          const Bytes &data);
