 ...
 * @brief Maintains global state information (committee_member list, current
 * fees)
 * @ingroup object
 * @ingroup implementation
 *
 * This is an implementation detail. The values here are set by
 * committee_members to tune the blockchain parameters.
 */
class global_property_object
    : public chainbase::object<global_property_object_type,
 ...
 * @ingroup implementation
 *
 * This is an implementation detail. The values here are calculated during
 * normal chain operations and reflect the
 * current values of global blockchain properties.
 */
class dynamic_global_property_object
    : public chainbase::object<dynamic_global_property_object_type,
...
using global_property_multi_index = chainbase::shared_multi_index_container<
    ...
    indexed_by<
        ordered_unique<tag<by_id>, BOOST_MULTI_INDEX_MEMBER(
                                       global_property_object,
                                       global_property_object::id_type, id)>>>;

using dynamic_global_property_multi_index =
    chainbase::shared_multi_index_container<
        dynamic_global_property_object,
        indexed_by<ordered_unique<
            tag<by_id>, BOOST_MULTI_INDEX_MEMBER(
                            dynamic_global_property_object,
                            dynamic_global_property_object::id_type, id)>>>;
}
}

CHAINBASE_SET_INDEX_TYPE(eos::chain::global_property_object,
                         eos::chain::global_property_multi_index)
...
FC_REFLECT(eos::chain::dynamic_global_property_object,
           (head_block_number)(head_block_id)(time)(current_producer)(
               accounts_registered_this_interval)(current_absolute_slot)(
               recent_slots_filled)(last_irreversible_block_num))

FC_REFLECT(eos::chain::global_property_object,
           (configuration)(active_producers))
