   ...
   struct injector_utils {
      ...
      static void add_import(Module& module, const char* func_name, int32_t& index ) {
         if (module.functions.imports.size() == 0 || registered_injected.find(func_name) == registered_injected.end() ) {
            ...
            decltype(module.functions.imports) new_import = { {{func_type_index}, ARISEN_INJECTED_MODULE_NAME, std::move(func_name)} };
...
#undef INSERT_INJECTED
   ...
   struct f32_promote_injector {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         int32_t idx;
         injector_utils::add_import<ResultType::f64, ValueType::f32>( *(arg.module), u8"_arisen_f32_promote", idx );
   ...
   struct f64_demote_injector {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         int32_t idx;
         injector_utils::add_import<ResultType::f32, ValueType::f64>( *(arg.module), u8"_arisen_f64_demote", idx );
         wasm_ops::op_types<>::call_t f32promote;
         f32promote.field = idx;
         f32promote.pack(arg.new_code);
      }
      ...
      public:
         ...
         void inject() {
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::ARISEN_OperatorDecoderStream<pre_op_injectors> pre_decoder(fd.code);
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::ARISEN_OperatorDecoderStream<post_op_injectors> post_decoder(fd.code);
