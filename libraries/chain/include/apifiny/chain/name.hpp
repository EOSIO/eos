...
namespace fc {
  class variant;
  void to_variant(const apifiny::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, apifiny::chain::name& check);
} // fc
...
namespace apifiny::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, apifiny::chain::name& check);
...
#define N(X) apifiny::chain::string_to_name(#X)

} // apifiny::chain

namespace std {
   template<> struct hash<apifiny::chain::name> : private hash<uint64_t> {
      typedef apifiny::chain::name argument_type;
...
FC_REFLECT( apifiny::chain::name, (value) )
