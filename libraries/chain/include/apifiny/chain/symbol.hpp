...
#include <apifiny/chain/core_symbol.hpp>
#include <string>
#include <functional>
...
#define SY(P,X) ::apifiny::chain::string_to_symbol_c(P,#X)
      ...
      class symbol : fc::reflect_init {
         public:
            ...
            explicit operator string() const
            {
               uint64_t v = m_value;
               uint8_t p = v & 0xFF;
               string ret = apifiny::chain::to_string(p);
               ret += ',';
               ret += name();
               return ret;
            }
...
namespace fc {
   inline void to_variant(const apifiny::chain::symbol& var, fc::variant& vo) { vo = var.to_string(); }
   inline void from_variant(const fc::variant& var, apifiny::chain::symbol& vo) {
      vo = apifiny::chain::symbol::from_string(var.get_string());
   }
}

namespace fc {
   inline void to_variant(const apifiny::chain::symbol_code& var, fc::variant& vo) {
      vo = apifiny::chain::symbol(var.value << 8).name();
   }
   inline void from_variant(const fc::variant& var, apifiny::chain::symbol_code& vo) {
      vo = apifiny::chain::symbol(0, var.get_string().c_str()).to_symbol_code();
   }
}

FC_REFLECT(apifiny::chain::symbol_code, (value))
FC_REFLECT(apifiny::chain::symbol, (m_value))
FC_REFLECT(apifiny::chain::extended_symbol, (sym)(contract))
