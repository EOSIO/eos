...
#define CHAINBASE_OBJECT_CTOR1(NAME) \
    NAME() = delete; \
    public: \
    template<typename Constructor, typename Allocator> \
    NAME(Constructor&& c, chainbase::allocator<Allocator>) \
    { c(*this); }
#define CHAINBASE_OBJECT_CTOR2_MACRO(x, y, field) ,field(a)
#define CHAINBASE_OBJECT_CTOR2(NAME, FIELDS) \
    NAME() = delete; \
    public: \
    template<typename Constructor, typename Allocator> \
    NAME(Constructor&& c, chainbase::allocator<Allocator> a) \
    : id(0) BOOST_PP_SEQ_FOR_EACH(CHAINBASE_OBJECT_CTOR2_MACRO, _, FIELDS) \
    { c(*this); }
#define CHAINBASE_OBJECT_CTOR(...) BOOST_PP_OVERLOAD(CHAINBASE_OBJECT_CTOR, __VA_ARGS__)(__VA_ARGS__)
