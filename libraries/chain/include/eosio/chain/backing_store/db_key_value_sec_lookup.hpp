...
namespace eosio { namespace chain { namespace backing_store {
   ...
   class db_key_value_sec_lookup : public db_key_value_any_lookup<Session>
   {
   public:
      db_key_value_sec_lookup( db_context& p, Session& s ) : db_key_value_any_lookup<Session>(p, s) {}

      int store( name scope, name table, const account_name& payer,
                 uint64_t id, const SecondaryKey& secondary ) {
         EOS_ASSERT( payer != account_name(), invalid_table_payer, "must specify a valid account to pay for new record" );

         const sec_pair_bundle secondary_key = get_secondary_slices_in_secondaries(this->parent.receiver, scope, table, secondary, id);

         this->add_table_if_needed(secondary_key.full_secondary_key, payer);

         auto old_value = this->current_session.read(secondary_key.full_secondary_key);
         ...
         set_value(secondary_key.full_primary_to_sec_key, this->useless_value);
      ...
      void remove( int iterator ) {
         ...
         EOS_ASSERT( table.contract == this->parent.receiver, table_access_violation, "db access violation" );

         const sec_pair_bundle secondary_key = get_secondary_slices_in_secondaries(this->parent.receiver, table.scope, table.table, key_store.secondary, key_store.primary);
         auto old_value = this->current_session.read(secondary_key.full_secondary_key);
         ...
         this->current_session.erase(secondary_key.full_primary_to_sec_key);
         this->remove_table_if_empty(secondary_key.full_secondary_key);
      ...
      void update( int iterator, account_name payer, const SecondaryKey& secondary ) {
         ...
         auto& context = this->parent.context;
         std::string event_id;
         ...
         if (secondary != key_store.secondary) {
            ...
            set_value(db_key_value_format::create_full_key(new_secondary_keys.secondary_key, this->parent.receiver), helper.value(secondary, payer));
...
#warning currently using useless_value to distinguish between no key and no value, eventually will just store empty value
            set_value(db_key_value_format::create_full_key(new_secondary_keys.primary_to_secondary_key, this->parent.receiver), this->useless_value);
         }
         else {
      ...
      int find_secondary( name code, name scope, name table, const SecondaryKey& secondary, uint64_t& primary ) {
         ...
         if (!this->match_prefix(secondary_key.full_key, session_iter)) {
            return table_ei;
         }
      ...
      int end_secondary( name code, name scope, name table ) {
         return this->get_end_iter(name{code}, name{scope}, name{table}, iter_store);
      }
      ...
      int previous_secondary( int iterator, uint64_t& primary ) {
         if( iterator < iter_store.invalid_iterator() ) {
            ...
            const auto type_prefix = db_key_value_format::create_full_key(legacy_type_key, table->contract);
            const auto next_type_prefix = type_prefix.next();
            auto session_iter = this->current_session.lower_bound(next_type_prefix);
            ...
            if (!this->match_prefix(type_prefix, session_iter)) {
               return iter_store.invalid_iterator();
            }
      ...
      int find_primary( name code, name scope, name table, SecondaryKey& secondary, uint64_t primary ) {
         ...
         const auto secondary_key = db_key_value_format::create_full_key(legacy_secondary_key, code);
         const auto old_value = this->current_session.read(secondary_key);
         EOS_ASSERT( old_value, db_rocksdb_invalid_operation_exception,
      ...
      int previous_primary( int iterator, uint64_t& primary ) {
         if( iterator < iter_store.invalid_iterator() ) {
            ...
            const auto type_prefix = db_key_value_format::create_full_key(types_key, table->contract);
            const auto next_type_prefix = type_prefix.next();
            auto session_iter = this->current_session.lower_bound(next_type_prefix); //upperbound of this primary_to_sec key type
      ...
      struct sec_pair_bundle {
         ...
         eosio::session::shared_bytes full_secondary_key;
         eosio::session::shared_bytes full_primary_to_sec_key;
         eosio::session::shared_bytes prefix_secondary_key;       // points to the type prefix for composite_key
         eosio::session::shared_bytes prefix_primary_to_sec_key;  // points to the primary key in full_primary_to_sec_key (all except the secondary key)
      };
      ...
      void set_value(const eosio::session::shared_bytes& full_key, const eosio::session::shared_bytes& payload) {
         this->current_session.write(full_key, payload);
      }
   ...
   private:
      ...
      std::optional<found_keys> find_matching_sec_prim(const std::pair<eosio::session::shared_bytes, std::optional<eosio::session::shared_bytes> >& found_kv,
      ...
      std::optional<found_keys> find_matching_prim_sec(const std::pair<eosio::session::shared_bytes, std::optional<eosio::session::shared_bytes>>& found_kv,
                                                       const eosio::session::shared_bytes& prefix) {
         SecondaryKey secondary_key;
         uint64_t primary_key;
         ...
         auto session_iter = this->current_session.read(full_secondary_key);
      ...
      int bound_secondary( name code, name scope, name table, bound_type bt, SecondaryKey& secondary, uint64_t& primary ) {
         ...
         if (!this->match_prefix(type_prefix, session_iter)) {
            ...
            while (this->match_prefix(secondary_key.full_key, session_iter)) {
               ++session_iter;
               if (!this->match_prefix(type_prefix, session_iter)) {
