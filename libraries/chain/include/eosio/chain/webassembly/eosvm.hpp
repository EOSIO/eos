...
namespace eosio { namespace chain { namespace webassembly { namespace eosvm_runtime {

using namespace fc;
using namespace eosio::chain::webassembly::common;
using namespace eosio::wasm_backend; 

struct eosvm_apply_instance_vars {
   wasm_allocator& alloc;
   apply_context&  ctx;
   char* get_validated_pointer(uint32_t offset, uint32_t size) {
      EOS_ASSERT(offset + size <= alloc.get_current_page()*constants::page_size && offset + size >= offset, wasm_execution_error, "access violation");
      return alloc.get_base_ptr<char>() + offset;
   }
};
...
#define _REGISTER_EOSVM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
