...
#warning: TODO avoid heap allocation
   ...
   struct trim_data {            //used by trim_blocklog_front(), trim_blocklog_end(), and smoke_test()
      trim_data(fc::path block_dir, bool check_file = false); // check_file indicates whether to validate block and index files or not. For now set true by trim_blocklog_end
      ~trim_data();
