...
void block_log::open(const fc::path &data_dir) {
  ...
  ilog("Opening block log at ${path}",
       ("path", my->block_file.generic_string()));
  ...
  /* On startup of the block log, there are several states the log file and the
   * index file can be
   ...
   *  - If the index file head is not in the log file, delete the index and
   * replay.
   *  - If the index file head is in the log, but not up to date, replay from
   * index head.
   */
...
uint64_t block_log::append(const signed_block &b) {
  try {
    ...
    FC_ASSERT(my->index_stream.tellp() ==
                  sizeof(uint64_t) * (b.block_num() - 1),
...
uint64_t block_log::get_block_pos(uint32_t block_num) const {
  ...
  if (!(my->head.valid() &&
        block_num <= block_header::num_from_id(my->head_id) && block_num > 0))
    return npos;
