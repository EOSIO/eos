...
namespace eosio { namespace chain {
   ...
   fc::path block_log::repair_log( const fc::path& data_dir, uint32_t stop_at_block ) {
      ...
      } else if( block_num == stop_at_block && pos < end_pos ) {
         ilog( "Stopped recovery of block log early at specified block number: ${stop}.", ("stop", stop_at_block) );
      } else {
