...
DEFINE_INTRINSIC_FUNCTION0(env, checktime, checktime, none) {
  checktime(wasm_interface::get().current_execution_time(),
            wasm_interface::get().checktime_limit);
}
...
int32_t validate_str(int32_t keyptr, int32_t keylen, int32_t valueptr,
                     int32_t valuelen, Function func) {
...
#define READ_RECORD(READFUNC, INDEX, SCOPE)                                    \
  auto lambda = [&](apply_context *ctx, INDEX::value_type::key_type *keys,     \
                    char *data, uint32_t datalen) -> int32_t {                 \
    auto res = ctx->READFUNC<INDEX, SCOPE>(Name(scope), Name(code),            \
                                           Name(table), keys, data, datalen);  \
    if (res >= 0)                                                              \
      res += INDEX::value_type::number_of_keys *                               \
             sizeof(INDEX::value_type::key_type);                              \
    return res;                                                                \
  };                                                                           \
  return validate<decltype(lambda), INDEX::value_type::key_type,               \
                  INDEX::value_type::number_of_keys>(valueptr, valuelen,       \
                                                     lambda);

#define UPDATE_RECORD(UPDATEFUNC, INDEX, DATASIZE)                             \
  auto lambda = [&](apply_context *ctx, INDEX::value_type::key_type *keys,     \
                    char *data, uint32_t datalen) -> int32_t {                 \
    return ctx->UPDATEFUNC<INDEX::value_type>(                                 \
        Name(scope), Name(ctx->code.value), Name(table), keys, data, datalen); \
  };                                                                           \
  return validate<decltype(lambda), INDEX::value_type::key_type,               \
                  INDEX::value_type::number_of_keys>(valueptr, DATASIZE,       \
                                                     lambda);

#define DEFINE_RECORD_UPDATE_FUNCTIONS(OBJTYPE, INDEX)                         \
  DEFINE_INTRINSIC_FUNCTION4(env, store_##OBJTYPE, store_##OBJTYPE, i32, i64,  \
                             scope, i64, table, i32, valueptr, i32,            \
                             valuelen) {                                       \
    UPDATE_RECORD(store_record, INDEX, valuelen);                              \
  }                                                                            \
  DEFINE_INTRINSIC_FUNCTION4(env, update_##OBJTYPE, update_##OBJTYPE, i32,     \
                             i64, scope, i64, table, i32, valueptr, i32,       \
                             valuelen) {                                       \
    UPDATE_RECORD(update_record, INDEX, valuelen);                             \
  }                                                                            \
  DEFINE_INTRINSIC_FUNCTION3(env, remove_##OBJTYPE, remove_##OBJTYPE, i32,     \
                             i64, scope, i64, table, i32, valueptr) {          \
    UPDATE_RECORD(remove_record, INDEX,                                        \
                  sizeof(typename INDEX::value_type::key_type) *               \
                      INDEX::value_type::number_of_keys);                      \
  }

#define DEFINE_RECORD_READ_FUNCTIONS(OBJTYPE, FUNCPREFIX, INDEX, SCOPE)        \
  DEFINE_INTRINSIC_FUNCTION5(env, load_##FUNCPREFIX##OBJTYPE,                  \
                             load_##FUNCPREFIX##OBJTYPE, i32, i64, scope, i64, \
                             code, i64, table, i32, valueptr, i32, valuelen) { \
    READ_RECORD(load_record, INDEX, SCOPE);                                    \
  }                                                                            \
  DEFINE_INTRINSIC_FUNCTION5(                                                  \
      env, front_##FUNCPREFIX##OBJTYPE, front_##FUNCPREFIX##OBJTYPE, i32, i64, \
      scope, i64, code, i64, table, i32, valueptr, i32, valuelen) {            \
    READ_RECORD(front_record, INDEX, SCOPE);                                   \
  }                                                                            \
  DEFINE_INTRINSIC_FUNCTION5(env, back_##FUNCPREFIX##OBJTYPE,                  \
                             back_##FUNCPREFIX##OBJTYPE, i32, i64, scope, i64, \
                             code, i64, table, i32, valueptr, i32, valuelen) { \
    READ_RECORD(back_record, INDEX, SCOPE);                                    \
  }                                                                            \
  DEFINE_INTRINSIC_FUNCTION5(env, next_##FUNCPREFIX##OBJTYPE,                  \
                             next_##FUNCPREFIX##OBJTYPE, i32, i64, scope, i64, \
                             code, i64, table, i32, valueptr, i32, valuelen) { \
    READ_RECORD(next_record, INDEX, SCOPE);                                    \
  }                                                                            \
  DEFINE_INTRINSIC_FUNCTION5(                                                  \
      env, previous_##FUNCPREFIX##OBJTYPE, previous_##FUNCPREFIX##OBJTYPE,     \
      i32, i64, scope, i64, code, i64, table, i32, valueptr, i32, valuelen) {  \
    READ_RECORD(previous_record, INDEX, SCOPE);                                \
  }                                                                            \
  DEFINE_INTRINSIC_FUNCTION5(env, lower_bound_##FUNCPREFIX##OBJTYPE,           \
                             lower_bound_##FUNCPREFIX##OBJTYPE, i32, i64,      \
                             scope, i64, code, i64, table, i32, valueptr, i32, \
                             valuelen) {                                       \
    READ_RECORD(lower_bound_record, INDEX, SCOPE);                             \
  }                                                                            \
  DEFINE_INTRINSIC_FUNCTION5(env, upper_bound_##FUNCPREFIX##OBJTYPE,           \
                             upper_bound_##FUNCPREFIX##OBJTYPE, i32, i64,      \
                             scope, i64, code, i64, table, i32, valueptr, i32, \
                             valuelen) {                                       \
...
DEFINE_RECORD_READ_FUNCTIONS(i128i128, primary_, key128x128_value_index,
                             by_scope_primary);
DEFINE_RECORD_READ_FUNCTIONS(i128i128, secondary_, key128x128_value_index,
                             by_scope_secondary);

DEFINE_RECORD_UPDATE_FUNCTIONS(i64i64i64, key64x64x64_value_index);
DEFINE_RECORD_READ_FUNCTIONS(i64i64i64, primary_, key64x64x64_value_index,
                             by_scope_primary);
DEFINE_RECORD_READ_FUNCTIONS(i64i64i64, secondary_, key64x64x64_value_index,
                             by_scope_secondary);
DEFINE_RECORD_READ_FUNCTIONS(i64i64i64, tertiary_, key64x64x64_value_index,
                             by_scope_tertiary);

#define UPDATE_RECORD_STR(FUNCTION)                                            \
  auto lambda = [&](apply_context *ctx, std::string *keys, char *data,         \
                    uint32_t datalen) -> int32_t {                             \
    return ctx->FUNCTION<keystr_value_object>(                                 \
        Name(scope), Name(ctx->code.value), Name(table), keys, data, datalen); \
  };                                                                           \
  return validate_str<decltype(lambda)>(keyptr, keylen, valueptr, valuelen,    \
                                        lambda);

#define READ_RECORD_STR(FUNCTION)                                              \
  auto lambda = [&](apply_context *ctx, std::string *keys, char *data,         \
                    uint32_t datalen) -> int32_t {                             \
    auto res = ctx->FUNCTION<keystr_value_index, by_scope_primary>(            \
        Name(scope), Name(code), Name(table), keys, data, datalen);            \
    return res;                                                                \
  };                                                                           \
  return validate_str<decltype(lambda)>(keyptr, keylen, valueptr, valuelen,    \
                                        lambda);

DEFINE_INTRINSIC_FUNCTION6(env, store_str, store_str, i32, i64, scope, i64,
                           table, i32, keyptr, i32, keylen, i32, valueptr, i32,
                           valuelen) {
  UPDATE_RECORD_STR(store_record)
}
DEFINE_INTRINSIC_FUNCTION6(env, update_str, update_str, i32, i64, scope, i64,
                           table, i32, keyptr, i32, keylen, i32, valueptr, i32,
                           valuelen) {
  UPDATE_RECORD_STR(update_record)
}
DEFINE_INTRINSIC_FUNCTION4(env, remove_str, remove_str, i32, i64, scope, i64,
                           table, i32, keyptr, i32, keylen) {
  int32_t valueptr = 0, valuelen = 0;
  UPDATE_RECORD_STR(remove_record)
}

DEFINE_INTRINSIC_FUNCTION7(env, load_str, load_str, i32, i64, scope, i64, code,
                           i64, table, i32, keyptr, i32, keylen, i32, valueptr,
                           i32, valuelen) {
  READ_RECORD_STR(load_record)
}
DEFINE_INTRINSIC_FUNCTION5(env, front_str, front_str, i32, i64, scope, i64,
                           code, i64, table, i32, valueptr, i32, valuelen) {
  int32_t keyptr = 0, keylen = 0;
  READ_RECORD_STR(front_record)
}
DEFINE_INTRINSIC_FUNCTION5(env, back_str, back_str, i32, i64, scope, i64, code,
                           i64, table, i32, valueptr, i32, valuelen) {
  int32_t keyptr = 0, keylen = 0;
  READ_RECORD_STR(back_record)
}
DEFINE_INTRINSIC_FUNCTION7(env, next_str, next_str, i32, i64, scope, i64, code,
                           i64, table, i32, keyptr, i32, keylen, i32, valueptr,
                           i32, valuelen) {
  READ_RECORD_STR(next_record)
}
DEFINE_INTRINSIC_FUNCTION7(env, previous_str, previous_str, i32, i64, scope,
                           i64, code, i64, table, i32, keyptr, i32, keylen, i32,
                           valueptr, i32, valuelen) {
  READ_RECORD_STR(previous_record)
}
DEFINE_INTRINSIC_FUNCTION7(env, lower_bound_str, lower_bound_str, i32, i64,
                           scope, i64, code, i64, table, i32, keyptr, i32,
                           keylen, i32, valueptr, i32, valuelen) {
  READ_RECORD_STR(lower_bound_record)
}
DEFINE_INTRINSIC_FUNCTION7(env, upper_bound_str, upper_bound_str, i32, i64,
                           scope, i64, code, i64, table, i32, keyptr, i32,
                           keylen, i32, valueptr, i32, valuelen) {
  READ_RECORD_STR(upper_bound_record)
}

DEFINE_INTRINSIC_FUNCTION3(env, assert_sha256, assert_sha256, none, i32,
                           dataptr, i32, datalen, i32, hash) {
  FC_ASSERT(datalen > 0);
...
DEFINE_INTRINSIC_FUNCTION3(env, sha256, sha256, none, i32, dataptr, i32,
                           datalen, i32, hash) {
  FC_ASSERT(datalen > 0);
...
DEFINE_INTRINSIC_FUNCTION2(env, multeq_i128, multeq_i128, none, i32, self, i32,
                           other) {
  auto &wasm = wasm_interface::get();
  auto mem = wasm.current_memory;
...
DEFINE_INTRINSIC_FUNCTION2(env, diveq_i128, diveq_i128, none, i32, self, i32,
                           other) {
  auto &wasm = wasm_interface::get();
  auto mem = wasm.current_memory;
  auto &v = memoryRef<unsigned __int128>(mem, self);
...
DEFINE_INTRINSIC_FUNCTION1(env, double_to_i64, double_to_i64, i64, i64, a) {
  return DOUBLE(*reinterpret_cast<double *>(&a)).convert_to<uint64_t>();
}
...
DEFINE_INTRINSIC_FUNCTION2(env, getActiveProducers, getActiveProducers, none,
                           i32, producers, i32, datalen) {
  auto &wasm = wasm_interface::get();
  auto mem = wasm.current_memory;
  types::AccountName *dst =
      memoryArrayPtr<types::AccountName>(mem, producers, datalen);
  return wasm_interface::get().current_validate_context->get_active_producers(
      dst, datalen);
}

DEFINE_INTRINSIC_FUNCTION0(env, now, now, i32) {
  return wasm_interface::get()
      .current_validate_context->controller.head_block_time()
      .sec_since_epoch();
}
...
DEFINE_INTRINSIC_FUNCTION1(env, requireAuth, requireAuth, none, i64, account) {
  wasm_interface::get().current_validate_context->require_authorization(
      Name(account));
}

DEFINE_INTRINSIC_FUNCTION1(env, requireNotice, requireNotice, none, i64,
                           account) {
...
DEFINE_INTRINSIC_FUNCTION3(env, memcpy, memcpy, i32, i32, dstp, i32, srcp, i32,
                           len) {
  auto &wasm = wasm_interface::get();
  auto mem = wasm.current_memory;
  ...
  if (dst > src)
    FC_ASSERT(dst >= (src + len), "overlap of memory range is undefined",
              ("d", dstp)("s", srcp)("l", len));
  else
    FC_ASSERT(src >= (dst + len), "overlap of memory range is undefined",
              ("d", dstp)("s", srcp)("l", len));
...
DEFINE_INTRINSIC_FUNCTION3(env, memcmp, memcmp, i32, i32, dstp, i32, srcp, i32,
                           len) {
  auto &wasm = wasm_interface::get();
  auto mem = wasm.current_memory;
  char *dst = memoryArrayPtr<char>(mem, dstp, len);
...
DEFINE_INTRINSIC_FUNCTION3(env, memset, memset, i32, i32, rel_ptr, i32, value,
                           i32, len) {
  auto &wasm = wasm_interface::get();
  auto mem = wasm.current_memory;
...
DEFINE_INTRINSIC_FUNCTION1(env, sbrk, sbrk, i32, i32, num_bytes) {
  ...
  FC_ASSERT(
      wasm.current_memory_management != nullptr,
      "sbrk can only be called during the scope of wasm_interface::vm_call");
  U32 previous_bytes_allocated =
      wasm.current_memory_management->sbrk(num_bytes);
...
DEFINE_INTRINSIC_FUNCTION0(env, transactionCreate, transactionCreate, i32) {
  auto &ptrx =
      wasm_interface::get().current_apply_context->create_pending_transaction();
  return ptrx.handle;
}
...
DEFINE_INTRINSIC_FUNCTION3(env, transactionRequireScope,
                           transactionRequireScope, none, i32, handle, i64,
                           scope, i32, readOnly) {
  auto &ptrx =
      wasm_interface::get().current_apply_context->get_pending_transaction(
          handle);
  if (readOnly == 0) {
...
DEFINE_INTRINSIC_FUNCTION2(env, transactionAddMessage, transactionAddMessage,
                           none, i32, handle, i32, msg_handle) {
...
DEFINE_INTRINSIC_FUNCTION1(env, transactionSend, transactionSend, none, i32,
                           handle) {
  auto apply_context = wasm_interface::get().current_apply_context;
  auto &ptrx = apply_context->get_pending_transaction(handle);
...
DEFINE_INTRINSIC_FUNCTION1(env, transactionDrop, transactionDrop, none, i32,
                           handle) {
  wasm_interface::get().current_apply_context->release_pending_transaction(
      handle);
}

DEFINE_INTRINSIC_FUNCTION4(env, messageCreate, messageCreate, i32, i64, code,
                           i64, type, i32, data, i32, length) {
  auto &wasm = wasm_interface::get();
  auto mem = wasm.current_memory;
  ...
  if (length > 0) {
    try {
      ...
      // within the bounds of the memory segment so that transactions cannot
      // pass
      // bad values in attempts to read improper memory
      const char *buffer =
          memoryArrayPtr<const char>(mem, uint32_t(data), uint32_t(length));
      payload.insert(payload.end(), buffer, buffer + length);
    } catch (const Runtime::Exception &e) {
      FC_THROW_EXCEPTION(tx_unknown_argument,
                         "Message data is not a valid memory range");
    }
  }

  auto &pmsg = wasm.current_apply_context->create_pending_message(
      Name(code), Name(type), payload);
  return pmsg.handle;
}

DEFINE_INTRINSIC_FUNCTION3(env, messageRequirePermission,
                           messageRequirePermission, none, i32, handle, i64,
                           account, i64, permission) {
  auto apply_context = wasm_interface::get().current_apply_context;
  // if this is not sent from the code account with the permission of "code"
  // then we must
  // presently have the permission to add it, otherwise its a failure
  if (!(account == apply_context->code.value &&
...
DEFINE_INTRINSIC_FUNCTION2(env, readMessage, readMessage, i32, i32, destptr,
                           i32, destsize) {
  ...
  int minlen =
      std::min<int>(wasm.current_validate_context->msg.data.size(), destsize);
...
DEFINE_INTRINSIC_FUNCTION2(env, assert, assert, none, i32, test, i32, msg) {
  const char *m =
      &Runtime::memoryRef<char>(wasm_interface::get().current_memory, msg);
...
DEFINE_INTRINSIC_FUNCTION1(env, malloc, malloc, i32, i32, size) {
  FC_ASSERT(size > 0);
  int32_t &end = Runtime::memoryRef<int32_t>(
      Runtime::getDefaultMemory(wasm_interface::get().current_module), 0);
  int32_t old_end = end;
  end += 8 * ((size + 7) / 8);
...
DEFINE_INTRINSIC_FUNCTION1(env, prints, prints, none, i32, charptr) {
  ...
  std::cerr << std::string(str,
                           strnlen(str, wasm.current_state->mem_end - charptr));
}

DEFINE_INTRINSIC_FUNCTION2(env, prints_l, prints_l, none, i32, charptr, i32,
                           len) {
  auto &wasm = wasm_interface::get();
  auto mem = wasm.current_memory;
...
DEFINE_INTRINSIC_FUNCTION2(env, printhex, printhex, none, i32, data, i32,
                           datalen) {
  auto &wasm = wasm_interface::get();
  auto mem = wasm.current_memory;
...
char *wasm_interface::vm_allocate(int bytes) {
  FunctionInstance *alloc_function =
      asFunctionNullable(getInstanceExport(current_module, "alloc"));
    ...
    try {
      ...
      */
      ...
      FunctionInstance *call =
          asFunctionNullable(getInstanceExport(current_module, name));
      if (!call) {
        // wlog( "unable to find call ${name}", ("name",name));
        return;
      }
      // FC_ASSERT( apply, "no entry point found for ${call}", ("call",
      // std::string(name))  );
      ...
      std::vector<Value> args = {
          Value(uint64_t(current_validate_context->msg.code)),
...
void wasm_interface::vm_onInit() {
  try {
    try {
      FunctionInstance *apply =
          asFunctionNullable(getInstanceExport(current_module, "init"));
      if (!apply) {
...
void wasm_interface::load(const AccountName &name,
...
#warning TODO: free existing module if the code has been updated, currently leak memory
    ...
    try {
      ...
      Serialization::MemoryInputStream stream((const U8 *)recipient.code.data(),
                                              recipient.code.size());
      WASM::serializeWithInjection(stream, *state.module);

      RootResolver rootResolver;
      LinkResult linkResult = linkModule(*state.module, rootResolver);
      state.instance = instantiateModule(*state.module,
      ...
      const auto allocated_memory =
          Runtime::getDefaultMemorySize(state.instance);
      ...
      memcpy(state.init_memory.data(), memstart,
             state.mem_end); // state.init_memory.size() );
      // std::cerr <<"\n";
      ...
      std::cerr << "Memory allocation failed: input is likely malformed"
                << std::endl;
      throw;
    }
  }
...
U32 wasm_memory::sbrk(I32 num_bytes) {
  ...
  // Round the absolute value of num_bytes to an alignment boundary, and ensure
  // it won't allocate too much or too little memory.
  ...
  // Update the number of bytes allocated, and compute the number of pages
  // needed for it.
  _num_bytes += num_bytes;
  const Uptr num_desired_pages =
      (_num_bytes + IR::numBytesPerPage - 1) >> IR::numBytesPerPageLog2;

  // Grow or shrink the memory object to the desired number of pages.
  if (num_desired_pages > _num_pages)
    Runtime::growMemory(_wasm_interface.current_memory,
                        num_desired_pages - _num_pages);
  else if (num_desired_pages < _num_pages)
    Runtime::shrinkMemory(_wasm_interface.current_memory,
