...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::rem_code_name ) // virtual rem.code permission does not really exist but is allowed
         continue;
 ...
 */
void apply_rem_newaccount(apply_context& context) {
...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   if( !creator.is_privileged() ) {
      EOS_ASSERT( name_str.find( "rem." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'rem.'" );
   }
...
void apply_rem_setcode(apply_context& context) {
...
void apply_rem_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_rem_updateauth(apply_context& context) {
   ...
   EOS_ASSERT( update.permission.to_string().find( "rem." ) != 0, action_validate_exception,
               "Permission names that start with 'rem.' are reserved" );
...
void apply_rem_deleteauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_rem_linkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   try {
      ...
      if( requirement.requirement != config::rem_any_name ) {
...
void apply_rem_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_rem_canceldelay(apply_context& context) {
