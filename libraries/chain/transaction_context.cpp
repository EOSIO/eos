...
namespace eosio { namespace chain {
   ...
   {
      const transaction& trx = packed_trx.get_transaction();
      if( (trx.expiration.time_since_epoch().count() != 0) && (trx.transaction_extensions.size() > 0) ) {
         disallow_transaction_extensions( "no transaction extensions supported yet for deferred transactions" );
      }
      // If (trx.expiration.time_since_epoch().count() == 0) then it was created after NO_DUPLICATE_DEFERRED_ID activation,
