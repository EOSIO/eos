...
/**
 * Only return true *if* the transaction has not expired or been invalidated. If
 * this
 * method is called with a VERY old transaction we will return false, they
 * should
 * query things by blocks if they are that old.
 */
bool chain_controller::is_known_transaction(
...
chain_controller::fetch_block_by_id(const block_id_type &id) const {
...
optional<signed_block>
chain_controller::fetch_block_by_number(uint32_t num) const {
  if (const auto &block = _block_log.read_block_by_num(num))
    return *block;

  // Not in _block_log, so it must be since the last irreversible block. Grab it
  // from _fork_db instead
...
const SignedTransaction &chain_controller::get_recent_transaction(
    const transaction_id_type &trx_id) const {
...
std::vector<block_id_type>
chain_controller::get_block_ids_on_fork(block_id_type head_of_fork) const {
  ...
  if (!((branches.first.back()->previous_id() ==
    ...
    edump((head_of_fork)(head_block_id())(branches.first.size())(
...
const GeneratedTransaction &chain_controller::get_generated_transaction(
    const generated_transaction_id_type &id) const {
  auto &index = _db.get_index<generated_transaction_multi_index,
                              generated_transaction_object::by_trx_id>();
 ...
 */
bool chain_controller::push_block(const signed_block &new_block,
                                  uint32_t skip) {
  try {
    return with_skip_flags(skip, [&]() {
      return without_pending_transactions([&]() {
        return _db.with_write_lock([&]() { return _push_block(new_block); });
      });
    });
  }
...
bool chain_controller::_push_block(const signed_block &new_block) {
  try {
    ...
    if (!(skip & skip_fork_db)) {
      /// TODO: if the block is greater than the head block and before the next
      /// maintenance interval
      // verify that the block signer is in the current set of active producers.

      shared_ptr<fork_item> new_head = _fork_db.push_block(new_block);
      // If the head block from the longest chain does not build off of the
      // current head, we need to switch forks.
      if (new_head->data.previous != head_block_id()) {
        // If the newly pushed block is the same height as head, we get head
        // back in new_head
        // Only switch forks if new_head is actually higher than head
        if (new_head->data.block_num() > head_block_num()) {
          wlog("Switching to fork: ${id}", ("id", new_head->data.id()));
          auto branches =
              _fork_db.fetch_branch_from(new_head->data.id(), head_block_id());
          ...
          for (auto ritr = branches.first.rbegin();
            ...
            if (except) {
              ...
              // remove the rest of branches.first from the fork_db, those
              // blocks are invalid
    ...
    try {
      ...
      if ((fc::time_point::now() - new_block.timestamp) < fc::seconds(60)) {
        ...
        ilog("${producer} #${num} @${time}  | ${trxcount} trx, ${pending} "
             "pending, exectime_ms=${extm}",
             ("producer", new_block.producer)("time", new_block.timestamp)(
                 "num", new_block.block_num())("trxcount", trxcount)(
                 "pending", _pending_transactions.size())("extm",
                                                          exec_ms.count()));
      }
      session.push();
    } catch (const fc::exception &e) {
 ...
 * skip_block_size_check bit on the skip argument. This
 * will allow the transaction to be pushed even if it causes the pending block
 * size to exceed the maximum block size.
 * Although the transaction will probably not propagate further now, as the
 * peers are likely to have their pending
 * queues full as well, it will be kept in the queue to be propagated later when
 * a new block flushes out the pending
 * queues.
 */
ProcessedTransaction
chain_controller::push_transaction(const SignedTransaction &trx,
                                   uint32_t skip) {
  try {
    return with_skip_flags(skip, [&]() {
      return _db.with_write_lock([&]() { return _push_transaction(trx); });
    });
  }
...
ProcessedTransaction
chain_controller::_push_transaction(const SignedTransaction &trx) {
  // If this is the first transaction pushed after applying a block, start a new
  // undo session.
  // This allows us to quickly rewind to the clean state of the head block, in
  // case a new block arrives.
  ...
  // The transaction applied successfully. Merge its changes into the pending
  // block session.
  temp_session.squash();
...
signed_block chain_controller::generate_block(
    fc::time_point_sec when, const AccountName &producer,
    const fc::ecc::private_key &block_signing_private_key,
    block_schedule::factory
        scheduler, /* = block_schedule::by_threading_conflits */
    uint32_t skip  /* = 0 */
    ) {
  try {
    return with_skip_flags(skip, [&]() {
...
signed_block chain_controller::_generate_block(
  ...
  try {
    ...
    if (!(skip & skip_producer_signature))
      FC_ASSERT(producer_obj.signing_key ==
    ...
    const auto &generated =
        _db.get_index<generated_transaction_multi_index,
                      generated_transaction_object::by_status>()
            .equal_range(generated_transaction_object::PENDING);

    vector<pending_transaction> pending;
    std::set<transaction_id_type> invalid_pending;
    pending.reserve(std::distance(generated.first, generated.second) +
                    _pending_transactions.size());
    for (auto iter = generated.first; iter != generated.second; ++iter) {
      const auto &gt = *iter;
      pending.emplace_back(
          std::reference_wrapper<const GeneratedTransaction>{gt.trx});
    }
      ...
      for (const auto &t : c) {
        ...
        for (const auto &trx : t.transactions) {
          try {
            auto temp_session = _db.start_undo_session(true);
            if (trx.contains<
                    std::reference_wrapper<const SignedTransaction>>()) {
              const auto &t =
                  trx.get<std::reference_wrapper<const SignedTransaction>>()
                      .get();
            ...
            } else if (trx.contains<std::reference_wrapper<
                           const GeneratedTransaction>>()) {
              const auto &t =
                  trx.get<std::reference_wrapper<const GeneratedTransaction>>()
                      .get();
              auto processed = apply_transaction(t);
              block_thread.generated_input.emplace_back(processed);
            } else {
              FC_THROW_EXCEPTION(tx_scheduling_exception,
            ...
            elog("Transaction was not processed while generating block due to "
                 "${e}",
                 ("e", e));
            if (trx.contains<
                    std::reference_wrapper<const SignedTransaction>>()) {
        ...
        if (!(block_thread.generated_input.empty() &&
              block_thread.user_input.empty())) {
    ...
    size_t postponed_tx_count =
        pending.size() - valid_transaction_count - invalid_transaction_count;
    if (postponed_tx_count > 0) {
      wlog("Postponed ${n} transactions due to block size limit",
           ("n", postponed_tx_count));
    }

    if (invalid_transaction_count > 0) {
      wlog("Postponed ${n} transactions errors when processing",
           ("n", invalid_transaction_count));

      // remove pending transactions determined to be bad during scheduling
      if (invalid_pending.size() > 0) {
        for (auto itr = _pending_transactions.begin();
             itr != _pending_transactions.end();) {
          auto &tx = *itr;
    ...
    pending_block.transaction_merkle_root =
        pending_block.calculate_merkle_root();

    pending_block.producer = producer_obj.owner;

    // If this block is last in a round, calculate the schedule for the new
    // round
    if (pending_block.block_num() % config::BlocksPerRound == 0) {
      auto new_schedule = _admin->get_next_round(_db);
      pending_block.producer_changes =
          get_global_properties().active_producers - new_schedule;
    }
    ...
    get_global_properties().parameters.maximum_block_size );
    }
    */
...
/**
 * Removes the most recent block from the database and undoes any changes it
 * made.
 */
void chain_controller::pop_block() {
  try {
    ...
    EOS_ASSERT(head_block.valid(), pop_empty_chain,
...
void chain_controller::apply_block(const signed_block &next_block,
                                   uint32_t skip) {
...
struct path_cons_list {
  ...
  path_cons_list(int index, const path_cons_list &rest)
      : head(head_type(index)), tail(tail_ref_type(&rest)) {}

  path_cons_list(const char *path, const path_cons_list &rest)
      : head(head_type(path)), tail(tail_ref_type(&rest)) {}

  path_cons_list(const char *path) : head(head_type(path)), tail() {}
...
void check_output(const T &expected, const T &actual,
                  const path_cons_list &path) {
  try {
    EOS_ASSERT((expected == actual), block_tx_output_exception,
               "expected: ${expected}, actual: ${actual}",
               ("expected", expected)("actual", actual));
  }
  FC_RETHROW_EXCEPTIONS(warn, "at: ${path}",
                        ("path", resolve_path_string(path)));
}
...
template <typename T>
void check_output(const fc::optional<T> &expected,
                  const fc::optional<T> &actual, const path_cons_list &path) {
...
void check_output(const types::Bytes &expected, const types::Bytes &actual,
  ...
  auto cmp_result =
      std::memcmp(expected.data(), actual.data(), expected.size());
  check_output(cmp_result, 0, path("@memcmp()"));
}

template <>
void check_output(const types::AccountPermission &expected,
                  const types::AccountPermission &actual,
                  const path_cons_list &path) {
...
template <>
void check_output(const InlineTransaction &expected,
                  const InlineTransaction &actual, const path_cons_list &path) {
  check_output<Transaction>(expected, actual, path);
  check_output(expected.output, actual.output, path(".output"));
}

template <>
void check_output(const GeneratedTransaction &expected,
                  const GeneratedTransaction &actual,
                  const path_cons_list &path) {
...
void check_output(const MessageOutput &expected, const MessageOutput &actual,
  ...
  check_output(expected.inline_transaction, actual.inline_transaction,
               path(".inline_transaction"));
  check_output(expected.deferred_transactions, actual.deferred_transactions,
               path(".deferred_transactions"));
}

template <typename T>
void chain_controller::check_transaction_output(
    const T &expected, const T &actual, const path_cons_list &path) const {
  ...
  try {
    ...
    FC_ASSERT((skip & skip_merkle_check) ||
                  next_block.transaction_merkle_root ==
                      next_block.calculate_merkle_root(),
              "", ("next_block.transaction_merkle_root",
                   next_block.transaction_merkle_root)(
                      "calc", next_block.calculate_merkle_root())(
                      "next_block", next_block)("id", next_block.id()));

    const producer_object &signing_producer =
        validate_block_header(skip, next_block);
...
auto make_authority_checker(const chainbase::database &db,
                            const flat_set<public_key_type> &signingKeys) {
  auto getPermission = make_get_permission(db);
  auto getAuthority =
      [getPermission](const types::AccountPermission &permission) {
        return getPermission(permission).auth;
      };
  auto depthLimit =
      db.get<global_property_object>().configuration.authDepthLimit;
  return MakeAuthorityChecker(std::move(getAuthority), depthLimit, signingKeys);
}
}

flat_set<public_key_type> chain_controller::get_required_keys(
    ...
    for (const auto &declaredAuthority : message.authorization) {
      if (!checker.satisfied(declaredAuthority)) {
        EOS_ASSERT(checker.satisfied(declaredAuthority), tx_missing_sigs,
                   "Transaction declares authority '${auth}', but does not "
...
void chain_controller::check_transaction_authorization(
    const SignedTransaction &trx, bool allow_unused_signatures) const {
  if ((_skip_flags & skip_transaction_signatures) &&
      (_skip_flags & skip_authority_check)) {
...
#warning TODO: Use a real chain_id here (where is this stored? Do we still need it?)
  auto checker =
      make_authority_checker(_db, trx.get_signature_keys(chain_id_type{}));

  for (const auto &message : trx.messages)
    for (const auto &declaredAuthority : message.authorization) {
      const auto &minimumPermission = lookup_minimum_permission(
          declaredAuthority.account, message.code, message.type);
      if ((_skip_flags & skip_authority_check) == false) {
        const auto &index = _db.get_index<permission_index>().indices();
        EOS_ASSERT(getPermission(declaredAuthority)
                       .satisfies(minimumPermission, index),
                   tx_irrelevant_auth, "Message declares irrelevant authority "
                                       "'${auth}'; minimum authority is ${min}",
  ...
  if (!allow_unused_signatures &&
      (_skip_flags & skip_transaction_signatures) == false)
    EOS_ASSERT(
        checker.all_keys_used(), tx_irrelevant_sig,
        "Transaction bears irrelevant signatures from these keys: ${keys}",
        ("keys", checker.unused_keys()));
}

void chain_controller::validate_scope(const Transaction &trx) const {
  EOS_ASSERT(trx.scope.size() + trx.readscope.size() > 0, transaction_exception,
             "No scope specified by transaction");
  for (uint32_t i = 1; i < trx.scope.size(); ++i)
    EOS_ASSERT(trx.scope[i - 1] < trx.scope[i], transaction_exception,
               "Scopes must be sorted and unique");
  for (uint32_t i = 1; i < trx.readscope.size(); ++i)
    EOS_ASSERT(trx.readscope[i - 1] < trx.readscope[i], transaction_exception,
               "Scopes must be sorted and unique");
  ...
  FC_ASSERT(intersection.size() == 0,
            "a transaction may not redeclare scope in readscope");
}

const permission_object &chain_controller::lookup_minimum_permission(
    types::AccountName authorizer_account, types::AccountName code_account,
  ...
  try {
    ...
    auto permissionKey = boost::make_tuple<AccountName, PermissionName>(
        authorizer_account, "active");
    if (link != nullptr)
...
void chain_controller::validate_uniqueness(
    const GeneratedTransaction &trx) const {
  if (!should_check_for_duplicate_transactions())
    return;
}
...
void chain_controller::record_transaction(const GeneratedTransaction &trx) {
  _db.modify(_db.get<generated_transaction_object,
                     generated_transaction_object::by_trx_id>(trx.id),
             [&](generated_transaction_object &transaction) {
...
void chain_controller::validate_tapos(const Transaction &trx) const {
  if (!should_check_tapos())
    return;

  const auto &tapos_block_summary =
      _db.get<block_summary_object>((uint16_t)trx.refBlockNum);

  // Verify TaPoS block summary has correct ID prefix, and that this block's
  // time is not past the expiration
  EOS_ASSERT(
      transaction_verify_reference_block(trx, tapos_block_summary.block_id),
      transaction_exception, "Transaction's reference block did not match. Is "
                             "this transaction from a different fork?",
      ("tapos_summary", tapos_block_summary));
}

void chain_controller::validate_referenced_accounts(
...
void chain_controller::validate_expiration(const Transaction &trx) const {
  try {
    ...
    const BlockchainConfiguration &chain_configuration =
        get_global_properties().configuration;

    EOS_ASSERT(trx.expiration <=
                   now + int32_t(chain_configuration.maxTrxLifetime),
               transaction_exception,
               "Transaction expiration is too far in the future",
               ("trx.expiration", trx.expiration)("now", now)(
                   "max_til_exp", chain_configuration.maxTrxLifetime));
    EOS_ASSERT(now <= trx.expiration, transaction_exception,
...
void chain_controller::process_message(const Transaction &trx, AccountName code,
  ...
  if (apply_ctx.inline_messages.size() > 0) {
    ...
    (*output.inline_transaction).messages =
        std::move(apply_ctx.inline_messages);
  }
  ...
  for (auto &asynctrx : apply_ctx.deferred_transactions) {
    ...
    _db.create<generated_transaction_object>(
        [&](generated_transaction_object &transaction) {
  ...
  // process_message recurses for each notified account, but we only want to run
  // this check at the top level
  if (parent_context == nullptr &&
      (_skip_flags & skip_authority_check) == false)
    EOS_ASSERT(
        apply_ctx.all_authorizations_used(), tx_irrelevant_auth,
...
void chain_controller::apply_message(apply_context &context) {
  try {
    ...
    if (contract_handlers_itr != apply_handlers.end()) {
      auto message_handler_itr =
          contract_handlers_itr->second.find({m.code, m.type});
    ...
    if (recipient.code.size()) {
      ...
      const uint32_t execution_time =
          _skip_flags | received_block
              ...
              : _skip_flags | created_block ? _create_block_trans_execution_time
...
/**
 *  @pre the transaction is assumed valid and all signatures / duplicate checks
 * have bee performed
 */
template <typename T>
typename T::Processed
chain_controller::process_transaction(const T &trx, int depth,
                                      const fc::time_point &start_time) {
  try {
    const BlockchainConfiguration &chain_configuration =
        get_global_properties().configuration;
    EOS_ASSERT(
        (fc::time_point::now() - start_time).count() <
            chain_configuration.maxTrxRuntime,
        checktime_exceeded,
        "Transaction exceeded maximum total transaction time of ${limit}ms",
        ("limit", chain_configuration.maxTrxRuntime));

    EOS_ASSERT(
        depth < chain_configuration.inlineDepthLimit, tx_resource_exhausted,
        "Transaction exceeded maximum inline recursion depth of ${limit}",
        ("limit", chain_configuration.inlineDepthLimit));
    ...
    for (uint32_t i = 0; i < trx.messages.size(); ++i) {
      ...
      if (output.inline_transaction.valid()) {
        ...
        output.inline_transaction = process_transaction(
            PendingInlineTransaction(trx), depth + 1, start_time);
      }
    }
...
chain_controller::validate_block_header(uint32_t skip,
                                        const signed_block &next_block) const {
  EOS_ASSERT(
      head_block_id() == next_block.previous, block_validate_exception, "",
      ("head_block_id", head_block_id())("next.prev", next_block.previous));
  EOS_ASSERT(
      head_block_time() < next_block.timestamp, block_validate_exception, "",
      ("head_block_time", head_block_time())("next", next_block.timestamp)(
          "blocknum", next_block.block_num()));
  if (next_block.block_num() % config::BlocksPerRound != 0) {
    ...
    EOS_ASSERT(is_sorted(keys(next_block.producer_changes)) &&
                   is_sorted(values(next_block.producer_changes)),
               block_validate_exception,
               "Producer changes are not sorted correctly",
               ("changes", next_block.producer_changes));
  }
  const producer_object &producer = get_producer(
      get_scheduled_producer(get_slot_at_time(next_block.timestamp)));

  if (!(skip & skip_producer_signature))
    EOS_ASSERT(
        next_block.validate_signee(producer.signing_key),
        block_validate_exception,
  ...
  if (!(skip & skip_producer_schedule_check)) {
    EOS_ASSERT(next_block.producer == producer.owner, block_validate_exception,
               ...
               ("block producer", next_block.producer)("scheduled producer",
                                                       producer.owner));
  }
...
void chain_controller::create_block_summary(const signed_block &next_block) {
  ...
  _db.modify(_db.get<block_summary_object, by_id>(sid),
             [&](block_summary_object &p) { p.block_id = next_block.id(); });
}

void chain_controller::update_global_properties(const signed_block &b) {
  // If we're at the end of a round, update the BlockchainConfiguration,
  // producer schedule
  ...
  if (b.block_num() % config::BlocksPerRound == 0) {
    ...
    _db.modify(gpo,
               [ schedule = std::move(schedule),
                 config = std::move(config) ](global_property_object & gpo) {
                 gpo.active_producers = std::move(schedule);
                 gpo.configuration = std::move(config);
               });

    auto active_producers_authority =
        types::Authority(config::ProducersAuthorityThreshold, {}, {});
    for (auto &name : gpo.active_producers) {
      active_producers_authority.accounts.push_back(
          {{name, config::ActiveName}, 1});
    }

    auto &po = _db.get<permission_object, by_owner>(
        boost::make_tuple(config::ProducersAccountName, config::ActiveName));
...
void chain_controller::add_checkpoints(
...
chain_controller::get_dynamic_global_properties() const {
...
const producer_object &
chain_controller::get_producer(const types::AccountName &ownerName) const {
  ...
  try {
    if (!_db.find<global_property_object>()) {
      _db.with_write_lock([this, &starter] {
        ...
        FC_ASSERT(initial_timestamp != time_point_sec(),
                  "Must initialize genesis timestamp.");
        FC_ASSERT(initial_timestamp.sec_since_epoch() %
                          config::BlockIntervalSeconds ==
                      0,
                  "Genesis timestamp must be divisible by "
                  "config::BlockIntervalSeconds.");

        // Create global properties
        _db.create<global_property_object>(
            [&starter](global_property_object &p) {
              p.configuration = starter.get_chain_start_configuration();
              p.active_producers = starter.get_chain_start_producers();
            });
        _db.create<dynamic_global_property_object>(
        ...
        std::for_each(messages.begin(), messages.end(), [&](const Message &m) {
          ...
          with_skip_flags(skip_scope_check | skip_transaction_signatures |
                              skip_authority_check | received_block,
...
chain_controller::chain_controller(
    database &database, fork_database &fork_db, block_log &blocklog,
    chain_initializer_interface &starter,
    unique_ptr<chain_administration_interface> admin,
    ...
    : _db(database), _fork_db(fork_db), _block_log(blocklog),
      _admin(std::move(admin)), _trans_execution_time(trans_execution_time),
      _rcvd_block_trans_execution_time(rcvd_block_trans_execution_time),
      _create_block_trans_execution_time(create_block_trans_execution_time) {

  initialize_indexes();
  starter.register_types(*this, _db);

  // Behave as though we are applying a block during chain initialization (it's
  // the genesis block!)
  with_applying_block([&] { initialize_chain(starter); });

  spinup_db();
  spinup_fork_db();

  if (_block_log.read_head() &&
      head_block_num() < _block_log.read_head()->block_num())
    replay();
}
...
void chain_controller::replay() {
  ...
  for (uint32_t i = 1; i <= last_block_num; ++i) {
    if (i % 5000 == 0)
      std::cerr << "   " << double(i * 100) / last_block_num << "%   " << i
                << " of " << last_block_num << "   \n";
    fc::optional<signed_block> block = _block_log.read_block_by_num(i);
    FC_ASSERT(block, "Could not find block #${n} in block_log!", ("n", i));
    apply_block(*block,
                skip_producer_signature | skip_transaction_signatures |
                    skip_transaction_dupe_check | skip_tapos_check |
                    skip_producer_schedule_check | skip_authority_check |
                    received_block);
  }
...
void chain_controller::spinup_db() {
  ...
  _db.with_write_lock([&] {
    ...
    FC_ASSERT(_db.revision() == head_block_num(),
...
void chain_controller::spinup_fork_db() {
  ...
  if (last_block.valid()) {
    ...
    if (last_block->id() != head_block_id()) {
      FC_ASSERT(head_block_num() == 0,
                "last block ID does not match current chain state",
                ("last_block->id", last_block->id())("head_block_num",
                                                     head_block_num()));
    }
  }
}
...
ProducerRound
chain_controller::calculate_next_round(const signed_block &next_block) {
  auto schedule = _admin->get_next_round(_db);
  auto changes = get_global_properties().active_producers - schedule;
  EOS_ASSERT(boost::range::equal(next_block.producer_changes, changes),
             block_validate_exception,
             "Unexpected round changes in new block header",
             ("expected changes", changes)("block changes",
                                           next_block.producer_changes));
...
void chain_controller::update_global_dynamic_data(const signed_block &b) {
  const dynamic_global_property_object &_dgp =
      _db.get<dynamic_global_property_object>();

  uint32_t missed_blocks =
      head_block_num() == 0 ? 1 : get_slot_at_time(b.timestamp);
  assert(missed_blocks != 0);
  missed_blocks--;

  //   if (missed_blocks)
  //      wlog("Blockchain continuing after gap of ${b} missed blocks", ("b",
  //      missed_blocks));
  ...
  for (uint32_t i = 0; i < missed_blocks; ++i) {
    ...
    if (producer_missed.owner != b.producer) {
      ...
      if( (fc::time_point::now() - b.timestamp) < fc::seconds(30) )
         wlog( "Producer ${name} missed block ${n} around ${t}",
      ("name",producer_account.name)("n",b.block_num())("t",b.timestamp) );
         */

      _db.modify(producer_missed,
                 [&](producer_object &w) { w.total_missed++; });
    }
  }
  ...
  _db.modify(_dgp, [&](dynamic_global_property_object &dgp) {
    ...
    // If we've missed more blocks than the bitmap stores, skip calculations and
    // simply reset the bitmap
  ...
  _fork_db.set_max_size(_dgp.head_block_number -
                        _dgp.last_irreversible_block_num + 1);
}

void chain_controller::update_signing_producer(
    const producer_object &signing_producer, const signed_block &new_block) {
  const dynamic_global_property_object &dpo = get_dynamic_global_properties();
  uint64_t new_block_aslot =
      dpo.current_absolute_slot + get_slot_at_time(new_block.timestamp);
...
void chain_controller::update_last_irreversible_block() {
  ...
  std::transform(
      gpo.active_producers.begin(), gpo.active_producers.end(),
      std::back_inserter(producer_objs),
      [this](const AccountName &owner) { return &get_producer(owner); });

  static_assert(config::IrreversibleThresholdPercent > 0,
                "irreversible threshold must be nonzero");

  size_t offset =
      EOS_PERCENT(producer_objs.size(),
                  config::Percent100 - config::IrreversibleThresholdPercent);
  std::nth_element(producer_objs.begin(), producer_objs.begin() + offset,
                   producer_objs.end(),
  ...
  uint32_t new_last_irreversible_block_num =
      producer_objs[offset]->last_confirmed_block_num;
  ...
  // Write newly irreversible blocks to disk. First, get the number of the last
  // block on disk...
  ...
  if (last_block_on_disk < new_last_irreversible_block_num)
    for (auto block_to_write = last_block_on_disk + 1;
         block_to_write <= new_last_irreversible_block_num; ++block_to_write) {
...
void chain_controller::clear_expired_transactions() {
  try {
    ...
    // Transactions must have expired by at least two forking windows in order
    // to be removed.
    auto &transaction_idx = _db.get_mutable_index<transaction_multi_index>();
    const auto &dedupe_index = transaction_idx.indices().get<by_expiration>();
    while ((!dedupe_index.empty()) &&
           (head_block_time() > dedupe_index.rbegin()->trx.expiration))
      transaction_idx.remove(*dedupe_index.rbegin());

    // Look for expired transactions in the pending generated list, and remove
    // them.
    // Transactions must have expired by at least two forking windows in order
    // to be removed.
    auto &generated_transaction_idx =
        _db.get_mutable_index<generated_transaction_multi_index>();
    const auto &generated_index =
        generated_transaction_idx.indices()
            .get<generated_transaction_object::by_expiration>();
    while ((!generated_index.empty()) &&
           (head_block_time() > generated_index.rbegin()->trx.expiration))
...
types::AccountName
chain_controller::get_scheduled_producer(uint32_t slot_num) const {
  const dynamic_global_property_object &dpo = get_dynamic_global_properties();
...
uint32_t chain_controller::producer_participation_rate() const {
  const dynamic_global_property_object &dpo = get_dynamic_global_properties();
  return uint64_t(config::Percent100) *
         __builtin_popcountll(dpo.recent_slots_filled) / 64;
}

void chain_controller::set_apply_handler(const AccountName &contract,
                                         const AccountName &scope,
...
ProcessedTransaction
chain_controller::transaction_from_variant(const fc::variant &v) const {
  const variant_object &vo = v.get_object();
#define GET_FIELD(VO, FIELD, RESULT)                                           \
  if (VO.contains(#FIELD))                                                     \
  fc::from_variant(VO[#FIELD], RESULT.FIELD)
    ...
    for (uint32_t i = 0; i < msgs.size(); ++i) {
      ...
      if (vo.contains("data")) {
        ...
        if (data.is_string()) {
          ...
          result.messages[i].data = message_to_binary(
              result.messages[i].code, result.messages[i].type, data);
          /*
          const auto& code_account = _db.get<account_object,by_name>(
          result.messages[i].code );
          if( code_account.abi.size() > 4 ) { /// 4 == packsize of empty Abi
             fc::datastream<const char*> ds( code_account.abi.data(),
          code_account.abi.size() );
             eos::types::Abi abi;
             fc::raw::unpack( ds, abi );
             types::AbiSerializer abis( abi );
             result.messages[i].data = abis.variantToBinary( abis.getActionType(
          result.messages[i].type ), data );
          }
          */
        }
      }
...
vector<char> chain_controller::message_to_binary(Name code, Name type,
                                                 const fc::variant &obj) const {
  try {
...
fc::variant
chain_controller::message_from_binary(Name code, Name type,
...
fc::variant chain_controller::transaction_to_variant(
    const ProcessedTransaction &trx) const {
