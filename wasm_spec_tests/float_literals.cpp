...
const string wasm_str_float_literals_0 = base_dir + "/wasm_spec_tests/wasms/float_literals.0.wasm";
...
BOOST_DATA_TEST_CASE(float_literals_0_pass, boost::unit_test::data::xrange(0,1), index) { try {
   ...
   tester.produce_block();
   tester.set_code(N(wasmtest), wasm_float_literals_0);
   tester.produce_block();
...
const string wasm_str_float_literals_1 = base_dir + "/wasm_spec_tests/wasms/float_literals.1.wasm";
...
BOOST_DATA_TEST_CASE(float_literals_1_pass, boost::unit_test::data::xrange(0,1), index) { try {
   ...
   tester.create_account( N(wasmtest) );
   tester.produce_block();
   tester.set_code(N(wasmtest), wasm_float_literals_1);
   tester.produce_block();
