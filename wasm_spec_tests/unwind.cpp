...
const string wasm_str_unwind_0 = base_dir + "/wasm_spec_tests/wasms/unwind.0.wasm";
std::vector<uint8_t> wasm_unwind_0= read_wasm(wasm_str_unwind_0.c_str());

BOOST_DATA_TEST_CASE(unwind_0_check_throw, boost::unit_test::data::xrange(0,8), index) { try {
   TESTER tester;
   tester.produce_block();
   tester.create_account( N(wasmtest) );
   tester.produce_block();
   tester.set_code(N(wasmtest), wasm_unwind_0);
   tester.produce_block();
...
BOOST_DATA_TEST_CASE(unwind_0_pass, boost::unit_test::data::xrange(8,9), index) { try {
   TESTER tester;
   tester.produce_block();
   tester.create_account( N(wasmtest) );
   tester.produce_block();
