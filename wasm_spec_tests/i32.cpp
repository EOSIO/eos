...
const string wasm_str_i32_0 = base_dir + "/wasm_spec_tests/wasms/i32.0.wasm";
std::vector<uint8_t> wasm_i32_0= read_wasm(wasm_str_i32_0.c_str());

BOOST_DATA_TEST_CASE(i32_0_check_throw, boost::unit_test::data::xrange(0,9), index) { try {
   TESTER tester;
   tester.produce_block();
   tester.create_account( N(wasmtest) );
   tester.produce_block();
   tester.set_code(N(wasmtest), wasm_i32_0);
   tester.produce_block();
...
BOOST_DATA_TEST_CASE(i32_0_pass, boost::unit_test::data::xrange(9,13), index) { try {
   TESTER tester;
   tester.produce_block();
   tester.create_account( N(wasmtest) );
   tester.produce_block();
