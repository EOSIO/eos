...
const string wasm_str_globals_0 = base_dir + "/wasm_spec_tests/wasms/globals.0.wasm";
...
BOOST_DATA_TEST_CASE(globals_0_check_throw, boost::unit_test::data::xrange(0,1), index) { try {
   ...
   tester.produce_block();
   tester.set_code(N(wasmtest), wasm_globals_0);
   tester.produce_block();
...
BOOST_DATA_TEST_CASE(globals_0_pass, boost::unit_test::data::xrange(1,2), index) { try {
   TESTER tester;
   tester.produce_block();
   tester.create_account( N(wasmtest) );
   tester.produce_block();
