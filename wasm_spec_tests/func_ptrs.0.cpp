...
BOOST_AUTO_TEST_SUITE(func_ptrs_0)

const string wasm_str = base_dir + "/wasm_spec_tests/wasms/func_ptrs.0.wasm";
std::vector<uint8_t> wasm = read_wasm(wasm_str.c_str());

BOOST_AUTO_TEST_CASE(func_ptrs_0_sub_apply_0) { try {
   TESTER tester;
