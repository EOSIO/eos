...
BOOST_AUTO_TEST_SUITE(memory_grow_4)

const string wasm_str = base_dir + "/wasm_spec_tests/wasms/memory_grow.4.wasm";
std::vector<uint8_t> wasm = read_wasm(wasm_str.c_str());

BOOST_AUTO_TEST_CASE(memory_grow_4_sub_apply_0) { try {
   TESTER tester;
...
BOOST_AUTO_TEST_CASE(memory_grow_4_sub_apply_1) { try {
   TESTER tester;
   tester.produce_block();
   tester.create_account( N(wasmtest) );
   tester.produce_block();
