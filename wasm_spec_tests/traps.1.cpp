...
BOOST_AUTO_TEST_SUITE(traps_1)

const string wasm_str = base_dir + "/wasm_spec_tests/wasms/traps.1.wasm";
std::vector<uint8_t> wasm = read_wasm(wasm_str.c_str());

BOOST_AUTO_TEST_CASE(traps_1_sub_apply_0) { try {
   TESTER tester;
...
BOOST_AUTO_TEST_CASE(traps_1_sub_apply_1) { try {
   TESTER tester;
   tester.produce_block();
   tester.create_account( N(wasmtest) );
   tester.produce_block();
...
BOOST_AUTO_TEST_CASE(traps_1_sub_apply_2) { try {
   TESTER tester;
   tester.produce_block();
   tester.create_account( N(wasmtest) );
   tester.produce_block();
...
BOOST_AUTO_TEST_CASE(traps_1_sub_apply_3) { try {
   TESTER tester;
   tester.produce_block();
   tester.create_account( N(wasmtest) );
   tester.produce_block();
