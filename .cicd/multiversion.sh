...
[[ ! -d $EOS_LOCATION/eos_multiversion_builder ]] && mkdir $EOS_LOCATION/eos_multiversion_builder
# pipeline config
...
if [[ -f "$PIPELINE_CONFIG" ]]; then
    ...
    mv -f $EOS_LOCATION/multiversion.conf $EOS_LOCATION/tests
...
cd $EOS_LOCATION
cp $EOS_LOCATION/tests/multiversion_paths.conf $EOS_LOCATION/build/tests
cd $EOS_LOCATION/build
# count tests
