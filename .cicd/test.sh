...
if [[ $(uname) == 'Darwin' ]]; then # macOS
    ...
    echo "$ docker run --rm --init -v $(pwd):$MOUNTED_DIR -v $(buildkite-intrinsics) -e JOBS $FULL_TAG bash -c \"$COMMANDS\""
    set +e # defer error handling to end
    eval docker run --rm --init -v $(pwd):$MOUNTED_DIR -v $(buildkite-intrinsics) -e JOBS $FULL_TAG bash -c \"$COMMANDS\"
    EXIT_STATUS=$?
fi
# buildkite
