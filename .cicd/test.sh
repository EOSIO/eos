...
if [[ $(uname) == 'Darwin' ]]; then # macOS
    ...
    echo "$ docker run --rm --init -v -e JOBS $(pwd):$MOUNTED_DIR $(buildkite-intrinsics) $FULL_TAG bash -c \"$MOUNTED_DIR/$@\""
    set +e # defer error handling to end
    eval docker run --rm --init -v -e JOBS $(pwd):$MOUNTED_DIR $(buildkite-intrinsics) $FULL_TAG bash -c \"$MOUNTED_DIR/$@\"
    EXIT_STATUS=$?
fi
# buildkite
