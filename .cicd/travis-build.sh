...
if [[ "$(uname)" == Darwin ]]; then
    ...
    # DOCKER
    docker run --rm -v $(pwd):/eos -v /usr/lib/ccache -v $HOME/.ccache:/opt/.ccache -e CCACHE_LOGFILE=/dev/stdout -e CCACHE_DIR=/opt/.ccache eosio/producer:ci-$IMAGE_TAG bash -c " \
