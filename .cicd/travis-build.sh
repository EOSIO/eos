...
if [[ "$(uname)" == Darwin ]]; then
    ...
    [[ $IMAGE_TAG == ubuntu-16.04 ]] && EXPORTS="export CCACHE_EXTRAFILES=\"/eos/unittests/wasm_tests.cpp:/eos/unittests/incbin.h:/eos/plugins/producer_api_plugin/producer_api_plugin.cpp\" &&" && CMAKE_EXTRAS="$CMAKE_EXTRAS -DCMAKE_TOOLCHAIN_FILE='/tmp/pinned_toolchain.cmake' -DCMAKE_CXX_COMPILER_LAUNCHER=ccache" # pinned only
