...
if [[ "$(uname)" == Darwin ]]; then
    ...
    echo "$ docker pull eosio/producer:ci-${IMAGE_TAG}${IMAGE_SUFFIX}"
    docker pull eosio/producer:ci-${IMAGE_TAG}${IMAGE_SUFFIX}
    ...
    # DOCKER
    docker run --rm -v $(pwd):/eos -v /usr/lib/ccache -v $HOME/.ccache:/opt/.ccache -e CCACHE_DIR=/opt/.ccache eosio/producer:ci-${IMAGE_TAG}${IMAGE_SUFFIX} bash -c " \
