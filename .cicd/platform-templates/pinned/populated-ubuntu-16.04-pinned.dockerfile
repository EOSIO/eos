...
RUN export EOSIO_LOCATION=$HOME/eosio
RUN git clone git@github.com:EOSIO/eos.git -b develop $EOSIO_INSTALL_LOCATION
RUN export EOSIO_INSTALL_LOCATION=$EOSIO_LOCATION/install
RUN mkdir $EOSIO_INSTALL_LOCATION
RUN apt-get update && \
    apt-get upgrade -y && \
...
RUN git clone --single-branch --branch release_80 https://git.llvm.org/git/llvm.git clang8 && cd clang8 && git checkout 18e41dc && \
    ...
    rm -rf clang8
RUN cp -rfp $EOSIO_LOCATION/scripts/pinned_toolchain.cmake $EOSIO_INSTALL_LOCATION/pinned_toolchain.cmake
...
ENV PATH=$PATH:$EOSIO_INSTALL_LOCATION/mongodb-linux-x86_64-ubuntu1604-3.6.3/bin
