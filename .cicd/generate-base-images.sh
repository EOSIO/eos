...
if [[ $(uname) == 'Darwin' ]]; then # macOS
    [[ "$BUILD_TYPE" == 'conan' ]] && sed -n '/## Environment/,/## Build/p' $CONAN_DIR/macos-10.14-$BUILD_TYPE.md | grep -v -e '```' -e '\#\#' -e '^$' >> $CICD_DIR/platforms/$BUILD_TYPE/macos-10.14-conan.sh
    . $CICD_DIR/platforms/$BUILD_TYPE/macos-10.14-$BUILD_TYPE.sh
else # Linux
    [[ "$BUILD_TYPE" == 'conan' ]] && sed -n '/## Environment/,/## Build/p' $CONAN_DIR/$IMAGE_TAG.md | grep -v -e '```' -e '\#\#' -e '^$' -e 'export' | sed -e 's/^/RUN /' >> $CICD_DIR/platforms/$BUILD_TYPE/$IMAGE_TAG.dockerfile
    ...
    if [[ $EXISTS =~ '404 page not found' || $EXISTS =~ 'manifest unknown' || $FORCE_BASE_IMAGE == 'true' ]]; then # if we cannot pull the image, we build and push it first
        docker build -t $FULL_TAG -f $CICD_DIR/platforms/$BUILD_TYPE/${IMAGE_TAG}.dockerfile .
