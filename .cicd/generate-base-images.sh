...
if [[ $(uname) == 'Darwin' ]]; then # macOS
    [[ "$PLATFORM_TYPE" == 'conan' ]] && sed -n '/## Environment/,/## Build/p' $CONAN_DIR/macos-10.14-$PLATFORM_TYPE.md | grep -v -e '```' -e '\#\#' -e '^$' >> $CICD_DIR/platforms/$PLATFORM_TYPE/macos-10.14-$PLATFORM_TYPE.sh
    . $CICD_DIR/platforms/$PLATFORM_TYPE/macos-10.14-$PLATFORM_TYPE.sh
else # Linux
