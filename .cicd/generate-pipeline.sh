...
cat <<EOF
  ...
  - label: ":boat: Launcher service tests"
    command: |
      ...
      # Put the contracts in the appropriate place.
      mkdir -p eosio.contracts/build/
      pushd eosio.contracts/build/
      ln -s ../../eos/build/unittests/contracts/ contracts
      popd

      # Run the launcher service in the background.
      pushd eos/build/
