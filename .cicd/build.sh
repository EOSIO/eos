...
if [[ $(uname) == 'Darwin' ]]; then
    ...
    if [[ "$USE_CONAN" == 'true' ]]; then
        sed -n '/```/,/```/p' $CONAN_DIR/MACOS-10.14.md | grep -v '```' >> $CONAN_DIR/conan-build.sh
    ...
    if [[ $IMAGE_TAG == 'amazon_linux-2' ]]; then
        ...
        sed -n '/```/,/```/p' $CONAN_DIR/AMAZON_LINUX-2.md | grep -v '```' >> $CONAN_DIR/conan-build.sh
    elif [[ $IMAGE_TAG == 'centos-7.6-conan' ]]; then
        sed -n '/```/,/```/p' $CONAN_DIR/CENTOS-7.6.md | grep -v '```' >> $CONAN_DIR/conan-build.sh
    elif [[ $IMAGE_TAG == 'ubuntu-18.04-conan' ]]; then
        sed -n '/```/,/```/p' $CONAN_DIR/UBUNTU-18.04.md | grep -v '```' >> $CONAN_DIR/conan-build.sh
    fi
    ...
    [[ "$USE_CONAN" == 'true' ]] && COMMANDS="$CONAN_DIR/conan-build.sh"
