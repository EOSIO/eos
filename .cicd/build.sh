...
if [[ "$(uname)" == 'Darwin' ]]; then
    ...
    if [[ $TRAVIS == true ]]; then
        ...
        echo "export NVM_DIR=~/.nvm" >> ~/.bash_profile && echo "source \$(brew --prefix nvm)/nvm.sh" >> ~/.bash_profile && cat ~/.bash_profile && . ~/.bash_profile && echo $NVM_DIR && nvm install --lts=dubnium # Support ship_test
    else
