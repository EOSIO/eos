...
if [[ "$(uname)" == 'Darwin' ]]; then
    ...
    if [[ $TRAVIS == true ]]; then
        ...
        echo "export NVM_DIR=\$HOME/.nvm" >> ~/.bash_profile && echo ". \"/usr/local/opt/nvm/nvm.sh\"" >> ~/.bash_profile && cat ~/.bash_profile && source ~/.bash_profile && echo $NVM_DIR && nvm install --lts=dubnium # Support ship_test
    else
