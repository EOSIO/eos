...
const char* error_advice_public_key_type_exception = R"=====(Public key should be encoded in base58 and starts with RSN prefix)=====";
...
const char* error_advice_authority_type_exception = R"=====(Ensure that your authority JSON is valid follows the following format!
{
  ...
  "keys": [         <keys must be alpha-numerically sorted by their string representations and unique>
    ...
    {
      "key":        <STRING: RSN compatible Public Key>,
...
You can refer to contracts/arisenlib/transaction.hpp for reference)=====";
...
const char* error_advice_tx_duplicate =  "You can try embedding arisen nonce action inside your transaction to ensure uniqueness.";

const char* error_advice_invalid_action_args_exception = R"=====(Ensure that your arguments follow the contract abi!
You can check the contract's abi by using 'arisecli get code' command.)=====";
...
bool print_recognized_errors(const fc::exception& e, const bool verbose_errors) {
   ...
   // refer to libraries/chain/include/arisen/chain/exceptions.hpp
