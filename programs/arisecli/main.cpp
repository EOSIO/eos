...
Options:
  ...
  --wallet-url TEXT=http://localhost:12618/
                              the http/https URL where aWallet is running
...
Subcommands:
  ...
  transfer                    Transfer RSN from account to account
...
//copy pasta from awallet's main.cpp
...
fc::variant call( const std::string& url,
   ...
   try {
      ...
      if(url == ::url)
         std::cerr << localized("Failed to connect to aOS at ${u}; is aOS running?", ("u", url)) << std::endl;
      else if(url == ::wallet_url)
         std::cerr << localized("Failed to connect to aWallet at ${u}; is aWallet running?", ("u", url)) << std::endl;
...
bytes json_or_file_to_bin( const account_name& account, const action_name& action, const string& data_or_filename ) {
   ...
   if( !data_or_filename.empty() ) {
      ...
      } RSN_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data_or_filename));
   }
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } RSN_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'", ("data",authorityJsonOrFile))
}
...
void try_local_port( const string& lo_address, uint16_t port, uint32_t duration ) {
   ...
   while ( !local_port_used(lo_address, port)) {
      if (duration_cast<std::chrono::milliseconds>( system_clock::now().time_since_epoch()).count() - start_time > duration ) {
         std::cerr << "Unable to connect to awallet, if awallet is running please kill the process and try again.\n";
         throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to awallet")});
      }
   }
}

void ensure_awallet_running(CLI::App* app) {
    if (no_auto_awallet)
        return;
    // get, version, net do not require awallet
    ...
    for (const auto& addr: resolved_url.resolved_addresses)
       if (local_port_used(addr, resolved_url.resolved_port))  // Hopefully taken by awallet
          return;
    ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if arisecli and awallet are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("awallet").append(key_store_executable_name);
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        std::cerr << "No wallet service listening on " << lo_address << ":" << std::to_string(resolved_url.resolved_port)
                  << ". Cannot automatically start awallet because awallet was not found." << std::endl;
    }
}
...
CLI::callback_t obsoleted_option_host_port = [](CLI::results_t) {
   std::cerr << localized("Host and port options (-H, --wallet-host, etc.) have been replaced with -u/--url and --wallet-url\n"
                          "Use for example -u http://localhost:12618 or --url https://example.invalid/\n");
   exit(1);
   return false;
};
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake-net", stake_net,
                                   (localized("The amount of RSN delegated for net bandwidth")))->required();
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of RSN to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of RSN to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_option("--buyram", buy_ram_amount, localized("The amount of RSN to buyram"));
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake RSN to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of RSN to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of RSN to undelegate for CPU bandwidth"))->required();
...
struct bidname_subcommand {
   ...
   bidname_subcommand(CLI::App *actionRoot) {
      ...
      bidname->add_option("bid", bid_amount, localized("The amount of RSN to bid"))->required();
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("amount", amount, localized("The amount of RSN to pay for RAM, or number of kbytes of RAM if --kbytes is set"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive RSN for sold RAM"))->required();
...
int main( int argc, char** argv ) {
   ...
   app.add_option( "--wallet-host", obsoleted_option_host_port, localized("the host where awallet is running") )->group("hidden");
   app.add_option( "--wallet-port", obsoleted_option_host_port, localized("the port where awallet is running") )->group("hidden");

   app.add_option( "-u,--url", url, localized("the http/https URL where aOS is running"), true );
   app.add_option( "--wallet-url", wallet_url, localized("the http/https URL where awallet is running"), true );

   app.add_option( "-r,--header", header_opt_callback, localized("pass specific HTTP header; repeat this option to pass multiple headers"));
   app.add_flag( "-n,--no-verify", no_verify, localized("don't verify peer certificate when using HTTPS"));
   app.add_flag( "--no-auto-awallet", no_auto_awallet, localized("don't automatically launch a awallet if one is not currently running"));
   app.set_callback([&app]{ ensure_awallet_running(&app);});
   ...
   pack_transaction->add_flag("--pack-action-data", pack_action_data_flag, localized("Pack all action data within transaction, needs interaction with aOS"));
   pack_transaction->set_callback([&] {
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string( plain_signed_transaction_json );
      } RSN_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to parse plain transaction JSON '${data}'", ("data", plain_signed_transaction_json))
   ...
   unpack_transaction->add_flag("--unpack-action-data", unpack_action_data_flag, localized("Unpack all action data within transaction, needs interaction with aOS"));
   unpack_transaction->set_callback([&] {
      ...
      } RSN_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to parse packed transaction JSON '${data}'", ("data", packed_transaction_json))
   ...
   getCode->set_callback([&] {
      ...
      try {
         ...
         if(code_as_wasm) {
            ...
            std::cout << localized("Warning: communicating to older aOS which returns malformed binary wasm") << std::endl;
         }
         else
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        RSN_ASSERT( !wasm.empty(), wast_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   string memo;
   bool pay_ram = false;
   auto transfer = app.add_subcommand("transfer", localized("Transfer RSN from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending RSN"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving RSN"))->required();
   transfer->add_option("amount", amount, localized("The amount of RSN to send"))->required();
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         ...
         RSN_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
      }
   ...
   auto stopAwallet = wallet->add_subcommand("stop", localized("Stop aWallet (doesn't work with aOS)."), false);
   stopAwallet->set_callback([] {
      const auto& v = call(wallet_url, awallet_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success awallet responds with empty object
   ...
   sign->set_callback([&] {
      ...
      if( str_chain_id.size() == 0 ) {
         ilog( "grabbing chain_id from aOS" );
         auto info = get_info();
   ...
   actionsSubcommand->set_callback([&] {
      ...
      if( !data.empty() ) {
         ...
         } RSN_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data))
      }
   ...
   trxSubcommand->set_callback([&] {
      ...
      } RSN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_push))
      try {
         signed_transaction trx = trx_var.as<signed_transaction>();
   ...
   trxsSubcommand->set_callback([&] {
      ...
      } RSN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trxsJson))
   ...
   propose_action->set_callback([&] {
      ...
      } RSN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",requested_perm))
      fc::variant transaction_perm_var;
      try {
         transaction_perm_var = json_from_file_or_string(transaction_perm);
      } RSN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",transaction_perm))
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string(proposed_transaction);
      } RSN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",proposed_transaction))
      ...
      send_actions({chain::action{accountPermissions, "arisen.msig", "propose", variant_to_bin( N(arisen.msig), N(propose), args ) }});
   });
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      } RSN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",perm))
      auto args = fc::mutable_variant_object()
         ("proposer", proposer)
         ("proposal_name", proposal_name)
         ("level", perm_var);
      ...
      send_actions({chain::action{accountPermissions, "arisen.msig", action, variant_to_bin( N(arisen.msig), action, args ) }});
   };
   ...
   cancel->set_callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, "arisen.msig", "cancel", variant_to_bin( N(arisen.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "arisen.msig", "exec", variant_to_bin( N(arisen.msig), N(exec), args ) }});
      }
   );
   ...
   sudo_exec->set_callback([&] {
      ...
      } RSN_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_exec))
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send arisen.system contract action to the blockchain."), false);
