...
struct Error : public std::runtime_error {
  ...
  Error(std::string parent, std::string name,
        ExitCodes exit_code = ExitCodes::BaseClass, bool print_help = true)
      : runtime_error(parent + ": " + name),
        exit_code(static_cast<int>(exit_code)), print_help(print_help) {}
  Error(std::string parent, std::string name,
...
struct ConstructionError : public Error {
  ...
  ConstructionError(std::string parent, std::string name,
...
struct IncorrectConstruction : public ConstructionError {
  IncorrectConstruction(std::string name)
      : ConstructionError("IncorrectConstruction", name,
                          ExitCodes::IncorrectConstruction) {}
};
...
struct OptionAlreadyAdded : public ConstructionError {
  OptionAlreadyAdded(std::string name)
      : ConstructionError("OptionAlreadyAdded", name,
...
/// Thrown when conversion call back fails, such as when an int fails to coerse
/// to a string
...
struct RequiresError : public ParseError {
  RequiresError(std::string name, std::string subname)
      : ParseError("RequiresError", name + " requires " + subname,
                   ExitCodes::Requires) {}
};
...
struct ExcludesError : public ParseError {
  ExcludesError(std::string name, std::string subname)
      : ParseError("ExcludesError", name + " excludes " + subname,
                   ExitCodes::Excludes) {}
};
...
struct HorribleError : public ParseError {
  HorribleError(std::string name)
      : ParseError("HorribleError", "(You should never see this error) " + name,
                   ExitCodes::Horrible) {}
};
...
template <typename T,
          enable_if_t<std::is_floating_point<T>::value, detail::enabler> =
              detail::dummy>
constexpr const char *type_name() {
  return "FLOAT";
}
...
/// Integers / enums
template <typename T,
          enable_if_t<std::is_integral<T>::value || std::is_enum<T>::value,
                      detail::enabler> = detail::dummy>
...
inline void format_help(std::stringstream &out, std::string name,
  ...
  for (std::string name : input) {
    if (name.length() == 0)
      ...
      if (pos_name.length() > 0)
        throw BadNameString("Only one positional name allowed, remove: " +
                            name);
      pos_name = name;
    }
  }
...
*  add_option methods make it easy to prepare options. Remember to call `.start`
...
protected:
  ...
  ///  If true, return immediatly on an unrecognised option (implies
  ///  allow_extras)
...
public:
  ...
  /// Does not return a pointer since it is supposed to be called on the main
  /// App.
  ...
  /// Add a subcommand. Like the constructor, you can override the help message
  /// addition by setting help=false
      ...
      for (const std::string &group : groups) {
        ...
        for (const Option_p &opt : options_) {
          if (opt->nonpositional() && opt->get_group() == group)
            detail::format_help(out, opt->help_name(), opt->get_description(),
                                wid);
        }
        out << std::endl;
      }
...
protected:
  ...
  void _parse(std::vector<std::string> &args) {
    ...
    if (require_subcommand_ < 0 && selected_subcommands.empty())
      throw RequiredError("Subcommand required");
             ...
             static_cast<int>(selected_subcommands.size()) !=
                 require_subcommand_)
      throw RequiredError(std::to_string(require_subcommand_) +
                          " subcommand(s) required");

    // Convert missing (pairs) to extras (string only)
    if (parent_ == nullptr) {
      args.resize(missing()->size());
      std::transform(std::begin(*missing()), std::end(*missing()),
                     std::begin(args),
      ...
      size_t num_left_over = std::count_if(
          ...
          [](std::pair<detail::Classifer, std::string> &val) {
        ...
        if (current.inputs.size() == 1) {
          ...
          if (val == "true" || val == "on" || val == "yes")
            ...
            try {
              ...
              throw ConversionError(current.fullname +
  ...
  size_t _count_remaining_required_positionals() const {
    size_t retval = 0;
    for (const Option_p &opt : options_)
      if (opt->get_positional() && opt->get_required() &&
          opt->get_expected() > 0 &&
          static_cast<int>(opt->count()) < opt->get_expected())
  ...
  void _parse_short(std::vector<std::string> &args) {
    ...
    auto op_ptr = std::find_if(
        std::begin(options_), std::end(options_),
        [name](const Option_p &opt) { return opt->check_sname(name); });
...
struct AppFriend {
  ...
  static auto parse_short(App *app, Args &&... args) ->
      typename std::result_of<decltype (&App::_parse_short)(App,
                                                            Args...)>::type {
  ...
  static auto parse_long(App *app, Args &&... args) ->
      typename std::result_of<decltype (&App::_parse_long)(App,
                                                           Args...)>::type {
