...
string producer_names::producer_name(unsigned int producer_number) {
   ...
   if (string(prod_name) == "defproducera" && producer_number != 0)
      throw std::runtime_error( "Arisen Launcher not designed to handle numbers this large " );
   return prod_name;
}
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many aOS instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if (vmap.count("specific-num")) {
    ...
    const auto specific_args = vmap["specific-aos"].as<vector<string>>();
    if (specific_nums.size() != specific_args.size()) {
      cerr << "ERROR: every specific-num argument must be paired with a specific-aos argument" << endl;
      exit (-1);
    }
...
launcher_def::bind_nodes () {
   ...
   for (auto &h : bindings) {
      for (auto &inst : h.instances) {
         ...
         node.instance = &inst;
         auto kp = is_bios ?
            private_key_type(string("5HrpMSjfpEtWkaJALRBNPNysX7mv3juwAnY2bLK4A1ofMMuD9Qq")) :
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      do_command(host, node.name,
                 { { "ARISEN_HOME", host.arisen_home }, { "ARISEN_NODE", node_num }, { "ARISEN_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
