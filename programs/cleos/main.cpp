...
uint32_t max_cpu_usage = 0;
uint32_t max_net_usage = 0;
...
void add_standard_transaction_options(CLI::App* cmd, string default_permission = "") {
   ...
   cmd->add_option("--max-cpu-usage", max_cpu_usage, localized("set an upper limit on the cpu usage budget, in instructions-retired, for the execution of the transaction (defaults to 0 which means no limit)"));
   cmd->add_option("--max-net-usage", max_net_usage, localized("set an upper limit on the net usage budget, in bytes, for the transaction (defaults to 0 which means no limit)"));
}
...
fc::variant push_transaction( signed_transaction& trx, int32_t extra_kcpu = 1000, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   trx.max_kcpu_usage = (max_cpu_usage + 1023)/1024;
   trx.max_net_usage_words = (max_net_usage + 7)/8;
