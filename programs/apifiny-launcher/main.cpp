...
using namespace apifiny::launcher::config;
...
public:
  ...
  apifinyd_def*       instance;
  string          gelf_endpoint;
  bool            dont_start = false;
};

void
apifinyd_def::mk_dot_label () {
  dot_label_str = name + "\\nprod=";
...
void
apifinyd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string apifiny_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), apifiny_home(), instances(1) {}
};
...
struct launcher_def {
   ...
   string apifinyd_extra_args;
   std::map<uint,string> specific_nodapifiny_args;
   std::map<uint,string> specific_nodapifiny_installation_paths;
   testnet_def network;
   string gelf_endpoint;
   ...
   apifiny::chain::genesis_state genesis_from_file;

   void assign_name (apifinyd_def &node, bool is_bios);
   ...
   void prep_remote_config_dir (apifinyd_def &node, host_def *host);
   void launch (apifinyd_def &node, string &gts);
   void kill (launch_modes mode, string sig_opt);
   static string get_node_num(uint16_t node_num);
   pair<host_def, apifinyd_def> find_node(uint16_t node_num);
   vector<pair<host_def, apifinyd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    (node_executable_name, bpo::value<string>(&apifinyd_extra_args), ("forward " + string(node_executable_name) + " command line argument(s) to each instance of " + string(node_executable_name) + ", enclose arg(s) in quotes").c_str())
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  retrieve_paired_array_parameters(vmap, "specific-num", "specific-" + string(node_executable_name), specific_nodapifiny_args);
  retrieve_paired_array_parameters(vmap, "spcfc-inst-num", "spcfc-inst-" + string(node_executable_name), specific_nodapifiny_installation_paths);
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &apifinyd : binding.instances) {
          apifinyd.host = binding.host_name;
          apifinyd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(apifinyd.p2p_port);

          aliases.push_back (apifinyd.name);
        }
      }
  ...
  if (vmap.count("specific-num")) {
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_nodapifiny_args[num] = specific_args[i];
    }
  }
...
void
launcher_def::assign_name (apifinyd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.apifiny_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      apifinyd_def apifinyd;
      assign_name(apifinyd, i == 0);
      aliases.push_back(apifinyd.name);
      apifinyd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(apifinyd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->apifiny_home =
          (local_id.contains (lhost->host_name) || servers.default_apifiny_home.empty()) ?
          erd : servers.default_apifiny_home;
        host_ndx++;
      } // ph_count == 0

      apifinyd_def apifinyd;
      assign_name(apifinyd, do_bios);
      apifinyd.has_db = false;

      if (servers.db.size()) {
        for (auto &dbn : servers.db) {
          if (lhost->host_name == dbn) {
            apifinyd.has_db = true;
            break;
         }
        }
      }
      aliases.push_back(apifinyd.name);
      apifinyd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(apifinyd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  apifinyd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->apifiny_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->apifiny_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->apifiny_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->apifiny_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->apifiny_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = apifiny::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = apifiny::mongo_db_plugin\n";
  }
  cfg << "plugin = apifiny::net_plugin\n";
  cfg << "plugin = apifiny::chain_api_plugin\n"
...
launcher_def::init_genesis () {
   ...
   if (!bfs::exists(genesis_path)) {
      ...
      apifiny::chain::genesis_state default_genesis;
      fc::json::save_to_file( default_genesis, genesis_path, true );
   }
   string bioskey = string(network.nodes["bios"].keys[0].get_public_key());

   fc::json::from_file(genesis_path).as<apifiny::chain::genesis_state>(genesis_from_file);
...
void
launcher_def::prep_remote_config_dir (apifinyd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->apifiny_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->apifiny_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->apifiny_home;

  cmd = "cd " + host->apifiny_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->apifiny_home
...
void
launcher_def::launch (apifinyd_def &instance, string &gts) {
  ...
  string install_path;
  if (instance.name != "bios" && !specific_nodapifiny_installation_paths.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodapifiny_installation_paths.count(node_num)) {
        install_path = specific_nodapifiny_installation_paths[node_num] + "/";
     }
  }
  string apifinydcmd = install_path + "programs/nodapifiny/" + string(node_executable_name) + " ";
  if (skip_transaction_signatures) {
    apifinydcmd += "--skip-transaction-signatures ";
  }
  if (!apifinyd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +apifiny::mongo_db_plugin");
       string args = std::regex_replace (apifinyd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       apifinydcmd += args + " ";
    }
    else {
       apifinydcmd += apifinyd_extra_args + " ";
    }
  }
  if (instance.name != "bios" && !specific_nodapifiny_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodapifiny_args.count(node_num)) {
        apifinydcmd += specific_nodapifiny_args[node_num] + " ";
     }
  }

  if( add_enable_stale_production ) {
    apifinydcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  apifinydcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  apifinydcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    apifinydcmd += " --genesis-timestamp " + gts;
  }
  ...
  if (!host->is_local()) {
    ...
    string cmdl ("cd ");
    cmdl += host->apifiny_home + "; nohup " + apifinydcmd + " > "
    ...
    string cmd = "cd " + host->apifiny_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else if (!instance.node->dont_start) {
    cerr << "spawning child, " << apifinydcmd << endl;

    bp::child c(apifinydcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << apifinydcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
    ...
    cerr << "not spawning child, " << apifinydcmd << endl;
...
#if 0
void
launcher_def::kill_instance(apifinyd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, apifinyd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, apifinyd_def>> node_list;
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.apifiny_home + "; ";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const apifinyd_def& node = node_pair.second;
      const string node_num = node.get_node_num();
      cout << "Bouncing " << node.name << endl;
      string cmd = "./scripts/apifiny-tn_bounce.sh " + apifinyd_extra_args;
      if (node_num != "bios" && !specific_nodapifiny_args.empty()) {
         const auto node_num_i = boost::lexical_cast<uint16_t,string>(node_num);
         if (specific_nodapifiny_args.count(node_num_i)) {
            cmd += " " + specific_nodapifiny_args[node_num_i];
         }
      }
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the apifiny-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the apifiny-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the apifiny-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << apifiny::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (apifiny_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_apifiny_home) (ssh) )
