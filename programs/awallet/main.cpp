...
int main(int argc, char** argv)
{
   try {
      ...
      http_plugin::set_defaults({
         ...
         .default_unix_socket_path = awallet::config::key_store_executable_name + ".sock",
      ...
      http.add_handler("/v1/awallet/stop", [](string, string, url_response_callback cb) { cb(200, "{}"); std::raise(SIGTERM); } );
      app().startup();
      app().exec();
