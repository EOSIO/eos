...
struct callbacks {
   ...
   bool exec_deferred(uint32_t chain_index, uint32_t cb_alloc_data, uint32_t cb_alloc) {
      ...
      if (itr != idx.end() && itr->delay_until <= chain.control->pending_block_time()) {
         ...
         set_data(cb_alloc_data, cb_alloc,
                  convert_to_bin(chain_types::transaction_trace{ eosio::state_history::convert(*trace) }));
         return true;
      }
      return false;
   }
...
void register_callbacks() {
   ...
   rhf_t::add<&callbacks::coverage_inc_fun_cnt>("env", "coverage_inc_fun_cnt");
   rhf_t::add<&callbacks::coverage_inc_line_cnt>("env", "coverage_inc_line_cnt");
   rhf_t::add<&callbacks::coverage_get_fun_cnt>("env", "coverage_get_fun_cnt");
   rhf_t::add<&callbacks::coverage_get_line_cnt>("env", "coverage_get_line_cnt");
   rhf_t::add<&callbacks::coverage_dump>("env", "coverage_dump");
   rhf_t::add<&callbacks::coverage_reset>("env", "coverage_reset");
}
