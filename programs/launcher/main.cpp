  ...
  void initialize() {
    ...
    if (::getifaddrs(&ifap) == 0) {
      for (ifaddrs *p_if = ifap; p_if != 0; p_if = p_if->ifa_next) {
        if (p_if->ifa_addr != 0 && p_if->ifa_addr->sa_family == AF_INET &&
...
struct eosd_def {
  eosd_def()
      : genesis("genesis.json"), remote(false), ssh_identity(""), ssh_args(""),
        eos_root_dir(), data_dir(), hostname("127.0.0.1"),
        public_name("localhost"), p2p_port(9876), http_port(8888),
        filesize(8192), keys(), peers(), producers(), onhost_set(false),
        onhost(true), localaddrs(), dot_alias_str() {}
  ...
  string p2p_endpoint() {
    if (p2p_endpoint_str.empty()) {
      p2p_endpoint_str =
          public_name + ":" + boost::lexical_cast<string, uint16_t>(p2p_port);
    }
...
enum launch_modes { LM_NONE, LM_LOCAL, LM_REMOTE, LM_ALL };
...
struct launcher_def {
  ...
  void make_custom();
  void write_dot_file();
  void format_ssh(const string &cmd, const string &hostname,
                  string &ssh_cmd_line);
...
void launcher_def::set_options(bpo::options_description &cli) {
  cli.add_options()("nodes,n", bpo::value<int>()->default_value(1),
                    "total number of nodes to configure and launch")(
      "pnodes,p", bpo::value<int>()->default_value(1),
      "number of nodes that are producers")(
      "shape,s", bpo::value<string>()->default_value("ring"),
      "network topology, use \"ring\" \"star\" \"mesh\" or give a filename for "
      "custom")("genesis,g",
                bpo::value<bf::path>()->default_value("./genesis.json"),
                "set the path to genesis.json")(
      "output,o", bpo::value<bf::path>(),
      "save a copy of the generated topology in this file")(
      "skip-signature", bpo::bool_switch()->default_value(false),
      "EOSD does not require transaction signatures.")(
      "eosd", bpo::value<string>(), "forward eosd command line argument(s) to "
                                    "each instance of eosd, enclose arg in "
                                    "quotes");
}
...
void launcher_def::write_dot_file() {
  ...
  for (auto &node : network.nodes) {
    for (const auto &p : node.second.peers) {
      ...
      df << "\"" << node.second.dot_alias(node.first) << "\"->\"" << pname
...
void launcher_def::define_nodes() {
...
void launcher_def::write_config_file(eosd_def &node) {
  bf::path filename;
  boost::system::error_code ec;
  ...
  if (node.on_host()) {
    ...
    if (!bf::create_directory(node.data_dir, ec) && ec.value()) {
      cerr << "could not create new directory: " << node.data_dir << " errno "
           << ec.value() << " " << strerror(ec.value()) << endl;
      exit(-1);
    }
  } else {
  ...
  if (!node.on_host()) {
    ...
    const string &args =
        node.ssh_args.length() ? node.ssh_args : network.ssh_helper.ssh_args;
    if (args.length()) {
      scp_cmd_line += args + " ";
    }
    scp_cmd_line += filename.string() + " ";

    const string &uid = node.ssh_identity.length()
                            ? node.ssh_identity
...
void launcher_def::format_ssh(const string &cmd, const string &hostname,
                              string &ssh_cmd_line) {
...
bool launcher_def::do_ssh(const string &cmd, const string &hostname) {
  string ssh_cmd_line;
...
void launcher_def::prep_remote_config_dir(eosd_def &node) {
  ...
  if (!do_ssh(cmd, node.hostname)) {
    cerr << "Unable to switch to path " << node.eos_root_dir << " on host "
         << node.hostname << endl;
    exit(-1);
  }
  cmd = "cd " + add;
  if (do_ssh(cmd, node.hostname)) {
    cmd = "rm -rf " + add + "/block*";
    if (!do_ssh(cmd, node.hostname)) {
      cerr << "Unable to remove old data directories on host " << node.hostname
           << endl;
      exit(-1);
    }
  } else {
    cmd = "mkdir " + add;
    if (!do_ssh(cmd, node.hostname)) {
      cerr << "Unable to invoke " << cmd << " on host " << node.hostname
           << endl;
      exit(-1);
    }
  }
...
void launcher_def::launch(eosd_def &node, string &gts) {
  bf::path dd = node.data_dir;
  ...
  if (!node.on_host()) {
    string cmdl("cd ");
    cmdl += node.eos_root_dir + "; nohup " + eosdcmd + " > " + reout.string() +
            " 2> " + reerr.string() + "& echo $! > " + pidf.string();
    if (!do_ssh(cmdl, node.hostname)) {
      cerr << "Unable to invoke " << cmdl << " on host " << node.hostname
           << endl;
      exit(-1);
    }
...
void launcher_def::kill(launch_modes mode, string sig_opt) {
  if (mode == LM_NONE) {
    return;
  }
...
void launcher_def::start_all(string &gts, launch_modes mode) {
  if (mode == LM_NONE)
    return;
...
int main(int argc, char *argv[]) {
  ...
  opts.add_options()("timestamp,i", bpo::value<string>(),
                     "set the timestamp for the first block. Use \"now\" to "
                     "indicate the current time")(
      "launch,l", bpo::value<string>(),
      "select a subset of nodes to launch. Currently may be \"all\", \"none\", "
      "or \"local\". If not set, the default is to launch all unless an output "
      "file is named, in which case it starts none.")(
      "kill,k", bpo::value<string>(),
      "The launcher retrieves the previously started process ids and issue a "
      "kill signal to each.")("help,h", "print this list");
...
FC_REFLECT(remote_deploy, (ssh_cmd)(scp_cmd)(ssh_identity)(ssh_args))

FC_REFLECT(eosd_def, (genesis)(remote)(ssh_identity)(ssh_args)(eos_root_dir)(
                         data_dir)(hostname)(public_name)(p2p_port)(http_port)(
                         filesize)(keys)(peers)(producers))
