...
//                                ->check(CLI::ExistingFile);
   ...
   sign->callback([&] {
      ...
      const auto& [pubkey, provider] = eosio::app().get_plugin<signature_provider_plugin>().signature_provider_for_specification(str_private_key);
      ...
      if( str_public_key.size() > 0 ) {
         ...
         digest_type digest = trx.sig_digest(*chain_id, trx.context_free_data);
         signature_type siguature = provider(digest);
