...
void streamer_plugin::plugin_initialize(const variables_map& options) {
   try {
      ...
      if (options.count("stream-rabbits")) {
         ...
         rabbits.insert(std::end(rabbits), std::begin(rabbits_options), std::end(rabbit_options));
      }
      ...
      if (rabbits_exchange.size()) initialize_rabbits_exchange(my->streams, rabbits, my->publish_immediately, stream_data_path);
