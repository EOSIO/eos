...
void handle_request(const wasm_ql::http_config& http_config, const wasm_ql::shared_state& shared_state,
   ...
   //std::cout << "req.keep_alive(): " << req.keep_alive() << "\n";
   ...
   try {
      if (req.target() == "/v1/chain/get_info") {
         ...
         //               " us, avg = " + std::to_string(acct / accn) + ", threads = " + std::to_string(nt) + "\n");
         std::unique_lock l{ m };
         ++nfinished;
         auto x = std::chrono::duration_cast<std::chrono::microseconds>(stop - lastTime).count();
         if (x >= 1'000'000) {
            std::cout << (std::to_string(x) + " us, " + std::to_string(nfinished) + " finished, " + std::to_string(nthreads) + " threads\n");
            lastTime  = stop;
            nfinished = 0;
         }
         return;
 ...
 public:
   ...
   http_session(const std::shared_ptr<const wasm_ql::http_config>&  http_config,
       ...
       : stream(std::move(socket)), http_config(http_config), shared_state(shared_state), state_cache(state_cache),
         queue_(*this) {
      std::cout << "http_session  " << ++nn << "\n";
   }

   ~http_session() { std::cout << "~http_session " << --nn << "\n"; }
