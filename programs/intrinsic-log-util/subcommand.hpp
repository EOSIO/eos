...
namespace detail {
   ...
   protected:
      ...
      static std::string _get_description() {
         if constexpr( detail::has_description<U>::value ) {
      ...
      void _initialize( bpo::variables_map&& vm ) {
         if constexpr( detail::has_initialize<U>::value ) {
            T& derived = static_cast<T&>( *this );
...
class subcommand<T, subcommand_style::contains_subcommands, Us...>
   : public detail::subcommand_base<T,  Us...>
{
protected:
   using base_type = detail::subcommand_base<T,  Us...>;
   using subcommands_type = std::variant<detail::no_subcommand, Us...>;
...
public:
                         ...
                         const std::vector<std::string>& subcommand_context = std::vector<std::string>() )
   {
      ...
      if( vm_temp.count( "__command" ) > 0 ) {
         ...
         bool success = detail::construct_and_visit_variant_by_name( _subcommand, cmd_name,
         ...
         detail::static_visit_all<std::decay_t<decltype(_subcommand)>>(
...
template<typename T, typename... Us>
class subcommand<T, subcommand_style::terminal_subcommand_with_positional_arguments, Us...>
   : public detail::subcommand_base<T,  Us...>
{
protected:
   using base_type = detail::subcommand_base<T,  Us...>;
...
public:
                         ...
                         const std::vector<std::string>& subcommand_context = std::vector<std::string>() )
   {
      parse_metadata pm( subcommand_context );
      pm.subcommand_desc  = base_type::_get_description();
      pm.opts_description = std::make_shared<bpo::options_description>( base_type::_get_options(subcommand_context) );
...
template<typename T, typename... Us>
class subcommand<T, subcommand_style::terminal_subcommand_without_positional_arguments, Us...>
   : public detail::subcommand_base<T,  Us...>
{
protected:
   using base_type = detail::subcommand_base<T,  Us...>;
...
parse_program_options( T& root, const std::vector<std::string>& opts, //int argc, const char** argv,
