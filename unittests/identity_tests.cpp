...
public:
   ...
   fc::variant get_identity(uint64_t idnt) {
      ...
      vector<char> data;
      copy_row(*itr, data);
      return abi_ser.binary_to_variant("identrow", data, control->get_abi_serializer_max_time_ms());
   }
   ...
   fc::variant get_certrow(uint64_t identity, const string& property, uint64_t trusted, const string& certifier) {
      ...
      if (itr != idx.end() && itr->t_id == t_id->id && itr->secondary_key == key.get_array()) {
         ...
         vector<char> data;
         copy_row(*itr, data);
         return abi_ser.binary_to_variant("certrow", data, control->get_abi_serializer_max_time_ms());
      } else {
         return fc::variant(nullptr);
      }
   }
   ...
   fc::variant get_accountrow(const string& account) {
      ...
      if( itr != idx.end() && itr->t_id == t_id->id && N(account) == itr->primary_key) {
         vector<char> data;
         copy_row(*itr, data);
         return abi_ser.binary_to_variant("accountrow", data, control->get_abi_serializer_max_time_ms());
      } else {
         return fc::variant(nullptr);
      }
   }
