...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for apifiny reqauth action with alice's spending key
   chain.link_authority(name("alice"), name("apifiny"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("apifiny"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with apifiny reqauth
   chain.unlink_authority(name("alice"), name("apifiny"), name("reqauth"));
   ...
   // Link authority for any apifiny action with alice's scud key
   chain.link_authority(name("alice"), name("apifiny"), name("scud"));
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority(name("alice"), name("apifiny"), name("first"), name("reqauth"));
...
try {
   ...
   // Creating account with apifiny. prefix with privileged account
   chain.create_account(name("apifiny.test1"));

   // Creating account with apifiny. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("apifiny.test2"), name("joe")), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'apifiny.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("apifiny"), name("apifiny.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("apifiny"), name("apifiny.any"), name("reqauth") );

   /// this should succeed because apifiny::reqauth is linked to any permission
   chain.push_reqauth(name("alice"), { permission_level{N(alice), name("spending")} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = apifiny::chain::resource_limits::resource_usage_object;
   using by_owner = apifiny::chain::resource_limits::by_owner;
