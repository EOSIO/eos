...
const std::string eosio_token = name(N(enumivo.token)).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.set_code(N(enumivo.token), eosio_token_wast);
   chain.set_abi(N(enumivo.token), eosio_token_abi);
