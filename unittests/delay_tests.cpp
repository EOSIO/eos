...
asset get_currency_balance(const TESTER& chain, account_name account) {
   return chain.get_currency_balance(N(enumivo.coin), symbol(SY(4,CUR)), account);
}

const std::string enumivo_token = name(N(enumivo.coin)).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.create_account(N(enumivo.coin));
   chain.produce_blocks(10);

   chain.set_code(N(enumivo.coin), enumivo_token_wast);
   chain.set_abi(N(enumivo.coin), enumivo_token_abi);
   ...
   auto trace = chain.push_action(N(enumivo.coin), name("transfer"), N(enumivo.coin), fc::mutable_variant_object()
       ("from", enumivo_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, N(enumivo.coin));
   ...
   liquid_balance = get_currency_balance(chain, N(enumivo.coin));
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester));
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>(N(enumivo.coin));
   const auto abi = acnt.get_abi();
   chain::abi_serializer abis(abi);
   const auto a = chain.control->db().get<account_object,by_name>(N(enumivo.coin)).get_abi();
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action(N(enumivo.coin), name("transfer"), vector<permission_level>{{N(tester), N(first)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(enumivo.coin), name("transfer"), vector<permission_level>{{N(tester), N(second)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(enumivo.coin), name("transfer"), vector<permission_level>{{N(tester), config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
