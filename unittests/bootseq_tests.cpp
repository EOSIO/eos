...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(apifiny.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create apifiny.msig and apifiny.token
        create_accounts({N(apifiny.msig), N(apifiny.token), N(apifiny.ram), N(apifiny.ramfee), N(apifiny.stake), N(apifiny.vpay), N(apifiny.bpay), N(apifiny.saving) });
        // Set code for the following accounts:
        //  - apifiny (code: apifiny.bios) (already set by tester constructor)
        //  - apifiny.msig (code: apifiny.msig)
        //  - apifiny.token (code: apifiny.token)
        // set_code_abi(N(apifiny.msig), contracts::apifiny_msig_wasm(), contracts::apifiny_msig_abi().data());//, &apifiny_active_pk);
        // set_code_abi(N(apifiny.token), contracts::apifiny_token_wasm(), contracts::apifiny_token_abi().data()); //, &apifiny_active_pk);

        set_code_abi(N(apifiny.msig),
                     contracts::apifiny_msig_wasm(),
                     contracts::apifiny_msig_abi().data());//, &apifiny_active_pk);
        set_code_abi(N(apifiny.token),
                     contracts::apifiny_token_wasm(),
                     contracts::apifiny_token_abi().data()); //, &apifiny_active_pk);

        // Set privileged for apifiny.msig and apifiny.token
        set_privileged(N(apifiny.msig));
        set_privileged(N(apifiny.token));

        // Verify apifiny.msig and apifiny.token is privileged
        const auto& apifiny_msig_acc = get<account_metadata_object, by_name>(N(apifiny.msig));
        BOOST_TEST(apifiny_msig_acc.is_privileged() == true);
        const auto& apifiny_token_acc = get<account_metadata_object, by_name>(N(apifiny.token));
        BOOST_TEST(apifiny_token_acc.is_privileged() == true);


        // Create SYS tokens in apifiny.token, set its manager as apifiny
        ...
        create_currency(N(apifiny.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to apifiny.system
        issue(N(apifiny.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(apifiny.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("apifiny"));
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), apifiny_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), apifiny_assert_message_exception);
