...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(config::token_account_name, symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        create_accounts({config::msig_account_name, config::token_account_name,
            config::ram_account_name, config::ramfee_account_name,
            config::stake_account_name, config::vpay_account_name, config::bpay_account_name,
            config::saving_account_name});

        // Set code for the following accounts:
        //  - eosio (code: eosio.bios) (already set by tester constructor)
        //  - eosio.msig (code: eosio.msig)
        //  - eosio.token (code: eosio.token)
        set_code_abi(config::msig_account_name, eosio_msig_wast, eosio_msig_abi);//, &eosio_active_pk);
        set_code_abi(config::token_account_name, eosio_token_wast, eosio_token_abi); //, &eosio_active_pk);

        // Set privileged for eosio.msig and eosio.token
        set_privileged(config::msig_account_name);
        ...
        create_currency(config::token_account_name, config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to eosio.system
        issue(config::token_account_name, config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(config::stake_account_name, a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name{config::system_account_name}.to_string());
