...
static const char import_register_transaction_hook_wast[] = R"=====(
(module
 (import "env" "register_transaction_hook" (func $register_transaction_hook (param i32 i64 i64)(result i32)))
 (memory $0 1)
 (export "apply" (func $apply))
 (func $apply (param $0 i64) (param $1 i64) (param $2 i64)
   (drop
     (call $register_transaction_hook
       (i32.const 0)
       (i64.const 43)
       (i64.const 43)
     )
...
BOOST_AUTO_TEST_CASE( register_transaction_hook_test ) { try {
   tester c( setup_policy::preactivate_feature_and_new_bios );

   const auto& pfm = c.control->get_protocol_feature_manager();
   const auto& d = pfm.get_builtin_digest(builtin_protocol_feature_t::register_transaction_hook);
   BOOST_REQUIRE(d);
   ...
   BOOST_CHECK_EXCEPTION(  c.set_code( alice_account, import_register_transaction_hook_wast ),
                           wasm_exception,
                           fc_exception_message_is( "env.register_transaction_hook unresolveable" ) );

   c.preactivate_protocol_features( {*d} );
   c.produce_block();

   // ensure it now resolves
   c.set_code( alice_account, import_register_transaction_hook_wast );
