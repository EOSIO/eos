...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(enu.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(enu.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
      :TESTER(),abi_ser(json::from_string(enu_token_abi).as<abi_def>())
      {
         create_account( N(enu.token));
         set_code( N(enu.token), enu_token_wast );

         auto result = push_action(N(enu.token), N(create), mutable_variant_object()
                 ("issuer",       enu_token)
         ...
         result = push_action(N(enu.token), N(issue), mutable_variant_object()
                 ("to",       enu_token)
      ...
      static const std::string enu_token;
};

const std::string currency_tester::enu_token = name(N(enu.token)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(enu.token), expected.get_symbol(), N(enu.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(enu.token), N(transfer), mutable_variant_object()
         ("from", enu_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(enu.token), N(transfer), mutable_variant_object()
                                    ("from", enu_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      BOOST_CHECK_EXCEPTION(symbol malformed(SY(6,EnU)),
                            fc::assert_exception, fc_assert_exception_message_is("invalid symbol: EnU"));
   }
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(enu_token, N(alice), "100.0000 CUR");
