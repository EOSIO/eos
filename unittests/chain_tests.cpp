...
BOOST_AUTO_TEST_CASE( replace_producer_keys ) try {
   ...
   BOOST_ASSERT(!gpo.proposed_schedule_block_num);
   BOOST_ASSERT(gpo.proposed_schedule.version == 0);
   BOOST_ASSERT(gpo.proposed_schedule.producers.empty());
...
BOOST_AUTO_TEST_CASE( replace_account_keys ) try {
   validating_tester tester;
   const name usr("eosio");
   const name active_permission("active");
