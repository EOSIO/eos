...
FC_REFLECT(account_t, (owner)(rsn_balance)(open_offers)(open_games));
...
BOOST_FIXTURE_TEST_CASE( dice_test, dice_tester ) try {
   ...
   create_accounts( {N(arisen.token), N(dice),N(alice),N(bob),N(carol),N(david)}, false);
   ...
   push_action(N(arisen.token), N(create), N(arisen.token), mvo()
     ("issuer", "arisen.token")
   ...
   transfer( config::system_account_name, N(alice), core_from_string("10000.0000"), "", N(arisen.token) );
   transfer( config::system_account_name, N(bob),   core_from_string("10000.0000"), "", N(arisen.token) );
   transfer( config::system_account_name, N(carol), core_from_string("10000.0000"), "", N(arisen.token) );
   ...
   BOOST_REQUIRE_EQUAL(
      get_currency_balance(N(arisen.token), symbol(CORE_SYMBOL), N(alice)),
