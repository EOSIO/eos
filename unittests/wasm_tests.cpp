...
BOOST_FIXTURE_TEST_CASE( simple_no_memory_check, TESTER ) try {
   ...
   trx.actions.push_back(act);
   trx.expiration = fc::time_point_cast<fc::seconds>( control->head_block_time() );
   set_transaction_headers(trx);
...
BOOST_DATA_TEST_CASE_F( old_wasm_tester, table_init_tests, bdata::make({false, true}), activate_wasm_config ) try {
   ...
   BOOST_CHECK_THROW(set_code(N(tableinit), too_big_table), eosio::chain::wasm_exception);
...
BOOST_FIXTURE_TEST_CASE( memory_init_border, TESTER ) try {
   ...
   BOOST_CHECK_THROW(set_code(N(memoryborder), memory_init_toolong), eosio::chain::wasm_exception);
   BOOST_CHECK_THROW(set_code(N(memoryborder), memory_init_negative), eosio::chain::wasm_exception);
...
BOOST_FIXTURE_TEST_CASE( nested_limit_test, TESTER ) try {
   ...
   }
   {
      ...
      for(unsigned int i = 0; i < 1024; ++i)
         ss << ")";
      ss << "))";
      BOOST_CHECK_THROW(set_code(N(nested), ss.str().c_str()), eosio::chain::wasm_exception);
   }
...
BOOST_FIXTURE_TEST_CASE( offset_check_old, old_wasm_tester ) try {
   ...
   for(const string& s : loadops) {
      ...
      BOOST_CHECK_THROW(set_code(N(offsets), ss.str().c_str()), eosio::chain::wasm_exception);
      produce_block();
   }
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
...
BOOST_FIXTURE_TEST_CASE( fuzz, TESTER ) try {
   ...
   */
   ...
   }
   {
      ...
      BOOST_CHECK_THROW(set_code(N(fuzzy), wasm), wasm_exception);
   }
   {
...
BOOST_AUTO_TEST_CASE( billed_cpu_test ) try {
   ...
   auto create_trx = [&](auto trx_max_ms) {
      ...
      auto fut = transaction_metadata::start_recover_keys( ptrx, chain.control->get_thread_pool(), chain.control->get_chain_id(), fc::microseconds::max() );
      return fut.get();
   };
   ...
   push_trx( ptrx, fc::time_point::max(), 0, false ); // non-explicit billing
   ...
   push_trx( ptrx, fc::time_point::max(), max_cpu_time_us, true );
   chain.produce_block();
   ...
   BOOST_CHECK_EXCEPTION( push_trx( ptrx, fc::time_point::max(), max_cpu_time_us + 1, true ), tx_cpu_usage_exceeded,
   ...
   push_trx( ptrx, fc::time_point::max(), 5 * 1000, true );
   chain.produce_block();
   ...
   // indicate explicit billing at max + 1
   BOOST_CHECK_EXCEPTION( push_trx( ptrx, fc::time_point::max(), 5 * 1000 + 1, true ), tx_cpu_usage_exceeded,
                          fc_exception_message_starts_with("billed") );

   // bill at minimum
   ptrx = create_trx(0);
   // indicate explicit billing at transaction minimum
   push_trx( ptrx, fc::time_point::max(), min_cpu_time_us, true );
   chain.produce_block();
   ...
   BOOST_CHECK_EXCEPTION( push_trx( ptrx, fc::time_point::max(), min_cpu_time_us - 1, true ), transaction_exception,
   ...
   BOOST_CHECK_EXCEPTION( push_trx( ptrx, fc::time_point::max(), cpu_limit+1, false ), tx_cpu_usage_exceeded,
   ...
   auto r = push_trx( ptrx, fc::time_point::max(), cpu_limit, false );
   ...
   cpu_limit = mgr.get_account_cpu_limit_ex(acc).first.max;
   BOOST_CHECK_EXCEPTION( push_trx( ptrx, fc::time_point::max(), cpu_limit+1, true ), tx_cpu_usage_exceeded,
                          fc_exception_message_starts_with("billed") );
