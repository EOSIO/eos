...
(module
 ...
 (func $apply (param $0 i64) (param $1 i64) (param $2 i64)
   ;;(drop (i64.load (i32.const 4)))
   ;;(drop (i64.load (i32.const 10)))
   ;;;;;;;(call $printi (i64.load (i32.const 8)))
   ;;;;(loop (br 0))
   ;;(call $prints (i32.const 64))
   ;;(drop (call $memcpy (i32.const 128) (i32.const 64) (i32.const 16)))
   ;;(call $prints (i32.const 128))
   ;;(drop (grow_memory (i32.const 1)))
   ;;(drop (i64.load (i32.const 100000)))
   ;;(call $printi (i64.extend_u/i32 (current_memory)))
   ;;(drop (grow_memory (i32.const 1)))
   ;;(call $printi (i64.extend_u/i32 (current_memory)))
   ;;(drop (i64.load (i32.const 140000)))
   ;;;(i64.store (i32.const 32) (get_global $depth))
   ;;;(i32.store (i32.const 40) (get_global $shorty))
   ;;;;;;;;(f64.store (i32.const 48) (get_global $floaty))
   ;;;(i64.store (i32.const 64) (get_global $changeme))
   ;;;;;;;(f64.store (i32.const 64) (f64.add (f64.const 1.0) (get_global $floaty)))
   ;;(call $apply (get_local $0) (get_local $1) (get_local $2))
   ;;(call_indirect (type $xxx) (i64.const 666) (i32.const 3))
   (call $overhere (i32.const 2))
   (call $overhere (i32.const 3))
 )
 (func $overhere (param $0 i32)
   (call_indirect (type $xxx) (i64.const 666) (get_local $0))
 )
 (func $yum 
   (call $printi (i64.const 50))
 )
 (func $nomnom (param $0 i64)
   (call $printi (get_local $0))
 )
 (elem (i32.const 2) $nomnom $nomnom)
 ;;(global $floaty f64 (f64.const 3.1415))
 ;;(global $shorty (mut i32) (i32.const 22))
 ;;(global $changeme (mut i64) (i64.const 4))
 ;;(global $depth (mut i64) (i64.const 250))
 ;;(global $mcfloaty (mut f64) (f64.const 3.14))
)
)=====";

BOOST_AUTO_TEST_CASE(playground, * boost::unit_test::disabled()) { try {
   tester chain(setup_policy::none);

   chain.produce_blocks(2);
   chain.create_accounts( {N(foo)} );
   chain.produce_block();

   chain.set_code(N(foo), foo_wast);
   ...
   act.name = N(foo);
   act.account = N(foo);
   act.authorization = vector<permission_level>{{N(foo),config::active_name}};
   trx.actions.push_back(act);
   chain.set_transaction_headers(trx);
   trx.sign(chain.get_private_key( N(foo), "active" ), chain.control->get_chain_id());
