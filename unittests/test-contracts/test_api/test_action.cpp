...
#include <apifinylib/privileged.h>
...
void test_action::read_action_normal() {
   ...
   apifiny_assert( action_data_size() == sizeof(dummy_action), "action_size() == sizeof(dummy_action)" );

   total = read_action_data( buffer, 30 );
   apifiny_assert( total == sizeof(dummy_action) , "read_action(30)" );

   total = read_action_data( buffer, 100 );
   apifiny_assert(total == sizeof(dummy_action) , "read_action(100)" );

   total = read_action_data(buffer, 5);
   apifiny_assert( total == 5 , "read_action(5)" );

   total = read_action_data(buffer, sizeof(dummy_action) );
   apifiny_assert( total == sizeof(dummy_action), "read_action(sizeof(dummy_action))" );

   dummy_action *dummy13 = reinterpret_cast<dummy_action *>(buffer);

   apifiny_assert( dummy13->a == DUMMY_ACTION_DEFAULT_A, "dummy13->a == DUMMY_ACTION_DEFAULT_A" );
   apifiny_assert( dummy13->b == DUMMY_ACTION_DEFAULT_B, "dummy13->b == DUMMY_ACTION_DEFAULT_B" );
   apifiny_assert( dummy13->c == DUMMY_ACTION_DEFAULT_C, "dummy13->c == DUMMY_ACTION_DEFAULT_C" );
}
...
void test_action::test_dummy_action() {
   ...
   apifiny_assert( total > 0, "get_action failed" );
   apifiny::action act = apifiny::get_action( 1, 0 );
   apifiny_assert( act.authorization.back().actor == "testapi"_n, "incorrect permission actor" );
   apifiny_assert( act.authorization.back().permission == "active"_n, "incorrect permission name" );
   apifiny_assert( apifiny::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
   apifiny_assert( act.account == "testapi"_n, "expected testapi account" );
   ...
   if ( dum13.b == 200 ) {
      ...
      apifiny_assert( false, "get_context_free_data() not allowed in non-context free action" );
   } else {
      apifiny_assert( dum13.a == DUMMY_ACTION_DEFAULT_A, "dum13.a == DUMMY_ACTION_DEFAULT_A" );
      apifiny_assert( dum13.b == DUMMY_ACTION_DEFAULT_B, "dum13.b == DUMMY_ACTION_DEFAULT_B" );
      apifiny_assert( dum13.c == DUMMY_ACTION_DEFAULT_C, "dum13.c == DUMMY_ACTION_DEFAULT_C" );
   }
}
...
void test_action::test_cf_action() {
   ...
   apifiny::action act = apifiny::get_action( 0, 0 );
   ...
   if ( cfa.payload == 100 ) {
      ...
      apifiny_assert( size > 0, "size determination failed" );
      std::vector<char> cfd( static_cast<size_t>(size) );
      size = get_context_free_data( cfa.cfd_idx, &cfd[0], static_cast<size_t>(size) );
      apifiny_assert(static_cast<size_t>(size) == cfd.size(), "get_context_free_data failed" );
      uint32_t v = apifiny::unpack<uint32_t>( &cfd[0], cfd.size() );
      apifiny_assert( v == cfa.payload, "invalid value" );
      ...
      apifiny_assert( false, "db_api should not be allowed" );
      ...
      apifiny::action dum_act;
      dum_act.send();
      apifiny_assert( false, "action send should not be allowed" );
   } else if ( cfa.payload == 206 ) {
      apifiny::require_auth("test"_n);
      apifiny_assert( false, "authorization_api should not be allowed" );
   } else if ( cfa.payload == 207 ) {
      now();
      apifiny_assert( false, "system_api should not be allowed" );
      ...
      apifiny_assert( false, "transaction_api should not be allowed" );
...
void test_action::require_notice( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   if( receiver == "testapi"_n.value ) {
      apifiny::require_recipient( "acc1"_n );
      apifiny::require_recipient( "acc2"_n );
      apifiny::require_recipient( "acc1"_n, "acc2"_n );
      apifiny_assert( false, "Should've failed" );
...
void test_action::require_notice_tests( uint64_t receiver, uint64_t code, uint64_t action ) {
   apifiny::print( "require_notice_tests" );
   if( receiver == "testapi"_n.value ) {
      apifiny::print("require_recipient( \"acc5\"_n )");
      apifiny::require_recipient("acc5"_n);
   } else if( receiver == "acc5"_n.value ) {
      apifiny::print("require_recipient( \"testapi\"_n )");
      apifiny::require_recipient("testapi"_n);
   }
}

void test_action::require_auth() {
   prints("require_auth");
   apifiny::require_auth("acc3"_n);
   apifiny::require_auth("acc4"_n);
}

void test_action::assert_false() {
   apifiny_assert( false, "test_action::assert_false" );
}

void test_action::assert_true() {
   apifiny_assert( true, "test_action::assert_true" );
}
...
void test_action::test_abort() {
   abort();
   apifiny_assert( false, "should've aborted" );
}

void test_action::test_publication_time() {
   uint64_t pub_time = 0;
   uint32_t total = read_action_data( &pub_time, sizeof(uint64_t) );
   apifiny_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)" );
   apifiny_assert( pub_time == publication_time(), "pub_time == publication_time()" );
}
...
void test_action::test_current_receiver( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   apifiny_assert( receiver == cur_rec.value, "the current receiver does not match" );
}
...
void test_action::test_current_time() {
   ...
   apifiny_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)" );
   apifiny_assert( tmp == current_time(), "tmp == current_time()" );
}
...
void test_action::test_assert_code() {
   ...
   apifiny_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)" );
   apifiny_assert_code( false, code );
}
...
void test_action::test_ram_billing_in_notify( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   apifiny_assert( total == sizeof(uint128_t), "total == sizeof(uint128_t)" );

   uint64_t to_notify = tmp >> 64;
   uint64_t payer = tmp & 0xFFFFFFFFFFFFFFFFULL;

   if( code == receiver ) {
      apifiny::require_recipient( name{to_notify} );
   } else {
      apifiny_assert( to_notify == receiver, "notified recipient other than the one specified in to_notify" );
...
void test_action::test_action_ordinal1(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (receiver == "testapi"_n.value) {
      print("exec 1");
      apifiny::require_recipient( "bob"_n ); //-> exec 2 which would then cause execution of 4, 10

      apifiny::action act1({name(_self), "active"_n}, name(_self),
      ...
      if (is_account("fail1"_n)) {
         apifiny_assert(false, "fail at point 1");
      }

      apifiny::action act2({name(_self), "active"_n}, name(_self),
                         name(WASM_TEST_ACTION("test_action", "test_action_ordinal3")),
                         std::tuple<>());
      act2.send(); // -> exec 9

      set_action_return_value( &apifiny::pack(unsigned_int(1))[0], apifiny::pack_size(unsigned_int(1)) );
      apifiny::require_recipient( "charlie"_n ); // -> exec 3 which would then cause execution of 11
      ...
      set_action_return_value( &apifiny::pack(std::string("bob"))[0], apifiny::pack_size(std::string("bob")) );
      apifiny::require_recipient( "david"_n );  // -> exec 4
      ...
      set_action_return_value( &apifiny::pack(std::string("charlie"))[0], apifiny::pack_size(std::string("charlie")) );
      if (is_account("fail3"_n)) {
         apifiny_assert(false, "fail at point 3");
      }

   } else if (receiver == "david"_n.value) {
      print("exec 4");
      set_action_return_value( &apifiny::pack(std::string("david"))[0], apifiny::pack_size(std::string("david")) );
   } else {
      apifiny_assert(false, "assert failed at test_action::test_action_ordinal1");
   }
}
void test_action::test_action_ordinal2(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (receiver == "testapi"_n.value) {
      print("exec 5");
      apifiny::require_recipient( "david"_n ); // -> exec 6
      apifiny::require_recipient( "erin"_n ); // -> exec 7

      apifiny::action act1({name(_self), "active"_n}, name(_self),
                         name(WASM_TEST_ACTION("test_action", "test_action_ordinal4")),
                         std::tuple<>());
      act1.send(); // -> exec 8
      set_action_return_value( &apifiny::pack("five"_n)[0], apifiny::pack_size("five"_n) );
   } else if (receiver == "david"_n.value) {
      print("exec 6");
      set_action_return_value( &apifiny::pack(true)[0], apifiny::pack_size(true) );
   } else if (receiver == "erin"_n.value) {
      print("exec 7");
      set_action_return_value( &apifiny::pack(signed_int(7))[0], apifiny::pack_size(signed_int(7)) );
   } else {
      apifiny_assert(false, "assert failed at test_action::test_action_ordinal2");
   }
}
...
void test_action::test_action_ordinal3(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (is_account("failnine"_n)) {
      apifiny_assert(false, "fail at point 9");
   }
   set_action_return_value( &apifiny::pack(unsigned_int(9))[0], apifiny::pack_size(unsigned_int(9)) );
}
void test_action::test_action_ordinal_foo(uint64_t receiver, uint64_t code, uint64_t action) {
   print("exec 10");
   set_action_return_value( &apifiny::pack(13.23)[0], apifiny::pack_size(13.23) );
}
void test_action::test_action_ordinal_bar(uint64_t receiver, uint64_t code, uint64_t action) {
   print("exec 11");
   set_action_return_value( &apifiny::pack(11.42f)[0], apifiny::pack_size(11.42f) );
}
