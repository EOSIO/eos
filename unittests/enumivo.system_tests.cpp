...
struct enumivo_parameters : enumivo::chain::chain_config {
   uint64_t  max_ram_size;
};

FC_REFLECT_DERIVED(enumivo_parameters, (enumivo::chain::chain_config), (max_ram_size));
...
BOOST_FIXTURE_TEST_CASE( buysell, enumivo_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "enumivo", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000") ) );
   ...
   transfer( "enumivo", "alice1111111", core_from_string("100000000.0000"), "enumivo" );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, enumivo_system_tester ) try {
   ...
   transfer( "enumivo", "bob111111111", core_from_string("1000.0000"), "enumivo" );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, enumivo_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "enumivo", "alice1111111", core_from_string("2000.0000"), core_from_string("1000.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( vote_for_producer, enumivo_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_from_string("2.0000"), core_from_string("0.0002")/*"2.0000 ENU", "0.0002 ENU"*/ ) );
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, enumivo_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // producvoterd votes for abcproducera ... abcproducern
   {
      ...
      BOOST_REQUIRE_EQUAL(success(), push_action(N(producvoterc), N(voteproducer), mvo()
                                                 ("voter",  "producvoterc")
                                                 ("proxy", name(0).to_string())
                                                 ("producers", vector<account_name>(producer_names.begin(), producer_names.begin()+26))
                                                 )
                          );
      ...
      BOOST_REQUIRE_EQUAL(success(), push_action(N(producvoterd), N(voteproducer), mvo()
                                                 ("voter",  "producvoterd")
                                                 ("proxy", name(0).to_string())
                                                 ("producers", vector<account_name>(producer_names.begin()+26, producer_names.end()))
                                                 )
                          );
   }
...

   {
      ...
      const uint32_t initial_tot_unpaid_blocks = initial_global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    initial_supply            = get_token_supply();
      const asset    initial_saving_balance    = get_balance(N(enumivo.save));
      ...
      const uint32_t tot_unpaid_blocks = global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    supply            = get_token_supply();
      const asset    saving_balance    = get_balance(N(enumivo.save));
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, enumivo_system_tester ) try {
   ...
   transfer( "enumivo", "alice1111111", core_from_string("600000000.0000"), "enumivo" );
...
BOOST_FIXTURE_TEST_CASE( setparams, enumivo_system_tester ) try {
   ...
   enumivo_parameters params;
   (enumivo::chain::chain_config&)params = control->get_global_properties().configuration;
