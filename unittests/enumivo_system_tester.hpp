...
public:
   ...
   enumivo_system_tester(Lambda setup) {
      ...
      create_accounts({ N(enumivo.tkn), N(enumivo.ram), N(enumivo.rfee), N(enumivo.stk),
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(enumivo.tkn), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
