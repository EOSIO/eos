...
class exchange_tester : public TESTER {
   public:
      ...
      :TESTER(),abi_ser(json::from_string(exchange_abi).as<abi_def>())
      {
         ...
         deploy_currency( N(enumivo.token) );

         create_currency( N(enumivo.token), N(enumivo.token), A(1000000.00 USD) );
         create_currency( N(enumivo.token), N(enumivo.token), A(1000000.00 BTC) );

         issue( N(enumivo.token), N(enumivo.token), N(dan), A(1000.00 USD) );
         issue( N(enumivo.token), N(enumivo.token), N(dan), A(1000.00 BTC) );

         deposit( N(exchange), N(dan), extended_asset( A(500.00 USD), N(enumivo.token) ) );
         deposit( N(exchange), N(dan), extended_asset( A(500.00 BTC), N(enumivo.token) ) );

         create_exchange( N(exchange), N(dan),
                            extended_asset( A(400.00 USD), N(enumivo.token) ),
                            extended_asset( A(400.00 BTC), N(enumivo.token) ),
                            A(10000000.00 EXC) );
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   exchange_tester t;
   t.create_currency( N(enumivo.token), N(enumivo.token), expected );
   t.issue( N(enumivo.token), N(enumivo.token), N(enumivo.token), expected );
...
BOOST_AUTO_TEST_CASE( exchange_create ) try {
   ...
   t.issue( N(enumivo.token), N(enumivo.token), N(trader), A(2000.00 BTC) );
   t.issue( N(enumivo.token), N(enumivo.token), N(trader), A(2000.00 USD) );

   t.deposit( N(exchange), N(trader), extended_asset( A(1500.00 USD), N(enumivo.token) ) );
   t.deposit( N(exchange), N(trader), extended_asset( A(1500.00 BTC), N(enumivo.token) ) );

   auto trader_ex_usd = t.get_exchange_balance( N(exchange), N(enumivo.token), symbol(2,"USD"), N(trader) );
   auto trader_ex_btc = t.get_exchange_balance( N(exchange), N(enumivo.token), symbol(2,"BTC"), N(trader) );
   auto dan_ex_usd    = t.get_exchange_balance( N(exchange), N(enumivo.token), symbol(2,"USD"), N(dan) );
   auto dan_ex_btc    = t.get_exchange_balance( N(exchange), N(enumivo.token), symbol(2,"BTC"), N(dan) );
   ...
   auto result = t.trade( N(exchange), N(trader), symbol(2,"EXC"),
                          extended_asset( A(10.00 BTC), N(enumivo.token) ),
                          extended_asset( A(0.01 USD), N(enumivo.token) ) );

   for( const auto& at : result->action_traces )
      ilog( "${s}", ("s",at.console) );

   trader_ex_usd = t.get_exchange_balance( N(exchange), N(enumivo.token), symbol(2,"USD"), N(trader) );
   trader_ex_btc = t.get_exchange_balance( N(exchange), N(enumivo.token), symbol(2,"BTC"), N(trader) );
   wdump((trader_ex_btc.quantity));
   wdump((trader_ex_usd.quantity));

   result = t.trade( N(exchange), N(trader), symbol(2,"EXC"),
                          extended_asset( A(9.75 USD), N(enumivo.token) ),
                          extended_asset( A(0.01 BTC), N(enumivo.token) ) );
...
BOOST_AUTO_TEST_CASE( exchange_lend ) try {
   ...
   t.issue( N(enumivo.token), N(enumivo.token), N(lender), A(2000.00 BTC) );
   t.issue( N(enumivo.token), N(enumivo.token), N(lender), A(2000.00 USD) );

   t.deposit( N(exchange), N(lender), extended_asset( A(1500.00 USD), N(enumivo.token) ) );
   t.deposit( N(exchange), N(lender), extended_asset( A(1500.00 BTC), N(enumivo.token) ) );

   auto lender_ex_usd = t.get_exchange_balance( N(exchange), N(enumivo.token), symbol(2,"USD"), N(lender) );
   auto lender_ex_btc = t.get_exchange_balance( N(exchange), N(enumivo.token), symbol(2,"BTC"), N(lender) );

   t.lend( N(exchange), N(lender), extended_asset( A(1000.00 USD), N(enumivo.token) ), symbol(2,"EXC") );

   lender_ex_usd = t.get_exchange_balance( N(exchange), N(enumivo.token), symbol(2,"USD"), N(lender) );
   lender_ex_btc = t.get_exchange_balance( N(exchange), N(enumivo.token), symbol(2,"BTC"), N(lender) );
   ...
   t.unlend( N(exchange), N(lender), lentshares, extended_symbol{ symbol(2,"USD"), N(enumivo.token)}, symbol(2,"EXC") );
