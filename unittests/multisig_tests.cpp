...
public:
   ...
   asset get_balance( const account_name& act ) {
      //return get_currency_balance( config::system_account_name, symbol(SY(4,ENU)), act );
      ...
      if (tbl) {
         const auto *obj = db.find<key_value_object, by_scope_primary>(boost::make_tuple(tbl->id, symbol(SY(4,ENU)).to_symbol_code()));
         if (obj) {
      ...
      return asset( result, symbol(SY(4,ENU)) );
   }
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, enumivo_msig_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION(create_account_with_resources( N(alice1111112), N(enumivo), asset::from_string("1.0000 ENU"), false ),
      fc::assert_exception,
