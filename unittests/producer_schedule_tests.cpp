...
BOOST_AUTO_TEST_CASE( extra_signatures_test ) try {
   ...
   vector<producer_authority> sch1 = {
      producer_authority{N(alice), block_signing_authority_v0{1,  {
                                                                     {get_public_key(N(alice), "bs1"), 1},
                                                                     {get_public_key(N(alice), "bs2"), 1},
                                                                     {get_public_key(N(alice), "bs3"), 1},
                                                                  }
                                                             }
                        }
   };
   ...
   BOOST_REQUIRE_EQUAL( true, main.control->proposed_producers().valid() );

   main.block_signing_private_keys.emplace(get_public_key(N(alice), "bs1"), get_private_key(N(alice), "bs1"));
   main.block_signing_private_keys.emplace(get_public_key(N(alice), "bs2"), get_private_key(N(alice), "bs2"));

   BOOST_REQUIRE( main.control->pending_block_producer() == N(eosio) );
   main.produce_blocks(3);
   BOOST_REQUIRE( main.control->pending_block_producer() == N(alice) );
   ...
   // Generate a valid block and then corrupt it by adding an extra signature.
   {
      ...
      remote.block_signing_private_keys.emplace(get_public_key(N(alice), "bs1"), get_private_key(N(alice), "bs1"));
      remote.block_signing_private_keys.emplace(get_public_key(N(alice), "bs2"), get_private_key(N(alice), "bs2"));

      // Generate the block that will be corrupted.
      auto valid_block = remote.produce_block();

      BOOST_REQUIRE( valid_block->producer == N(alice) );
      ...
      auto additional_sigs = exts.lower_bound( additional_sigs_eid )->second.get<additional_block_signatures_extension>().signatures;
      ...
      additional_sigs.emplace_back( remote.get_private_key(N(alice), "bs3").sign(sig_digest) );
      additional_sigs.emplace_back( remote.get_private_key(N(alice), "bs4").sign(sig_digest) );
