...
BOOST_FIXTURE_TEST_CASE( producer_register_unregister, eosio_system_tester ) try {
   ...
   auto key =  fc::crypto::public_key( std::string("GLS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV") );
   ...
   auto key2 =  fc::crypto::public_key( std::string("GLS5jnmSKrzdBHE9n8hw58y7yxFWBC8SNiG7m8S1crJH3KvAnf9o6") );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(alice1111111), N(regproducer), mvo()
                                               ("producer",  "alice1111111")
                                               ("producer_key", key2 )
                                               ("url", "http://block.two")
