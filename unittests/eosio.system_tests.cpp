...
BOOST_FIXTURE_TEST_CASE( buyname, eosio_system_tester ) try {
   ...
   transfer( "eosio", "dan", core_from_string( "10000.0000" ) );
   transfer( "eosio", "sam", core_from_string( "10000.0000" ) );
   stake_with_transfer( "eosio", "sam", core_from_string( "80000000.0000" ), core_from_string( "80000000.0000" ) );

   regproducer( N(eosio) );
   push_action( N(sam), N(voteproducer), mvo()
               ("voter",  "sam")
               ("proxy", name(0).to_string() )
               ("producers", vector<account_name>{ N(eosio) } ) );
