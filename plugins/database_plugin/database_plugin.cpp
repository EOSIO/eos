...
void database_plugin::set_program_options(options_description &,
                                          options_description &cfg) {
  cfg.add_options()("readonly", bpo::value<bool>()->default_value(false),
                    "open the database in read only mode")(
      "shared-file-dir", bpo::value<bfs::path>()->default_value("blockchain"),
      "the location of the chain shared memory files (absolute path or "
...
void database_plugin::wipe_database() {
  if (!my->shared_memory_dir.empty() && !my->db.valid())
    ...
    elog("ERROR: database_plugin::wipe_database() called before configuration "
         "or after startup. Ignoring.");
}
...
void database_plugin::plugin_initialize(const variables_map &options) {
  ...
  my->shared_memory_size =
      options.at("shared-file-size").as<uint64_t>() * 1024 * 1024;
...
void database_plugin::plugin_startup() {
  ...
  my->db = chainbase::database(my->shared_memory_dir,
                               my->readonly ? database::read_only
                                            : database::read_write,
                               my->shared_memory_size);
}
