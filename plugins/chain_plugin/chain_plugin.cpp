...
#ifdef NDEBUG
const uint32_t chain_plugin::DEFAULT_RECEIVED_BLOCK_TRANSACTION_EXECUTION_TIME =
    12;
const uint32_t chain_plugin::DEFAULT_TRANSACTION_EXECUTION_TIME = 3;
const uint32_t chain_plugin::DEFAULT_CREATE_BLOCK_TRANSACTION_EXECUTION_TIME =
    3;
#else
const uint32_t chain_plugin::DEFAULT_RECEIVED_BLOCK_TRANSACTION_EXECUTION_TIME =
    72;
...
void chain_plugin::set_program_options(options_description &cli,
  ...
  cfg.add_options()("genesis-json", bpo::value<boost::filesystem::path>(),
      ...
      "the location of the block log (absolute path or relative to application "
      "data dir)")(
      "checkpoint,c", bpo::value<vector<string>>()->composing(),
      "Pairs of [BLOCK_NUM,BLOCK_ID] that should be enforced as checkpoints.")(
      "rcvd-block-trans-execution-time",
      bpo::value<uint32_t>()->default_value(
          DEFAULT_RECEIVED_BLOCK_TRANSACTION_EXECUTION_TIME),
      "Limits the maximum time (in milliseconds) that is allowed a "
      "transaction's code to execute from a received block.")(
      "trans-execution-time",
      bpo::value<uint32_t>()->default_value(DEFAULT_TRANSACTION_EXECUTION_TIME),
      "Limits the maximum time (in milliseconds) that is allowed a pushed "
      "transaction's code to execute.")(
      "create-block-trans-execution-time",
      bpo::value<uint32_t>()->default_value(
          DEFAULT_CREATE_BLOCK_TRANSACTION_EXECUTION_TIME),
      "Limits the maximum time (in milliseconds) that is allowed a "
      "transaction's code to execute while creating a block.");
  cli.add_options()("replay-blockchain",
...
void chain_plugin::plugin_initialize(const variables_map &options) {
  ...
  if (options.count("genesis-timestamp")) {
    ...
    if (strcasecmp(tstr.c_str(), "now") == 0) {
      ...
      auto diff = my->genesis_timestamp.sec_since_epoch() %
  ...
  if (options.count("checkpoint")) {
    ...
    for (auto cp : cps) {
      auto item =
          fc::json::from_string(cp).as<std::pair<uint32_t, block_id_type>>();
      my->loaded_checkpoints[item.first] = item.second;
    }
  }

  my->rcvd_block_trans_execution_time =
      options.at("rcvd-block-trans-execution-time").as<uint32_t>() * 1000;
  my->trans_execution_time =
      options.at("trans-execution-time").as<uint32_t>() * 1000;
  my->create_block_trans_execution_time =
      options.at("create-block-trans-execution-time").as<uint32_t>() * 1000;
}
...
void chain_plugin::plugin_startup() {
  try {
    ...
    FC_ASSERT(
        fc::exists(my->genesis_file),
        "unable to find genesis file '${f}', check --genesis-json argument",
        ("f", my->genesis_file.generic_string()));

    auto genesis = fc::json::from_file(my->genesis_file)
                       .as<native_contract::genesis_state_type>();
    ...
    my->chain = chain_controller(
        db, *my->fork_db, *my->block_logger, initializer,
        native_contract::make_administrator(), my->trans_execution_time,
        my->rcvd_block_trans_execution_time,
        my->create_block_trans_execution_time);
    ...
    ilog(
        ...
        ("num", my->chain->head_block_num())(
            "ts", genesis.initial_timestamp.to_iso_string()));
  }
...
bool chain_plugin::accept_block(const chain::signed_block &block,
                                bool currently_syncing) {
  if (currently_syncing && block.block_num() % 10000 == 0) {
    ilog("Syncing Blockchain --- Got block: #${n} time: ${t} producer: ${p}",
         ("t", block.timestamp)("n", block.block_num())("p", block.producer));
  }
...
bool chain_plugin::block_is_on_preferred_chain(
    const chain::block_id_type &block_id) {
  // If it's not known, it's not preferred.
  if (!chain().is_known_block(block_id))
    return false;
  // Extract the block number from block_id, and fetch that block number's ID
  // from the database.
  // If the database's block ID matches block_id, then block_id is on the
  // preferred chain. Otherwise, it's on a fork.
  return chain().get_block_id_for_num(
             chain::block_header::num_from_id(block_id)) == block_id;
}
...
const chain::chain_controller &chain_plugin::chain() const {
  return *my->chain;
}
...
read_only::get_info(const read_only::get_info_params &) const {
  return {
      db.head_block_num(), db.last_irreversible_block_num(), db.head_block_id(),
      db.head_block_time(), db.head_block_producer(),
...
types::Abi getAbi(const chain_controller &db, const Name &account) {
  ...
  if (code_accnt.abi.size() > 4) {
    fc::datastream<const char *> ds(code_accnt.abi.data(),
                                    code_accnt.abi.size());
...
string getTableType(const types::Abi &abi, const Name &tablename) {
  ...
  FC_ASSERT(!"Abi does not define table", "Table ${table} not specified in ABI",
            ("table", tablename));
}

read_only::get_table_rows_result
read_only::get_table_rows(const read_only::get_table_rows_params &p) const {
  const auto &d = db.get_database();
  ...
  if (table_type == KEYi64) {
    return get_table_rows_ex<chain::key_value_index, chain::by_scope_primary>(
        p, abi);
  } else if (table_type == KEYstr) {
    return get_table_rows_ex<chain::keystr_value_index,
                             chain::by_scope_primary>(p, abi);
  } else if (table_type == KEYi128i128) {
    if (table_key == PRIMARY)
      return get_table_rows_ex<chain::key128x128_value_index,
                               chain::by_scope_primary>(p, abi);
    if (table_key == SECONDARY)
      return get_table_rows_ex<chain::key128x128_value_index,
                               chain::by_scope_secondary>(p, abi);
  } else if (table_type == KEYi64i64i64) {
    if (table_key == PRIMARY)
      return get_table_rows_ex<chain::key64x64x64_value_index,
                               chain::by_scope_primary>(p, abi);
    if (table_key == SECONDARY)
      return get_table_rows_ex<chain::key64x64x64_value_index,
                               chain::by_scope_secondary>(p, abi);
    if (table_key == TERTIARY)
      return get_table_rows_ex<chain::key64x64x64_value_index,
                               chain::by_scope_tertiary>(p, abi);
  }
  FC_ASSERT(false, "invalid table type/key ${type}/${key}",
            ("type", table_type)("key", table_key)("abi", abi));
}

read_only::get_block_results
read_only::get_block(const read_only::get_block_params &params) const {
  try {
    if (auto block =
            db.fetch_block_by_id(fc::json::from_string(params.block_num_or_id)
  ...
  try {
    if (auto block =
            db.fetch_block_by_number(fc::to_uint64(params.block_num_or_id)))
      return *block;
  } catch (fc::bad_cast_exception) { /* do nothing */
  }
...
read_write::push_transaction_results read_write::push_transaction(
    const read_write::push_transaction_params &params) {
...
read_write::push_transactions_results read_write::push_transactions(
    const read_write::push_transactions_params &params) {
  FC_ASSERT(params.size() <= 1000,
  ...
  for (const auto &item : params) {
    try {
      ...
      result.emplace_back(read_write::push_transaction_results{
          chain::transaction_id_type(),
...
read_only::get_code(const get_code_params &params) const {
...
read_only::get_account(const get_account_params &params) const {
  ...
  while (perm != permissions.end() && perm->owner == params.name) {
    ...
    result.permissions.push_back(
        permission{perm->name, parent, perm->auth.to_authority()});
    ++perm;
  }

  return result;
}
read_only::abi_json_to_bin_result read_only::abi_json_to_bin(
    const read_only::abi_json_to_bin_params &params) const {
  abi_json_to_bin_result result;
  result.binargs =
      db.message_to_binary(params.code, params.action, params.args);
  return result;
}
read_only::abi_bin_to_json_result read_only::abi_bin_to_json(
    const read_only::abi_bin_to_json_params &params) const {
  abi_bin_to_json_result result;
  result.args =
      db.message_from_binary(params.code, params.action, params.binargs);
  return result;
}
...
read_only::get_required_keys(const get_required_keys_params &params) const {
  ...
  auto required_keys_set =
      db.get_required_keys(pretty_input, params.available_keys);
