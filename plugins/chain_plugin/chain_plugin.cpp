...
static fc::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis, const fc::microseconds& abi_serializer_max_time_ms ) {
   ...
   return abis.binary_to_variant(abis.get_table_type(N(global)), data, abi_serializer_max_time_ms, true ); // TODO: Allow configuration to output verbose error messages
}
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const {
   ...
   for( ; it != secondary_index_by_secondary.end() && it->t_id == secondary_table_id->id; ++it ) {
      ...
      if (p.json)
         result.rows.emplace_back(abis.binary_to_variant(abis.get_table_type(N(producers)), data, abi_serializer_max_time, true)); // TODO: Allow configuration to output verbose error messages
      else
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      if (t_id != nullptr) {
         ...
         if ( it != idx.end() ) {
            ...
            result.total_resources = abis.binary_to_variant( "user_resources", data, abi_serializer_max_time, true ); // TODO: Allow configuration to output verbose error messages
         }
      }
      ...
      if (t_id != nullptr) {
         ...
         if ( it != idx.end() ) {
            vector<char> data;
            copy_inline_row(*it, data);
            result.self_delegated_bandwidth = abis.binary_to_variant( "delegated_bandwidth", data, abi_serializer_max_time, true ); // TODO: Allow configuration to output verbose error messages
         }
      }
      ...
      if (t_id != nullptr) {
         ...
         if ( it != idx.end() ) {
            vector<char> data;
            copy_inline_row(*it, data);
            result.refund_request = abis.binary_to_variant( "refund_request", data, abi_serializer_max_time, true ); // TODO: Allow configuration to output verbose error messages
         }
      }
      ...
      if (t_id != nullptr) {
         ...
         if ( it != idx.end() ) {
            vector<char> data;
            copy_inline_row(*it, data);
            result.voter_info = abis.binary_to_variant( "voter_info", data, abi_serializer_max_time, true ); // TODO: Allow configuration to output verbose error messages
         }
      }
   }
   return result;
...
read_only::abi_bin_to_json_result read_only::abi_bin_to_json( const read_only::abi_bin_to_json_params& params )const {
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      abi_serializer abis( abi, abi_serializer_max_time );
      result.args = abis.binary_to_variant( abis.get_action_type( params.action ), params.binargs, abi_serializer_max_time, true ); // TODO: Allow configuration to output verbose error messages
   } else {
      EOS_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
