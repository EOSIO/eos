...
public:
  ...
  read_only(const chain_controller &db) : db(db) {}
  ...
  abi_json_to_bin(const abi_json_to_bin_params &params) const;
  ...
  abi_bin_to_json(const abi_bin_to_json_params &params) const;
  ...
  get_required_keys(const get_required_keys_params &params) const;
  ...
  struct get_block_results : public chain::signed_block {
    get_block_results(const chain::signed_block &b)
        : signed_block(b), id(b.id()), block_num(b.block_num()),
  ...
  struct get_table_rows_result {
    ...
    bool more; ///< true if last element in data is not the end and sizeof
               ///data() < limit
  };

  get_table_rows_result
  get_table_rows(const get_table_rows_params &params) const;
  ...
  void copy_row(const chain::keystr_value_object &obj,
  ...
  void copy_row(const chain::key128x128_value_object &obj,
                vector<char> &data) const {
    data.resize(2 * sizeof(uint128_t) + obj.value.size());
    memcpy(data.data(), &obj.primary_key, sizeof(uint128_t));
    memcpy(data.data() + sizeof(uint128_t), &obj.secondary_key,
           sizeof(uint128_t));
    memcpy(data.data() + 2 * sizeof(uint128_t), obj.value.data(),
           obj.value.size());
  }

  void copy_row(const chain::key64x64x64_value_object &obj,
                vector<char> &data) const {
    data.resize(3 * sizeof(uint64_t) + obj.value.size());
    memcpy(data.data(), &obj.primary_key, sizeof(uint64_t));
    memcpy(data.data() + sizeof(uint64_t), &obj.secondary_key,
           sizeof(uint64_t));
    memcpy(data.data() + 2 * sizeof(uint64_t), &obj.tertiary_key,
           sizeof(uint64_t));
    memcpy(data.data() + 3 * sizeof(uint64_t), obj.value.data(),
           obj.value.size());
  }
  ...
  get_table_rows_ex(const read_only::get_table_rows_params &p,
                    const types::Abi &abi) const {
    read_only::get_table_rows_result result;
    const auto &d = db.get_database();
    const auto &code_account =
        d.get<chain::account_object, chain::by_name>(p.code);
    ...
    auto upper = idx.upper_bound(
        boost::make_tuple(p.scope, p.code, Name(uint64_t(p.table) + 1)));

    if (p.lower_bound.size())
      lower = idx.lower_bound(boost::make_tuple(
          p.scope, p.code, p.table,
          fc::variant(p.lower_bound)
              .as<typename IndexType::value_type::key_type>()));
    if (p.upper_bound.size())
      upper = idx.lower_bound(boost::make_tuple(
          p.scope, p.code, p.table,
          fc::variant(p.upper_bound)
              .as<typename IndexType::value_type::key_type>()));

    vector<char> data;

    auto end =
        fc::time_point::now() + fc::microseconds(1000 * 10); /// 10ms max time
    ...
    for (itr = lower; itr != upper && itr->table == p.table; ++itr) {
      ...
      if (p.json)
        result.rows.emplace_back(
            abis.binaryToVariant(abis.getTableType(p.table), data));
      else
...
public:
  read_write(chain_controller &db, uint32_t skip_flags)
      : db(db), skip_flags(skip_flags) {}
  ...
  push_transaction(const push_transaction_params &params);
  ...
  push_transactions(const push_transactions_params &params);
};
} // namespace chain_apis
...
FC_REFLECT(
    eos::chain_apis::read_only::get_info_results,
    (head_block_num)(last_irreversible_block_num)(head_block_id)(
        head_block_time)(head_block_producer)(recent_slots)(participation_rate))
FC_REFLECT(eos::chain_apis::read_only::get_block_params, (block_num_or_id))

FC_REFLECT_DERIVED(eos::chain_apis::read_only::get_block_results,
                   (eos::chain::signed_block), (id)(block_num)(refBlockPrefix));
FC_REFLECT(eos::chain_apis::read_write::push_transaction_results,
           (transaction_id)(processed))

FC_REFLECT(
    eos::chain_apis::read_only::get_table_rows_params,
    (json)(table_key)(scope)(code)(table)(lower_bound)(upper_bound)(limit))
FC_REFLECT(eos::chain_apis::read_only::get_table_rows_result, (rows)(more));

FC_REFLECT(eos::chain_apis::read_only::get_account_results,
           (name)(eos_balance)(staked_balance)(unstaking_balance)(
               last_unstaking_time)(permissions)(producer))
FC_REFLECT(eos::chain_apis::read_only::get_code_results,
           (name)(code_hash)(wast)(abi))
...
FC_REFLECT(eos::chain_apis::read_only::abi_json_to_bin_params,
           (code)(action)(args))
FC_REFLECT(eos::chain_apis::read_only::abi_json_to_bin_result,
           (binargs)(required_scope)(required_auth))
FC_REFLECT(eos::chain_apis::read_only::abi_bin_to_json_params,
           (code)(action)(binargs))
FC_REFLECT(eos::chain_apis::read_only::abi_bin_to_json_result,
           (args)(required_scope)(required_auth))
FC_REFLECT(eos::chain_apis::read_only::get_required_keys_params,
           (transaction)(available_keys))
FC_REFLECT(eos::chain_apis::read_only::get_required_keys_result,
           (required_keys))
