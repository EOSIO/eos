...
private:
   ...
   void store_block_trace( const chain::block_state_ptr& block_state ) {
      try {
         block_trace_v1<action_trace_v1> bt = create_block_trace<block_trace_v1<action_trace_v1>>( block_state );

         std::vector<transaction_trace_v1<action_trace_v1>>& traces = bt.transactions_v1;
         traces.reserve( block_state->block->transactions.size() + 1 );
         if( onblock_trace )
            traces.emplace_back( to_transaction_trace<transaction_trace_v1<action_trace_v1>>( *onblock_trace ));
         for( const auto& r : block_state->block->transactions ) {
            ...
            if( it != cached_traces.end() ) {
               traces.emplace_back( to_transaction_trace<transaction_trace_v1<action_trace_v1>>( it->second ));
            }
         }
         clear_caches();
