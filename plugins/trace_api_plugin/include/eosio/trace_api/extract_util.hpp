...
inline transaction_trace_v3 to_transaction_trace_v3( const cache_trace& t ) {
    transaction_trace_v3 r;
    if( !t.trace->failed_dtrx_trace ) {
        r.id = t.trace->id;
    } else {
        r.id = t.trace->failed_dtrx_trace->id; // report the failed trx id since that is the id known to user
...
/// @return block_trace_v2 without any transaction_trace_v2
inline block_trace_v2 create_block_trace_v2( const chain::block_state_ptr& bsp ) {
   block_trace_v2 r;
   r.id = bsp->id;
   r.number = bsp->block_num;
   r.previous_id = bsp->block->previous;
...
/// @return block_trace_v3 without any transaction_trace_v3
inline block_trace_v3 create_block_trace_v3( const chain::block_state_ptr& bsp ) {
    block_trace_v3 r;
    r.id = bsp->id;
    r.number = bsp->block_num;
    r.previous_id = bsp->block->previous;
