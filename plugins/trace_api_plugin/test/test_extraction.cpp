...
struct extraction_test_fixture {
   ...
   struct mock_logfile_provider_type {
       ...
       */
      void append( const block_trace_v0<action_trace_v0>& entry ) {
         fixture.data_log.emplace_back(entry);
      }

      void append( const block_trace_v1<action_trace_v0>& entry ) {
         fixture.data_log.emplace_back(entry);
      }

      void append( const block_trace_v1<action_trace_v1>& entry ) {
         fixture.data_log.emplace_back(entry);
      }
...
BOOST_AUTO_TEST_SUITE(block_extraction)
   ...
   BOOST_FIXTURE_TEST_CASE(basic_single_transaction_block, extraction_test_fixture)
   {
      ...
      const block_trace_v1<action_trace_v1> expected_trace{
         {
         bsp1->id,
         1,
         bsp1->prev(),
