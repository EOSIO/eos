...
class debug_plugin_impl {
   public:
      ...
      typedef void (*apply_function_type)(uint64_t, uint64_t);
...
void debug_plugin_impl::dummy_action_handler(chain::apply_context& context) {
   ...
   if (fn_handle != 0 && a.code.size() > 0) {
      auto code = chain.get_wasm_cache().checkout_scoped(a.code_version, a.code.data(), a.code.size());
      eosio::chain::wasm_interface::get().apply_native_debug(code, context, fn_handle);
   }
   else {
