...
producer_plugin_impl::producer_plugin_impl(boost::asio::io_service &io)
: _timer(io)
, _transaction_ack_channel(app().get_channel<compat::channels::transaction_ack>())
{
}

void producer_plugin_impl::consider_new_watermark( account_name producer, uint32_t block_num, block_timestamp_type timestamp) {
...
std::optional<producer_watermark> producer_plugin_impl::get_watermark( account_name producer ) const {
   auto itr = _producer_watermarks.find( producer );
   if( itr == _producer_watermarks.end() ) return {};
   return itr->second;
}

void producer_plugin_impl::on_block( const block_state_ptr& bsp ) {
...
void producer_plugin_impl::on_block_header( const block_state_ptr& bsp ) {
   consider_new_watermark( bsp->header.producer, bsp->block_num, bsp->block->timestamp );
}

void producer_plugin_impl::on_irreversible_block( const signed_block_ptr& lib ) {
...
bool producer_plugin_impl::on_incoming_block(const signed_block_ptr& block, const std::optional<block_id_type>& block_id) {
...
void producer_plugin_impl::on_incoming_transaction_async(const packed_transaction_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
...
bool producer_plugin_impl::process_incoming_transaction_async(const transaction_metadata_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
