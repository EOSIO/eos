...
public:
   ...
   void subjective_bill( const transaction_id_type& id, const fc::time_point& expire, const account_name& authorizer,
                         const fc::microseconds& elapsed, bool in_pending_block )
   {
      if( !_disabled && !_disabled_accounts.count( authorizer ) ) {
         ...
         auto p = _trx_cache_index.emplace(
               trx_cache_entry{id,
                               ...
                               expire} );
         if( p.second ) {
            _account_subjective_bill_cache[authorizer].pending_cpu_us += bill;
            if( in_pending_block ) {
               _block_subjective_bill_cache[authorizer] += bill;
            }
         }
      }
   }

   void subjective_bill_failure( const account_name& authorizer, const fc::microseconds& elapsed, const fc::time_point& now )
   {
      if( !_disabled && !_disabled_accounts.count( authorizer ) ) {
         uint32_t bill = std::max<int64_t>( 0, elapsed.count() );
         const auto time_ordinal = time_ordinal_for(now);
         _account_subjective_bill_cache[authorizer].expired_accumulator.add(bill, time_ordinal, expired_accumulator_average_window);
      }
   }

   uint32_t get_subjective_bill( const account_name& authorizer, const fc::time_point& now ) const {
      if( _disabled || _disabled_accounts.count( authorizer ) ) return 0;
      const auto time_ordinal = time_ordinal_for(now);
      const subjective_billing_info* sub_bill_info = nullptr;
      auto aitr = _account_subjective_bill_cache.find( authorizer );
      if( aitr != _account_subjective_bill_cache.end() ) {
         sub_bill_info = &aitr->second;
      }
      uint64_t in_block_pending_cpu_us = 0;
      auto bitr = _block_subjective_bill_cache.find( authorizer );
      if( bitr != _block_subjective_bill_cache.end() ) {
         in_block_pending_cpu_us = bitr->second;
      }

      if (sub_bill_info) {
         EOS_ASSERT(sub_bill_info->pending_cpu_us >= in_block_pending_cpu_us, chain::tx_resource_exhaustion, "Logic error subjective billing ${a}", ("a", authorizer) );
