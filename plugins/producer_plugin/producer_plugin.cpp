   ...
   public:
               ...
               app().post( priority::low, [self, future{std::move(future)}, persist_until_expired, next{std::move( next )}, trx{std::move(trx)}, return_failure_trace]() mutable {
                  auto exception_handler = [self, &next, trx{std::move(trx)}](fc::exception_ptr ex) {
                     fc_dlog(_trx_failed_trace_log, "[TRX_TRACE] Speculative execution is REJECTING tx: ${txid}, auth: ${a} : ${why} ",
                            ("txid", trx->id())("a",trx->get_transaction().authorizer())("why",ex->what()));
                     next(ex);
      ...
      bool process_incoming_transaction_async(const transaction_metadata_ptr& trx,
            ...
            if (std::holds_alternative<fc::exception_ptr>(response)) {
               ...
               if (_pending_block_mode == pending_block_mode::producing) {
                  fc_dlog(_trx_failed_trace_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is REJECTING tx: ${txid}, auth: ${a} : ${why} ",
                        ...
                        ("txid", trx->id())
                        ("a", trx->packed_trx()->get_transaction().authorizer())
         ...
         try {
            ...
            auto authorizer = trx->packed_trx()->get_transaction().authorizer();
            uint32_t sub_bill = 0;
            if( !disable_subjective_billing )
               sub_bill = _subjective_billing.get_subjective_bill( authorizer, fc::time_point::now() );

            auto trace = chain.push_transaction( trx, deadline, trx->billed_cpu_time_us, false, sub_bill );
            fc_dlog( _trx_failed_trace_log, "Subjective bill for ${a}: ${b} elapsed ${t}us", ("a",authorizer)("b",sub_bill)("t",trace->elapsed));
            if( trace->except ) {
               ...
               if( persist_until_expired && !_disable_persist_until_expired ) {
                  ...
                  _subjective_billing.subjective_bill( trx->id(), expire, authorizer, trace->elapsed,
   ...
   if( !_unapplied_transactions.empty() ) {
      ...
      while( itr != end_itr ) {
         ...
         try {
            ...
            auto authorizer = trx->packed_trx()->get_transaction().authorizer();
            if( account_fails.failure_limit( authorizer ) ) {
               ++num_failed;
            ...
            if( !_subjective_billing.is_disabled() && prev_billed_cpu_time_us > 0 && !rl.is_unlimited_cpu( authorizer )) {
            ...
            fc_dlog( _trx_failed_trace_log, "Subjective unapplied bill for ${a}: ${b} prev ${t}us", ("a",authorizer)("b",prev_billed_cpu_time_us)("t",trace->elapsed));
            if( trace->except ) {
               if( exception_is_exhausted( *trace->except, deadline_is_subjective ) ) {
                  ...
                  fc_dlog( _trx_failed_trace_log, "Subjective unapplied bill for failed ${a}: ${b} prev ${t}us", ("a",authorizer)("b",prev_billed_cpu_time_us)("t",trace->elapsed));
                  ...
                  if( failure_code != tx_duplicate::code_value ) {
                     ...
                     account_fails.add( authorizer, failure_code );
                     _subjective_billing.subjective_bill_failure( authorizer, trace->elapsed, fc::time_point::now() );
                  }
                  ++num_failed;
               ...
               fc_dlog( _trx_successful_trace_log, "Subjective unapplied bill for success ${a}: ${b} prev ${t}us", ("a",authorizer)("b",prev_billed_cpu_time_us)("t",trace->elapsed));
               // if db_read_mode SPECULATIVE then trx is in the pending block and not immediately reverted
               _subjective_billing.subjective_bill( trx->id(), trx->packed_trx()->expiration(), authorizer, trace->elapsed,
                                                    chain.get_read_mode() == chain::db_read_mode::SPECULATIVE );
               ++num_applied;
