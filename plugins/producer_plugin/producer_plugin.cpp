...
public:
  ...
  eos::chain::block_schedule::factory _production_scheduler =
      eos::chain::block_schedule::in_single_thread;
...
void new_chain_banner(const eos::chain::chain_controller &db) {
  ...
  if (db.get_slot_at_time(fc::time_point::now()) > 200) {
    std::cerr << "Your genesis seems to have an old timestamp\n"
                 "Please consider using the --genesis-timestamp option to give "
                 "your genesis a recent timestamp\n"
                 "\n";
  }
  return;
}
...
void producer_plugin::set_program_options(
  ...
  auto default_priv_key =
      fc::ecc::private_key::regenerate(fc::sha256::hash(std::string("nathan")));

  auto private_key_default =
      std::make_pair(chain::public_key_type(default_priv_key.get_public_key()),
  ...
  producer_options.add_options()(
      ...
      boost::program_options::bool_switch()->notifier(
          [this](bool e) { my->_production_enabled = e; }),
      "Enable block production, even if the chain is stale.")(
      "required-participation",
      boost::program_options::bool_switch()->notifier([this](int e) {
        my->_required_producer_participation = uint32_t(e * config::Percent1);
      }),
      "Percent of producers (0-99) that must be participating in order to "
      "produce blocks")("producer-name,p",
      ...
      boost::program_options::value<vector<string>>()
          ->composing()
          ->multitoken()
          ->default_value({fc::json::to_string(private_key_default)},
  ...
  command_line_options.add_options()(
      ...
      boost::program_options::value<string>()
          ->default_value("single-thread")
          ->notifier([this](const string &v) {
            if (v == "single-thread") {
              my->_production_scheduler =
                  eos::chain::block_schedule::in_single_thread;
            } else if (v == "cycling-conflicts") {
              ilog("Using scheduler by_cycling_conflicts");
              my->_production_scheduler =
                  eos::chain::block_schedule::by_cycling_conflicts;
            } else if (v == "threading-conflicts") {
              ilog("Using scheduler by_threading_conflicts");
              my->_production_scheduler =
                  eos::chain::block_schedule::by_threading_conflicts;
            } else {
      ...
      "  single-thread\n"
      "    \tA reference scheduler that puts all transactions in a single "
      "thread (FIFO).\n"
      "  cycling-conflicts\n"
      "    \tA greedy scheduler that attempts to cycle through threads to "
      "resolve scope contention before falling back on cycles.\n"
      "  threading-conflicts\n"
      "    \tA greedy scheduler that attempts to make short threads to resolve "
      "scope contention before falling back on cycles.");
}

template <typename T> T dejsonify(const string &s) {
...
#define LOAD_VALUE_SET(options, name, container, type)                         \
  if (options.count(name)) {                                                   \
    const std::vector<std::string> &ops =                                      \
        options[name].as<std::vector<std::string>>();                          \
    std::copy(ops.begin(), ops.end(),                                          \
              std::inserter(container, container.end()));                      \
  }

void producer_plugin::plugin_initialize(
  ...
  try {
    ...
    if (options.count("private-key")) {
      const std::vector<std::string> key_id_to_wif_pair_strings =
          options["private-key"].as<std::vector<std::string>>();
      for (const std::string &key_id_to_wif_pair_string :
           key_id_to_wif_pair_strings) {
        auto key_id_to_wif_pair =
            dejsonify<std::pair<chain::public_key_type, std::string>>(
        ...
        fc::optional<fc::ecc::private_key> private_key =
            eos::utilities::wif_to_key(key_id_to_wif_pair.second);
...
void producer_plugin::plugin_startup() {
  try {
    ...
    if (!my->_producers.empty()) {
      ilog("Launching block production for ${n} producers.",
           ("n", my->_producers.size()));
      if (my->_production_enabled) {
        if (chain.head_block_num() == 0)
          new_chain_banner(chain);
        my->_production_skip_flags |=
            eos::chain::chain_controller::skip_undo_history_check;
      }
      my->schedule_production_loop();
    } else
      elog("No producers configured! Please add producer IDs and private keys "
           "to configuration.");
...
void producer_plugin_impl::schedule_production_loop() {
  ...
  int64_t time_to_next_second =
      1000000 - (now.time_since_epoch().count() % 1000000);
  if (time_to_next_second < 50000) // we must sleep for at least 50ms
    time_to_next_second += 1000000;

  _timer.expires_from_now(boost::posix_time::microseconds(time_to_next_second));
  _timer.async_wait(
      boost::bind(&producer_plugin_impl::block_production_loop, this));
}
...
producer_plugin_impl::block_production_loop() {
  ...
  case block_production_condition::produced: {
    ...
    wlog("${p} generated block #${n} @ ${t} with ${count} trxs  ${pending} "
         "pending",
         ("p", producer)(capture)("pending", pending));
    break;
  }
  ...
  case block_production_condition::no_private_key:
    ilog("Not producing block because I don't have the private key for "
         "${scheduled_key}",
         (capture));
    break;
  case block_production_condition::low_participation:
    elog("Not producing block because node appears to be on a minority fork "
         "with only ${pct}% producer participation",
         (capture));
    break;
...
block_production_condition::block_production_condition_enum
producer_plugin_impl::maybe_produce_block(fc::mutable_variant_object &capture) {
  chain::chain_controller &chain = app().get_plugin<chain_plugin>().chain();
  ...
  eos::types::AccountName scheduled_producer =
      chain.get_scheduled_producer(slot);
  ...
  eos::chain::public_key_type scheduled_key =
      chain.get_producer(scheduled_producer).signing_key;
  ...
  auto block = chain.generate_block(
      scheduled_time, scheduled_producer, private_key_itr->second,
      _production_scheduler, _production_skip_flags);
  ...
  capture("n", block.block_num())("t", block.timestamp)("c", now)("count",
                                                                  count);
