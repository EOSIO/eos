...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      void on_incoming_transaction_async(const packed_transaction_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
         chain::controller& chain = chain_plug->chain();
         bool is_transaction_sponsorship_fp_activated = chain.is_builtin_activated(builtin_protocol_feature_t::transaction_sponsorship);
      ...
      bool process_incoming_transaction_async(const transaction_metadata_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
         ...
         auto send_response = [this, &trx, &chain, &next, is_transaction_sponsorship_fp_activated](const std::variant<fc::exception_ptr, transaction_trace_ptr>& response) {
            next(response);
            ...
            if (std::holds_alternative<fc::exception_ptr>(response)) {
               ...
               if (_pending_block_mode == pending_block_mode::producing) {
                  fc_dlog(_trx_failed_trace_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is REJECTING tx: ${txid}, auth: ${a} : ${why} ",
                        ...
                        ("txid", trx->id())
                        ("a", trx->packed_trx()->get_transaction().resource_payer(is_transaction_sponsorship_fp_activated))
         ...
         try {
            ...
            auto resource_payer = trx->packed_trx()->get_transaction().resource_payer(is_transaction_sponsorship_fp_activated);
            uint32_t sub_bill = 0;
