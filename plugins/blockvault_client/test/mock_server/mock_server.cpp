...
#include "test_server_certificate.hpp"
...
#define CLUSTER_INIT_FILE_PATH             "/Users/keke.li/blockvault_cluster.ini"
...
private:
    ...
    void process_request(http::request<request_body_t, http::basic_fields<alloc_t>> const& req)
    {
        ...
        case http::verb::get:{
            ...
            std::string response = "Receiced:"; 
            std::string _req(req.target().data(),req.target().size());
            response += _req;
            send_string_response(http::status::ok, response);
            break;
         }
        default:
        ...
        std::string const& body)
    {
        ...
        string_response_->result(status);
        string_response_->keep_alive(false);
        string_response_->set(http::field::server, "BV mock server");
        string_response_->set(http::field::content_type, "text/plain");
    ...
    void send_file_response(beast::string_view target)
    {
        ...
        file_response_->set(http::field::server, "BV mock server");
...
std::vector<std::pair<std::string, std::string>> get_init_config(const std::string& config_file_path){  // fetch init cluster from file      
      ...
      std::ifstream infile(config_file_path, std::ifstream::in);
      std::string line;
      int line_num = 0;
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        std::string default_config_file = std::string(CLUSTER_INIT_FILE_PATH);
        std::cout << "config file: " << default_config_file << std::endl;  
        std::vector<std::pair<std::string, std::string>> cluster_config;
        cluster_config = get_init_config(default_config_file);
        std::cout << "cluster config begin : " << std::endl;
        for(const auto & p : cluster_config) {
         std::cout << p.first << ":" << p.second << std::endl;
        }
        std::cout << "cluster config end. " << std::endl;
        net::io_context ioc_array[MAX_CLUSTER_NUM];
        tcp::acceptor * ppacceptor[MAX_CLUSTER_NUM]={nullptr};
        net::io_context * pioc = ioc_array;

        std::vector<std::thread> thv;
        thv.reserve(cluster_config.size());

        for(int i = 0; i < cluster_config.size(); ++i){
           auto const address = net::ip::make_address(cluster_config[i].first.c_str());
           unsigned short port = static_cast<unsigned short>(std::atoi(cluster_config[i].second.c_str()));
           int num_workers = 1;
           bool spin = false;

           //ppioc[i] = new net::io_context;
           //if(ppioc[i] == nullptr) {
           //   std::cout << "new ioc failed!, return.  " << std::endl;
           //}
 ...
 // Create and launch a listening port
           ...
           ppacceptor[i] = new tcp::acceptor {pioc[i], {address, port}};
           if(ppacceptor[i] == nullptr) {
               std::cout << "new acceptro failed! return -1!" << std::endl;
               return -1;
            }
           ...
           {
               workers.emplace_back(*ppacceptor[i], std::string("~/"));
               workers.back().start();
           }

           thv.emplace_back([=] { pioc[i].run(); });

         // (If we get here, it means we got a SIGINT or SIGTERM)

           // Block until all the threads exit

           std::cout << address << ":" << port << " is up." << std::endl;
           //if (spin)
           //  for (;;) ioc.poll();
           //else
           //  ioc.run();
        ...
        for(int k = 0; k < MAX_CLUSTER_NUM; ++k){
           if(ppacceptor[k] != nullptr) delete ppacceptor[k];
        }
    }
