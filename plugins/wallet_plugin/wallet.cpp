...
fc::ecc::private_key derive_private_key(const std::string &prefix_string,
  ...
  fc::ecc::private_key derived_key =
      fc::ecc::private_key::regenerate(fc::sha256::hash(h));
  return derived_key;
}
...
public:
  wallet_api &self;
  wallet_api_impl(wallet_api &s, const wallet_data &initial_data) : self(s) {}
  ...
  bool copy_wallet_file(string destination_filename) {
    ...
    while (fc::exists(dest_path)) {
      ++suffix;
      dest_path = destination_filename + "-" + std::to_string(suffix) +
  ...
  bool is_locked() const { return _checksum == fc::sha512(); }

  string get_wallet_filename() const { return _wallet_filename; }

  optional<fc::ecc::private_key>
  try_get_private_key(const public_key_type &id) const {
  ...
  // imports the private key into the wallet, and associate it in some way (?)
  // with the
  // given account name.
  // @returns true if the key matches a current active/owner/memo key for the
  // named
  ...
  bool import_key(string wif_key) {
    ...
    eos::chain::public_key_type wif_pub_key =
        optional_private_key->get_public_key();
...
bool wallet_api::is_locked() const { return my->is_locked(); }

bool wallet_api::is_new() const { return my->_wallet.cipher_keys.size() == 0; }

void wallet_api::encrypt_keys() { my->encrypt_keys(); }
...
wallet_api::get_private_key_from_password(string account, string role,
