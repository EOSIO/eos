...
std::string wallet_manager::create(const std::string &name) {
  ...
  if (fc::exists(wallet_filename)) {
    FC_THROW("Wallet with name: '${n}' already exists at ${path}",
             ("n", name)("path", fc::path(wallet_filename)));
  }
  ...
  // If we have name in our map then remove it since we want the emplace below
  // to replace.
...
void wallet_manager::unlock(const std::string &name,
...
void wallet_manager::import_key(const std::string &name,
...
wallet_manager::sign_transaction(const chain::SignedTransaction &txn,
