...
/// Provides associate of wallet name to wallet and manages the interaction with
/// each wallet.
///
/// The name of the wallet is also used as part of the file name by wallet_api.
/// See wallet_manager::create.
...
public:
  ...
  /// Use chain_controller::get_required_keys to determine which keys are needed
  /// for txn.
  /// @param txn the transaction to sign.
  /// @param keys the public keys of the corresponding private keys to sign the
  /// transaction with
  /// @param id the chain_id to sign transaction with.
  /// @return txn signed
  /// @throws fc::exception if corresponding private keys not found in unlocked
  /// wallets
  chain::SignedTransaction
  sign_transaction(const chain::SignedTransaction &txn,
  ...
  /// @return Plaintext password that is needed to unlock wallet. Caller is
  /// responsible for saving password otherwise
  ///         they will not be able to unlock their wallet. Note user supplied
  ///         passwords are not supported.
  /// @throws fc::exception if wallet with name already exists (or filename
  /// already exists)
  ...
  /// @return A list of wallet names with " *" appended if the wallet is
  /// unlocked.
  ...
  /// @return A set of public keys from all unlocked wallets, use with
  /// chain_controller::get_required_keys.
  ...
  /// @param wif_key the WIF Private Key to import, e.g.
...
private:
  ...
  std::chrono::seconds timeout =
      std::chrono::seconds::max(); ///< how long to wait before calling
                                   ///lock_all()
  mutable timepoint_t timeout_time =
      timepoint_t::max(); ///< when to call lock_all()
