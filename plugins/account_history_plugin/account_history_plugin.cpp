...
using ordered_transaction_results =
    account_history_apis::read_only::ordered_transaction_results;
using get_transactions_results =
    account_history_apis::read_only::get_transactions_results;

class account_history_plugin_impl {
public:
  ProcessedTransaction
  get_transaction(const chain::transaction_id_type &transaction_id) const;
  get_transactions_results
  get_transactions(const AccountName &account_name,
                   const optional<uint32_t> &skip_seq,
                   const optional<uint32_t> &num_seq) const;
  vector<AccountName> get_key_accounts(const public_key_type &public_key) const;
  vector<AccountName>
  get_controlled_accounts(const AccountName &controlling_account) const;
...
private:
  struct block_comp {
    bool operator()(const block_id_type &a, const block_id_type &b) const {
      return chain::block_header::num_from_id(a) >
             chain::block_header::num_from_id(b);
    }
  };
  typedef std::multimap<block_id_type, transaction_id_type, block_comp>
      block_transaction_id_map;

  optional<block_id_type>
  find_block_id(const chainbase::database &db,
                const transaction_id_type &transaction_id) const;
  ProcessedTransaction
  ...
  get_transactions_results
  ordered_transactions(const block_transaction_id_map &block_transaction_ids,
                       const fc::time_point &start_time, const uint32_t begin,
                       const uint32_t end) const;
  static void add(chainbase::database &db,
                  const vector<types::KeyPermissionWeight> &keys,
                  const AccountName &account_name,
                  const PermissionName &permission);
  template <typename MultiIndex, typename LookupType>
  static void remove(chainbase::database &db, const AccountName &account_name,
                     const PermissionName &permission) {
  ...
  static void
  add(chainbase::database &db,
      const vector<types::AccountPermissionWeight> &controlling_accounts,
      const AccountName &account_name, const PermissionName &permission);
...
optional<block_id_type> account_history_plugin_impl::find_block_id(
    const chainbase::database &db,
    const transaction_id_type &transaction_id) const {
  optional<block_id_type> block_id;
  const auto &trx_idx =
      db.get_index<transaction_history_multi_index, by_trx_id>();
...
ProcessedTransaction account_history_plugin_impl::find_transaction(
    const chain::transaction_id_type &transaction_id,
  ...
  FC_THROW("Transaction with ID ${tid} was indexed as being in block ID "
           "${bid}, but was not found in that block",
           ("tid", transaction_id)("bid", block.id()));
}

ProcessedTransaction account_history_plugin_impl::get_transaction(
    const chain::transaction_id_type &transaction_id) const {
  const auto &db = chain_plug->chain().get_database();
  optional<block_id_type> block_id;
  db.with_read_lock([&]() { block_id = find_block_id(db, transaction_id); });
  if (block_id.valid()) {
    auto block = chain_plug->chain().fetch_block_by_id(*block_id);
    FC_ASSERT(block, "Transaction with ID ${tid} was indexed as being in block "
                     "ID ${bid}, but no such block was found",
              ("tid", transaction_id)("bid", block_id));
...
#warning TODO: lookup of recent transactions
  FC_THROW_EXCEPTION(chain::unknown_transaction_exception,
                     "Could not find transaction for: ${id}",
                     ("id", transaction_id.str()));
}

get_transactions_results account_history_plugin_impl::get_transactions(
    const AccountName &account_name, const optional<uint32_t> &skip_seq,
    const optional<uint32_t> &num_seq) const {
  ...
  db.with_read_lock([&]() {
    const auto &account_idx =
        db.get_index<account_transaction_history_multi_index,
                     by_account_name>();
    ...
    for (auto obj = range.first; obj != range.second; ++obj) {
      ...
      FC_ASSERT(block_id, "Transaction with ID ${tid} was tracked for "
                          "${account}, but no corresponding block id was found",
                ("tid", obj->transaction_id)("account", account_name));
      block_transaction_ids.emplace(
          std::make_pair(*block_id, obj->transaction_id));
    }
  });
...
get_transactions_results account_history_plugin_impl::ordered_transactions(
    const block_transaction_id_map &block_transaction_ids,
    const fc::time_point &start_time, const uint32_t begin,
    const uint32_t end) const {
  ...
  while (block_transaction != block_transaction_ids.cend() &&
         !time_exceeded(start_time)) {
    optional<signed_block> block =
        chain_plug->chain().fetch_block_by_id(block_transaction->first);
    FC_ASSERT(
        block, "Transaction with ID ${tid} was indexed as being in block ID "
               "${bid}, but no such block was found",
        ("tid", block_transaction->second)("bid", block_transaction->first));

    auto range = block_transaction_ids.equal_range(block_transaction->first);

    std::set<transaction_id_type> trans_ids_for_block;
    for (block_transaction = range.first; block_transaction != range.second;
         ++block_transaction) {
    ...
    for (auto cycle = block->cycles.crbegin();
         cycle != block->cycles.crend() && current < trx_after_block; ++cycle) {
      for (auto thread = cycle->crbegin();
           thread != cycle->crend() && current < trx_after_block; ++thread) {
        for (auto trx = thread->user_input.crbegin();
             trx != thread->user_input.crend() && current < trx_after_block;
             ++trx) {
          if (trans_ids_for_block.count(trx->id())) {
            if (++current > begin) {
              const auto pretty_trx =
                  chain_plug->chain().transaction_to_variant(*trx);
              results.transactions.emplace_back(ordered_transaction_results{
                  (current - 1), trx->id(), pretty_trx});
            ...
            // just check after finding transaction or transitioning to next
            // block to avoid spending all our time checking
    ...
    // just check after finding transaction or transitioning to next block to
    // avoid spending all our time checking
    if (time_exceeded(start_time)) {
      results.time_limit_exceeded_error = true;
      return results;
    }
...
bool account_history_plugin_impl::time_exceeded(
    const fc::time_point &start_time) const {
  return (fc::time_point::now() - start_time).count() > transactions_time_limit;
}

vector<AccountName> account_history_plugin_impl::get_key_accounts(
    const public_key_type &public_key) const {
  std::set<AccountName> accounts;
  const auto &db = chain_plug->chain().get_database();
  db.with_read_lock([&]() {
    const auto &pub_key_idx =
        db.get_index<public_key_history_multi_index, by_pub_key>();
...
vector<AccountName> account_history_plugin_impl::get_controlled_accounts(
    const AccountName &controlling_account) const {
  std::set<AccountName> accounts;
  const auto &db = chain_plug->chain().get_database();
  db.with_read_lock([&]() {
    const auto &account_control_idx =
        db.get_index<account_control_history_multi_index, by_controlling>();
  ...
  for (const auto &cycle : block.cycles) {
    for (const auto &thread : cycle) {
      for (const auto &trx : thread.user_input) {
        ...
        db.create<transaction_history_object>(
            [&block_id, &trx](transaction_history_object &transaction_history) {
              transaction_history.block_id = block_id;
              transaction_history.transaction_id = trx.id();
            });

        for (const auto &account_name : trx.scope) {
          db.create<account_transaction_history_object>([&trx, &account_name](
              account_transaction_history_object &account_transaction_history) {
        ...
        for (const chain::Message &msg : trx.messages) {
          if (msg.code == config::EosContractName) {
            if (msg.type == NEW_ACCOUNT) {
              ...
              remove<account_control_history_multi_index,
                     by_controlled_authority>(db, update.account,
                                              update.permission);
              add(db, update.authority.accounts, update.account,
                  update.permission);
              ...
              remove<account_control_history_multi_index,
                     by_controlled_authority>(db, del.account, del.permission);
            }
          }
        }
      }
...
void account_history_plugin_impl::add(
    chainbase::database &db, const vector<types::KeyPermissionWeight> &keys,
    const AccountName &account_name, const PermissionName &permission) {
...
void account_history_plugin_impl::add(
  ...
  for (auto controlling_account : controlling_accounts) {
    db.create<account_control_history_object>(
        [&](account_control_history_object &obj) {
...
bool account_history_plugin_impl::is_scope_relevant(
    const eos::types::Vector<AccountName> &scope) {
...
void account_history_plugin::set_program_options(options_description &cli,
                                                 options_description &cfg) {
  cfg.add_options()("filter_on_accounts,f",
                    bpo::value<vector<string>>()->composing(),
                    "Track only transactions whose scopes involve the listed "
                    "accounts. Default is to track all transactions.")(
      "get-transactions-time-limit",
      bpo::value<int>()->default_value(
          account_history_plugin_impl::DEFAULT_TRANSACTION_TIME_LIMIT),
      "Limits the maximum time (in milliseconds) processing a single "
      "get_transactions call.");
}

void account_history_plugin::plugin_initialize(const variables_map &options) {
  my->transactions_time_limit =
      options.at("get-transactions-time-limit").as<int>() * 1000;
...
void account_history_plugin::plugin_startup() {
  ...
  my->chain_plug->chain().applied_block.connect([&impl = my](
      const signed_block &block) { impl->applied_block(block); });
}
...
read_only::get_transaction_results read_only::get_transaction(
    const read_only::get_transaction_params &params) const {
  auto trx = account_history->get_transaction(params.transaction_id);
  return {params.transaction_id,
          account_history->chain_plug->chain().transaction_to_variant(trx)};
}

read_only::get_transactions_results read_only::get_transactions(
    const read_only::get_transactions_params &params) const {
  return account_history->get_transactions(params.account_name, params.skip_seq,
                                           params.num_seq);
}

read_only::get_key_accounts_results
read_only::get_key_accounts(const get_key_accounts_params &params) const {
  return {account_history->get_key_accounts(params.public_key)};
}

read_only::get_controlled_accounts_results read_only::get_controlled_accounts(
