...
namespace eosio {
   ...
   constexpr auto     def_max_just_send = 1500 * 3; // "mtu" * 8
   constexpr auto     def_send_whole_blocks = true;

   constexpr auto     message_header_size = 4;
   constexpr auto     tag_2018_01_25 = -22219;
   /**
    *  Index by id
   ...
   public:
      ...
      optional<sync_state>    peer_supplying; // this peer is providing to us
   ...
   public:
      ...
      void bcast_summary (const block_summary_message& msg, connection_ptr skip = connection_ptr());
      ...
      void bcast_rejected_transaction (const signed_transaction& msg);
   ...
   void connection::request_sync_blocks (uint32_t start, uint32_t end) {
      ...
      peer_supplying = sync_state(start, end, start-1);
   ...
   void sync_manager::reset_lib_num() {
      ...
      for (auto& c : my_impl->connections) {
         ...
         if(c->peer_supplying && c->peer_supplying->end_block > sync_last_requested_num) {
            sync_last_requested_num = c->peer_supplying->end_block;
         }
      }
      fc_dlog(logger, "resetting, our last req is ${cc}",
   ...
   void sync_manager::recv_notice (connection_ptr c, const notice_message &msg) {
      ...
      if (msg.known_blocks.mode == catch_up) {
         if (msg.known_blocks.ids.size() == 0) {
            if (c->last_handshake_recv.network_version == tag_2018_01_25) {
   ...
   void big_msg_manager::bcast_summary (const block_summary_message &bsm, connection_ptr skip) {
   ...
   void big_msg_manager::bcast_block (const signed_block &sb, connection_ptr skip) {
      ...
      if (msgsiz > just_send_it_max) {
         ...
         for (auto cp : my_impl->connections) {
            ...
            const auto& prev = cp->blk_state.find (sb.previous);
   ...
   void big_msg_manager::bcast_rejected_transaction (const signed_transaction& txn) {
      transaction_id_type tid = txn.id();
      fc_wlog(logger,"sending rejected transaction ${tid}",("tid",tid));
      bcast_transaction (txn);
   }
   ...
   void big_msg_manager::bcast_transaction (const signed_transaction& txn) {
      ...
      if( !skip && bufsiz <= just_send_it_max) {
         my_impl->send_all( txn, [txnid](connection_ptr c) -> bool {
               if( c->syncing ) {
                  return false;
               }
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const signed_transaction &msg) {
      ...
      big_msg_master->bcast_rejected_transaction(msg);
   }
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const signed_block &msg) {
      ...
      if( c->peer_supplying) {
         if (blk_num == c->peer_supplying->end_block) {
            c->peer_supplying.reset();
         }
         else {
            c->peer_supplying->last = blk_num;
         }
      }

      try {
         if( cc.is_known_block(blk_id)) {
            sync_master->recv_block(c, msg, true);
            return;
         }
      } catch( ...) {
