...
namespace eosio {
   ...
   public:
      ...
      void init_ssl_context(const std::string& cert, const std::string& pkey, const std::string& ca);
   };
...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   public:
      flex_socket                               socket;     // only accessed through strand after construction
      boost::asio::io_context::strand           strand;
   ...
   private:
      void extract_participant_name(X509* cert)
      {
         ...
         int length = X509_NAME_get_text_by_NID(subj, NID_organizationName, buf, std::size(buf));
         ...
         if (my_impl->ssl_enabled) {
            socket.ssl_socket()->async_handshake(server ? ssl::stream_base::server : ssl::stream_base::client,
                                       ...
                                       [start_read, c = shared_from_this(), socket=socket, callback](const auto& ec){
                                          ...
                                          std::ignore = socket;
                                          if (ec) {
                                             peer_elog( c, "ssl handshake error: ${e}", ("e", ec.message()) );
                                             c->close();
                                             return;
                                          }
   ...
   void connection::blk_send( const block_id_type& blkid ) {
      ...
      app().post( priority::medium, [blkid, weak{std::move(weak)}]() {
         ...
         try {
            ...
            if( b ) {
               peer_dlog( c, "found block for id at num ${n}", ("n", b->block_num()) );
               my_impl->dispatcher->add_peer_block( blkid, c->connection_id );
               c->strand.post( [c, b{std::move(b)}]() {
                  c->enqueue_block( b );
               } );
            } else {
               peer_ilog( c, "fetch block by id returned null, id ${id}", ("id", blkid) );
            }
         } catch( const assert_exception& ex ) {
            peer_elog( c, "caught assert on fetch_block_by_id, ${ex}, id ${id}", ("ex", ex.to_string())("id", blkid) );
         } catch( ... ) {
            peer_elog( c, "caught other exception fetching block id ${id}", ("id", blkid) );
         }
      });
   }
   ...
   void connection::do_queue_write() {
      ...
      strand.post( [c{std::move(c)}, bufs{std::move(bufs)}]() {
         //capture by value is const so we need mutable keyword
         auto write_lambda = [c, socket=c->socket]( boost::system::error_code ec, std::size_t w ) mutable {
            try {
      ...
      queue_write(send_buffer,
            [conn{std::move(self)}, close_after_send](boost::system::error_code ec, std::size_t ) {
                        ...
                        if (close_after_send != no_reason) {
                           peer_ilog( conn, "sent a go away message: ${r}, closing connection", ("r", reason_str(close_after_send)) );
                           conn->close();
                           return;
                        }
                  },
   ...
   void dispatch_manager::bcast_transaction(const packed_transaction_ptr& trx) {
      ...
      for_each_connection( [this, &trx, &nts, &buff_factory]( auto& cp ) {
         ...
         if( !sb ) return true;
         peer_dlog( cp, "sending trx: ${id}", ("id", trx->id()) );
   ...
   void connection::handle_message( const handshake_message& msg ) {
      ...
      if (msg.generation == 1) {
         ...
         app().post( priority::medium, [peer_lib, chain_plug = my_impl->chain_plug, weak{std::move(weak)},
            ...
            peer_dlog( c, "handshake, check for fork lib_num = ${ln} peer_lib = ${pl}", ("ln", lib_num)( "pl", peer_lib ) );
   ...
   void connection::process_signed_block( const block_id_type& blk_id, signed_block_ptr msg ) {
      ...
      try {
         ...
         peer_elog( c, "Caught an unknown exception trying to recall blockID" );
      }
   ...
   void net_plugin_impl::init_ssl_context(const std::string& cert, const std::string& pkey, const std::string& ca){
