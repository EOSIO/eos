...
typedef multi_index_container<
    ...
    indexed_by<
        ordered_unique<tag<by_id>,
                       member<node_transaction_state, transaction_id_type,
                              &node_transaction_state::id>>,
        ordered_non_unique<tag<by_expiry>,
                           member<node_transaction_state, fc::time_point_sec,
                                  &node_transaction_state::expires>>,

        ordered_non_unique<tag<by_block_num>,
                           member<node_transaction_state, uint32_t,
...
public:
  ...
  boost::asio::steady_timer::duration keepalive_interval{
...
/**
 *  Index by id
 *  Index by is_known, block_num, validated_time, this is the order we will
 * broadcast
 *  to peer.
...
struct transaction_state {
  transaction_id_type id;
  bool is_known_by_peer = false;   ///< true if we sent or received this trx to
                                   ///this peer or received notice from peer
  bool is_noticed_to_peer = false; ///< have we sent peer notice we know it
                                   ///(true if we receive from this peer)
...
typedef multi_index_container<
    ...
    indexed_by<ordered_unique<tag<by_id>,
                              member<transaction_state, transaction_id_type,
...
typedef multi_index_container<
    ...
    indexed_by<ordered_unique<
        tag<by_id>, member<block_state, block_id_type, &block_state::id>>>>
    block_state_index;
...
public:
  connection(string endpoint, size_t send_buf_size = def_buffer_size,
             size_t recv_buf_size = def_buffer_size);

  connection(socket_ptr s, size_t send_buf_size = def_buffer_size,
             size_t recv_buf_size = def_buffer_size);
  ~connection();
  void initialize();
...
#warning("TODO: Rework Message Caching for efficiency")
  ...
  // large as possible. We can query the recv buffer size. When using
  // async_read_some(),
  // we will frequently end a read in the middle of a message. That could happen
  // with
  // small messages, in which a simple "normalize" call could reset the buffer
  // and allow a
  // subsequent read to pick up the rest. In the case of very large messages, we
  // should allow
  // for the creation of a secondary buffer big enough to hold the entire
  // message. Better
  // still would be to have a mempool class that has a series of fixed size
  // blocks that can
  ...
  char ts[ts_buffer_size]; //!< working buffer for making human readable
                           //!timestamps
  /** @} */
   ...
   */
  void send_time();
  /** \brief Populate and queue time_message immediately using incoming
   * time_message
   */
...
connection::connection(string endpoint, size_t send_buf_size,
                       size_t recv_buf_size)
    : block_state(), trx_state(), sync_receiving(), sync_requested(),
      socket(std::make_shared<tcp::socket>(std::ref(app().get_io_service()))),
      pending_message_size(0), pending_message_buffer(recv_buf_size),
      send_buffer(send_buf_size), node_id(), last_handshake(), out_queue(),
      connecting(false), syncing(false), peer_addr(endpoint),
      response_expected(), pending_fetch() {
  wlog("created connection to ${n}", ("n", endpoint));
  initialize();
}

connection::connection(socket_ptr s, size_t send_buf_size, size_t recv_buf_size)
    : block_state(), trx_state(), sync_receiving(), sync_requested(), socket(s),
      pending_message_size(0), pending_message_buffer(recv_buf_size),
      send_buffer(send_buf_size), node_id(), last_handshake(), out_queue(),
      connecting(false), syncing(false), peer_addr(), response_expected(),
      pending_fetch() {
...
void connection::initialize() {
  ...
  response_expected.reset(
      new boost::asio::steady_timer(app().get_io_service()));
}

bool connection::connected() { return (socket->is_open() && !connecting); }

bool connection::current() { return (connected() && !syncing); }
...
void connection::send_next_message() {
  ...
  fc::datastream<char *> ds(send_buffer.data(),
                            send_message_size + sizeof(send_message_size));
  ds.write((char *)&send_message_size, sizeof(send_message_size));
  fc::raw::pack(ds, m);

  boost::asio::async_write(
      *socket, boost::asio::buffer(
                   send_buffer, send_message_size + sizeof(send_message_size)),
...
void connection::sync_wait() {
  ...
  response_expected->async_wait(boost::bind(&connection::sync_timeout, this,
                                            boost::asio::placeholders::error));
}

void connection::fetch_wait() {
  response_expected->expires_from_now(my_impl->resp_expected_period);
  response_expected->async_wait(boost::bind(&connection::fetch_timeout, this,
                                            boost::asio::placeholders::error));
}
...
void connection::sync_timeout(boost::system::error_code ec) {
  if (!ec) {
    ...
    elog("setting timer for sync request got error ${ec}",
         ("ec", ec.message()));
  }
}
...
void connection::fetch_timeout(boost::system::error_code ec) {
  if (!ec) {
    ...
    if (!(pending_fetch->req_trx.empty() ||
          pending_fetch->req_blocks.empty())) {
      enqueue((request_message){vector<transaction_id_type>(),
                                vector<block_id_type>()});
    ...
    elog("setting timer for fetch request got error ${ec}",
         ("ec", ec.message()));
  }
}
//-----------------------------------------------------------
...
sync_manager::sync_manager(uint32_t span)
    : sync_head(0), sync_req_head(0), sync_req_span(span) {
...
struct postcache : public fc::visitor<void> {
  ...
  void operator()(const signed_block &block) const {
    try {
      ...
      elog("unlinkable_block_exception accept block #${n}",
           ("n", block.block_num()));
      // close (c);
...
void sync_manager::take_chunk(connection_ptr c) {
  ...
  if (!ss->block_cache.empty()) {
    ...
    if (ss->start_block !=
        chain_plug->chain().last_irreversible_block_num() + 1) {
      bool found = false;
      for (auto pos = full_chunks.begin(); !found && pos != full_chunks.end();
           ++pos) {
      ...
      if (!found) {
        full_chunks.push_back(
            ss); // either full chunks is empty or pos ran off the end
      }
    } else {
      apply_chunk(ss);
    }
  ...
  if (chain_plug->chain().head_block_num() == sync_head) {
    ...
    handshake_message hello;
    handshake_initializer::populate(hello);
    my_impl->send_all(hello,
                      [](connection_ptr /* no arg */) -> bool { return true; });
  }
  if (c->connected()) {
    assign_chunk(c);
  }
...
void net_plugin_impl::connect(connection_ptr c) {
  ...
  resolver->async_resolve(
      query, [c, this](const boost::system::error_code &err,
...
void net_plugin_impl::connect(connection_ptr c,
  ...
  c->socket->async_connect(
      current_endpoint,
      [c, endpoint_itr, this](const boost::system::error_code &err) {
        if (!err) {
          start_session(c);
        } else {
  ...
  acceptor->async_accept(*socket, [socket, this](boost::system::error_code ec) {
    if (!ec) {
      if (max_client_count == 0 || num_clients < max_client_count) {
        ...
        elog("Error max_client_count ${m} exceeded", ("m", max_client_count));
        socket->close();
      }
      start_listen_loop();
    } else {
      ...
      *conn->socket,
      ...
      [this, c](boost::system::error_code ec,
        ...
        if (!ec) {
          ...
          if (conn->pending_message_size <=
              conn->pending_message_buffer.size()) {
          ...
          elog("Error reading message from connection: ${m}",
               ("m", ec.message()));
        }
        close(c.lock());
      });
...
void net_plugin_impl::handle_message(connection_ptr c,
...
void net_plugin_impl::handle_message(connection_ptr c,
  ...
  //
  //  dlog("Clock offset is ${o}ns (${us}us)", ("o", c->offset)("us",
  //  c->offset/NsecPerUsec));
  c->org = 0;
  c->rec = 0;
}
...
void net_plugin_impl::handle_message(connection_ptr c,
  ...
  // peer tells us about one or more blocks or txns. When done syncing, forward
  // on
  ...
  for (const auto &t : msg.known_trx) {
    ...
    if (tx == my_impl->local_txns.end()) {
      c->trx_state.insert((transaction_state){
          t, true, true, (uint32_t)-1, fc::time_point(), fc::time_point()});
  ...
  if (fwd.known_trx.size() > 0) {
    send_all(fwd, [c, fwd](connection_ptr cptr) -> bool { return cptr != c; });
    c->enqueue(req);
  }
}
  ...
  if (!msg.req_trx.empty()) {
    ...
    for (auto t : msg.req_trx) {
      try {
        ...
        elog("caught assert on get_recent_transaction, ${ex} txnid = ${t}",
             ("ex", ex.get_log())("t", t));
        // keep going, client can ask another peer
      } catch (...) {
...
void net_plugin_impl::handle_message(connection_ptr c,
                                     const sync_request_message &msg) {
  if (msg.end_block == 0) {
    c->sync_requested.reset();
  } else {
    c->sync_requested.reset(
        new sync_state(msg.start_block, msg.end_block, msg.start_block - 1));
    c->enqueue_sync_block();
  }
}

void net_plugin_impl::handle_message(connection_ptr c,
                                     const block_summary_message &msg) {
  const auto &itr = c->block_state.get<by_id>();
  auto bs = itr.find(msg.block_header.id());
  if (bs == c->block_state.end()) {
    c->block_state.insert(
        (block_state){msg.block_header.id(), true, true, fc::time_point()});
    send_all(msg, [c](connection_ptr cptr) -> bool { return cptr != c; });
  } else {
    if (!bs->is_known) {
      ...
      for (auto &cyc_thr_id : cyc) {
        ...
        for( auto &gt : cyc_thr_id.gen_trx ) {
          try {
            ...
            cyc_thr.generated_input.push_back( ProcessedGeneratedTransaction(
        gen ) );
          } catch ( const exception &ex) {
            fetch_error = true;
            elog( "unable to retieve generated transaction, caught {ex}",
        ("ex",ex) );
            break;
          } catch ( ... ) {
            fetch_error = true;
        ...
        */
        for (auto &ut : cyc_thr_id.user_trx) {
          try {
            cyc_thr.user_input.push_back(
                ProcessedTransaction(cc.get_recent_transaction(ut)));
          } catch (const exception &ex) {
            fetch_error = true;
...
void net_plugin_impl::handle_message(connection_ptr c,
  ...
  if (tx == c->trx_state.end()) {
    c->trx_state.insert(
        (transaction_state){txnid, true, true, (uint32_t)msg.refBlockNum,
...
void net_plugin_impl::handle_message(connection_ptr c,
                                     const signed_block &msg) {
  //    dlog ("got block #${num}, from
  //    ${id}",("num",msg.block_num())("id",c->node_id));
  chain_controller &cc = chain_plug->chain();
  try {
  ...
  if (syncing) {
    ...
    if (!has_chunk) {
      ...
      if (c->sync_receiving->last + 1 != num) {
        elog("expected block ${next} but got ${num}",
             ("next", c->sync_receiving->last + 1)("num", num));
      }
      c->sync_receiving->last = num;
    }
  }
  bool accepted = false;
  //      dlog ("last irrevesible block = ${lib}", ("lib",
  //      cc.last_irreversible_block_num()));
  if (!syncing || num == cc.head_block_num() + 1 ||
      num > cc.last_irreversible_block_num()) {
    try {
...
struct precache : public fc::visitor<void> {
  ...
  void operator()(const signed_block &msg) const {
    ...
    memcpy(c->blk_buffer.data(), c->pending_message_buffer.data(),
           c->message_size);
  }
...
void net_plugin_impl::start_reading_pending_buffer(connection_ptr c) {
  boost::asio::async_read(
      *c->socket, boost::asio::buffer(c->pending_message_buffer.data(),
...
void net_plugin_impl::ticker() {
  ...
  keepalive_timer->async_wait([&](boost::system::error_code ec) {
    ticker();
    if (ec) {
      wlog("Peer keepalive ticked sooner than expected: ${m}",
           ("m", ec.message()));
    }
    for (auto &c : connections) {
      if (c->socket->is_open()) {
...
void net_plugin_impl::send_all_txn(const SignedTransaction &txn) {
  ...
  node_transaction_state nts = {
      txnid, time_point::now(), txn.expiration, buff, bn, true};
  ...
  if (fc::raw::pack_size(txn) <= just_send_it_max) {
    send_all(txn, [txn, txnid](connection_ptr c) -> bool {
      ...
      if (unknown)
        c->trx_state.insert(
            transaction_state({txnid, true, true, (uint32_t)-1,
                               fc::time_point(), fc::time_point()}));
      return unknown;
    });
  } else {
    ...
    send_all(nm, [txn, txnid](connection_ptr c) -> bool {
      ...
      if (unknown)
        c->trx_state.insert(
            transaction_state({txnid, false, true, (uint32_t)-1,
                               fc::time_point(), fc::time_point()}));
      return unknown;
    });
...
void net_plugin_impl::broadcast_block_impl(const chain::signed_block &sb) {
          ...
          */
          for (auto ui : thr.user_input) {
            ...
            if (lt != local_txns.end()) {
              ...
              transaction_id_type txnid = txn.id();
              if (local_txns.get<by_id>().find(txnid) ==
                  local_txns.end()) { // not found
                uint16_t bn = static_cast<uint16_t>(txn.refBlockNum);
  ...
  if (!send_whole_blocks) {
    ...
    send_all(bsm, [sb](connection_ptr c) -> bool {
      ...
      if (bs == c->block_state.end()) {
        c->block_state.insert(
            (block_state){sb.id(), true, true, fc::time_point()});
        return true;
      }
      return false;
    });
...
void handshake_initializer::populate(handshake_message &hello) {
  ...
  try {
    hello.head_id =
        cc.get_block_id_for_num(hello.head_num = cc.head_block_num());
  } catch (const unknown_block_exception &ex) {
...
void net_plugin::set_program_options(options_description &cli,
  ...
  cfg.add_options()(
      "listen-endpoint", bpo::value<string>()->default_value("0.0.0.0:9876"),
      "The local IP address and port to listen for incoming connections.")(
      "remote-endpoint", bpo::value<vector<string>>()->composing(),
      "The IP address and port of a remote peer to sync with.")(
      "public-endpoint", bpo::value<string>(),
      "Overrides the advertised listen endpointlisten ip address.")(
      "agent-name", bpo::value<string>()->default_value("EOS Test Agent"),
      "The name supplied to identify this node amongst the peers.")(
      "send-whole-blocks",
      bpo::value<bool>()->default_value(def_send_whole_blocks),
...
void net_plugin::plugin_initialize(const variables_map &options) {
  ...
  my->resolver =
      std::make_shared<tcp::resolver>(std::ref(app().get_io_service()));
  ...
  if (options.count("public-endpoint")) {
    ...
    if (my->listen_endpoint.address().to_v4() == address_v4::any()) {
      ...
      auto port = my->p2p_address.substr(my->p2p_address.find(':'),
                                         my->p2p_address.size());
...
void net_plugin::plugin_startup() {
  ...
  my->chain_plug->chain().on_pending_transaction.connect(
      &net_plugin_impl::transaction_ready);
  my->start_monitors();
