...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   bool connection::process_next_trx_message(uint32_t message_length) {
      ...
      auto report_dropping_trx = [](const packed_transaction_ptr& packed_trx_ptr, unsigned long trx_in_progress_sz) {
         char reason[72];
         snprintf(reason, 72, "Dropping trx, too many trx in progress %lu bytes", trx_in_progress_sz);
         my_impl->producer_plug->log_failed_transaction(packed_trx_ptr, reason);
      };
      ...
      if( which == trx_message_v1_which ) {
         ...
         if( trx_id ) {
            if (trx_in_progress_sz > def_max_trx_in_progress_size) {
               report_dropping_trx(ptr, trx_in_progress_sz);
               return true;
            }
         ...
         if( have_trx ) {
            ...
            if (ptr && trx_id && *trx_id != ptr->id()) {
               my_impl->producer_plug->log_failed_transaction(ptr, "Provided trx_id does not match provided packed_transaction");
