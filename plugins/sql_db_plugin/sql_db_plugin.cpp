...
{
   cfg.add_options()
         ("sqldb-queue-size,q", bpo::value<uint>()->default_value(256),
         "The queue size between nodeos and SQL DB plugin thread.")
         ("sqldb-uri,m", bpo::value<std::string>(),
...
//      my->init();
//   } else {
//      wlog("eosio::sql_db_plugin configured, but no --mongodb-uri specified.");
//      wlog("sql_db_plugin disabled.");
//   }
}
