...
class sync_callback {
 public:
   ///
   /// \brief The primary method for adding constructed blocks to the
   /// Blockvault. If a proposed constructed block is accepted, the Blockvault
   ...
   /// violations. If the Blockvault cannot make that guarantee for any reason,
   /// it must reject the proposal.
   ///
   /// Notice that handler would be called from a thread different from the
   /// invoker of this member function.
   ///
   ...
   ///
   /// \param handler The callback function to inform caller whether the block
   /// has accepted the Blockvault or not.
...
class block_vault_interface {
 public:
   ...
   /// \param handler The callback function to inform caller whether the block
   /// has accepted the Blockvault or not.
   virtual void async_propose_constructed_block(watermark_t watermark,
                                                uint32_t lib,
   ...
   /// Blockvault. If an external block is accepted, the Blockvault cluster will
   ...
   /// conflicts with the Blockvault state, then it will be rejected. If the
   /// block is older than the currently available snapshot, it is still
   /// accepted but will not affect the Blockvault cluster state.
   ///
   /// Notice that handler would be called from a thread different from the
   /// invoker of this member function.
   ///
   ...
   /// has accepted the Blockvault or not.
   ///
   virtual void async_append_external_block(uint32_t lib,
                                            eosio::chain::signed_block_ptr block,
                                            std::function<void(bool)> handler) = 0;

   ///
   /// \brief The primary method for a nodeos node to offer snapshot data to
   /// Blockvault that facilitates log pruning. If a snapshot's height is
   /// greater than the current snapshot AND less than or equal to the current
   /// implied LIB height, it will be accepted, and Blockvault will be able to
   ...
   /// the Blockvault. Syncing is semi-session based. A syncing `nodeos` node
   /// will establish a session with a single Blockvault node which is stored on
   /// that node for the duration of the syncing process.  This session is used
   /// to guarantee that the Blockvault node does not prune any data that will
   /// be needed to complete this sync process until it is completed OR the
   /// session is timed-out. This session is not replicated amongst other
   /// Blockvault nodes.  As a result, if the syncing process is interrupted due
   /// to inability to communicate with the chosen Blockvault node, it must
