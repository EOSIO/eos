...
def startWallet():
    ...
    background(args.remvault + ' --unlock-timeout %d --http-server-address 0.0.0.0:6666 --wallet-dir %s' % (
...
def startNode(nodeIndex, account):
    ...
    sleep(2)
    run(
        f'curl -X POST http://127.0.0.1:{8000 + nodeIndex}/v1/producer/schedule_protocol_feature_activations -d \'{{"protocol_features_to_activate": ["0ec7e080177b2c02b278d5088611686b49d739925a92d9bfcacd7fc6b74053bd"]}}\'')
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        print('%s: total funds=%s, ram=%s, net=%s, cpu=%s, unstaked=%s' % (
        a['name'], intToCurrency(a['funds']), intToCurrency(ramFunds), intToCurrency(stakeNet), intToCurrency(stakeCpu),
        intToCurrency(unstaked)))
...
def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(
            args.remcli + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' +
            a['pub'])
...
def stepCreateTokens():
    run(args.remcli + 'push action rem.token create \'["rem", "11000000000.0000 %s"]\' -p rem.token' % (args.symbol))
...
def stepSetSystemContract():
    retry(args.remcli + 'set contract rem ' + args.contracts_dir + '/rem.bios/ -p rem')
    retry(
        args.remcli + 'push action rem activate \'["f0af56d2c5a48d60a4a5b5c903edfb7db3a736a94ed589d0b797df33ff9d3e1d"]\' -p rem')
    retry(
        args.remcli + 'push action rem activate \'["2652f5f96006294109b3dd0bbde63693f55324af452b799ee137a81a905eed25"]\' -p rem')
    retry(
        args.remcli + 'push action rem activate \'["8ba52fe7a3956c5cd3a656a3174b931d3bb2abb45578befc59f283ecd816a405"]\' -p rem')
    retry(
        args.remcli + 'push action rem activate \'["ad9e3d8f650687709fd68f4b90b41f7d825a365b02c23a636cef88ac2ac00c43"]\' -p rem')
    retry(
        args.remcli + 'push action rem activate \'["68dcaa34c0517d19666e6b33add67351d8c5f69e999ca1e37931bc410a297428"]\' -p rem')
    retry(
        args.remcli + 'push action rem activate \'["e0fb64b1085cc5538970158d05a009c24e276fb94e1a0bf6a528b48fbc4ff526"]\' -p rem')
    retry(
        args.remcli + 'push action rem activate \'["ef43112c6543b88db2283a2e077278c315ae2c84719a8b25f25cc88565fbea99"]\' -p rem')
    retry(
        args.remcli + 'push action rem activate \'["4a90c00d55454dc5b059055ca213579c6ea856967712a56017487886a4d4cc0f"]\' -p rem')
    retry(
        args.remcli + 'push action rem activate \'["1a99a59d87e06e09ec5b028a9cbb7749b4a5ad8819004365d02dc4379a8b7241"]\' -p rem')
    retry(
        args.remcli + 'push action rem activate \'["4e7bf348da00a945489b2a681749eb56f5de00b900014e137ddae39f48f69d67"]\' -p rem')
...
parser.add_argument('--remcli', metavar='', help="Cleos command",
                    default='../../build/programs/remcli/remcli --wallet-url http://127.0.0.1:6666 ')
parser.add_argument('--remnode', metavar='', help="Path to remnode binary",
                    default='../../build/programs/remnode/remnode')
parser.add_argument('--remvault', metavar='', help="Path to remvault binary",
                    default='../../build/programs/remvault/remvault')
